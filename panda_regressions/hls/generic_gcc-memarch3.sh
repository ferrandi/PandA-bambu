...
python3 $script_dir/../../etc/scripts/mantis.py --tool=bambu \
   ...
   --args="--configuration-name=${compiler}-O1-wp-NN-LSS --memory-allocation-policy=LSS --channels-type=MEM_ACC_NN ${BATCH_ARGS[*]}"\
   --args="--configuration-name=${compiler}-O1-wp-NN-GSS --memory-allocation-policy=GSS --channels-type=MEM_ACC_NN ${BATCH_ARGS[*]}"\
