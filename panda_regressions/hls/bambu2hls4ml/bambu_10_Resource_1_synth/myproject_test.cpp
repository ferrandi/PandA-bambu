...
int main(int argc, char **argv) {
    ...
    if (fin.is_open() && fpr.is_open()) {
      ...
      nnet::copy_data<float, input_t, 0, 16*4>(in, conv1d_input);
      hls::stream<result_t> layer2_out("layer2_out");
            ...
            if (e % CHECKPOINT == 0) {
                ...
                nnet::print_result<result_t, 16*1>(layer2_out, std::cout, true);
            }
            e++;

            // hls-fpga-machine-learning insert tb-output
            nnet::print_result<result_t, 16*1>(layer2_out, fout);
        }
        fin.close();
        fpr.close();
    } else {
        ...
        for (unsigned i = 0; i < NUM_TEST_SAMPLES; i++) {
            ...
            hls::stream<input_t> conv1d_input("conv1d_input");
            nnet::fill_zero<input_t, 16*4>(conv1d_input);
            hls::stream<result_t> layer2_out("layer2_out");
