#!/bin/bash
$(dirname $0)/../../etc/scripts/test_panda.py -l function_pointers_list_bsearch --tool=bambu \
   ...
   -o output_function_pointers_bsearch -b$(dirname $0) --table=function_pointers_bsearch_output.tex --name="FunctionPointerBSearch" $@
return_value=$?
if test $return_value != 0; then
   exit $return_value
fi
$(dirname $0)/../../etc/scripts/test_panda.py -l function_pointers_list_qsort --tool=bambu \
   ...
   -o output_function_pointers_qsort -b$(dirname $0) --table=function_pointers_qsort_output.tex --name="FunctionPointerQSort" $@
return_value=$?
if test $return_value != 0; then
   exit $return_value
fi
