...
int main(void)
{
	...
	check_d1(lgamma, -__builtin_inff(), __builtin_inff());
	//check_d1(lgamma, __builtin_inff(), NAN);
	check_d1(lgamma, 0.0, __builtin_inff()); /* pole */
	check_d1(lgamma, minus_zero, __builtin_inff()); /* pole */
	check_d1(lgamma, 1.0, 0.0);
	check_d1(lgamma, 2.0, 0.0);
	check_d1(lgamma, FLT_MAX/2, __builtin_inff()); /* overflow to inf */
	check_d1(lgamma, FLT_MAX, __builtin_inff()); /* overflow to inf */
	check_d1(lgamma, __builtin_inff(), __builtin_inff()); /* overflow to inf */
	check_d1(lgamma, 7, logf(2*3*4*5*6)); /* normal value */
