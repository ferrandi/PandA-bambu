...
#define MANT_DIG CHOOSE((LDBL_MANT_DIG - 1), (DBL_MANT_DIG - 1), (FLT_MANT_DIG - 1), (LDBL_MANT_DIG - 1), (DBL_MANT_DIG - 1), (FLT_MANT_DIG - 1))
...
{
   if(output_points && (verbose > 1 || (verbose == 1 && ok == xfail)))
      return 1;
   return 0;
}
...
{
   if(output_max_error && (verbose > 1 || ((verbose == 1) && (ok == xfail))))
      return 1;
   return 0;
}
...
static void print_complex_function_ulps(const char* function_name, FLOAT real_ulp, FLOAT imag_ulp)
{
#if 0
  if (output_ulps)
    {
...
static void print_complex_max_error(const char* func_name, __complex__ FLOAT allowed, __complex__ int xfail)
{
   int ok = 0;

   if((real_max_error == 0 && imag_max_error == 0) || (real_max_error <= __real__ allowed && imag_max_error <= __imag__ allowed && !ignore_max_ulp))
   {
      ok = 1;
   }
...
static void test_exceptions(const char* test_name, int exception)
{
   ...
   if((exception & DIVIDE_BY_ZERO_EXCEPTION_OK) == 0)
      test_single_exception(test_name, exception, DIVIDE_BY_ZERO_EXCEPTION, FE_DIVBYZERO, "Divide by zero");
#endif
#ifdef FE_INVALID
...
static void check_float_internal(const char* test_name, FLOAT computed, FLOAT expected, FLOAT max_ulp, int xfail, int exceptions, FLOAT* curr_max_error)
{
   ...
   else if(isinf(computed) && isinf(expected))
   {
      ...
      if((exceptions & IGNORE_ZERO_INF_SIGN) == 0 && signbit(computed) != signbit(expected))
      {
         ok = 0;
   ...
   else
   {
      ...
      print_diff = 1;
      if((exceptions & IGNORE_ZERO_INF_SIGN) == 0 && computed == 0.0 && expected == 0.0 && signbit(computed) != signbit(expected))
         ok = 0;
...
{
   check_float_internal(test_name, computed, expected, max_ulp, xfail, exceptions, &max_error);
}

static void check_complex(const char* test_name, __complex__ FLOAT computed, __complex__ FLOAT expected, __complex__ FLOAT max_ulp, __complex__ int xfail, int exception)
{
   ...
   check_float_internal(str, part_comp, part_exp, part_max_ulp, part_xfail, exception, &real_max_error);
   ...
   /* Don't check again for exceptions, just pass through the
      zero/inf sign test.  */
   check_float_internal(str, part_comp, part_exp, part_max_ulp, part_xfail, exception & IGNORE_ZERO_INF_SIGN, &imag_max_error);
}
...
int matherr(struct exception* x __attribute__((unused)))
{
   return 1;
}
    ...
    atan2_test(void)
{
   ...
   check_float("atan2 (-0.00756827042671106339, -.001792735857538728036) == -1.80338464113663849327153994379639112", FUNC(atan2)(identityFloat(-0.00756827042671106339L), identityFloat(-.001792735857538728036L)), -1.80338464113663849327153994379639112L, 1,
               0, 0);

   print_max_error("atan2", 1, 0);
}

static void cabs_test(void)
{
   errno = 0;
...
static void initialize(void)
{
   ...
   plus_infty = CHOOSE(HUGE_VALL, HUGE_VAL, HUGE_VALF, HUGE_VALL, HUGE_VAL, HUGE_VALF);
   minus_infty = CHOOSE(-HUGE_VALL, -HUGE_VAL, -HUGE_VALF, -HUGE_VALL, -HUGE_VAL, -HUGE_VALF);
   max_value = CHOOSE(LDBL_MAX, DBL_MAX, FLT_MAX, LDBL_MAX, DBL_MAX, FLT_MAX);
   min_value = CHOOSE(LDBL_MIN, DBL_MIN, FLT_MIN, LDBL_MIN, DBL_MIN, FLT_MIN);
