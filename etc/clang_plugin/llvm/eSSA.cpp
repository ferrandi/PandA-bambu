...
namespace eSSAInfoClasses {
   ...
   class PredicateWithEdge : public PredicateWithCondition {
      ...
      protected:
         PredicateWithEdge(PredicateType PType, llvm::Value *Op, llvm::BasicBlock *From,
                           llvm::BasicBlock *To, llvm::Value *Cond)
   ...
   class PredicateBranch : public PredicateWithEdge {
      public:
         ...
         bool TrueEdge;
         PredicateBranch(llvm::Value *Op, llvm::BasicBlock *BranchBB, llvm::BasicBlock *SplitBB,
                         llvm::Value *Condition, bool TakenEdge)
   ...
   class PredicateSwitch : public PredicateWithEdge {
      public:
         ...
         PredicateSwitch(llvm::Value *Op, llvm::BasicBlock *SwitchBB, llvm::BasicBlock *TargetBB,
                         llvm::Value *CaseValue, llvm::SwitchInst *SI)
   ...
   const llvm::BasicBlock *getBranchBlock(const PredicateBase *PB) {
      assert(llvm::isa<PredicateWithEdge>(PB) &&
   ...
   static bool valueComesBefore(llvm::OrderedInstructions &OI, const llvm::Value *A,
                                const llvm::Value *B) {
   ...
   struct ValueDFS_Compare {
         ...
         const std::pair<llvm::BasicBlock *, llvm::BasicBlock *>
         getBlockEdge_local(const ValueDFS &VD) const {
            if (!VD.Def && VD.U) {
         ...
         llvm::Value *getMiddleDef(const ValueDFS &VD) const {
            ...
            if (!VD.U) {
               ...
               assert(llvm::isa<PredicateAssume>(VD.PInfo) &&
   ...
   void convertUsesToDFSOrdered(
         llvm::Value *Op, llvm::SmallVectorImpl<ValueDFS> &DFSOrderedSet,
         const llvm::DominatorTree *DT) {
      for (auto &U : Op->uses()) {
   ...
   const ValueInfo &getValueInfo(llvm::Value *Operand,
   ...
   llvm::Value *materializeStack(unsigned int &Counter,
                                 ValueDFSStack &RenameStack,
                                 llvm::Value *OrigOp, llvm::Function&F,
      ...
      for (auto RenameIter = RenameStack.end() - Start;
         ...
         if (llvm::isa<PredicateWithEdge>(ValInfo)) {
            ...
            auto *PIC =
                  B.CreatePHI(Op->getType(), 1, Op->getName() + "." + llvm::Twine(Counter++));
   ...
   void renameUses(llvm::SmallPtrSetImpl<llvm::Value *> &OpSet,
                   llvm::DenseMap<llvm::Value *, unsigned int>&ValueInfoNums,
                   llvm::SmallVector<ValueInfo, 32>& ValueInfos,
                   llvm::Function&fun,
                   const llvm::DominatorTree *DT,
                   llvm::OrderedInstructions *OI,
      ...
      auto Comparator = [&](const llvm::Value *A, const llvm::Value *B) {
      ...
      for (auto *Op : OpsToRename) {
         ...
         for (auto &VD : OrderedUses) {
            ...
            if (RenameStack.empty()) {
               ...
               llvm::errs() << "Rename Stack Top DFS numbers are ("
                            << RenameStack.back().DFSIn << ","
                            << RenameStack.back().DFSOut << ")\n";
            }

            llvm::errs() << "Current DFS numbers are (" << VD.DFSIn << ","
                         << VD.DFSOut << ")\n";
#endif
...
#if DEBUG_ESSA
            llvm::errs() << "Found replacement " << *Result.Def << " for "
   ...
   ValueInfo &getOrCreateValueInfo(llvm::Value *Operand,
                                   llvm::DenseMap<llvm::Value *, unsigned int>& ValueInfoNums,
                                   llvm::SmallVector<eSSAInfoClasses::ValueInfo, 32>& ValueInfos) {
   ...
   void addInfoFor(llvm::SmallPtrSetImpl<llvm::Value *> &OpsToRename, llvm::Value *Op,
                   PredicateBase *PB,
                   llvm::DenseMap<llvm::Value *, unsigned int>& ValueInfoNums,
                   llvm::SmallVector<eSSAInfoClasses::ValueInfo, 32>& ValueInfos)
   {
   ...
   void processAssume(llvm::IntrinsicInst *II, llvm::BasicBlock *AssumeBB,
      ...
      for (auto Cond : ConditionsToProcess) {
         if (auto *Cmp = llvm::dyn_cast<llvm::CmpInst>(Cond)) {
            ...
            assert(BinOp->getOpcode() == llvm::Instruction::And &&
   ...
   void processBranch(llvm::BranchInst *BI, llvm::BasicBlock *BranchBB,
      ...
      for (auto Cond : ConditionsToProcess) {
         if (auto *Cmp = llvm::dyn_cast<llvm::CmpInst>(Cond)) {
            ...
            assert((BinOp->getOpcode() == llvm::Instruction::And ||
                    BinOp->getOpcode() == llvm::Instruction::Or) &&
