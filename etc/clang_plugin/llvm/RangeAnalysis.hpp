...
class Range_base
{
   ...
   public:
     Range_base();
     Range_base(const APInt &lb, const APInt &ub, RangeType rType = Regular);
     ~Range_base() = default;
...
public:
  Range() : Range_base() {}
  Range(const APInt &lb, const APInt &ub, RangeType rType = Regular) : Range_base(lb,ub,rType) {}
  ~Range() = default;
  ...
  Range lshr(const Range &other, unsigned int from_bw) const;
...
public:
  ...
  bool runOnModule(Module &M, ModulePass *modulePass);
  static unsigned getMaxBitWidth(Module &M);
