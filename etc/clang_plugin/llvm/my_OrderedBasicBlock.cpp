...
OrderedBasicBlock::OrderedBasicBlock(const BasicBlock* BasicB) : NextInstPos(0), BB(BasicB)
{
...
bool OrderedBasicBlock::comesBefore(const Instruction* A, const Instruction* B)
{
   const Instruction* Inst = nullptr;
   assert(!(LastInstFound == BB->end() && NextInstPos != 0) && "Instruction supposed to be in NumberedInsts");
...
{
   assert(A->getParent() == B->getParent() && "Instructions must be in the same basic block!");
