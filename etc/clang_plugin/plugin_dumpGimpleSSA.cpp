...
namespace clang
{
   ...
   static void writeXML_interfaceFile(const std::string filename)
   {
      ...
      {
         if(TopFunctionName != "" && Fun2Demangled.find(funArgPair.first)->second != TopFunctionName && funArgPair.first != TopFunctionName)
            continue;
            ...
            for(const auto par : FD->parameters())
            {
               if(const ParmVarDecl* ND = dyn_cast<ParmVarDecl>(par))
               {
                  ...
                  else if(argType->isPointerType() || argType->isReferenceType())
                  {
                     ...
                     if(UDIT_p)
                     {
                        if(UserDefinedInterfaceType != "none" && UserDefinedInterfaceType != "handshake" && UserDefinedInterfaceType != "valid" && UserDefinedInterfaceType != "acknowledge" && UserDefinedInterfaceType != "fifo" &&
                  ...
                  else
                  {
                     ...
                     if(UDIT_p)
                     {
                        if(UserDefinedInterfaceType != "none" && UserDefinedInterfaceType != "handshake" && UserDefinedInterfaceType != "valid" && UserDefinedInterfaceType != "acknowledge")
                        {
                           DiagnosticsEngine& D = CI.getDiagnostics();
         ...
         while(Tok.isNot(tok::eod))
         {
            ...
            if(Tok.isNot(tok::eod))
            {
               ...
               else if(index >= 1)
               {
                  ...
                  if(index == 1)
                  {
                     if(tokString != "none" && tokString != "array" && tokString != "bus" && tokString != "fifo" && tokString != "handshake" && tokString != "valid" && tokString != "acknowledge")
                     {
                        DiagnosticsEngine& D = PP.getDiagnostics();
                        unsigned ID = D.getCustomDiagID(DiagnosticsEngine::Error, "#pragma HLS_interface unexpected interface type. Currently accepted keywords are: none,array,bus,fifo,handshake,valid,acknowledge");
