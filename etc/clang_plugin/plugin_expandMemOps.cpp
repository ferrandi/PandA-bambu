...
namespace llvm
{
   ...
   {
    private:
      bool addrIsOfIntArrayType(llvm::Value* dst_addr, unsigned& Align, const llvm::DataLayout* DL)
      {
      ...
      llvm::Type* getMemcpyLoopLoweringTypeLocal(llvm::LLVMContext& Context, llvm::ConstantInt* Length,
                                                 unsigned src_align, unsigned dst_align, llvm::Value* src_addr,
                                                 llvm::Value* dst_addr, const llvm::DataLayout* DL, bool is_volatile,
                                                 bool& Optimize)
      {
         if(!is_volatile)
         {
      ...
      void createMemCpyLoopKnownSizeLocal(llvm::Instruction* InsertBefore, llvm::Value* src_addr, llvm::Value* dst_addr,
                                          llvm::ConstantInt* CopyLen, unsigned src_align, unsigned dst_align,
                                          bool src_volatile, bool dst_volatile, const llvm::DataLayout* DL)
      {
         ...
         const auto LoopOpType = getMemcpyLoopLoweringTypeLocal(Ctx, CopyLen, src_align, dst_align, src_addr, dst_addr,
                                                                DL, src_volatile || dst_volatile, PeelCandidate);
         ...
         LLVM_DEBUG(llvm::dbgs() << "  size: " << LoopOpSize << ", count: " << LoopEndCount
                                 << ", left: " << RemainingBytes << "\n");
            ...
            llvm::dyn_cast<llvm::BitCastInst>(dst_addr) && PeelCandidate)
         {
            ...
            auto srcAddress =
                Builder.CreateBitCast(srcIsAlloca ? llvm::dyn_cast<llvm::BitCastInst>(src_addr)->getOperand(0) :
                                                    cast<llvm::ConstantExpr>(src_addr)->getOperand(0),
                                      SrcOpType);
            auto dstAddress =
                Builder.CreateBitCast(llvm::dyn_cast<llvm::BitCastInst>(dst_addr)->getOperand(0), DstOpType);
...
#endif
            auto* DstGEP = LoopBuilder.CreateInBoundsGEP(LoopOpType, dst_addr, LoopIndex);
            LoopBuilder.CreateStore(Load, DstGEP, dst_volatile);

            auto* NewIndex = LoopBuilder.CreateAdd(LoopIndex, llvm::ConstantInt::get(TypeOfCopyLen, 1U));
         ...
         if(RemainingBytes)
         {
            ...
            for(auto OpTy : RemainingOps)
            {
               ...
               auto* DstPtrType = llvm::PointerType::get(OpTy, DstAS);
               auto* CastedDst =
      ...
      void expandMemSetAsLoopLocal(llvm::MemSetInst* Memset, const llvm::DataLayout* DL)
      {
         ...
         if(AlignCanBeUsed)
         {
            ...
            LoopBuilder.CreateAlignedStore(
                SetValue, LoopBuilder.CreateInBoundsGEP(SetValue->getType(), dst_addr, LoopIndex), Align, IsVolatile);
#endif
         }
         else
         {
