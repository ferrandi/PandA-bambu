...
void* __m_cosim_main(void*)
{
   ...
   pthread_exit((void*)static_cast<size_t>((retval & 0xFF) << 8 | MDPI_COSIM_END));
   return NULL;
}
...
void __m_exit(int __status)
{
   ...
   __m_signal_to(MDPI_ENTITY_SIM, MDPI_COSIM_END);
   pthread_exit((void*)static_cast<size_t>((__status & 0xFF) << 8 | MDPI_COSIM_END));
}
...
void __m_abort()
{
   ...
   debug("Simulator reported state: %s\n", mdpi_state_str(state));
   __m_signal_to(MDPI_ENTITY_SIM, MDPI_COSIM_END);
   pthread_exit((void*)static_cast<size_t>(MDPI_COSIM_ABORT));
}
