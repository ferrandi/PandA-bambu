...
float __hide_ieee754_jnf(int n, float x)
{
	...
	if(FLT_UWORD_IS_ZERO(ix)||FLT_UWORD_IS_INFINITE(ix))
	    ...
	    a = __hide_ieee754_j0f(x);
	    b = __hide_ieee754_j1f(x);
	    for(i=1;i<n;i++){
		temp = b;
     ...
     */
	    ...
	    /* determine k */
		float t,v;
		float q0,q1,h,tmp; unsigned k,m;
		...
		if(tmp<(float)8.8721679688e+01) {
	    	...
	    	b = (t*__hide_ieee754_j0f(x)/b);
	    }
	}
	if(sgn==1) return -b; else return b;
}

float __hide_ieee754_ynf(int n, float x) 
{
	int i,hx,ix,ib;
	int sign;
	float a, b, temp;
	...
	if(n==0) return(__hide_ieee754_y0f(x));
	if(n==1) return(sign*__hide_ieee754_y1f(x));
	if(FLT_UWORD_IS_INFINITE(ix)) return zero;

	a = __hide_ieee754_y0f(x);
	b = __hide_ieee754_y1f(x);
	/* quit if b is -inf */
	GET_FLOAT_WORD(ib,b);
