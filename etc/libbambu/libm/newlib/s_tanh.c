...
#ifdef __STDC__
static const RAISE_VOLATILE double one=1.0, two=2.0, tiny = 1.0e-300;
#else
static double RAISE_VOLATILE one=1.0, two=2.0, tiny = 1.0e-300;
#endif
...
#else
		return ix == 0x7ff00000 ? (((jx >> 31)&1)?-one:one) : __math_invalid(x);
#endif
	}
