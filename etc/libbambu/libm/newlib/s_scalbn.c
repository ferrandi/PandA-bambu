...
#endif
{
	...
	EXTRACT_WORDS(hx,lx,x);
	    ...
	    k = ((hx&0x7ff00000)>>20) - 54; 
            if (n< -50000) return __math_raise(tiny*x, __math_uflow((hx >> 31) & 1)); 	/*underflow*/
	    }
        if (k==0x7ff) return __math_raise(x+x, x);		/* NaN or Inf */
        if (n > 50000) 	/* in case integer overflow in n+k */
	    return __math_raise(huge*copysign(huge,x), __math_oflow((hx >> 31) & 1));	/*overflow*/
        k = k+n; 
        if (k >  0x7fe) return __math_raise(huge*copysign(huge,x), __math_oflow((hx >> 31) & 1)); /* overflow  */
        if (k > 0) 				/* normal result */
	    {SET_HIGH_WORD(x,(hx&0x800fffff)|(k<<20)); return x;}
        if (k <= -54)
	    return __math_raise(tiny*copysign(tiny,x), __math_uflow((hx >> 31) & 1)); 	/*underflow*/
