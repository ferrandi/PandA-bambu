...
#endif
{
	...
	if(ix<0x32000000) {	/* |x|<2**-27 */
	    if(__math_raise(huge+x>one, 1)) return (float)0.5*x;/* inexact if x!=0 necessary */
	}
	z = x*x;
...
#endif
{
	...
	if(!FLT_UWORD_IS_FINITE(ix)) return one/(x+x*x); 
        if(FLT_UWORD_IS_ZERO(ix)) return __math_raise(-one/zero, __math_oflowf(1));
        if(hx<0) return __math_raise(zero/zero, __math_invalid(x));
        if(ix >= 0x40000000) {  /* |x| >= 2.0 */
                s = sinf(x);
                c = cosf(x);
                ss = -s-c;
