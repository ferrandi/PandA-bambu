/* origin: FreeBSD lib/msun/src/s_sincos.c */
/*-
 * ====================================================
...
sincos(double x, double *sn, double *cs)
{
	...
	if (ix <= 0x3fe921fb) {
		if (ix < 0x3e400000) {		/* |x| < 2**-27 */
			if (__math_raise((int)x == 0, 1)) {	/* Generate inexact. */
				*sn = x;
				*cs = 1;
				return;
			}
	...
	if (ix >= 0x7ff00000) {
		*sn = __math_raise(x - x, __math_invalid(x));
		*cs = __math_raise(x - x, __math_invalid(x));
		return;
	}
