...
#endif
{
	...
	if (ix >= 0x40c00000) {		/* inf>|x|>=6 */
	    if(hx>=0) return __math_raise(one-tiny, one); else return __math_raise(tiny-one, -one);
	}
	x = fabsf(x);
 	s = one/(x*x);
...
#endif
{
	...
	if (ix < 0x41e00000) {		/* |x|<28 */
		...
		if(hx<0&&ix>=0x40c00000) return __math_raise(two-tiny, two);/* x < -6 */
	        R=rb0+s*(rb1+s*(rb2+s*(rb3+s*(rb4+s*(
				rb5+s*rb6)))));
	        S=one+s*(sb1+s*(sb2+s*(sb3+s*(sb4+s*(
				sb5+s*(sb6+s*sb7))))));
	    ...
	    if(hx>0) return __math_uflow(0); else return __math_raise(two-tiny, two);
	}
}
