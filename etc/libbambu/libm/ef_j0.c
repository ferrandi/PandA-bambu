...
static const float huge = 1e30f, one = 1.0f, invsqrtpi = 5.6418961287e-01f, /* 0x3f106ebb */
...
float __hide_ieee754_j0f(float x)
{
   ...
   { /* |x| >= 2.0 */
       ...
       */
      if(ix > 0x80000000)
         z = (invsqrtpi * cc) / __hide_ieee754_sqrtf(x);
      else
      {
         u = __hide_pzerof(x);
         v = __hide_qzerof(x);
         z = invsqrtpi * (u * cc - v * ss) / __hide_ieee754_sqrtf(x);
      }
      return z;
   }
   if(ix < 0x39000000)
...
float __hide_ieee754_y0f(float x)
{
   ...
   { /* |x| >= 2.0 */
      ...
      if(ix > 0x80000000)
         z = (invsqrtpi * ss) / __hide_ieee754_sqrtf(x);
      else
      {
         u = __hide_pzerof(x);
         v = __hide_qzerof(x);
         z = invsqrtpi * (u * ss + v * cc) / __hide_ieee754_sqrtf(x);
      }
      return z;
   }
   if(ix <= 0x32000000)
   { /* x < 2**-27 */
      return (u00 + tpi * __hide_ieee754_logf(x));
   }
   z = x * x;
   u = u00 + z * (u01 + z * (u02 + z * (u03 + z * (u04 + z * (u05 + z * u06)))));
   v = one + z * (v01 + z * (v02 + z * (v03 + z * v04)));
   return (u / v + tpi * (__hide_ieee754_j0f(x) * __hide_ieee754_logf(x)));
}
...
static float __hide_pzerof(float x)
{
   const float *p, *q;
   float z, r, s;
   int ix;
...
static float __hide_qzerof(float x)
{
   const float *p, *q;
   float s, r, z;
   int ix;
