...
void test_ac_hyperbolic_tan_pwl(
...
template <int Wfi, int Ifi, bool Sfi, int outWfi, int outIfi, bool outSfi>
int test_driver(
  double &cumulative_max_error_hyperbolic_tan,
...
)
{
  ...
  for (i = lower_limit; i < upper_limit; i += step) {
    ...
    if (this_error_hyperbolic_tan > max_hyperbolic_tan_error) { max_hyperbolic_tan_error = this_error_hyperbolic_tan; }
  }

  if (max_hyperbolic_tan_error > cumulative_max_error_hyperbolic_tan) { cumulative_max_error_hyperbolic_tan = max_hyperbolic_tan_error; }

  passed = (max_hyperbolic_tan_error < allowed_error);

  if (passed) { printf("PASSED , max err (%f)\n", max_hyperbolic_tan_error); }
  else        { printf("FAILED , max err (%f)\n", max_hyperbolic_tan_error); } // LCOV_EXCL_LINE
