...
void test_ac_exp2_cordic(const ac_fixed<Wfi, Ifi, Sfi, AC_TRN, AC_WRAP>& in, ac_fixed<outWfi, outIfi, false, AC_TRN, AC_WRAP>& out_exp2)
{
   ac_exp2_cordic(in, out_exp2);
}
...
template <int Wfi, int Ifi, bool Sfi, int outWfi, int outIfi>
int test_driver(double& cumulative_max_error_exp2, const double allowed_error, const double threshold, bool details = false)
{
   bool passed = true;
   double max_error_exp2 = 0.0; // reset for this run
   ...
   cout << "OUTPUT: ";
   cout.width(38);
   cout << left << output_exp2.type_name();
   cout << "RESULT: ";
   ...
   for(double i = lower_limit; i <= upper_limit; i += step)
   {
      ...
      double expected_value_exp2 = ((T_out)pow(2, input.to_double())).to_double();
      double actual_value_exp2 = output_exp2.to_double();

      double this_error_exp2;
      // If expected value of output falls below the threshold, calculate absolute error instead of relative
      if(expected_value_exp2 > threshold)
      {
         this_error_exp2 = abs_double((expected_value_exp2 - actual_value_exp2) / expected_value_exp2) * 100.0;
      }
      else
      {
         this_error_exp2 = abs_double(expected_value_exp2 - actual_value_exp2) * 100.0;
      }

#ifdef DEBUG
      if(this_error_exp2 > allowed_error)
      {
         cout << endl;
         cout << "  Error exceeds tolerance" << endl;
         cout << "  input               = " << input << endl;
         cout << "  expected_value_exp2 = " << expected_value_exp2 << endl;
         cout << "  actual_value_exp2   = " << actual_value_exp2 << endl;
         cout << "  this_error_exp2     = " << this_error_exp2 << endl;
         cout << "  threshold           = " << threshold << endl;
         assert(false);
      }
#endif

      if(this_error_exp2 > max_error_exp2)
      {
         max_error_exp2 = this_error_exp2;
      }
   }

   passed = (max_error_exp2 < allowed_error);
   ...
   if(max_error_exp2 > cumulative_max_error_exp2)
   {
      cumulative_max_error_exp2 = max_error_exp2;
   }
...
int main(int argc, char* argv[])
{
   ...
   cout << "    max_error_exp2 = " << max_error_exp2 << endl;

   bool test_fail = (max_error_exp2 > allowed_error);
