...
template <int Wfi, int Ifi, bool Sfi, int outWfi, int outIfi>
int test_driver(
  double &cumulative_max_error_exp2,
...
)
{
  ...
  for (double i = lower_limit; i <= upper_limit; i += step) {
    ...
    if (expected_value_exp2 > threshold) {this_error_exp2 = abs_double( (expected_value_exp2 - actual_value_exp2) / expected_value_exp2 ) * 100.0;}
    else {this_error_exp2 = abs_double(expected_value_exp2 - actual_value_exp2) * 100.0;}
    ...
    if (this_error_exp2 > max_error_exp2) {max_error_exp2 = this_error_exp2;}
  }

  passed = (max_error_exp2 < allowed_error);

  if (passed) { printf("PASSED , max err %f\n", max_error_exp2); }
  else        { printf("FAILED , max err %f\n", max_error_exp2); } // LCOV_EXCL_LINE

  if (max_error_exp2>cumulative_max_error_exp2) { cumulative_max_error_exp2 = max_error_exp2; }
