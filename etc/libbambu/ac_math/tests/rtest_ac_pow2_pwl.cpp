                ...
                bool details = false)
{
   ...
   for(double i = lower_limit; i <= upper_limit; i += step)
   {
      ...
      if(expected_value_pow2 > threshold)
      {
         this_error_pow2 = abs((expected_value_pow2 - actual_value_pow2) / expected_value_pow2) * 100.0;
      }
      else
      {
         this_error_pow2 = abs(expected_value_pow2 - actual_value_pow2) * 100.0;
      }
      ...
      if(this_error_pow2 > max_error_pow2)
      {
         max_error_pow2 = this_error_pow2;
      }
   }
   ...
   if(passed)
   {
      printf("PASSED , max err (%f pow2)\n", max_error_pow2);
   }
   else
   {
      printf("FAILED , max err (%f pow2)\n", max_error_pow2);
   } // LCOV_EXCL_LINE

   if(max_error_pow2 > cumulative_max_error_pow2)
   {
      cumulative_max_error_pow2 = max_error_pow2;
   }
