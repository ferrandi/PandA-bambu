...
)
{
  ...
  for (double i = lower_limit_base; i <= upper_limit_base; i += step_base) {
    for (double j = lower_limit_exponent; j < upper_limit_exponent; j += step_exponent) {
      ...
      if (expected_value_pow > threshold) {this_error_pow = abs_double( (expected_value_pow - actual_value_pow) / expected_value_pow ) * 100.0;}
      else {this_error_pow = abs_double(expected_value_pow - actual_value_pow) * 100.0;}
      ...
      if (this_error_pow > max_error_pow) { max_error_pow = this_error_pow; }
    }
  }
  ...
  else        { printf("FAILED , max err %f\n", max_error_pow); } // LCOV_EXCL_LINE

  if (max_error_pow > cumulative_max_error) { cumulative_max_error = max_error_pow; }
