...
template <int Wfi, int Ifi, bool Sfi, int outWfi, int outIfi>
int test_driver(
  double &cumulative_max_error_sigmoid,
...
)
{
  ...
  for (i = lower_limit; i < upper_limit; i += step) {
    ...
    if (check_monotonic) {
      // MONOTONIC: Make sure that function is monotonic. Compare old value (value of previous iteration) with current value. Since the sigmoid function that is being tested
    ...
    if (this_error_sigmoid > max_sigmoid_error) { max_sigmoid_error = this_error_sigmoid; }
  }

  if (max_sigmoid_error > cumulative_max_error_sigmoid) { cumulative_max_error_sigmoid = max_sigmoid_error; }

  passed = (max_sigmoid_error < allowed_error);

  if (passed) { printf("PASSED , max err (%f)\n", max_sigmoid_error); }
  else        { printf("FAILED , max err (%f)\n", max_sigmoid_error); } // LCOV_EXCL_LINE
