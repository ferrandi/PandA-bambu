...
)
{
  ...
  for (double i = lower_limit; i <= upper_limit; i += step) {
    ...
    if (expected_value_exp > threshold) {this_error_exp = abs_double( (expected_value_exp - actual_value_exp) / expected_value_exp ) * 100.0;}
    else {this_error_exp = abs_double(expected_value_exp - actual_value_exp) * 100.0;}
