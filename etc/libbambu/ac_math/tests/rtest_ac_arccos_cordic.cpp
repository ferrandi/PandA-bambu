...
void test_ac_arccos_cordic(const ac_fixed<Wfi, Ifi, Sfi, AC_TRN, AC_WRAP>& in, ac_fixed<outWfi, outIfi, outSfi, AC_TRN, AC_WRAP>& out_cos)
{
...
int test_driver(double& cummulative_max_error_arccosine, const double allowed_error, bool details = false)
{
   bool passed = true;
   ...
   for(double i = -1; i < 1; i += step)
   {
      ...
      if(this_error_arccosine > max_error_arccosine)
      {
         max_error_arccosine = this_error_arccosine;
      }
   }
   ...
   if(max_error_arccosine > cummulative_max_error_arccosine)
   {
      cummulative_max_error_arccosine = max_error_arccosine;
   }
