/* Specific functions for bambu architecture.
   Copyright (C) 2016 Politecnico di Milano (Italy).
...
static inline
unsigned long long int __local_get_uint64(double x)
{
...
static inline
double __local_get_double(unsigned long long int x)
{
  local_double_shape_type val;
  val.u_value = x;
  return val.value;
...
static inline 
double __local_copysign(double x, double y)
{
