...
#define LOOP_BODY(z,n,data) \
      ...
      sum_result_carry = (SELECT_BIT(sum_result,DIV_NUM_BIT_M1) & SELECT_BIT(w,DIV_NUM_BIT_M1)) | (SELECT_BIT(w,DIV_NUM_BIT_M1) & SELECT_BIT(partial_remainderH,DIV_NUM_BIT_M1)) | (SELECT_BIT(sum_result,DIV_NUM_BIT_M1) & SELECT_BIT(partial_remainderH,DIV_NUM_BIT_M1));\
...
static inline long long non_restoring_sdiv(long long a, long long b, long long * rem, _Bool signed_div)
{
  ...
  plus_one = (plus_divisor_sign && !sign_a) || 
             (!partial_remainder_sign && plus_divisor_sign && rem_nul) || 
