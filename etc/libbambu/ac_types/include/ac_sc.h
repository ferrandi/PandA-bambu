...
#if !defined(IEEE_1666_SYSTEMC) && !defined(SYSTEMC_VERSION) &&                \
    !defined(SC_API_VERSION_STRING)
...
template <int W> ac_int<W, true> to_ac(const sc_dt::sc_bigint<W> &val) {
  enum { N = (W + 31) / 32 };
...
template <int W> ac_int<W, false> to_ac(const sc_dt::sc_biguint<W> &val) {
  enum { N = (W + 31) / 32 };
...
template <int W> sc_dt::sc_bigint<W> to_sc(const ac_int<W, true> &val) {
  enum { N = (W + 31) / 32 };
...
template <int W> sc_dt::sc_biguint<W> to_sc(const ac_int<W, false> &val) {
  enum { N = (W + 31) / 32 };
  ac_int<N * 32, true> v = val;
...
#ifdef SC_INCLUDE_FX
template <ac_special_val V, int W, int I, sc_dt::sc_q_mode Q,
          sc_dt::sc_o_mode O, int nbits>
inline sc_dt::sc_fixed<W, I, Q, O, nbits>
value(sc_dt::sc_fixed<W, I, Q, O, nbits>) {
  sc_dt::sc_fixed<W, I> r;
  if (V == AC_VAL_DC) {
    int t;
    r = t;
...
template <ac_special_val V, int W, int I, sc_dt::sc_q_mode Q,
          sc_dt::sc_o_mode O, int nbits>
inline sc_dt::sc_ufixed<W, I, Q, O, nbits>
value(sc_dt::sc_ufixed<W, I, Q, O, nbits>) {
  sc_dt::sc_ufixed<W, I> r;
  if (V == AC_VAL_DC) {
    int t;
    r = t;
...
template <int W, bool S> class vcd_ac_int_trace : public sc_core::vcd_trace {
public:
  vcd_ac_int_trace(const ac_int<W, S> &object_, const std::string &name_,
                   const std::string &vcd_name_)
      : vcd_trace(name_, vcd_name_), object(object_) {
    vcd_var_typ_name =
        "wire";    // SystemC does not expose vcd_types[] in sc_vcd_trace.h
  ...
  virtual void print_variable_declaration_line(FILE *f) {
    ...
    std::sprintf(buf, "$var %s  % 3d  %s  %s [%d:0]  $end\n", vcd_var_typ_name,
                 bit_width, vcd_name.c_str(), namecopy.c_str(), bit_width - 1);
    std::fputs(buf, f);
  }
  ...
  virtual void write(FILE *f) {
    ...
    std::fprintf(
        f, "%s",
        compose_line(
            ((ac_int<W, false>)object).to_string(AC_BIN, true).substr(3))
            .c_str());
    old_value = object;
  }
...
inline void sc_trace(sc_core::sc_trace_file *tf, const ac_int<W, S> &a,
                     const std::string &name) {
  using namespace sc_core;
  if (tf) {
    ...
    //}
    vcd_trace *t = (vcd_trace *)new vcd_ac_int_trace<W, S>(
...
public:
  vcd_ac_fixed_trace(const ac_fixed<W, I, S, Q, O> &object_,
                     const std::string &name_, const std::string &vcd_name_)
      : vcd_trace(name_, vcd_name_), object(object_) {
    vcd_var_typ_name =
        "wire";    // SystemC does not expose vcd_types[] in sc_vcd_trace.h
    bit_width = W; // bit_width defined in base class 'vcd_trace'
  ...
  virtual void write(FILE *f) {
    ...
    std::fprintf(
        f, "%s",
        compose_line(
            ((ac_fixed<W, I, false>)object).to_string(AC_BIN, true).substr(3))
            .c_str());
    old_value = object;
  }
...
inline void sc_trace(sc_core::sc_trace_file *tf,
  ...
  if (tf) {
    ...
    //        "To add traces, create a new vcd trace file.", false );
    //}
    vcd_trace *t = (vcd_trace *)new vcd_ac_fixed_trace<W, I, S, Q, O>(
        a, name, ((vcd_trace_file *)tf)->obtain_name());
    ((vcd_trace_file *)tf)->traces.push_back(t);
  }
}
...
public:
  vcd_ac_float_trace(const ac_float<W, I, E, Q> &object_,
                     const std::string &name_, const std::string &vcd_name_)
      : vcd_trace(name_, vcd_name_), object(object_) {
    vcd_var_typ_name =
        "wire";    // SystemC does not expose vcd_types[] in sc_vcd_trace.h
  ...
  virtual void write(FILE *f) {
    ...
    // its own.
    std::fprintf(f, "%s",
                 compose_line(object.to_string(AC_BIN).substr(2)).c_str());
    old_value = object;
  }
...
template <int W, int I, int E, ac_q_mode Q>
inline void sc_trace(sc_core::sc_trace_file *tf, const ac_float<W, I, E, Q> &a,
                     const std::string &name) {
  using namespace sc_core;
  if (tf) {
    //--- SystemC 2.2.0 deficiency. The 'initialized' class member of
    ...
    //        "To add traces, create a new vcd trace file.", false );
    //}
    vcd_trace *t = (vcd_trace *)new vcd_ac_float_trace<W, I, E, Q>(
        a, name, ((vcd_trace_file *)tf)->obtain_name());
    ((vcd_trace_file *)tf)->traces.push_back(t);
  }
}
...
template <typename T>
inline void sc_trace(sc_core::sc_trace_file *tf, const ac_complex<T> &a,
                     const std::string &name) {
