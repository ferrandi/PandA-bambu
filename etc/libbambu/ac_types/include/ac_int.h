...
//
//   - Compiler support: Clang/llvm is able to lower ac_types to builtin data
types when optimization are enable.
//     GCC is working well but will not lower the ac_types to builtin when they
are used as top level function
//     parameter data types.
//     To improve the efficiency of hardware generated two optimization was
done:
//     - bit_adjust function is called on any input and output operand of basic
operators. This helps the
//       (bit) value range analysis to infer the minimum number of bits for
operands and results.
//     - the class iv use a specialized class to store the array of integers. In
particular, for ac_types
//       requiring less than 32bits a single integer object is used. This makes
the SROA compiler step more
//       effective.
//
...
#pragma clang loop unroll(full)
  ...
  if (n_mss < d_mss) {
    ...
    for (int k = n_mss; k >= d_mss; k--) {
      ...
      uw4 n1 =
          odd ? (uw4)((r1[k_msi + 1] << w1_length) | (r1[k_msi] >> w1_length))
                        << w2_length |
      ...
      if (q1 >> w1_length)
        q1--;
      AC_ASSERT(!(q1 >> w1_length),
                "Problem detected in long division algorithm, Please report");
      unsigned k2 = k - d_mss;
      unsigned k2_i = k2 >> 1;
      bool odd_2 = k2 & 1;
...
#pragma clang loop unroll(full)
      ...
      for (int i = ishift; i < M1; i++) {
        ...
        r.set(i, (hw << s31) |
                     (lw >> ((32 - s31) & 31))); // &31 is to quiet compilers
        lw = hw;
      }
      if (Nr > M1) {
...
#pragma clang loop unroll(full)
      ...
      for (int i = 0; i < Nr; i++) {
        unsigned hw = (i + ishift + 1 < N) ? op1[i + ishift + 1] : ext;
        r.set(i, (lw >> s31) |
                     (hw << ((32 - s31) & 31))); // &31 is to quiet compilers
        lw = hw;
      }
    } else {
#if defined(__clang__)
...
template <ac_base_mode b, int N> struct to_strImpl {
  static inline int to_str(iv_base<N> &v, int w, bool left_just, char *r) {
    ...
    const unsigned char B =
        b == AC_BIN ? 1 : (b == AC_OCT ? 3 : (b == AC_HEX ? 4 : 0));
    int k = (w + B - 1) / B;
    int n = (w + 31) >> 5;
    int bits = 0;
...
protected:
  class hex2doubleConverter {
    ...
    template <size_t begin, char charToFind, size_t NN> struct HfindCharImpl {
    ...
    struct getNumberImpl {
      ...
      getNumber(const char (&str)[NN], size_t begin, size_t end) {
        ...
        return increment +
               multiplier * getNumberImpl<Index - 1, base, Int, NN>::getNumber(
                                str, begin, end);
      }
    };
    ...
    static constexpr double Hscalbn(const double value, const int exponent) {
      ...
      if (in.ull_value << 1 == 0)
        return value;
      unsigned long long cur_exp =
          (((int)((in.ull_value >> 52) & 0x7ff)) + exponent);
    ...
    template <size_t NN>
    static constexpr size_t HpointPos1(const size_t mantissa_end,
                                       const size_t pp0) {
      return pp0 == 0 ? mantissa_end : pp0 - 1;
    }
    template <size_t NN>
    static constexpr size_t HpointPos(const char (&str)[NN],
      ...
      return mantissa_end == NN
                 ...
                 : (str[mantissa_end + 1] == '-' ? -1 : 1) *
                       getNumberImpl<NN - 1, 10, int, NN>::getNumber(
                           str, mantissa_end + 1 + HisSign(mantissa_end + 1),
                           NN - 1);
    }
    ...
    static constexpr size_t HmantissaBegin(const char (&str)[NN]) {
      return HisSign(str[0]) + 2 * (str[HisSign(str[0])] == '0' &&
    ...
    get0(const char (&str)[NN], const size_t mantissa_end,
      ...
      return (str[0] == '-' ? -1 : 1) *
             (Hscalbn(HbeforePoint(str, point_pos), exp) +
              Hscalbn(Hfraction(str, point_pos, mantissa_end),
...
#pragma clang diagnostic pop
  ...
  inline std::string to_string(ac_base_mode base_rep,
    ...
    if ((base_rep == AC_DEC || sign_mag) && is_neg()) {
      ...
      str_w =
          ac_private::to_string(mag.v, W + 1, sign_mag, base_rep, false, r + i);
    } else {
      ac_int<W, S> tmp = *this;
      str_w = ac_private::to_string(tmp.v, W + !S, sign_mag, base_rep, false,
                                    r + i);
    }
    if (!str_w) {
      r[i] = '0';
...
#pragma GCC diagnostic pop
  ...
  slc(const ac_int<WX, SX> &index) const {
    ...
    AC_ASSERT(index >= ac_intX(0),
              "Attempting to read slc with negative indices");
