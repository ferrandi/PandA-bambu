...
template <typename T> struct rt_ac_float_T {
  template <AC_FL_T0()> struct op1 {
    typedef AC_FL0() fl_t;
...
template <AC_FL_T()> class ac_float {
...
public:
  ...
  template <int WI, bool SI> struct rt_i {
    enum {
      lshift_w = W,
      lshift_i = I,
      lshift_s = S,
  ...
  template <typename T> struct rt_T {
    ...
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::mult mult;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::plus plus;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::minus minus;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::minus2 minus2;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::logic logic;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::div div;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::div2 div2;
  ...
  template <typename T> struct rt_T2 {
    typedef typename ac_private::map<T>::t map_T;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::mult mult;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::plus plus;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::minus2 minus;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::minus minus2;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::logic logic;
    typedef typename ac_private::rt_ac_float_T<map_T>::template op1<
        AC_FL_TV0()>::div2 div;
  ...
  template <AC_FL_T(2)>
  ac_float(const AC_FL(2) & op, bool assert_on_overflow = false,
           bool assert_on_rounding = false) {
    ...
    if (fx_t::width > W && assert_on_rounding)
      AC_ASSERT(m == t,
                "Loss of precision due to Rounding in ac_float constructor");
    ac_int<ET, true> exp =
        !m ? ac_int<ET, true>(0)
  ...
  ac_float(const ac_fixed<W, I, S> &m2, const ac_int<E, true> &e2,
           bool normalize = true) {
    m = m2;
    if (E) {
      if (!m)
  ...
  ac_float(const ac_fixed<WFX, IFX, SFX> &m2, const ac_int<E2, true> &e2,
           bool normalize = true) {
  ...
  template <int WI, bool SI> ac_float(const ac_int<WI, SI> &op) {
  ...
  bool normalize() {
    bool normalized = operator!() || !S && m[W - 1] ||
                      S && (m[W - 1] ^ m[W - 2]);
    if (E && !normalized)
  ...
  ac_float(double d, bool assert_on_overflow = false,
           bool assert_on_rounding = false) {
    enum {
      I_EXT = AC_MAX(I, 1),
  ...
  ac_float(float f, bool assert_on_overflow = false,
           bool assert_on_rounding = false) {
    enum {
      I_EXT = AC_MAX(I, 1),
  ...
  template <AC_FL_T(2)> bool compare(const AC_FL(2) & op2, bool *gt) const {
  ...
  void plus_minus(const AC_FL(2) & op2, AC_FL(R) & r, bool sub = false) const {
    ...
    ac_int<ET, true> r_e =
        op1_zero ? op2_e : (op2_zero ? op1_e : AC_MAX(op1_e, op2_e));
  ...
  template <AC_FL_T(2)> ac_float operator+=(const AC_FL(2) & op2) {
    ac_float r;
    plus_minus(op2, r);
    *this = r;
  }
  ...
  // Shift assign -------------------------------------------------------------
  template <int WI, bool SI> ac_float &operator<<=(const ac_int<WI, SI> &op2) {
    *this = operator<<(op2);
    return *this;
  }
  template <int WI, bool SI> ac_float &operator>>=(const ac_int<WI, SI> &op2) {
  ...
  template <AC_FL_T(2)> bool operator==(const AC_FL(2) & f) const {
    bool gt;
    return compare(f, &gt);
  }
  template <AC_FL_T(2)> bool operator!=(const AC_FL(2) & f) const {
  ...
  inline std::string to_string(ac_base_mode base_rep, bool sign_mag = false,
                               bool hw = true) const {
  ...
  template <ac_special_val V> inline ac_float &set_val() {
...
namespace ac_private {
template <typename T> bool ac_fpclassify(T x, bool &inf) {
  bool nan = !(x == x);
  if (!nan) {
    T d = x - x;
    inf = !(d == d);
...
namespace ac {
template <typename T> struct ac_float_represent {
  typedef typename ac_fixed_represent<T>::type fx_t;
  typedef ac_float<fx_t::width + !fx_t::sign, fx_t::i_width + !fx_t::sign, 1,
                   fx_t::q_mode>
      type;
};
...
namespace ac_private {
// with T == ac_float
template <AC_FL_T0(2)> struct rt_ac_float_T<AC_FL0(2)> {
...
// with T == ac_int
template <int WI, bool SI> struct rt_ac_float_T<ac_int<WI, SI>> {
  // For now E2 > 0
...
template <typename T> struct rt_ac_float_T<c_type<T>> {
  // For now E2 > 0
  enum {
...
#define FL_BIN_OP_WITH_CTYPE(BIN_OP, C_TYPE, RTYPE)                            \
  template <AC_FL_T()>                                                         \
  inline typename AC_FL()::template rt_T2<C_TYPE>::RTYPE operator BIN_OP(      \
      C_TYPE c_op, const AC_FL() & op) {                                       \
    typedef typename ac::template ac_float_represent<C_TYPE>::type fl2_t;      \
    return fl2_t(c_op).operator BIN_OP(op);                                    \
  }                                                                            \
  template <AC_FL_T()>                                                         \
  inline typename AC_FL()::template rt_T<C_TYPE>::RTYPE operator BIN_OP(       \
      const AC_FL() & op, C_TYPE c_op) {                                       \
    typedef typename ac::template ac_float_represent<C_TYPE>::type fl2_t;      \
...
#define FL_SHIFT_OP_WITH_INT_CTYPE(BIN_OP, C_TYPE, RTYPE)                      \
  template <AC_FL_T()>                                                         \
  inline typename AC_FL()::template rt_i<                                      \
      ac_private::c_type_params<C_TYPE>::W,                                    \
...
#define FL_BIN_OP_WITH_AC_INT_1(BIN_OP, RTYPE)                                 \
  ...
  inline typename AC_FL()::template rt_T2<ac_int<WI, SI>>::RTYPE               \
  operator BIN_OP(const ac_int<WI, SI> &i_op, const AC_FL() & op) {            \
    typedef                                                                    \
        typename ac::template ac_float_represent<ac_int<WI, SI>>::type fl2_t;  \
...
#define FL_BIN_OP_WITH_AC_INT_2(BIN_OP, RTYPE)                                 \
  template <AC_FL_T(), int WI, bool SI>                                        \
  inline typename AC_FL()::template rt_T2<ac_int<WI, SI>>::RTYPE               \
  operator BIN_OP(const AC_FL() & op, const ac_int<WI, SI> &i_op) {            \
    typedef                                                                    \
        typename ac::template ac_float_represent<ac_int<WI, SI>>::type fl2_t;  \
...
#define FL_ASSIGN_OP_WITH_AC_INT(ASSIGN_OP)                                    \
  template <AC_FL_T(), int WI, bool SI>                                        \
  inline AC_FL() &operator ASSIGN_OP(AC_FL() & op,                             \
...
#define FL_BIN_OP_WITH_AC_FIXED_1(BIN_OP, RTYPE)                               \
  ...
  inline typename AC_FL()::template rt_T2<ac_fixed<WF, IF, SF>>::RTYPE         \
  operator BIN_OP(const ac_fixed<WF, IF, SF, QF, OF> &f_op,                    \
                  const AC_FL() & op) {                                        \
    typedef                                                                    \
        typename ac::template ac_float_represent<ac_fixed<WF, IF, SF>>::type   \
            fl2_t;                                                             \
...
#define FL_BIN_OP_WITH_AC_FIXED_2(BIN_OP, RTYPE)                               \
  template <AC_FL_T(), int WF, int IF, bool SF, ac_q_mode QF, ac_o_mode OF>    \
  inline typename AC_FL()::template rt_T2<ac_fixed<WF, IF, SF>>::RTYPE         \
  operator BIN_OP(const AC_FL() & op,                                          \
                  const ac_fixed<WF, IF, SF, QF, OF> &f_op) {                  \
    typedef                                                                    \
        typename ac::template ac_float_represent<ac_fixed<WF, IF, SF>>::type   \
            fl2_t;                                                             \
...
#define FL_REL_OP_WITH_AC_FIXED(REL_OP)                                        \
  template <AC_FL_T(), int WF, int IF, bool SF, ac_q_mode QF, ac_o_mode OF>    \
  inline bool operator REL_OP(const AC_FL() & op,                              \
                              const ac_fixed<WF, IF, SF, QF, OF> &op2) {       \
    typedef                                                                    \
        typename ac::template ac_float_represent<ac_fixed<WF, IF, SF>>::type   \
            fl2_t;                                                             \
    return op.operator REL_OP(fl2_t(op2));                                     \
  ...
  template <AC_FL_T(), int WF, int IF, bool SF, ac_q_mode QF, ac_o_mode OF>    \
  inline bool operator REL_OP(ac_fixed<WF, IF, SF, QF, OF> &op2,               \
                              const AC_FL() & op) {                            \
    typedef                                                                    \
        typename ac::template ac_float_represent<ac_fixed<WF, IF, SF>>::type   \
            fl2_t;                                                             \
...
#define FL_ASSIGN_OP_WITH_AC_FIXED(ASSIGN_OP)                                  \
  template <AC_FL_T(), int WF, int IF, bool SF, ac_q_mode QF, ac_o_mode OF>    \
  inline AC_FL() &operator ASSIGN_OP(                                          \
      AC_FL() & op, const ac_fixed<WF, IF, SF, QF, OF> &op2) {                 \
    typedef                                                                    \
        typename ac::template ac_float_represent<ac_fixed<WF, IF, SF>>::type   \
            fl2_t;                                                             \
    return op.operator ASSIGN_OP(fl2_t(op2));                                  \
...
template <ac_special_val V, AC_FL_T()> inline AC_FL() value(AC_FL()) {
  AC_FL() r;
