...
template <class T> class ac_channel {
public:
  T read();
  void read(T &t);
  ...
  bool
  available(unsigned int k); // Return true if channel has at least k entries
  unsigned int size();
...
private:
  // Prevent the compiler from autogenerating these.  (This enforces that
...
#ifndef __BAMBU__
    : rSz(init), rVal(val), size_call_count(0)
#endif
{
  for (int i = init; i > 0; i--)
    write(val);
...
// in an integer variable similar to rSz.  The allows us to use a FIFO length
// which is independent of the startup time of the design.

template <class T> T ac_channel<T>::read() {
#ifndef __BAMBU__
  if (chan.empty()) {
...
template <class T> void ac_channel<T>::read(T &t) { t = read(); }

template <class T> void ac_channel<T>::write(const T &t) {
#ifndef __BAMBU__
  chan.push_back(t);
#endif
}

template <class T> unsigned int ac_channel<T>::size() {
#ifndef __BAMBU__
  size_call_count++;
...
template <class T> bool ac_channel<T>::available(unsigned int k) {
#ifndef __BAMBU__
  return chan.size() >= k;
#endif
}
