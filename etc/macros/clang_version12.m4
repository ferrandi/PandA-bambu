...
for compiler in $CLANG_TO_BE_CHECKED; do
   if test -f $compiler; then
      ...
      I386_LLVM12_CXXFLAGS="$LLVM12_CXXFLAGS -std=c++14 -O2 $3"
      ...
      LIBS=
      AC_LANG_PUSH([C])
      AC_LINK_IFELSE([AC_LANG_SOURCE([int main(void){ return 0;}])],I386_CLANG12_M32=yes,I386_CLANG12_M32=no)
      AC_LANG_POP([C])
      CC=$ac_save_CC
      CFLAGS=$ac_save_CFLAGS
      LDFLAGS=$ac_save_LDFLAGS
      LIBS=$ac_save_LIBS
      if test "x$I386_CLANG12_M32" == xyes; then
      ...
      LIBS=
      AC_LANG_PUSH([C])
      AC_LINK_IFELSE([AC_LANG_SOURCE([int main(void){ return 0;}])],I386_CLANG12_MX32=yes,I386_CLANG12_MX32=no)
      AC_LANG_POP([C])
      CC=$ac_save_CC
      CFLAGS=$ac_save_CFLAGS
      LDFLAGS=$ac_save_LDFLAGS
      LIBS=$ac_save_LIBS
      if test "x$I386_CLANG12_MX32" == xyes; then
      ...
      LIBS=
      AC_LANG_PUSH([C])
      AC_LINK_IFELSE([AC_LANG_SOURCE([int main(void){ return 0;}])],I386_CLANG12_M64=yes,I386_CLANG12_M64=no)
      AC_LANG_POP([C])
      CC=$ac_save_CC
      CFLAGS=$ac_save_CFLAGS
      LDFLAGS=$ac_save_LDFLAGS
      LIBS=$ac_save_LIBS
      if test "x$I386_CLANG12_M64" == xyes; then
...
PLUGIN_TEST
      for plugin_compiler in $I386_CLANGPP12_EXE; do
         ...
         case $host_os in
           mingw*) 
             I386_CLANG12_PLUGIN_COMPILER=$plugin_compiler
             ;;
           *)
             ...
             I386_CLANG12_PLUGIN_COMPILER=$plugin_compiler,I386_CLANG12_PLUGIN_COMPILER=)
             AC_LANG_POP([C])
             CC=$ac_save_CC
             CFLAGS=$ac_save_CFLAGS
             #If plugin compilation fails, skip this executable
             if test "x$I386_CLANG12_PLUGIN_COMPILER" = x; then
