...
for compiler in $CLANG_TO_BE_CHECKED; do
   if test -f $compiler; then
      ...
      AC_CACHE_CHECK(clang 4.0 supports -m32,
        ax_cv_clang4_m32,
        [
         ac_save_CC="$CC"
         ...
         LIBS=
         AC_LANG_PUSH([C])
         AC_LINK_IFELSE([AC_LANG_SOURCE([int main(void){ return 0;}])],ax_cv_clang4_m32=yes,ax_cv_clang4_m32=no)
         AC_LANG_POP([C])
         CC=$ac_save_CC
      ...
      if test "x$ax_cv_clang4_m32" == xyes; then
      ...
      AC_CACHE_CHECK(clang 4.0 supports -mx32,
        ax_cv_clang4_mx32,
        [
         ac_save_CC="$CC"
         ac_save_CFLAGS="$CFLAGS"
         ...
         LIBS=
         AC_LANG_PUSH([C])
         AC_LINK_IFELSE([AC_LANG_SOURCE([int main(void){ return 0;}])],ax_cv_clang4_mx32=yes,ax_cv_clang4_mx32=no)
         AC_LANG_POP([C])
         CC=$ac_save_CC
         CFLAGS=$ac_save_CFLAGS
         LDFLAGS=$ac_save_LDFLAGS
      ...
      if test "x$ax_cv_clang4_mx32" == xyes; then
      ...
      AC_CACHE_CHECK(clang 4.0 supports -m64,
        ax_cv_clang4_m64,
        [
         ac_save_CC="$CC"
         ac_save_CFLAGS="$CFLAGS"
         ...
         LIBS=
         AC_LANG_PUSH([C])
         AC_LINK_IFELSE([AC_LANG_SOURCE([int main(void){ return 0;}])],ax_cv_clang4_m64=yes,ax_cv_clang4_m64=no)
         AC_LANG_POP([C])
         CC=$ac_save_CC
         CFLAGS=$ac_save_CFLAGS
         LDFLAGS=$ac_save_LDFLAGS
      ...
      if test "x$ax_cv_clang4_m64" == xyes; then
...
PLUGIN_TEST
      AC_CACHE_CHECK(clang 4.0 supports plugins, 
        ax_cv_clang4_plugin_compiler,
        [
          for plugin_compiler in $I386_CLANGPP4_EXE; do
            ...
            ax_cv_clang4_plugin_compiler=$plugin_compiler,ax_cv_clang4_plugin_compiler=)
            AC_LANG_POP([C])
            CC=$ac_save_CC
            CFLAGS=$ac_save_CFLAGS
            #If plugin compilation fails, skip this executable
            if test "x$ax_cv_clang4_plugin_compiler" = x; then
...
if test x$ax_cv_clang4_plugin_compiler != x; then
  ...
  I386_CLANG4_PLUGIN_COMPILER=$ax_cv_clang4_plugin_compiler
