proc dump_statistics_map { report_file target_period } {
   set LSLICES 0
   set LUTs 0
   set has_LUTs 0
   set Registers 0
   ...
   set twr_file [open ./impl/top_impl.tw1 r]
   set twr_log [read $twr_file]
   regexp --  {\s*Report:\s*([[0-9.]+)} $twr_log ignore design_delay
   if { [expr {$design_delay == 0}] } {
      regexp --  {\s*Warning:\s*([[0-9.]+)} $twr_log ignore design_delay
   }
...
proc dump_statistics_par { report_file target_period } {
   set LSLICES 0
   set LUTs 0
   set has_LUTs 0
   set Registers 0
   ...
   set DSPs 0
   set design_delay 0
   set par_file [open ./impl/top_impl.par r]
   set par_log [read $par_file]
   set map_file [open ./impl/top_impl.mrp r]
   set map_log [read $map_file]
   ...
   set DSPs [expr {$DSP18s + $DSP9s + $ALU54s + $ALU24s}]
   regexp --  {\s*Number of registers:\s*([0-9]+)} $map_log ignore Registers
   regexp --  {\s*Number of PIO sites used:\s*([[0-9]+)} $map_log ignore BIOB
   file delete -force $report_file
   set twr_file [open ./impl/top_impl.twr r]
   set twr_log [read $twr_file]
   regexp --  {\s*Report:\s*([[0-9.]+)} $twr_log ignore design_delay
   if { [expr {$design_delay == 0}] } {
      regexp -- {\s*Warning:\s*([[0-9.]+MHz)} $twr_log ignore design_delay
      if { [expr {$design_delay == 0}] } {
         regexp --  {\s*Warning:\s*([[0-9.]+)} $twr_log ignore design_delay
      } else {
         regexp -- {\s*Warning:\s*([[0-9.]+)} $twr_log ignore design_delay
         set design_delay [expr {1000 / $design_delay}]
      }
   } else {
      set design_delay 0
      regexp -- {\s*Report:\s*([[0-9.]+MHz)} $twr_log ignore design_delay
      if { [expr {$design_delay == 0}] } {
         regexp --  {\s*Report:\s*([[0-9.]+)} $twr_log ignore design_delay
      } else {
         regexp -- {\s*Report:\s*([[0-9.]+)} $twr_log ignore design_delay
...
prj_strgy set_value -strategy Strategy1 par_create_delay_statistic_file=True
prj_strgy set_value -strategy Strategy1 par_place_effort_level=4
prj_strgy set_value -strategy Strategy1 {maptrce_analysis_option=Standard Setup With Hold Analysis on IOs}
prj_strgy set_value -strategy Strategy1 {partrce_analysis_option=Standard Setup With Hold Analysis on IOs}
prj_strgy set_value -strategy Strategy1 {par_routing_method=CDR}
...
dump_statistics_map $report_file $target_period
prj_run PAR -impl impl -forceOne
prj_run PAR -impl impl -task PARTrace -forceOne
dump_statistics_par $report_file $target_period
