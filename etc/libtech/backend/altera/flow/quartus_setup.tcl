...
if { [dict get $bambu_results /application/top_module@combinational] } {
   ...
   if { [dict get $bambu_results /application/target@connect_iob] && "$sdc_ext_file" eq "{}" } {
      puts $sdc_fd "set_min_delay 0 -from \[all_inputs\] -to \[all_registers\]"
      puts $sdc_fd "set_min_delay 0 -from \[all_registers\] -to \[all_outputs\]"
      puts $sdc_fd "set_max_delay [dict get $bambu_results /application/target@period] -from \[all_inputs\] -to \[all_outputs\]"
...
if { [string first "Cyclone" $target_family] } {
...
set vhdl_library ""
if { [dict exists $bambu_results /application/vhdl_library@sources] } {
   set vhdl_library "-library [dict get $bambu_results /application/vhdl_library@sources]"
}
set sources [dict get $bambu_results /application/outputs/file]
foreach file $sources {
   set file "$env(BAMBU_HLS_OUTDIR)/$file"
   set_global_assignment -name SOURCE_FILE $file {*}$vhdl_library
}
