...
std::cout << "reg [(PORTSIZE_" << _ports_out[1].name << "*BITSIZE_" << _ports_out[1].name << ")-1:0] "
          << _ports_out[1].name << " ;\n";
...
std::cout << "    started0[ii] = (started[ii] | " << _ports_in[2].name << "[ii]) & !(validated[ii] | "
          << _ports_in[5].name << ");\n";
...
std::cout << "always @(*)\n";
std::cout << "  for(ii=0; ii<PORTSIZE_" << _ports_out[1].name << "; ii=ii+1)\n";
std::cout << "    validated0[ii] <= (validated[ii] | " << _ports_in[5].name << ") & !(started[ii] | "
          << _ports_in[2].name << "[ii]);\n";
...
std::cout << "    " << _ports_out[1].name << "[(BITSIZE_" << _ports_out[1].name << ")*ii+:BITSIZE_"
          << _ports_out[1].name << "] = " << _ports_in[5].name << " ? (" << _ports_in[4].name << " >> (8*"
          << _ports_in[3].name << "[(BITSIZE_" << _ports_in[3].name << ")*ii+:BITSIZE_" << _ports_in[3].name
          << "])) : (reg_" << _ports_in[4].name << " >> (8*" << _ports_in[3].name << "[(BITSIZE_" << _ports_in[3].name
          << ")*ii+:BITSIZE_" << _ports_in[3].name << "]));" << std::endl;
std::cout << "end\n";

std::cout << "always @(*)\n";
std::cout << "begin\n";
std::cout << "  for(ii=0; ii<PORTSIZE_" << _ports_out[1].name << "; ii=ii+1)\n";
std::cout << "    " << _ports_out[0].name << "[ii] = (" << _ports_in[2].name << "[ii] & " << _ports_in[5].name
