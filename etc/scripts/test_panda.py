...
def execute_tests(named_list, thread_index):
    ...
    while local_index < len(lines) and not (failure and args.stop):
        ...
        failed_output_file_name = os.path.join(
            cwd, args.tool + "_failed_output")
        if os.path.exists(failed_output_file_name):
            os.remove(failed_output_file_name)
        timing_violation_report_file_name = os.path.join(
            cwd, "HLS_output/Synthesis/timing_violation_report")
        ...
        if args.restart and os.path.exists(os.path.join(cwd, args.tool + "_return_value")):
            ...
            if return_value == "0":
                with lock:
                    total_benchmark += 1
                    passed_benchmark += 1
                    logging.info("   SKIPPING --- OVERALL: " + str(passed_benchmark) + " passed, " + str(total_benchmark-passed_benchmark) +
                                 " failed, " + str(len(lines)-total_benchmark) + " queued --- " + lines[local_index].replace("\\", ""))
                    local_index = line_index
                    line_index += 1
                continue
        ...
        if not (failure and args.stop) or (return_value != -9 and return_value != 0):
            if return_value != 0:
                os.symlink(os.path.basename(output_file_name), str(os.path.join(
                    os.path.dirname(output_file_name), args.tool + "_failed_output")))
            if os.path.exists(timing_violation_report_file_name):
                annotated_timing_violation_report_file = open(
                    os.path.join(cwd, "timing_violation_report"), "w")
                annotated_timing_violation_report_file.write("#" * 80 + "\n")
                annotated_timing_violation_report_file.write(
                    "cd " + cwd + "; ")
                annotated_timing_violation_report_file.write(
                    local_command + "\n")
                annotated_timing_violation_report_file.write("#" * 80 + "\n")
                annotated_timing_violation_report_file.flush()
                timing_violation_report_file = open(
                    timing_violation_report_file_name)
                annotated_timing_violation_report_file.write(
                    timing_violation_report_file.read())
            ...
            with lock:
                ...
                if return_value == 0:
                    ...
                    if os.path.exists(os.path.join(cwd, args.tool + "_results_0.xml")):
                        logging.info("   SUCCESS (" + tool_results_string + ") --- OVERALL: " + str(passed_benchmark) + " passed, " + str(
                            total_benchmark-passed_benchmark) + " failed, " + str(len(lines)-total_benchmark) + " queued --- " + lines[local_index].replace("\\", ""))
                    else:
                        logging.info("   SUCCESS --- OVERALL: " + str(passed_benchmark) + " passed, " + str(total_benchmark-passed_benchmark) +
                                     " failed, " + str(len(lines)-total_benchmark) + " queued --- " + lines[local_index].replace("\\", ""))
                elif return_value == 124:
                    logging.info("   FAILURE (Timeout) --- OVERALL: " + str(passed_benchmark) + " passed, " + str(total_benchmark -
                                 passed_benchmark) + " failed, " + str(len(lines)-total_benchmark) + " queued --- " + lines[local_index].replace("\\", ""))
                elif return_value == 153:
                    logging.info("   FAILURE (File size limit exceeded) --- OVERALL: " + str(passed_benchmark) + " passed, " + str(total_benchmark -
                                 passed_benchmark) + " failed, " + str(len(lines)-total_benchmark) + " queued --- " + lines[local_index].replace("\\", ""))
                else:
                    logging.info("   FAILURE --- OVERALL: " + str(passed_benchmark) + " passed, " + str(total_benchmark-passed_benchmark) +
                                 " failed, " + str(len(lines)-total_benchmark) + " queued --- " + lines[local_index].replace("\\", ""))
            with lock:
                local_index = line_index
                line_index += 1
    ...
    for subdir in subdirs:
        if os.path.exists(os.path.join(directory, subdir, args.tool + "_return_value")):
            return_value_file_name = os.path.join(
                directory, subdir, args.tool + "_return_value")
            ...
            if return_value == "0":
                tool_results_file_name = os.path.join(
                    directory, subdir, args.tool + "_results")
        ...
        for line in lines:
            ...
            if os.path.exists(os.path.join(local_dir, args.tool + "_results_0.xml")):
                input_files = input_files + " " + \
                    os.path.join(local_dir, args.tool + "_results_0.xml")
        if len(input_files) > 0:
            # Generate experimental setup xml
            experimental_setup_file_name = os.path.join(
                abs_path, "experimental_setup.xml")
            ...
            if args.commonargs != None:
                bambu_arguments = ' '.join(' '.join(map(str, l))
                                           for l in args.commonargs)
            else:
                bambu_arguments = ""
            temp_list.write("<?xml version=\"1.0\"?>\n")
            temp_list.write("<experimental_setup>\n")
            temp_list.write("   <bambu_version value=\"" +
                            bambu_version + "\"/>\n")
            temp_list.write("   <timestamp value=\"" +
                            str(datetime.datetime.now()) + "\"/>\n")
            temp_list.write("   <script value=\"" + args.script + "\"/>\n")
            temp_list.write("   <bambu_arguments value=\"" +
                            bambu_arguments + "\"/>\n")
            ...
            for line in reordered_list.readlines():
                temp_list.write("      <benchmark value=\"" + line.rstrip().replace("\\\\\\-", "-").replace("\\\\\\=", "=").replace(
            ...
            if os.path.exists(args.spider_style):
                ...
                local_args = local_args + " " + \
                    os.path.join(os.path.dirname(spider),
...
def CreateJunitBody(directory, ju_file):
    ...
    if print_testsuite and len(subdirs) > 0:
        ju_file.write("  <testsuite disabled=\"0\" errors=\"0\" failures=\""+failed_counter+"\" name=\""+directory +
                      "\" tests=\"" + str(len(subdirs)) + "\" timestamp=\"" + str(datetime.datetime.now()) + "\">\n")
    for subdir in subdirs:
        if os.path.exists(os.path.join(directory, subdir, args.tool + "_return_value")):
            return_value_file_name = os.path.join(
                directory, subdir, args.tool + "_return_value")
            ...
            if return_value == "0":
                ...
                with open(os.path.join(directory, args.tool + "_failed_output")) as f:
                    for line in deque(f, maxlen=args.junit_logtail):
                        ju_file.write(line)
                ju_file.write("]]>\n")
                ju_file.write("      </system-out>\n")
...
def CreatePerfPublisherBody(directory, pp_file):
    ...
    for subdir in subdirs:
        if os.path.exists(os.path.join(directory, subdir, args.tool + "_return_value")):
            pp_file.write("  <test name=\"" + str(directory) +
                          "/" + str(subdir) + "\" executed=\"yes\">\n")
            ...
            if return_value == "0":
                ...
                if os.path.exists(os.path.join(directory, subdir, args.tool + "_results_0.xml")):
                    xml_document = minidom.parse(os.path.join(
                        directory, subdir, args.tool + "_results_0.xml"))
                    if len(xml_document.getElementsByTagName("CYCLES")) > 0:
                        cycles_tag = str(xml_document.getElementsByTagName(
                            "CYCLES")[0].attributes["value"].value)
                    if len(xml_document.getElementsByTagName("AREAxTIME")) > 0:
                        areatime_tag = str(xml_document.getElementsByTagName(
                            "AREAxTIME")[0].attributes["value"].value)
                    if len(xml_document.getElementsByTagName("SLICE")) > 0:
                        slice_tag = str(xml_document.getElementsByTagName(
                            "SLICE")[0].attributes["value"].value)
                    if len(xml_document.getElementsByTagName("SLICE_LUTS")) > 0:
                        sliceluts_tag = str(xml_document.getElementsByTagName(
                            "SLICE_LUTS")[0].attributes["value"].value)
                    if len(xml_document.getElementsByTagName("REGISTERS")) > 0:
                        registers_tag = str(xml_document.getElementsByTagName(
                            "REGISTERS")[0].attributes["value"].value)
                    if len(xml_document.getElementsByTagName("DSPS")) > 0:
                        dsps_tag = str(xml_document.getElementsByTagName(
                            "DSPS")[0].attributes["value"].value)
                    if len(xml_document.getElementsByTagName("BRAMS")) > 0:
                        brams_tag = str(xml_document.getElementsByTagName(
                            "BRAMS")[0].attributes["value"].value)
                    if len(xml_document.getElementsByTagName("PERIOD")) > 0:
                        period_tag = str(xml_document.getElementsByTagName(
                            "PERIOD")[0].attributes["value"].value)
                    if len(xml_document.getElementsByTagName("CLOCK_SLACK")) > 0:
                        slack_tag = str(xml_document.getElementsByTagName(
                            "CLOCK_SLACK")[0].attributes["value"].value)
                    if len(xml_document.getElementsByTagName("FREQUENCY")) > 0:
                        frequency_tag = str(xml_document.getElementsByTagName(
                            "FREQUENCY")[0].attributes["value"].value)
                    if len(xml_document.getElementsByTagName("HLS_execution_time")) > 0:
                        HLS_execution_time_tag = str(xml_document.getElementsByTagName(
                            "HLS_execution_time")[0].attributes["value"].value)

                if cycles_tag != "" and cycles_tag != "inf":
                    pp_file.write("      <performance unit=\"cycles\" mesure=\"" +
                                  cycles_tag + "\" isRelevant=\"true\"/>\n")
                if HLS_execution_time_tag != "":
                    pp_file.write("      <executiontime unit=\"s\" mesure=\"" +
                                  HLS_execution_time_tag + "\" isRelevant=\"true\"/>\n")
                ...
                if (areatime_tag != "" and areatime_tag != "inf") or (slice_tag != "" and slice_tag != "inf") or (sliceluts_tag != "" and sliceluts_tag != "inf") or (registers_tag != "" and registers_tag != "inf") or (dsps_tag != "" and dsps_tag != "inf") or (brams_tag != "" and brams_tag != "inf") or (period_tag != "" and period_tag != "inf") or (slack_tag != "" and slack_tag != "inf") or (frequency_tag != "" and frequency_tag != "inf"):
                    ...
                    if areatime_tag != "" and areatime_tag != "inf":
                        pp_file.write("        <areatime unit=\"lutxns\" mesure=\"" +
                                      areatime_tag + "\" isRelevant=\"true\"/>\n")
                    if slice_tag != "" and slice_tag != "inf":
                        pp_file.write("        <slices unit=\"ns\" mesure=\"" +
                                      slice_tag + "\" isRelevant=\"true\"/>\n")
                    if sliceluts_tag != "" and sliceluts_tag != "inf":
                        pp_file.write("        <sliceluts unit=\"slice\" mesure=\"" +
                                      sliceluts_tag + "\" isRelevant=\"true\"/>\n")
                    if registers_tag != "" and registers_tag != "inf":
                        pp_file.write("        <registers unit=\"registers\" mesure=\"" +
                                      registers_tag + "\" isRelevant=\"true\"/>\n")
                    if dsps_tag != "" and dsps_tag != "inf":
                        pp_file.write(
                            "        <dsps unit=\"dsp\" mesure=\"" + dsps_tag + "\" isRelevant=\"true\"/>\n")
                    if brams_tag != "" and brams_tag != "inf":
                        pp_file.write("        <brams unit=\"bram\" mesure=\"" +
                                      brams_tag + "\" isRelevant=\"true\"/>\n")
                    if period_tag != "" and period_tag != "inf":
                        pp_file.write("        <period unit=\"ns\" mesure=\"" +
                                      period_tag + "\" isRelevant=\"true\"/>\n")
                    if slack_tag != "" and slack_tag != "inf":
                        pp_file.write("        <slack unit=\"ns\" mesure=\"" +
                                      slack_tag + "\" isRelevant=\"true\"/>\n")
                    if frequency_tag != "" and frequency_tag != "inf":
                        pp_file.write("        <frequency unit=\"MHz\" mesure=\"" +
                                      frequency_tag + "\" isRelevant=\"true\"/>\n")
                    pp_file.write("      </metrics>\n")
            else:
                if return_value == "124":
                    pp_file.write(
...
parser = argparse.ArgumentParser(
    description="Performs panda tests", fromfile_prefix_chars='@')
parser.add_argument(
...
parser.add_argument("--spider", help="The spider executable (default=spider)",
                    default="spider")
parser.add_argument("--spider-style", help="The spider table style relative to the spider executable (default=../lib/latex_format_bambu_results.xml)",
                    default="../lib/latex_format_bambu_results.xml")
parser.add_argument(
    "--zebu", help="The zebu executable (default=zebu)", default="zebu")
parser.add_argument(
    '-t', "--timeout", help="Timeout for tool execution (default=75m)", default="75m")
parser.add_argument(
    '-a', "--args", help="A set of arguments to be passed to the tool", nargs='*', action='append')
parser.add_argument('-c', "--commonargs",
                    help="A set of arguments to be passed to the tool", nargs='*', action='append')
parser.add_argument(
    "--table", help="Print the results in tex format", default="results.tex")
...
parser.add_argument(
    "--name", help="Set the name of this regression (default=Bambu regression)", nargs='*', action='append')
...
parser.add_argument("--restart", help="Restart last execution (default=false)",
                    default=False, action="store_true")
parser.add_argument(
    "--script", help="Set the bash script in the generated tex", default="")
parser.add_argument("--junitdir", help="Set the JUnit directory", default="")
parser.add_argument(
    "--junit-logtail", help="Set the JUnit log tail length for failed tests (default=300)", default=300, type=positive_integer)
parser.add_argument("--perfpublisherdir",
                    help="Set the PerfPublisher directory", default="")
...
if args.junitdir != "":
    junit_index = 0
    junit_file_name = os.path.abspath(os.path.join(
        args.junitdir, "Junit_report"+str(junit_index)+".xml"))
...
if args.perfpublisherdir != "":
    ...
    perfpublisher_file_name = os.path.abspath(os.path.join(
        args.perfpublisherdir, perfpublisher_name+".xml"))
...
if args.benchmarks_root is None:
    ...
    if os.path.isabs(args.benchmarks_root):
        ...
        if os.path.exists(os.path.join(os.path.abspath(".."), args.benchmarks_root)):
            abs_benchmarks_root = os.path.join(
                os.path.abspath(".."), args.benchmarks_root)
        else:
            if os.path.exists(os.path.join(os.path.abspath(os.path.join(os.path.dirname(abs_script), "../../..")), args.benchmarks_root)):
                abs_benchmarks_root = os.path.join(os.path.abspath(os.path.join(
                    os.path.dirname(abs_script), "../../..")), args.benchmarks_root)
            else:
...
for thread_index in range(n_jobs):
    threads.insert(thread_index, threading.Thread(
        target=execute_tests, args=(named_list_name, thread_index)))
...
if args.junitdir != "":
    ...
    junit_file.write("<testsuites disabled=\"0\" errors=\"0\" failures=\""+str(total_benchmark -
                     passed_benchmark)+"\" name=\"" + abs_path + "\" tests=\"" + str(total_benchmark) + "\">\n")
...
if args.perfpublisherdir != "":
    ...
    perfpublisher_file.write(
        "<report name=\"" + perfpublisher_name + "\" categ=\"PandA-Report\">\n")
...
if args.tool == "bambu" or args.tool == "zebu":
    ...
    report_file.write(str(passed_benchmark) + "/" +
                      str(total_benchmark) + "\n\n")
