...
#define MAKE_ANN(binfo, gimple_val, statement_val) \
   (ZERO_OR_ONE(binfo) | (ZERO_OR_ONE(gimple_val) << 1) | (ZERO_OR_ONE(statement_val) << 2))
...
int tree_2_ints_eq(const void* va, const void* vb)
{
   const struct tree_2_ints *const a = (const struct tree_2_ints*)va, *const b = (const struct tree_2_ints*)vb;
   return (a->key == b->key && a->ann == b->ann);
}

/* Hash a from tree in a tree_2_ints.  */
unsigned int tree_2_ints_hash(const void* item)
{
   return htab_hash_pointer(((const struct tree_2_ints*)item)->key);
}

/* Return true if the DECL_UID in both trees are equal.  */
int dg_descriptor_tree_eq(const void* va, const void* vb)
{
...
unsigned int dg_descriptor_tree_hash(const void* item)
{
...
int tree_2_ints_marked_p(const void* p)
{
   return ggc_marked_p(((const struct tree_2_ints*)p)->key);
}

int dg_descriptor_tree_marked_p(const void* p)
{
...
#define serialize_gimple_child(field, child) queue_and_serialize_gimple_index(field, child)
/* Serialize the STATEMENT CHILD and its children.  */
#define serialize_statement_child(field, child) queue_and_serialize_statement_index(field, child)
 ...
 */
void SerializeGimpleUseDefs(const GIMPLE_type current, const GIMPLE_type next_def);
...
extern tree build_call_expr_internal_loc_array(location_t loc, internal_fn ifn, tree type, int n, const tree* args);

#endif
static void DumpGimpleWalker(void* arg ATTRIBUTE_UNUSED)
{
   if(di_local_nodes_index)
      ggc_mark(di_local_nodes_index);
   if(di_local_referenced_var_htab)
      ggc_mark(di_local_referenced_var_htab);
}
...
static void DumpGimpleTreeWalker(void* arg ATTRIBUTE_UNUSED)
{
}
...
#if(__GNUC__ > 4)
   loc = di_local_referenced_var_htab->find_slot_with_hash(&in, in.hash, INSERT);
#else
               loc = (struct dg_descriptor_tree**)htab_find_slot_with_hash(di_local_referenced_var_htab, &in, in.hash,
                                                                           INSERT);
#endif
...
#else
#if(__GNUC__ == 4 && __GNUC_MINOR__ == 6)
                  desc = ((struct dg_descriptor_tree*)(ggc_internal_alloc_stat(sizeof(struct dg_descriptor_tree)
                                                                                   MEM_STAT_INFO)));
#elif(__GNUC__ == 4 && __GNUC_MINOR__ > 6)
...
static void dump_pt_solution(struct pt_solution* pt, const char* first_tag, const char* second_tag)
{
   if(pt->anything)
...
static unsigned int queue(tree t)
{
   serialize_queue_p dq;
   unsigned int index;
   struct tree_2_ints in;
...
/* Add G to the end of the queue of nodes to serialize.  Returns the index
   assigned to G.  */
static unsigned int queue_gimple(GIMPLE_type g)
{
...
static void serialize_index(unsigned int index)
{
...
static void queue_and_serialize_gimple_index(const char* field, GIMPLE_type g)
{
   unsigned int index;
   struct tree_2_ints *n, in;
...
static void serialize_vops(GIMPLE_type gs)
{
   ...
   if(vuse != NULL_TREE)
   {
      ...
      for(ptr = other_uses->next; ptr != other_uses; ptr = ptr->next)
      {
         ...
         {
            /// Check if the operation containing the next vdef in the chain is the current one, we do not need to add
            /// anti dependency
...
static void serialize_string_cst(const char* field, const char* str, int length, unsigned int precision)
{
   int new_length;
   serialize_maybe_newline();
   if(precision == 8)
...
static void serialize_op(tree t)
{
#if(__GNUC__ > 7)
...
static void serialize_stmt(const_tree t)
{
...
static bool serialize_cpp_specifics(tree t)
{
   ...
   if(DECL_P(t))
   {
      ...
      if(DECL_LANG_SPECIFIC(templ_t))
      {
         ...
         if((TREE_CODE(templ) == VAR_DECL) || (TREE_CODE(templ) == FUNCTION_DECL) || (TREE_CODE(templ) == TYPE_DECL))
         {
   ...
   switch(code)
   {
      ...
      case UNION_TYPE:
         ...
         if(TYPE_CONTEXT(t) && TREE_CODE(TYPE_CONTEXT(t)) == TREE_CODE(t) && CLASSTYPE_AS_BASE(TYPE_CONTEXT(t)) == t)
         {
...
static void dequeue_and_serialize()
{
   ...
   if(TREE_CODE(t) == TARGET_MEM_REF)
    ...
    (__GNUC__ == 4 && __GNUC_MINOR__ >= 6 && __GNUC_PATCHLEVEL__ >= 1)
           ...
           TREE_CODE(TREE_TYPE(t)) == BOOLEAN_TYPE) /*incorrect encoding of not of a boolean expression*/
   ...
   if(IS_EXPR_CODE_CLASS(code_class))
   {
      ...
      {
         serialize_maybe_newline();
         fprintf(serialize_gimple_info.stream, "srcp: \"%s\":%-d:%-6d ", EXPR_FILENAME(t), EXPR_LINENO(t),
                 EXPR_COLUMNNO(t));
   ...
   else if(DECL_P(t))
   {
      ...
      if(HAS_DECL_ASSEMBLER_NAME_P(t) && DECL_ASSEMBLER_NAME_SET_P(t) && DECL_ASSEMBLER_NAME(t) != DECL_NAME(t))
   ...
   else if(code_class == tcc_type)
   {
      ...
      if(quals != TYPE_UNQUALIFIED)
      {
         if(quals & TYPE_QUAL_VOLATILE)
            fprintf(serialize_gimple_info.stream, "qual: %c%c%c     ", (quals & TYPE_QUAL_CONST) ? 'c' : ' ', 'v',
                    (quals & TYPE_QUAL_RESTRICT) ? 'r' : ' ');
         else
            fprintf(serialize_gimple_info.stream, "qual: %c%c     ", (quals & TYPE_QUAL_CONST) ? 'c' : ' ',
                    (quals & TYPE_QUAL_RESTRICT) ? 'r' : ' ');
         serialize_gimple_info.column += 14;
      }
   ...
   switch(code)
   {
      ...
      case SSA_NAME:
      {
...
#if(__GNUC__ > 4)
         if(!POINTER_TYPE_P(TREE_TYPE(t)) && SSA_NAME_RANGE_INFO(t))
         {
            wide_int min, max;
         ...
         {
            if(p_VRP_min[index_ssa])
               serialize_child("min", VRP_min_array[index_ssa]);
            if(p_VRP_max[index_ssa])
               serialize_child("max", VRP_max_array[index_ssa]);
         }
#endif
         break;
      }
      ...
      case RESULT_DECL:
         ...
         else if(TREE_READONLY(t) && TREE_CODE(t) == VAR_DECL && (TREE_STATIC(t) || DECL_EXTERNAL(t)))
         {
            ...
            if(node)
            {
               ...
               if(real_decl)
                  serialize_child("init", DECL_INITIAL(real_decl));
            }
         }
#endif
   ...
   switch(gimple_code(g))
   {
      ...
      case GIMPLE_ASSIGN:
         ...
         if(get_gimple_rhs_class(gimple_expr_code(g)) == GIMPLE_TERNARY_RHS)
         {
            ...
            serialize_child("op", build3(gimple_assign_rhs_code(g), TREE_TYPE(gimple_assign_lhs(g)),
                                         gimple_assign_rhs1(g), gimple_assign_rhs2(g), gimple_assign_rhs3(g)));
         }
         else
#endif
         ...
         {
            serialize_child("op", gimple_assign_lhs(g));
            serialize_child("op", build2(gimple_assign_rhs_code(g), TREE_TYPE(gimple_assign_lhs(g)),
         ...
         else if(get_gimple_rhs_class(gimple_expr_code(g)) == GIMPLE_UNARY_RHS)
         {
            serialize_child("op", gimple_assign_lhs(g));
            serialize_child("op",
                            build1(gimple_assign_rhs_code(g), TREE_TYPE(gimple_assign_lhs(g)), gimple_assign_rhs1(g)));
         }
...
tree gimple_call_expr_arglist(GIMPLE_type g)
{
   tree arglist = NULL_TREE;
   int i;
...
tree build_custom_function_call_expr(location_t loc, tree fn, tree arglist)
{
...
#if(__GNUC__ > 5)
tree build_custom_function_call_expr_internal_fn(location_t loc, internal_fn fn, tree type, tree arglist)
{
   int n = list_length(arglist);
   tree* argarray = (tree*)alloca(n * sizeof(tree));
   int i;
 ...
 */
static bool serialize_gimple_aliased_reaching_defs_1(ao_ref* dummy_1, tree vdef, void* dummy_2)
{
...
static void serialize_gimple_aliased_reaching_defs(GIMPLE_type stmt, tree ref)
{
   ...
   walk_aliased_vdefs(&refd, gimple_vuse(stmt), serialize_gimple_aliased_reaching_defs_1, gimple_bb(stmt), NULL);
}

static void serialize_all_gimple_aliased_reaching_defs(GIMPLE_type stmt)
{
   walk_aliased_vdefs(NULL, gimple_vuse(stmt), serialize_gimple_aliased_reaching_defs_1, NULL, NULL);
}

static bool serialize_gimple_aliased_reaching_defs_2(ao_ref* dummy_1, tree vdef, void* dummy_2)
{
   serialize_child("vover", vdef);
   return false;
}

static void serialize_gimple_aliased_reaching_defsdefs(GIMPLE_type stmt, tree ref)
{
   ao_ref refd;
   ao_ref_init(&refd, ref);
   walk_aliased_vdefs(&refd, gimple_vuse(stmt), serialize_gimple_aliased_reaching_defs_2, gimple_bb(stmt), NULL);
}
...
static void serialize_gimple_dependent_stmts_load(GIMPLE_type gs)
{
   ...
   if(is_gimple_call(gs))
   {
      ...
      if(callee != NULL_TREE && DECL_BUILT_IN_CLASS(callee) == BUILT_IN_NORMAL &&
         (DECL_FUNCTION_CODE(callee) == BUILT_IN_MEMSET || DECL_FUNCTION_CODE(callee) == BUILT_IN_MALLOC ||
          DECL_FUNCTION_CODE(callee) == BUILT_IN_FREE
#if(__GNUC__ > 4) || (__GNUC__ == 4 && __GNUC_MINOR__ >= 7)
          || DECL_FUNCTION_CODE(callee) == BUILT_IN_MEMSET_CHK || DECL_FUNCTION_CODE(callee) == BUILT_IN_CALLOC ||
          DECL_FUNCTION_CODE(callee) == BUILT_IN_VA_END || DECL_FUNCTION_CODE(callee) == BUILT_IN_ALLOCA ||
          (DECL_FUNCTION_CODE(callee) == BUILT_IN_ALLOCA_WITH_ALIGN) ||
          DECL_FUNCTION_CODE(callee) == BUILT_IN_STACK_SAVE || DECL_FUNCTION_CODE(callee) == BUILT_IN_STACK_RESTORE ||
   ...
   else if(gimple_assign_single_p(gs))
   {
      ...
      if(TREE_CODE(rhs) != SSA_NAME && !is_gimple_min_invariant(rhs) && TREE_CODE(rhs) != CONSTRUCTOR)
      {
   ...
   else if(gimple_code(gs) == GIMPLE_RETURN)
   {
      ...
      if(rhs && TREE_CODE(rhs) != SSA_NAME && !is_gimple_min_invariant(rhs) && TREE_CODE(rhs) != CONSTRUCTOR)
      {
         serialize_gimple_aliased_reaching_defs(gs, rhs);
      }
   }
...
static bool gimples_may_conflict(GIMPLE_type gs, GIMPLE_type next_gs)
{
   tree *op0, *op0_next, *op1 = 0;

   if((gimple_code(next_gs) == GIMPLE_CALL && !(gimple_call_flags(next_gs) & (ECF_CONST | ECF_PURE))) ||
      (gimple_code(next_gs) == GIMPLE_ASM && (gimple_asm_volatile_p(PTRCONV(gasm*, next_gs)) || gimple_vuse(next_gs))))
      return true;
   ...
   if((gimple_code(gs) == GIMPLE_CALL && !(gimple_call_flags(gs) & (ECF_CONST | ECF_PURE))) ||
      (gimple_code(gs) == GIMPLE_ASM && (gimple_asm_volatile_p(PTRCONV(gasm*, gs)) || gimple_vuse(gs))))
      return true;
   ...
   if((op0 && (DECL_P(*op0) || (REFERENCE_CLASS_P(*op0) && get_base_address(*op0)))) &&
      (op0_next && (DECL_P(*op0_next) || (REFERENCE_CLASS_P(*op0_next) && get_base_address(*op0_next)))))
   {
   ...
   if((op1 && (DECL_P(*op1) || (REFERENCE_CLASS_P(*op1) && get_base_address(*op1)))) &&
      (op0_next && (DECL_P(*op0_next) || (REFERENCE_CLASS_P(*op0_next) && get_base_address(*op0_next)))))
   {
      ao_ref refd, refd_next;
 ...
 * @param gs is the other gimple
 */
void SerializeGimpleUseDefs(GIMPLE_type current, GIMPLE_type next_def)
{
...
bool IsPragma(const GIMPLE_type gs)
{
   if(gimple_code(gs) == GIMPLE_CALL)
   {
      ...
      if((strcmp(called_function_name, "__pragma_start__\0") == 0) ||
         (strcmp(called_function_name, "__pragma_end__\0") == 0) ||
         (strcmp(called_function_name, "__pragma_single_line_one_argument__\0") == 0))
      {
         return true;
      }
      return false;
...
static void add_referenced_var_map(tree var)
{
   ...
   struct dg_descriptor_tree** loc;
   struct dg_descriptor_tree* desc;
   gcc_assert(TREE_CODE(var) == VAR_DECL || TREE_CODE(var) == PARM_DECL || TREE_CODE(var) == RESULT_DECL);
   in.hash = uid;
   in.vd = var;
#if(__GNUC__ > 4)
   h = di_local_referenced_var_htab->find_with_hash(&in, uid);
...
#if(__GNUC__ > 4)
   loc = di_local_referenced_var_htab->find_slot_with_hash(&in, uid, INSERT);
#else
         loc = (struct dg_descriptor_tree**)htab_find_slot_with_hash(di_local_referenced_var_htab, &in, uid, INSERT);
#endif
...
#if(__GNUC__ == 4 && __GNUC_MINOR__ == 6)
            desc = ((
                struct dg_descriptor_tree*)(ggc_internal_alloc_stat(sizeof(struct dg_descriptor_tree) MEM_STAT_INFO)));
#elif(__GNUC__ == 4 && __GNUC_MINOR__ > 6)
            desc = ggc_alloc_dg_descriptor_tree();
#else
            desc = (struct dg_descriptor_tree*)ggc_alloc(sizeof(struct dg_descriptor_tree));
