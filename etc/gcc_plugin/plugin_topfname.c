...
static bool cgraph_non_local_node_p_1(struct cgraph_node* node, void* data ATTRIBUTE_UNUSED)
{
   /* FIXME: Aliases can be local, but i386 gets thunks wrong then.  */
   return !(cgraph_only_called_directly_or_aliased_p(node) && !ipa_ref_has_aliases_p(&node->ref_list) &&
            node->definition && !DECL_EXTERNAL(node->decl) && !node->externally_visible &&
            !node->used_from_other_partition && !node->in_other_partition);
}

/* Return true when function can be marked local.  */

static bool cgraph_local_node_p(struct cgraph_node* node)
{
   ...
   if(n->thunk.thunk_p)
      return false;
   return !cgraph_for_node_and_aliases(n, cgraph_non_local_node_p_1, NULL, true);
}
...
static bool comdat_can_be_unshared_p_1(symtab_node* node)
{
   ...
   if(!DECL_VIRTUAL_P(node->decl) &&
      (TREE_CODE(node->decl) != FUNCTION_DECL ||
       (!DECL_CXX_CONSTRUCTOR_P(node->decl) && !DECL_CXX_DESTRUCTOR_P(node->decl))) &&
   ...
   if(is_a<varpool_node>(node) && (!TREE_READONLY(node->decl) || TREE_THIS_VOLATILE(node->decl)))
      return false;
   return true;
}
...
static bool cgraph_externally_visible_p(struct cgraph_node* node)
{
   ...
   if(lookup_attribute("externally_visible", DECL_ATTRIBUTES(node->decl)))
      return true;
   if(TARGET_DLLIMPORT_DECL_ATTRIBUTES && lookup_attribute("dllexport", DECL_ATTRIBUTES(node->decl)))
      return true;
   ...
   if(DECL_COMDAT(node->decl) && comdat_can_be_unshared_p(node))
      return false;
...
{
   return (symtab_node_availability(node) > AVAIL_OVERWRITABLE && !symtab_can_be_discarded(node));
}

/* Return true when variable VNODE should be considered externally visible.  */

static bool varpool_externally_visible_p_local(varpool_node* vnode)
{
   ...
   if(DECL_PRESERVE_P(vnode->decl))
      return true;
   if(lookup_attribute("externally_visible", DECL_ATTRIBUTES(vnode->decl)))
      return true;
   if(TARGET_DLLIMPORT_DECL_ATTRIBUTES && lookup_attribute("dllexport", DECL_ATTRIBUTES(vnode->decl)))
      return true;
   ...
   if(DECL_COMDAT(vnode->decl) && comdat_can_be_unshared_p(vnode))
      return false;
...
static unsigned int function_and_variable_visibility()
{
   ...
   FOR_EACH_FUNCTION(node)
   {
      ...
      if(optimize && (flags & (ECF_CONST | ECF_PURE)) && !(flags & ECF_LOOPING_CONST_OR_PURE))
      {
    ...
    what comdat group they are in when they won't be emitted in this TU.  */
      ...
      gcc_assert((!DECL_WEAK(node->decl) && !DECL_COMDAT(node->decl)) || TREE_PUBLIC(node->decl) || node->weakref ||
      ...
      if(!node->externally_visible && node->definition && !node->weakref && !DECL_EXTERNAL(node->decl))
      {
      ...
      if(node->thunk.thunk_p && TREE_PUBLIC(node->decl))
      {
         ...
         {
            gcc_checking_assert(DECL_COMDAT(node->decl) == DECL_COMDAT(decl_node->decl));
            gcc_checking_assert(DECL_COMDAT_GROUP(node->decl) == DECL_COMDAT_GROUP(decl_node->decl));
    ...
    Later we may bring the symbols static if they are not exported.  */
         ...
         (node->resolution == LDPR_PREVAILING_DEF_IRONLY || node->resolution == LDPR_PREVAILING_DEF_IRONLY_EXP))
      {
         ...
         if(node->same_comdat_group)
            for(next = node->same_comdat_group; next != node; next = next->same_comdat_group)
               if(next->externally_visible && (next->resolution != LDPR_PREVAILING_DEF_IRONLY &&
                                               next->resolution != LDPR_PREVAILING_DEF_IRONLY_EXP))
                  break;
         if(node == next)
         {
   ...
   FOR_EACH_VARIABLE(vnode)
   {
      ...
      gcc_assert(!DECL_WEAK(vnode->decl) || vnode->weakref || TREE_PUBLIC(vnode->decl) || DECL_EXTERNAL(vnode->decl));
    ...
    Canonicalize things here and clear the redundant flag.  */
      if(DECL_COMMON(vnode->decl) &&
         (!(TREE_PUBLIC(vnode->decl) || DECL_EXTERNAL(vnode->decl)) ||
          (DECL_INITIAL(vnode->decl) && DECL_INITIAL(vnode->decl) != error_mark_node) || DECL_WEAK(vnode->decl) ||
          DECL_SECTION_NAME(vnode->decl) != NULL || !(ADDR_SPACE_GENERIC_P(TYPE_ADDR_SPACE(TREE_TYPE(vnode->decl))))))
   ...
   FOR_EACH_DEFINED_VARIABLE(vnode)
   {
      ...
      if(!vnode->externally_visible && !vnode->weakref)
      {
...
static unsigned int whole_program_function_and_variable_visibility(void)
{
   printf("optimize\n");
...
#else
static struct ipa_opt_pass_d pass_topfname =
#endif
   {
   ...
   class pass_topfname : public ipa_opt_pass_d
   {
    public:
      pass_topfname(gcc::context* ctxt)
          : ipa_opt_pass_d(pass_data_topfname, ctxt, NULL, /* generate_summary */
                           NULL,                           /* write_summary */
                           NULL,                           /* read_summary */
                           NULL,                           /* write_optimization_summary */
                           NULL,                           /* read_optimization_summary */
...
ipa_opt_pass_d* make_pass_topfname(gcc::context* ctxt)
{
