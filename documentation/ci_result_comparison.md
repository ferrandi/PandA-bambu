...
Comparability reason codes are derived canonically from the check field. `task_id` maps to `task-id-differs`, `task_type` maps to `task-type-differs`, every supported configuration path maps to `configuration-differs`, and `build_profile` maps to `build-profile-differs`. The serialized `reason_code` is retained for readability only; standalone validation rejects unknown, missing, duplicated, reordered, or relabeled check fields and recomputes the expected reason code from each canonical field.
