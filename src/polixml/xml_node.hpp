...
class xml_node
{
   ...
   public:
      ...
      */
       ...
       */
      static
      void convert_escaped(std::string& ioString)
      {
         std::string::size_type lPos = 0;
...
class xml_child : public xml_node
{
   public:
       ...
       */
      xml_text_node* get_child_text() {return first_text;}

      /** Get the first child text content node.
       * This is a convenience method, meant as an alternative to iterating over all the child nodes to find the first suitable node then and getting the text directly.
       * @returns The first text node, if any.
       */
      const xml_text_node* get_child_text() const {return first_text;}
       ...
       */
      node_list const & get_children() {return child_list;}

      /** Obtain the list of child nodes.
       * @returns The list of child nodes.
       */
      const node_list& get_children() const {return child_list;}

      /**
       * @returns Whether this node has children, or is empty.
       */
      bool has_child() const {return !child_list.empty();}
