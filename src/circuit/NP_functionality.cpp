...
{
   return (descriptions.find(type) != descriptions.end() && descriptions.find(type)->second != "");
}
...
std::string NP_functionality::get_library_name() const
{
  ...
  if(library_description == "") return library_description;
...
NP_functionality::NP_functionality(const NP_functionalityRef& obj)
{
  for(unsigned int i = 0; i < UNKNOWN; i++)
  {
     ...
     if (val != "") descriptions[static_cast<NP_functionaly_type>(i)] = val;
  } 
}
