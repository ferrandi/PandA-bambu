...
void structural_manager::reconnect_signal_member(structural_objectRef &member, structural_objectRef &from_signal, structural_objectRef &to_signal)
{
   ...
   PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "moving member " + member->get_path() +
                 " from " + from_signal->get_path() + " to " + to_signal->get_path());
...
void structural_manager::INIT(bool permissive)
{
   ...
   if (og) delete og;
   if (data_graph) delete data_graph;
   if (circuit_graph) delete circuit_graph;
...
void circuit_add_edge(typename boost::graph_traits<Graph>::vertex_descriptor A, typename boost::graph_traits<Graph>::vertex_descriptor B, int selector, Graph& g, const structural_objectRef from1, structural_objectRef to1, bool is_critical = false)
{
   ...
   {
      if (boost::target(*oi, g) != B) continue;
...
static void add_directed_edge_single(graphs_collection *bg, const std::map<structural_objectRef, boost::graph_traits<graphs_collection>::vertex_descriptor> &module_vertex_rel, const structural_objectRef &p1, const structural_objectRef &p2, boost::graph_traits<graphs_collection>::vertex_descriptor en, boost::graph_traits<graphs_collection>::vertex_descriptor ex, bool is_critical = false)
{
   ...
   if (owner1 && owner1->get_kind() == port_vector_o_K) owner1 = owner1->get_owner();
   if (owner2 && owner2->get_kind() == port_vector_o_K) owner2 = owner2->get_owner();

   int edge_type = DATA_SELECTOR;
   if (GetPointer<port_o>(p_obj2)->get_is_clock() || GetPointer<port_o>(p_obj2)->get_is_clock()) edge_type = CLOCK_SELECTOR;
...
void structural_manager::change_connection(structural_objectRef old_obj, structural_objectRef new_obj, structural_objectRef owner)
{
   ...
   for(unsigned int i = 0; i < p_old->get_connections_size(); i++)
   {
      ...
      if (GetPointer<signal_o>(conn_comp) and  GetPointer<signal_o>(conn_comp)->get_owner()->get_kind() != signal_vector_o_K and conn_comp->get_owner() != owner) continue;
      if (GetPointer<port_o>(conn_comp) and conn_comp->get_owner() != owner and conn_comp->get_owner()->get_owner() != owner and (conn_comp->get_owner()->get_kind() != port_vector_o_K or conn_comp->get_owner()->get_owner() != owner)) continue;
