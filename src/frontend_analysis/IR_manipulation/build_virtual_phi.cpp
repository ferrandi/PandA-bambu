      ...
      while(to_be_processed.size())
      {
         ...
         else
         {
            ...
            for(boost::tie(ie, ie_end) = boost::in_edges(current, *basic_block_graph); ie != ie_end; ie++)
            {
               ...
               {
                  THROW_ASSERT(reaching_defs.find(virtual_ssa_definition.first) != reaching_defs.end() and reaching_defs.find(virtual_ssa_definition.first)->second.find(source) != reaching_defs.find(virtual_ssa_definition.first)->second.end(), "Definition coming from BB"+ STR(basic_block_graph->CGetBBNodeInfo(source)->block->number));
         ...
         for(boost::tie(oe, oe_end) = boost::out_edges(current, *basic_block_graph); oe != oe_end; oe++)
         {
            ...
            if(loop_basic_blocks.find(target) != loop_basic_blocks.end())
            {
               ...
               if((basic_block_graph->GetSelector(*oe) & FB_CFG_SELECTOR) != 0)
               {
                  ...
                  {
                     THROW_ASSERT(added_phis.find(virtual_ssa_definition.first) != added_phis.end() and added_phis.find(virtual_ssa_definition.first)->second.find(target) != added_phis.find(virtual_ssa_definition.first)->second.end(), "Phi for " + STR(virtual_ssa_definition.first) + " was not created in BB" + STR(basic_block_graph->CGetBBNodeInfo(target)->block->number));
                     GetPointer<gimple_phi>(GET_NODE(added_phis.find(virtual_ssa_definition.first)->second.find(target)->second))->AddDefEdge(TM, gimple_phi::DefEdge(reaching_defs[virtual_ssa_definition.first][current], basic_block_graph->CGetBBNodeInfo(current)->block->number));
