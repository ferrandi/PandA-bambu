...
FunctionCallTypeCleanup::FunctionCallTypeCleanup(const ParameterConstRef Param,
   ...
   for (auto block : sl->list_of_bloc)
   {
      ...
      for (const auto & stmt : block.second->CGetStmtList())
      {
            ...
            if (code1 == call_expr_K || code1 == aggr_init_expr_K)
            {
               ...
               if (GET_NODE(ce->fn)->get_kind() == addr_expr_K)
               {
                  ...
                  INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                        "---is a call_expr with LHS " + GET_NODE(ga->op0)->ToString());
                  INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                        "---the called function returns type " + ret_type_node->ToString());
                  if (code0 == ssa_name_K)
                  {
                     const tree_nodeConstRef assigned_ssa_type_node =
                        tree_helper::CGetType(GET_NODE(ga->op0));
                     INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                           "---the assigned ssa_name " + STR(GET_NODE(ga->op0)) +
                           " has type " + assigned_ssa_type_node->ToString());
                     if (conv_really_needed(TM, ret_type_node->index, assigned_ssa_type_node->index))
                     {
                        const tree_nodeRef new_ssa =
                           tree_man->create_ssa_name(tree_nodeRef(), ret_type_node);
                        INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                              "---create ssa " + GET_NODE(new_ssa)->ToString());

                        const auto ga_nop =
                           tree_man->CreateNopExpr(new_ssa, assigned_ssa_type_node);
                        INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                        ...
                        INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                              "---update call " + GET_NODE(stmt)->ToString());

                        block.second->PushAfter(ga_nop, stmt);
                        INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                              "---insert nop " + GET_NODE(ga_nop)->ToString());
                        changed = true;
                     }
                  }
                  ...
                  if (called_body_fun_ids.find(called_id) != called_body_fun_ids.end())
                  {
                     ...
                     {
                        if (GET_NODE(*arg_it)->get_kind() == integer_cst_K or
                              GET_NODE(*arg_it)->get_kind() == ssa_name_K)
                        {
         ...
         else if (GET_NODE(stmt)->get_kind() == gimple_call_K)
         {
            ...
            {
               THROW_UNREACHABLE("call node  " + STR(GET_NODE(gc->fn)) +
                  " is a " + GET_NODE(gc->fn)->get_kind_text());
            }
         }
...
const std::unordered_set<std::pair<FrontendFlowStepType, FrontendFlowStep::FunctionRelationship> >
   FunctionCallTypeCleanup::ComputeFrontendRelationships(
         const DesignFlowStep::RelationshipType relationship_type) const
{
   std::unordered_set<std::pair<FrontendFlowStepType, FrontendFlowStep::FunctionRelationship> > relationships;
