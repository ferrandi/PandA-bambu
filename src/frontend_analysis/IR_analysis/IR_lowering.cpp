...
tree_nodeRef IR_lowering::expand_sdiv_pow2(const tree_nodeRef& op0, unsigned long long int d, const tree_nodeRef& stmt, const blocRef& block, const tree_nodeRef& type, const std::string& srcp_default)
{
   ...
   if(d == 2)
   {
      ...
      t2_ga = tree_man->CreateGimpleAssign(type, nullptr, nullptr, sum_expr, function_id, block->number, srcp_default);
   }
   else
   {
      ...
      const auto cond_op = tree_man->create_ternary_operation(type, cond_op0_ga_var, t_ga_var, op0, srcp_default, cond_expr_K);
      t2_ga = tree_man->CreateGimpleAssign(type, nullptr, nullptr, cond_op, function_id, block->number, srcp_default);
   }
...
tree_nodeRef IR_lowering::expand_MC(const tree_nodeRef& op0, const integer_cst* ic_node, const tree_nodeRef& old_target, const tree_nodeRef& stmt, const blocRef& block, const tree_nodeRef& type_expr, const std::string& srcp_default)
{
   ...
   else
   {
      if(ext_op1 < 0)
      {
         ...
         short int max_cost = mult_plus_ratio; // static_cast<short int>(tree_helper::Size(tree_helper::CGetType(type_expr))/mult_cost_divisor);
...
tree_nodeRef IR_lowering::array_ref_lowering(array_ref* AR, const std::string& srcp_default, std::pair<unsigned int, blocRef> block, std::list<tree_nodeRef>::const_iterator it_los, bool temp_addr)
{
   ...
   THROW_ASSERT(GET_CONST_NODE(ar_op0_type_node)->get_kind() == array_type_K, "array_type expected: @" + STR(ar_op0_type_node->index));
