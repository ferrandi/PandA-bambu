...
unsigned int Vectorize::DuplicateIncrement(const unsigned int loop_id, const tree_nodeRef statement)
{
   ...
   if(use_stmts.size() > 1)
   {
      ...
      if(GET_CONST_NODE(header_guard_tn)->get_kind() != integer_cst_K)
      {
         ...
         bb_graph->GetBBNodeInfo(*(loop->exit_block_iter_begin()))->block->PushBefore(TM->GetTreeReindex(new_cond_computation), feedback_condition_sn->CGetDefStmt(), AppM);
      }
      else
      {
         bb_graph->GetBBNodeInfo(*(loop->exit_block_iter_begin()))->block->PushBack(TM->GetTreeReindex(new_cond_computation), AppM);
      }
   ...
   for(const auto basic_block : basic_blocks)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            const auto combined_condition = control_condition->index != true_value->index ? tree_man->CreateAndExpr(control_condition, edge_condition, blocRef(), function_id) : edge_condition;
            if(control_condition->index != true_value->index)
            {
               bb_node_info->block->PushFront(GetPointer<const ssa_name>(GET_NODE(combined_condition))->CGetDefStmt(), AppM);
            }
            if(edge_condition != gc->op0)
            {
               bb_node_info->block->PushFront(GetPointer<const ssa_name>(GET_NODE(edge_condition))->CGetDefStmt(), AppM);
            }
               ...
               for(boost::tie(oe, oe_end) = boost::out_edges(basic_block, *fcfg_bb_graph); oe != oe_end; oe++)
               {
                  if(fcfg_bb_graph->GetSelector(*oe) & FB_CFG_SELECTOR)
                  {
                     ...
                     if(basic_block_divergence.find(target_id)->second)
                     {
                        ...
                        const auto combined_condition = current_condition->index != true_value->index ? tree_man->CreateAndExpr(current_condition, feedback_condition, bb_node_info->block, function_id) : gc->op0;
      ...
      if(basic_block_divergence.find(basic_block_index) != basic_block_divergence.end() and basic_block_divergence.find(basic_block_index)->second)
      {
         ...
         for(const auto& stmt : bb_node_info->block->CGetStmtList())
         {
            ...
            if(ga && ga->predicate)
            {
               ...
               const auto sn = GetPointer<ssa_name>(GET_NODE(ga->predicate));
               if(sn)
               {
                  sn->AddUseStmt(stmt);
               }
   ...
   switch(transformation)
   {
      ...
      case(INIT):
      {
         ...
         for(const auto& def_edge : gp->CGetDefEdgesList())
         {
            if(def_edge.second == previous_id)
            {
               if(GET_NODE(def_edge.first)->get_kind() == integer_cst_K)
               {
                  ...
                  THROW_ASSERT(iv_increment.count(gp->res->index), "Increment variable of " + gp->res->ToString() + " is unknown");
               ...
               else if(GET_NODE(def_edge.first)->get_kind() == ssa_name_K)
               {
                  ...
                  for(size_t i = 1; i <= parallel_degree; i++)
                  {
                     ...
                     const auto new_ic = TM->CreateUniqueIntegerCst(increment, tree_man->create_default_unsigned_integer_type());
                  ...
                  {
                     const auto new_ic = TM->CreateUniqueIntegerCst(static_cast<long long int>(scalar - 1), tree_man->create_default_unsigned_integer_type());
         ...
         for(size_t scalar = 1; scalar <= parallel_degree; scalar++)
         {
            ...
            const auto offset = TM->CreateUniqueIntegerCst(((static_cast<long long int>(scalar) - 1) * bit_size), tree_man->create_default_unsigned_integer_type());
            const auto size = TM->CreateUniqueIntegerCst(static_cast<long long int>(bit_size), tree_man->create_default_unsigned_integer_type());
