   ...
   switch(transformation)
   {
      ...
      case(INIT):
      {
         ...
         for(size_t scalar = 1; scalar <= parallel_degree; scalar++)
         {
            ...
            const auto bit_size = element_type->get_kind() != boolean_type_K ? tree_helper::Size(element_type) : 32;
         ...
         switch(tn->get_kind())
         {
            case gimple_assign_K:
            {
               ...
               for(size_t scalar = 1; scalar <= parallel_degree; scalar++)
               {
                  ...
                  /// vector of Boolean types are mapped on vector of integer
                  const auto bit_size =
                      element_type->get_kind() != boolean_type_K ? tree_helper::Size(element_type) : 32;
                  const auto offset = TM->CreateUniqueIntegerCst(static_cast<long long int>((scalar - 1) * bit_size),
                                                                 tree_man->GetUnsignedIntegerType());
            ...
            case gimple_phi_K:
            {
               ...
               if(!gp->virtual_flag)
               {
                  ...
                  for(size_t scalar = 1; scalar <= parallel_degree; scalar++)
                  {
                     ...
                     /// vector of Boolean types are mapped on vector of integer
                     const auto bit_size =
                         element_type->get_kind() != boolean_type_K ? tree_helper::Size(element_type) : 32U;
                     const auto offset = TM->CreateUniqueIntegerCst(static_cast<long long int>((scalar - 1) * bit_size),
                                                                    tree_man->GetUnsignedIntegerType());
                     const auto size = TM->CreateUniqueIntegerCst(static_cast<long long int>(bit_size),
                                                                  tree_man->GetUnsignedIntegerType());
