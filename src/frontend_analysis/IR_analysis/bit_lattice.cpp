...
std::deque<bit_lattice> BitLatticeManipulator::sup(
      const std::deque<bit_lattice> & _a,
      const std::deque<bit_lattice> & _b,
      const unsigned int output_uid) const
{
   ...
   {
      THROW_UNREACHABLE("unexpected sup for output_uid " +
            STR(output_uid) + " of kind " + node->get_kind_text());
   }
   THROW_ASSERT(out_type_size, "");
   const bool out_is_bool = tree_helper::is_bool(TM, output_uid);
   THROW_ASSERT(not out_is_bool or (out_type_size == 1),
   ...
   if (out_is_signed)
   {
      ...
      const size_t final_size = std::min(out_type_size,
		      ((a_sign_is_x == b_sign_is_x) ? (std::min(_a.size(), _b.size())) : (a_sign_is_x ? _a.size() : _b.size())));
...
std::deque<bit_lattice> BitLatticeManipulator::inf(
      const std::deque<bit_lattice> & a,
      const std::deque<bit_lattice> & b,
      const unsigned int output_uid) const
{
   THROW_ASSERT(not (a.empty() and b.empty()),
         "a.size() = " + STR(a.size()) + " b.size() = " + STR(b.size()));
   ...
   bool out_is_signed = signed_var.find(output_uid) != signed_var.end() or
      tree_helper::is_int(TM, output_uid);
...
{
   return not tree_helper::is_real(TM, type_id) and
      not tree_helper::is_a_complex(TM, type_id) and
      not tree_helper::is_a_vector(TM, type_id) and
      not tree_helper::is_a_struct(TM, type_id);
}
