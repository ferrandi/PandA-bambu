   ...
   for(const auto function : functions)
   {
      ...
      for(boost::tie(ei, ei_end) = boost::out_edges(CG->GetVertex(function), *cg); ei != ei_end; ++ei)
      {
         ...
         else
         {
            ...
            else if(fun_name != "llabs" && fun_name != "labs" && fun_name != "abs" && fun_name != "putchar" && fun_name != "__builtin_putchar" && fun_name != "__bambu_read4c" && fun_name != "__bambu_readc" && fun_name != "abort" && fun_name != "exit")
            {
               ...
               if(ssa)
               {
                  ...
                  else if(GET_NODE(ga->op1)->get_kind() == call_expr_K || GET_NODE(ga->op1)->get_kind() == aggr_init_expr_K)
                  {
                     ...
                     if(not called_fd->body)
                     {
                        ...
                        else if(fun_name != "llabs" && fun_name != "labs" && fun_name != "abs" && fun_name != "putchar" && fun_name != "__builtin_putchar" && fun_name != "__bambu_read4c" && fun_name != "__bambu_readc")
                        {
