...
static tree_nodeRef extractOp1(tree_nodeRef opSSA)
{
   ...
   if(opSSA_def_stmt->get_kind() == gimple_nop_K ||
         opSSA_def_stmt->get_kind() == gimple_phi_K)
      return tree_nodeRef();
...
bool rebuild_initialization2::extract_var_decl_ppe(tree_nodeRef addr_assign_op1, unsigned &vd_index, tree_nodeRef &vd_node)
{
   ...
   if(!addr2_assign_op1)
      return false;
   if(addr2_assign_op1->get_kind()==view_convert_expr_K ||
         addr2_assign_op1->get_kind()==nop_expr_K)
   {
...
bool rebuild_initialization2::extract_var_decl(const mem_ref * me, unsigned &vd_index, tree_nodeRef &vd_node, tree_nodeRef & addr_assign_op1)
{
   ...
   else if(addr_assign_op1->get_kind() == ssa_name_K)
   {
      ...
      return varFound(addr_assign_op1,vd_index,vd_node);
           ...
           addr_assign_op1->get_kind() == nop_expr_K ||
...
tree_nodeRef getAssign(tree_nodeRef SSAop, unsigned vd_index, std::set<unsigned> &nonConstantVars, TreeNodeMap<std::map<long long int, tree_nodeRef> > &inits, tree_managerRef TM)
{
   ...
   if(ssa_def_stmt->get_kind() == gimple_nop_K ||
         ssa_def_stmt->get_kind() == gimple_phi_K)
   {
