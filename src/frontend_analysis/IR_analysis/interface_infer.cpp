                                           ...
                                           const std::list<tree_nodeRef>& usedStmt_defs, const tree_manipulationRef tree_man, const tree_managerRef TM, bool commonRWSignature)
{
   ...
   if(commonRWSignature)
   {
      ...
      const auto data_value = tree_man->CreateIntegerCst(readType, 0, data_value_id);
                                            ...
                                            unsigned n_resources, unsigned alignment)
{
   ...
   if(!TechMan->is_library_manager(INTERFACE_LIBRARY) || !TechMan->get_library_manager(INTERFACE_LIBRARY)->is_fu(ResourceName))
   {
      ...
      auto selPort = CM->add_port_vector("in1", port_o::IN, n_resources, interface_top, size1);
      auto sizePort = CM->add_port_vector("in2", port_o::IN, n_resources, interface_top, size1);
      auto dataPort = CM->add_port_vector("in3", port_o::IN, n_resources, interface_top, size1);
      auto addrPort = CM->add_port_vector("in4", port_o::IN, n_resources, interface_top, word_bool_type);
      ...
      technology_nodeRef bram_f_unit = TechMan->get_fu(ARRAY_1D_STD_BRAM_SDS, LIBRARY_STD_FU);
                     ...
                     else
                     {
                        ...
                        else if(isRead && !isWrite)
                        {
                           ...
                           for(auto rs : readStmt)
                           {
                              ...
                              create_Read_function(argName_string, rs, rs_ga->bb_index, sl, fd, instanceFname, argSSANode, a, GetPointer<mem_ref>(GET_NODE(rs_ga->op1))->type, usedStmt_defs, tree_man, TM, commonRWSignature);
                              addGimpleNOPxVirtual(rs, sl, TM);
                              usedStmt_defs.clear();
                              ++loadIdIndex;
                           }
