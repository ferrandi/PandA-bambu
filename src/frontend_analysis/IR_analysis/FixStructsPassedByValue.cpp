      ...
      )
{
   ...
   if (p_type_head and GetPointer<const void_type>(GET_CONST_NODE(p_type_head)))
   {
      ...
      THROW_ASSERT(fd->list_of_args.empty(),
            "function " + fu_name + " has void parameter type but has a parm_decl " +
   ...
   if (p_type_head and fd->list_of_args.empty())
   {
      while (p_type_head)
      {
         ...
         THROW_ASSERT(not tree_helper::is_a_struct(TM, p->valu->index) and
               ...
               "function " + fu_name + " has no parm_decl but in its "
               "function_type it takes a struct type " + STR(GET_NODE(p->valu)));
      }
      return true;
   }
   return false;
...
DesignFlowStep_Status FixStructsPassedByValue::InternalExec()
{
   ...
   if (not cannot_have_struct_parameters(fd, fu_type, TM, fu_name))
   {
      INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "-->Fixing declaration of function " + fu_name);
      ...
      for (; p_decl_it != fd->list_of_args.cend(); p_decl_it++, param_n++)
      {
         ...
         THROW_ASSERT(static_cast<bool>(has_param_types) == static_cast<bool>(p_type_head),
               "function " + fu_name + " has " + STR(fd->list_of_args.size()) +
         ...
         if (tree_helper::is_an_union(TM, p_type->index) or tree_helper::is_a_struct(TM, p_type->index))
         {
            ...
            const auto new_local_var_decl = tree_man->create_var_decl(
                  local_var_identifier, TM->CGetTreeReindex(p_type->index), pd->scpe, pd->size,
                  tree_nodeRef(), tree_nodeRef(), srcp,
                  GetPointer<const type_node>(p_type)->algn, pd->used,
                  false); // artificial flag (should be true???)
            INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                  "<--Created new local var_decl");

            // substitute var_decl to parm_decl in all the statements of the function
            {
               INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                     "-->Substituting var_decl " + STR(p_decl) + " to parm_decl " +
                     STR(new_local_var_decl) + " in all statements");
               for (auto & block : sl->list_of_bloc)
               {
                  INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                        "-->Examining BB" + STR(block.first));
                  ...
                  INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                        "<--Examined BB" + STR(block.first));
               }
               INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                     "<--Substituted var_decl " + STR(p_decl) + " to parm_decl " +
                     STR(new_local_var_decl) + " in all statements");
            }

            // create pointer type for the new pointer-to-struct parameter
            const auto ptr_type = tree_man->create_pointer_type(p_type);
            ...
            {
               INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                     "-->Substituting parm_decl of " + STR(p_decl));
               ...
               const auto ptr_p_decl = tree_man->create_parm_decl(
                     ptr_p_identifier, ptr_type, pd->scpe, GetPointer<type_node>(GET_NODE(ptr_type))->size,
                     ptr_type, tree_nodeRef(), tree_nodeRef(),
                     srcp, GetPointer<const type_node>(GET_NODE(ptr_type))->algn,
                     1, false, true);
               INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            ...
            {
               INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                     "-->Selecting first basic block of " + fu_name);
               ...
               THROW_ASSERT(succ_blocks.size() == 1,
                     "entry basic block of function " + fu_name +
                     " has " + STR(succ_blocks.size()) + " successors");
      ...
      INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "<--Fixed declaration of function " + fu_name);
   }
   // fix calls to other functions that accept structs passed by value as parameters
   {
      for (auto & block : sl->list_of_bloc)
      {
         ...
         for (const auto & stmt : block.second->CGetStmtList())
         {
            ...
            {
               INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                     "---Is a " + GET_NODE(stmt)->get_kind_text());
                  ...
                  while (p_type_head)
                  {
                     ...
                     if (tree_helper::is_an_union(TM, p->valu->index) or tree_helper::is_a_struct(TM, p->valu->index))
                     {
                        INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                              ...
                              " has a struct parameter with type: " + STR(GET_CONST_NODE(p->valu)));
                        if (ft->varargs_flag)
                        {
...
#endif
                        INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                              "---Actual argument " + STR(actual_argument_node) +
                              " is " + actual_argument_node->get_kind_text());
                        THROW_ASSERT(tree_helper::is_an_union(TM, arg_id) or tree_helper::is_a_struct(TM, arg_id),
                              "op: " + STR(stmt) + " id: " + STR(call_tree_node_id) +
                              " passes argument " + STR(actual_argument_node) +
                              " to a call to function " + called_ssa_name +
                              " which has a struct/union parameter with type: " + STR(GET_CONST_NODE(p->valu)) +
                              " but " + STR(actual_argument_node) + " is a " +
                     ...
                     INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                           "<--Analyzed parameter type" + STR(GET_CONST_NODE(p->valu)));
                     p_type_head = p->chan;
                  }
                  INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
               ...
               for (; p_decl_it != called_fd->list_of_args.cend(); p_decl_it++, param_n++)
               {
                  ...
                  {
                     INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                           "function " + called_fu_name + " has a struct parameter: " +
                           STR(p_decl) + " with type " + STR(p_type));
                     if (called_fu_type->varargs_flag)
                     {
                        THROW_ERROR("op: " + STR(stmt) + " id: " + STR(call_tree_node_id) +
                              " calls function " + called_fu_name +
                              ": varargs function taking structs argument not supported");
                     }
                     const auto & actual_argument_node = GET_NODE(arguments->at(param_n));
#if HAVE_ASSERTS
                     ...
                     THROW_ASSERT(tree_helper::is_an_union(TM, arg_id) or tree_helper::is_a_struct(TM, arg_id),
                           "op: " + STR(stmt) + " id: " + STR(call_tree_node_id) +
                           " passes argument " + STR(actual_argument_node) +
                           " to a call to function " + called_fu_name +
