...
void EdgeCWriter::writeRoutineInstructions_rec
(
...
)
{
   ...
   for (vRIter = bb_node_info->statements_list.rbegin(), vRIterEnd = bb_node_info->statements_list.rend(); vRIter != vRIterEnd ; vRIter++)
   {
      ...
      if ((GET_TYPE(cfgGraph, *vRIter) & TYPE_INIT) != 0) continue;
...
void EdgeCWriter::writeRoutineInstructions(const unsigned int function_index, const OpVertexSet & instructions, const var_pp_functorConstRef variableFunctor, vertex bb_start, std::set<vertex> bb_end)
{
   ...
   for (boost::tie(vi, vi_end) = boost::vertices(*bb_fcfgGraph); vi != vi_end; vi++)
   {
      ...
      basic_blocks_labels[bb_node_info->block->number ] = (le ? behavioral_helper->get_label_name(le) : ("BB_LABEL_" + boost::lexical_cast<std::string>(bb_node_info->block->number)) + (bb_label_counter == 1 ? "" : "_" + boost::lexical_cast<std::string>(bb_label_counter)));
   ...
   std::set_difference(
      goto_list.begin(), goto_list.end(), /*first set*/
   ...
   for(boost::tie(e, e_end) = boost::edges(*support_cfg); e != e_end; e++)
   {
      ...
      {
         WriteFile("Error.c");
         THROW_ERROR_CODE(PROFILING_EC, "Profiling Instrumentation of Edge of function " + behavioral_helper->get_function_name() + " from vertex BB" + boost::lexical_cast<std::string>(support_cfg->CGetBBNodeInfo(boost::source(*e, *support_cfg))->block->number) + " to BB" + boost::lexical_cast<std::string>(support_cfg->CGetBBNodeInfo(boost::target(*e, *support_cfg))->block->number) + " not printed");
      }
   }
