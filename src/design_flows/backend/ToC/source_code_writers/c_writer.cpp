      ...
      case(CBackend::CB_ESCAPED_SEQUENTIAL):
         {
            ...
            const InstructionWriterRef instruction_writer(new EscapeInstructionWriter(escape_c_backend_information->profiling_architecture, app_man, instrument_writer, indented_output_stream, escape_c_backend_information->exitings_after, escape_c_backend_information->exitings_before, parameters));
...
void CWriter::WriteHeader()
{
   ...
   if(is_readc_needed)
   {
      ...
      indented_output_stream->Append("long long int __bambu_read4c(int fd){long long int buf;int res = read(fd,&buf,4);return res == 4 ? buf : (res==3 ? (buf|(1ULL<<35)) : (res==2 ? (buf|(1ULL<<35)|(1ULL<<34)) : (res == 1 ? (buf|(1ULL<<35)|(1ULL<<34)|(1ULL<<33)) : (buf|(1ULL<<35)|(1ULL<<34)|(1ULL<<33)|(1ULL<<32))) ));}\n");
   }
...
void CWriter::writePreInstructionInfo
(const FunctionBehaviorConstRef, const vertex)
{}

void CWriter::writePostInstructionInfo
(const FunctionBehaviorConstRef, const vertex)
{}

void CWriter::writeRoutineInstructions_rec
(
   vertex current_vertex,
   bool bracket
)
{
   const BBGraphInfoConstRef &bb_graph_info =
      local_rec_bb_fcfgGraph->CGetBBGraphInfo();
   const BBNodeInfoConstRef &bb_node_info =
      local_rec_bb_fcfgGraph->CGetBBNodeInfo(current_vertex);
   ...
   if (this->verbose)
   {
      indented_output_stream->Append("//Basic block " + STR(bb_number) +
         " - loop " + STR(bb_node_info->loop_id) + "\n");
   }
   // check if some extra strings must be printed before or after the basic
   // block. this is used for splitting the phi nodes
   bool add_phi_nodes_assignment_prefix =
      basic_block_prefix.find(bb_number) != basic_block_prefix.end();
   bool add_phi_nodes_assignment =
      basic_block_tail.find(bb_number) != basic_block_tail.end();
...
#endif
   bool analyze_bb_PD = bb_frontier.find(bb_PD) == bb_frontier.end() &&
                        bb_analyzed.find(bb_PD) == bb_analyzed.end();
   if (analyze_bb_PD)
   {
   ...
   bool last_statement_is_a_cond_or_goto = is_there and
      local_rec_behavioral_helper->end_with_a_cond_or_goto(bb_node_info->block) != 0 &&
   ...
   if (!add_bb_label and !start_with_a_label and boost::in_degree(current_vertex, *local_rec_bb_fcfgGraph) > 1)
   {
      ...
      for (boost::tie(inE, inEEnd) = boost::in_edges(current_vertex, *local_rec_bb_fcfgGraph); inE != inEEnd; inE++)
      {
         ...
         if (pred_bb_node_info->statements_list.size() and
               (GET_TYPE(local_rec_cfgGraph, *(pred_bb_node_info->statements_list.rbegin())) & TYPE_SWITCH) and
         ...
         //Basic block start the body of a short circuit
               ...
               !((FB_CFG_SELECTOR & local_rec_bb_fcfgGraph->GetSelector(*inE))))
         {
            ...
            if (bb_frontier.find(false_vertex) == bb_frontier.end())
            {
               ...
               else
               {
                  ...
                  if (target_bb_node_info->statements_list.empty() or
                        ((GET_TYPE(local_rec_cfgGraph, *(target_bb_node_info->statements_list.rbegin())) & (TYPE_WHILE | TYPE_FOR)) == 0))
                  {
                     indented_output_stream->Append("else\n");
                     THROW_ASSERT(basic_blocks_labels.find(bb_node_info->block->false_edge) != basic_blocks_labels.end(), "I do not know the destination");
                     indented_output_stream->Append("   goto " + basic_blocks_labels.find(bb_node_info->block->false_edge)->second + ";\n");
            ...
            for (boost::tie(oE, oEEnd) = boost::out_edges(current_vertex, *local_rec_bb_fcfgGraph); oE != oEEnd; oE++)
            {
               ...
               if (FB_CFG_SELECTOR & local_rec_bb_fcfgGraph->GetSelector(*oE))
               {
                  ...
                  if (target_bb_node_info->statements_list.size() and
                        (GET_TYPE(local_rec_cfgGraph, *(target_bb_node_info->statements_list.rbegin())) & (TYPE_WHILE | TYPE_FOR)))
                  {
...
void CWriter::DeclareLocalVariables(const CustomSet<unsigned int> & to_be_declared, CustomSet<unsigned int> & already_declared_variables, CustomSet<std::string> & already_declared_types, const BehavioralHelperConstRef behavioral_helper, const var_pp_functorConstRef varFunc)
{
...
void CWriter::WriteBuiltinWaitCall()
{
   ...
   for (const unsigned int id : AppM->CGetCallGraphManager()->GetAddressedFunctions() )
   {
      ...
      {
         indented_output_stream->Append
            (I.first + " " + I.second + " = va_arg(ap, " + I.first + ");\n");
      }
      ...
      if (returnType != "void")
      {
         ...
         indented_output_stream->Append
            (returnType + "* " + "resultAddress = va_arg(ap, " + returnType+ " *);\n");
