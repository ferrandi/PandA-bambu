      ...
      for (boost::tie(v, vEnd) = boost::vertices(*op_graph); v != vEnd; v++)
      {
         ...
         if (index != ENTRY_ID and index != EXIT_ID)
         {
            INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                  "-->Analyzing includes for operation" + STR(*v));
            std::unordered_set<unsigned int> types;
            BH->get_typecast(index, types);
            for (const auto type_id : types)
            {
               INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                     "-->Analyzing includes for type" + STR(type_id));
               AnalyzeInclude(type_id, BH, includes_to_write);
               INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                     "<--Analyzed includes for type" + STR(type_id));
            }
            INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
                  "<--Analyzed includes for operation" + STR(*v));
         }
      }
