...
{
   if (op0 != op1) indented_output_stream->Append("  assign " + op0 + " = " + op1 + ";\n");
}
...
void verilog_writer::write_io_signal_post_fix(const structural_objectRef &port, const structural_objectRef &sig)
{
   ...
   if (port_string != signal_string) indented_output_stream->Append("assign " + port_string + " = " + signal_string + ";\n");
}
...
void verilog_writer::write_transition_output_functions(bool single_proc, unsigned int output_index, const structural_objectRef &cir, const std::string&reset_state, const std::string&reset_port, const std::string&start_port, const std::string&clock_port, std::vector<std::string>::const_iterator &first, std::vector<std::string>::const_iterator &end)
{
   ...
   for(std::vector<std::string>::const_iterator first_it = first; first_it != end; ++first_it)
   {
      ...
      if(!single_proc && output_index != mod->get_out_port_size() &&
            (default_output[output_index] == current_output[output_index] || current_output[output_index] == '-'))
      {
...
void verilog_writer::write_module_parametrization_decl(const structural_objectRef &cir)
{
   ...
   if(!first_it) indented_output_stream->Append(";\n");
}
