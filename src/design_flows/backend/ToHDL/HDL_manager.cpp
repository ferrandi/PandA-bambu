...
std::string HDL_manager::write_components(const std::string& filename, HDLWriter_Language language, const std::list<structural_objectRef>& components, bool equation, std::list<std::string> & aux_files) const
{
   ...
   + std::string(" - Date " + boost::posix_time::to_simple_string(current_time))
   + "\n");
   ...
   for (std::list<structural_objectRef>::const_iterator cit = components.begin(); cit != components.end(); ++cit)
   {
      ...
      else
      {
         ...
         else if (np && (np->exist_NP_functionality(NP_functionality::VERILOG_PROVIDED) || np->exist_NP_functionality(NP_functionality::VERILOG_FILE_PROVIDED)))
         {
            ...
            {
               if(module_type.find("gimple_asm") == std::string::npos and module_type.find("__builtin_trap") == std::string::npos and module_type.find("return_value_mm_register") == std::string::npos and module_type.find("notify_caller_minimal") == std::string::npos and module_type.find("memory_mapped_register") == std::string::npos and module_type.find("status_register") == std::string::npos and module_type.find("__builtin_wait_call") == std::string::npos and module_type.find("PROXY_CTRL") == std::string::npos and module_type.find("PRINTF") == std::string::npos and module_type.find("trunc_") == std::string::npos and module_type.find("__builtin_memstore") == std::string::npos and module_type != "ui_mult_expr_FU" and module_type != "mult_expr_FU" and module_type.find("ADDRESS_DECODING_LOGIC") == std::string::npos and module_type.find("BRAM") == std::string::npos and module_type != "STD_N21_BYTEMUX" and module_type != "STD_LIVE_VALUE_TABLE" and module_type.find("MEMORY_CTRL") == std::string::npos and module_type != "register_SARSE" and module_type.find("DISTRAM") == std::string::npos and (not fu or (fu->memory_type == "" or fu->memory_ctrl_type == "")))
               {
