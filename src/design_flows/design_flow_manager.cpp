...
#endif
         INDENT_OUT_MEX(OUTPUT_LEVEL_VERY_PEDANTIC, output_level, "<--Ended execution of " + step->GetName() + (design_flow_step_info->status == DesignFlowStep_Status::UNCHANGED ? ":=" : (design_flow_step_info->status == DesignFlowStep_Status::SUCCESS ? ":+" : "")) +" in " + print_cpu_time(step_execution_time) + " seconds" + memory_usage);
#ifndef NDEBUG
