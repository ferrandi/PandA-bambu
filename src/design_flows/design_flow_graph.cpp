...
DesignFlowStepWriter::DesignFlowStepWriter(const DesignFlowGraph * design_flow_graph, const CustomMap<vertex, DesignFlowStep_Status> & _vertex_history, const CustomMap<vertex, std::string> & _actor_names, const int _detail_level) :
   VertexWriter(design_flow_graph, _detail_level),
   vertex_history(_vertex_history),
...
DesignFlowEdgeWriter::DesignFlowEdgeWriter(const DesignFlowGraph * design_flow_graph, const CustomMap<vertex, DesignFlowStep_Status> & _vertex_history, const std::unordered_map<EdgeDescriptor, int> & _edge_history, const int _detail_level) :
   EdgeWriter(design_flow_graph, _detail_level),
   vertex_history(_vertex_history),
   edge_history(_edge_history)
{}
...
void DesignFlowEdgeWriter::operator()(std::ostream& out, const EdgeDescriptor & edge) const
{
   ...
   else
   {
      ...
      const bool source_executed = source_info->status == DesignFlowStep_Status::EMPTY or source_info->status == DesignFlowStep_Status::SKIPPED or source_info->status == DesignFlowStep_Status::SUCCESS or source_info->status == DesignFlowStep_Status::UNCHANGED;
      const bool target_executed = target_info->status == DesignFlowStep_Status::EMPTY or target_info->status == DesignFlowStep_Status::SKIPPED or target_info->status == DesignFlowStep_Status::SUCCESS or target_info->status == DesignFlowStep_Status::UNCHANGED;
