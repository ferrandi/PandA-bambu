...
class DesignFlowEdgeWriter : public EdgeWriter
{
   ...
   public:
       ...
       * @param detail_level is the detail level
       */
      DesignFlowEdgeWriter(const DesignFlowGraph * design_flow_graph, const CustomMap<vertex, DesignFlowStep_Status> & vertex_history = CustomMap<vertex, DesignFlowStep_Status>(), const std::unordered_map<EdgeDescriptor, int> & edge_history = std::unordered_map<EdgeDescriptor, int>(), const int detail_level = 0);
