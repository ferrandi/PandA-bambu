...
class Loop
{
   ...
   public:
      ...
      */
      ...
      std::set<vertex> get_entries() const {return alternative_entries;}

      /// add an entry of the loop
      void add_entry(vertex v) {alternative_entries.insert(v);}
      ...
      */
      ...
      void add_sp_back_edge(vertex vs, vertex vd) {std::pair<vertex, vertex> vpair(vs,vd); sp_back_edges.insert(vpair);}

      /// check if a pair of vertex is a spanning tree back edge for the loop
      bool is_sp_back_edge(vertex vs, vertex vd) const {std::pair<vertex, vertex> vpair(vs,vd); return sp_back_edges.find(vpair) != sp_back_edges.end();}

      /// return the list of spanning tree back edges
      std::set<std::pair<vertex,vertex>> get_sp_back_edges() const {return sp_back_edges;}
