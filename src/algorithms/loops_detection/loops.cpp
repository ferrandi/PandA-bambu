...
struct djgraph_dfs_tree_visitor : public boost::default_dfs_visitor
{
   private:
      ...
      std::unordered_map<vertex, unsigned int> & vertex_level;
   ...
   public:
      ...
      (
         std::unordered_map<vertex, unsigned int> & _vertex_level,
         std::unordered_map<vertex, unsigned int> & _dfs_order,
...
bool Loops::is_edge_in_list(std::unordered_set<vertex_pair>& l, vertex source, vertex target)
{
...
void Loops::DetectLoops()
{
   ...
   std::unordered_set<vertex_pair> cj_edges;
   std::unordered_set<vertex_pair> bj_edges;
   ...
   std::unordered_map<vertex, unsigned int> vertex_level_rel;
   std::unordered_map<vertex, unsigned int> dfs_order;
   ...
   std::unordered_set<vertex_pair> sp_back_edges;
   ...
   for (unsigned int index = max_level+1; index > 0; --index)
   {
      ...
      for (auto v: level_vertices_rel[lev])
      {
         ...
         for (boost::tie(e_iter, e_iter_end) = boost::in_edges(v, *djg) ; e_iter != e_iter_end ; ++e_iter )
         {
            ...
            {
               PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "Irreducible loop found: SP-BACK-EDGE=" + boost::lexical_cast<std::string>(cfg->CGetBBNodeInfo(m)->block->number) + "->" + boost::lexical_cast<std::string>(cfg->CGetBBNodeInfo(n)->block->number));
               irreducible = true;
            }
            if (is_edge_in_list(bj_edges, m, n))
            {
               ...
               PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "Found a reducible Loop: SP-BACK-EDGE=" + boost::lexical_cast<std::string>(cfg->CGetBBNodeInfo(m)->block->number) + "->" + boost::lexical_cast<std::string>(cfg->CGetBBNodeInfo(n)->block->number));
            }
         }
         if(reducible)
         {
...
void Loops::tarjan_scc(const BBGraphRef djg, vertex v,
                       ...
                       std::unordered_map<vertex, unsigned int>& lowlink,
                       std::list<vertex>& s,
                       std::set<vertex> &u, unsigned int &max_dfs)
{
   dfs_order[v] = max_dfs++;
   ...
   else if (lowlink[v] == dfs_order[v])
   {
      ...
      std::unordered_set<vertex> candidate_body_loop;
      vertex v1;
      do
      {
         v1 = s.back();
      ...
      while (v1 != v);
      std::unordered_set<vertex> real_body_loop;
      std::unordered_set<LoopRef> nested_loops;
