...
class bipartite_matching
{
   ...
   public:
      /// constructor
      bipartite_matching(size_t _num_rows, size_t _num_cols, const CostFunctor &_cost) : MINUS_ONE(std::numeric_limits<size_t>::max()), MINUS_TWO(std::numeric_limits<size_t>::max()-1),
         num_rows(_num_rows), num_cols(_num_cols), nel(std::max(_num_rows,_num_cols)),
         lx(nel), ly(nel,0), xy(nel, MINUS_ONE), yx(nel, MINUS_ONE), slack(nel), slackx(nel), S(nel),
         T(nel), prev(nel), cost(_cost)
      {
         /// initialize lx
      ...
      std::vector<size_t> & get_solution() {return xy;}
