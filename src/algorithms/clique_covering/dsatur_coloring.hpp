...
namespace boost
{
   ...
   class dsatur_coloring_helper
   {
      ...
      public:
         dsatur_coloring_helper(const VertexListGraph& _G, ColorMap &_CM, const size_type _num_node) :
            num_node(_num_node), satur_to_list(1), vertex_to_satur(_num_node,0), vertex_to_iter(_num_node), color_set(_num_node), G(_G), CM(_CM)
         {
         ...
         size_type SeqColor()
         {
            ...
            for(size_type i=0;i<num_node;i++) /// color each vertex
            {
               ...
               while(!satur_to_list[maxsatur]) maxsatur--;
               ...
               size_type color = 0;
               while(color_set[v].find(color) != color_set[v].end()) color++;
                  ...
                  if(vertex_to_iter[w])
                  {
                     ...
                     if (color_set[w].find(color) == color_set[w].end())
                     {
                        ...
                        else
                        {
                           ...
                           while(iter->next && !DPF(iter->next->data)) iter = iter->next;
   ...
   dsatur_coloring(const VertexListGraph& G,
                                         ColorMap color)
   {
      typedef graph_traits<VertexListGraph> GraphTraits;
      typedef typename GraphTraits::vertex_descriptor Vertex;
