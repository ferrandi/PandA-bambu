...
namespace boost
{
   ...
   unsorted_coloring(const VertexListGraph& G, ColorMap color)
   {
   ...
   class maxclique_dsatur_coloring_helper
   {
      ...
      public:
         maxclique_dsatur_coloring_helper(const VertexListGraph& _G, ColorMap &_CM, const size_type _num_node, size_type &_lb) : 
            num_node(_num_node), BestColoring(_num_node +1), ColorClass(_num_node,_num_node-1), valid(_num_node,true), ColorAdj(_num_node,_num_node), ColorCount(_num_node,0), DegreeCount(_num_node), lb(_lb), G(_G), CM(_CM), HCF(ColorCount,DegreeCount), clique_size(0), filter(&support), FG(G,keep_all(),filter)
         {
            typename GraphTraits::vertex_iterator v, vend;
         ...
         size_type SeqColorRec(size_type i, size_type current_color)
         {
            ...
            //prob_count++;
            if (current_color+1 >= BestColoring) return(current_color+1);
            if (BestColoring <= lb) return(BestColoring);

            if (i >= num_node) return(current_color+1);
   ...
   typename property_traits<ColorMap>::value_type
   maxclique_dsatur_coloring(const VertexListGraph& G,
                                         ColorMap color)
   {
      typedef graph_traits<VertexListGraph> GraphTraits;
      typedef typename GraphTraits::vertex_descriptor Vertex;
