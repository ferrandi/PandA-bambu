...
#define INDENT_DBG_MEX(dbgLevel, curDbgLevel, mex) \
   ((dbgLevel <= curDbgLevel) \
      ? ( panda_message = mex, (std::string(panda_message) == "-->") \
          ...
          : ( (std::string(panda_message).substr(0,3) == "-->") \
              ...
              : ( (std::string(panda_message).substr(0,3) == "---") \
                  ...
                  : ( (std::string(panda_message) == "<--") \
                      ? (null_stream << (indentation-=2)) \
                      : ( (std::string(panda_message).substr(0,3) == "<--") \
                          ? (std::cerr << std::string(indentation, ' ') << std::string(panda_message).substr(3) << std::endl, null_stream << (indentation-=2)) \
...
#define INDENT_OUT_MEX(profLevel, curprofLevel, mex) \
   (void (0))
#define PRINT_OUT_MEX(profLevel, curprofLevel, mex) \
   (void (0))
#define PRINT_OUT_STRING(profLevel, curprofLevel, mex) \
   (void (0))
