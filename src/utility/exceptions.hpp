...
inline T& throw_error(const T& t, const std::string&expression, const char * DEBUG_PARAMETER(pp), const char * DEBUG_PARAMETER(file), int DEBUG_PARAMETER(line), int code = EXIT_FAILURE)
{
   ...
   throw std::string (std::string("error -> ") + expression + std::string("\n\t") 
                        + std::string(pp) + std::string("\n\t") + std::string(file) + 
                              std::string(":") + boost::lexical_cast<std::string>(line));
#endif
   static T t_local=t;
   return t_local;
}
...
#define THROW_ERROR_CODE(code, str_expr) do {PRINT_STACK; throw_error(0, (str_expr), __PRETTY_FUNCTION__, __FILE__, __LINE__, code);} while(0)
