...
#define VISIT_SC(mask,superclass, method) if(((1<<GETID(superclass))&mask)==0) this->superclass::method
///macro used to visit a non null member
#define VISIT_MEMBER_NAMED(ref_obj_name,mask,ref_obj, method) if(((1<<GETID(ref_obj_name))&mask)==0 && ref_obj) ref_obj->method
...
#define SEQ_VISIT_MEMBER(mask,seq,seqbasetype,method,visitor_type,visitor_obj) if(((1<<GETID(seq))&mask)==0 && !seq.empty()) std::for_each(seq.begin(), seq.end(), for_each_functor<seqbasetype,visitor_type>(&seqbasetype::method, visitor_obj))
...
class for_each_functor : public std::unary_function<const refcount<T>&, void>
{
   ...
   public:
      ...
      ///functor
      void operator() (const refcount<T> &x) const { ((x.get())->*visitor_function)(vo); }
   private:
      ///visitor member
