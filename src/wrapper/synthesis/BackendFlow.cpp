...
std::string BackendFlow::CreateScripts(const DesignParametersRef dp)
{
   ...
   generated_synthesis_script = "./synthesize";
   if(exec_params->chain_name.size())
   {
      generated_synthesis_script += "_" + exec_params->chain_name;
   }
   if(exec_params->component_name.size())
   {
      generated_synthesis_script += "_" + exec_params->component_name;
   }
   generated_synthesis_script += ".sh";

   std::ofstream script(generated_synthesis_script);
   std::filesystem::permissions(generated_synthesis_script,
   ...
   script << "#!/bin/bash\n"
             ...
             "##########################################################\n"
             "# Synthesis script for COMPONENT: "
          << exec_params->component_name
          << "\n"
