                                                  ...
                                                  const std::list<std::string>& aux_files)
{
   ...
   if(library.size())
   {
      ...
      actual_parameters->parameter_values[PARAM_clk_period] = STR((is_time_unit_PS?1000:1)*GetPointer<functional_unit>(tn)->get_clock_period());
   ...
   if(!time_constrained)
   {
      actual_parameters->parameter_values[PARAM_clk_period] = STR((is_time_unit_PS?1000:1)*PARAM_clk_period_default);
   ...
   actual_parameters->parameter_values[PARAM_clk_freq] =
       STR((is_time_unit_PS?1000:1)*1000 / boost::lexical_cast<double>(actual_parameters->parameter_values[PARAM_clk_period]));
