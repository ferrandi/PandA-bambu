...
class BackendFlow
{
   ...
   public:
       ...
       */
      static
      BackendFlowRef CreateFlow(const ParameterConstRef Param, const std::string& flow_name, const target_managerRef target);

      /**
       * Determines the type of the backend flow based on the target device
       */
      static
      type_t DetermineBackendFlowType(const target_deviceRef device, const ParameterConstRef parameters);
       ...
       */
      static
      BackendFlowRef xload_generator_chain(const ParameterConstRef Param, const std::string& xml_file);
#endif
