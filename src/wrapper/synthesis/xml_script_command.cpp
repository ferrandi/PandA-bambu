...
bool xml_script_node_t::evaluate_condition(const std::string * condition)
{
   ...
   catch (const std::exception& ex) {}     // trimmed contains no numbers
...
bool xml_script_node_t::evaluate_condition(const std::string * condition, const DesignParametersRef& dp)
{
   ...
   boost::algorithm::trim(trimmed);
   bool negate = false;
   if (trimmed[0] == '!') { trimmed = trimmed.substr(1, trimmed.length()); negate = true; }
   if (trimmed[0] == '$')
   {
      std::string var;
...
xml_set_entry_t::xml_set_entry_t(const xml_element *element)
{
   ...
   if (a) value = a->get_value();
   else THROW_ERROR("Attribute \"value\" required for entry");

   a = element->get_attribute("condition");
   if (a) condition = new std::string(a->get_value());
}
...
)
{
   ...
   if (a) name = a->get_value();
   else THROW_ERROR("Attribute \"name\" must be defined");
   ...
   if (singleValue && multiValues.size())
...
{
   if (singleValue) delete singleValue;
   if (condition)   delete condition;
...
xml_parameter_t::xml_parameter_t(const xml_element * element)
{
   ...
   a = element->get_attribute("name");
   if (a) name = new std::string(a->get_value());
   ...
   if (singleValue && multiValues.size())
      THROW_ERROR("Only one between \"value\" and <entry> children can be defined");
   if (!name && !singleValue && !multiValues.size())
...
xml_command_t::xml_command_t(const xml_element * element)
{
   ...
   if (a) output = new std::string(a->get_value());
}
...
xml_ite_block_t::xml_ite_block_t(const xml_element * element) : condition (element->get_attribute("condition") ? element->get_attribute("condition")->get_value() : "")
{
   nodeType = NODE_ITE_BLOCK;
   thenNodes.clear();
   elseNodes.clear();
