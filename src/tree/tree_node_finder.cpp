...
#define TREE_NOT_YET_IMPLEMENTED(token) \
   THROW_ASSERT(tree_node_schema.find(TOK(token)) == tree_node_schema.end(), std::string("field not yet supported ") + STOK(token))
...
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res && (tree_node_schema.find(TOK(TOK_SRCP)) == tree_node_schema.end() ||  tree_node_schema.find(TOK(TOK_SRCP))->second == obj->include_name + ":" + boost::lexical_cast<std::string>(obj->line_number) + ":"  + boost::lexical_cast<std::string>(obj->column_number));
}

void tree_node_finder::operator()(const decl_node* obj, unsigned int & mask)
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res and
   CHECK_TREE_NODE_OPT(TOK_NAME, obj->name) and
   CHECK_TREE_NODE_OPT(TOK_MNGL, obj->mngl) and
   CHECK_TREE_NODE_OPT(TOK_ORIG, obj->orig) and
   CHECK_TREE_NODE_OPT(TOK_TYPE, obj->type) and
   CHECK_TREE_NODE_OPT(TOK_SCPE, obj->scpe) and
   CHECK_TREE_NODE_OPT(TOK_ATTRIBUTES, obj->attributes) and
   CHECK_TREE_NODE_OPT(TOK_CHAN, obj->chan) and
   CHECK_VALUE_OPT(TOK_ARTIFICIAL, obj->artificial_flag) and
...
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res and CHECK_TREE_NODE_OPT(TOK_VUSE, obj->memuse) and
              CHECK_TREE_NODE_OPT(TOK_VDEF, obj->memdef);
...
void tree_node_finder::operator()(const quaternary_expr* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_OP0, obj->op0) &&
         CHECK_TREE_NODE_OPT(TOK_OP1, obj->op1) &&
         CHECK_TREE_NODE_OPT(TOK_OP2, obj->op2) &&
         CHECK_TREE_NODE_OPT(TOK_OP3, obj->op3);
}

void tree_node_finder::operator()(const type_node* obj, unsigned int & mask)
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res && CHECK_VALUE_OPT(TOK_QUAL, obj->qual) and
         CHECK_TREE_NODE_OPT(TOK_NAME, obj->name) and
         CHECK_TREE_NODE_OPT(TOK_UNQL, obj->unql) and
         CHECK_TREE_NODE_OPT(TOK_SIZE, obj->size) and
         CHECK_TREE_NODE_OPT(TOK_SCPE, obj->scpe) and
         CHECK_VALUE_OPT(TOK_PACKED, obj->packed_flag) and
         CHECK_VALUE_OPT(TOK_SYSTEM, obj->system_flag) and
         CHECK_VALUE_OPT(TOK_ALGN, obj->algn);
}
...
void tree_node_finder::operator()(const array_type* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_ELTS, obj->elts) &&
         CHECK_TREE_NODE_OPT(TOK_DOMN, obj->domn);
}

void tree_node_finder::operator()(const gimple_asm* obj, unsigned int & mask)
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res && CHECK_VALUE_OPT(TOK_VOLATILE, obj->volatile_flag) &&
         CHECK_VALUE_OPT(TOK_STR, obj->str) &&
         CHECK_TREE_NODE_OPT(TOK_OUT, obj->out) &&
         CHECK_TREE_NODE_OPT(TOK_IN, obj->in) &&
...
void tree_node_finder::operator()(const binfo* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_TYPE,obj->type) &&
         CHECK_VALUE_OPT(TOK_VIRT,obj->virt_flag) && CHECK_VALUE_OPT(TOK_BASES,obj->bases);
...
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_REAL, obj->real) &&
         CHECK_TREE_NODE_OPT(TOK_IMAG, obj->imag);
}
...
void tree_node_finder::operator()(const enumeral_type* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_VALUE_OPT(TOK_PREC, obj->prec) &&
         ...
         CHECK_TREE_NODE_OPT(TOK_MIN, obj->min) &&
...
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res && CHECK_VALUE_OPT(TOK_LINE, obj->line) &&
         CHECK_TREE_NODE_OPT(TOK_EXPR, obj->expr) &&
...
void tree_node_finder::operator()(const field_decl* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_VALUE_OPT(TOK_ALGN, obj->algn) &&
         ...
         CHECK_TREE_NODE_OPT(TOK_BPOS, obj->bpos) &&
...
void tree_node_finder::operator()(const function_decl* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_VALUE_OPT(TOK_OPERATOR, obj->operator_flag) &&
         CHECK_TREE_NODE_OPT(TOK_TMPL_PARMS, obj->tmpl_parms) &&
         CHECK_TREE_NODE_OPT(TOK_TMPL_ARGS, obj->tmpl_args) &&

         CHECK_VALUE_OPT(TOK_FIXD, obj->fixd) &&
         CHECK_VALUE_OPT(TOK_VIRT, obj->virt) &&
         CHECK_TREE_NODE_OPT(TOK_FN, obj->fn) &&
         CHECK_VALUE_OPT(TOK_UNDEFINED, obj->undefined_flag) &&
         CHECK_VALUE_OPT(TOK_BUILTIN, obj->builtin_flag) &&
         ...
         CHECK_TREE_NODE_OPT(TOK_BODY, obj->body) &&
...
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_RETN, obj->retn) &&
...
void tree_node_finder::operator()(const gimple_assign* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_OP, obj->op0) &&
         CHECK_TREE_NODE_OPT(TOK_OP, obj->op1) && CHECK_TREE_NODE_OPT(TOK_PREDICATE, obj->predicate) && 
         CHECK_TREE_NODE_OPT(TOK_ORIG, obj->orig) && CHECK_VALUE_OPT(TOK_INIT, obj->init_assignment) && CHECK_VALUE_OPT(TOK_CLOBBER, obj->clobber) && CHECK_VALUE_OPT(TOK_ADDR, obj->temporary_address);
}
...
void tree_node_finder::operator()(const overload* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_CRNT, obj->crnt) &&
...
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_ARGT, obj->argt) &&
         CHECK_TREE_NODE_OPT(TOK_SIZE, obj->size) &&
         CHECK_VALUE_OPT(TOK_ALGN, obj->algn) &&
         CHECK_VALUE_OPT(TOK_USED, obj->used) &&
         CHECK_VALUE_OPT(TOK_REGISTER, obj->register_flag) &&
...
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res && CHECK_VALUE_OPT(TOK_OVERFLOW, obj->overflow_flag) &&
         CHECK_VALUE_OPT(TOK_VALR, obj->valr) &&
...
void tree_node_finder::operator()(const record_type* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_TMPL_PARMS, obj->tmpl_parms) &&
         CHECK_TREE_NODE_OPT(TOK_TMPL_ARGS, obj->tmpl_args) &&
         CHECK_VALUE_OPT(TOK_PTRMEM, obj->ptrmem_flag) &&
         CHECK_TREE_NODE_OPT(TOK_PTD, obj->ptd) &&
         CHECK_TREE_NODE_OPT(TOK_CLS, obj->cls) &&
         CHECK_TREE_NODE_OPT(TOK_BFLD, obj->bfld) &&
         CHECK_TREE_NODE_OPT(TOK_VFLD, obj->vfld) &&
         CHECK_VALUE_OPT(TOK_SPEC, obj->spec_flag) &&
         CHECK_VALUE_OPT(TOK_STRUCT, obj->struct_flag) &&
...
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_INIT, obj->init) &&
         CHECK_TREE_NODE_OPT(TOK_SIZE, obj->size) &&
         CHECK_VALUE_OPT(TOK_ALGN, obj->algn) &&
         CHECK_TREE_NODE_OPT(TOK_SMT_ANN, obj->smt_ann);
}
...
void tree_node_finder::operator()(const scope_ref* obj, unsigned int & mask)
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_OP, obj->op0) &&
         CHECK_TREE_NODE_OPT(TOK_OP, obj->op1);
}
...
void tree_node_finder::operator()(const ssa_name* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_VAR, obj->type) &&
              CHECK_TREE_NODE_OPT(TOK_VAR, obj->var) &&
         CHECK_VALUE_OPT(TOK_VERS, obj->vers) &&
         CHECK_VALUE_OPT(TOK_ORIG_VERS, obj->orig_vers) &&
         //CHECK_TREE_NODE_OPT(TOK_PTR_INFO, obj->ptr_info) &&
         CHECK_VALUE_OPT(TOK_VIRTUAL, obj->virtual_flag) &&
         CHECK_VALUE_OPT(TOK_VOLATILE, obj->volatile_flag) &&
         CHECK_TREE_NODE_OPT(TOK_MIN, obj->min) &&
         CHECK_TREE_NODE_OPT(TOK_MAX, obj->max) &&
...
{
   tree_node_mask::operator()(obj,mask);
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_RSLT, obj->rslt) &&
         CHECK_TREE_NODE_OPT(TOK_INST, obj->inst) &&
         CHECK_TREE_NODE_OPT(TOK_SPCS, obj->spcs) &&
         CHECK_TREE_NODE_OPT(TOK_PRMS, obj->prms);
}
...
void tree_node_finder::operator()(const tree_list* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_PURP, obj->purp) &&
         CHECK_TREE_NODE_OPT(TOK_VALU, obj->valu) &&
         CHECK_TREE_NODE_OPT(TOK_CHAN, obj->chan);
}
...
void tree_node_finder::operator()(const try_block* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_VALUE_OPT(TOK_LINE, obj->line) &&
         CHECK_TREE_NODE_OPT(TOK_BODY, obj->body) &&
         CHECK_TREE_NODE_OPT(TOK_HDLR, obj->hdlr) &&
         CHECK_TREE_NODE_OPT(TOK_NEXT, obj->next);
}
...
void tree_node_finder::operator()(const target_mem_ref* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_TYPE, obj->type) &&
         CHECK_TREE_NODE_OPT(TOK_SYMBOL, obj->symbol) &&
         CHECK_TREE_NODE_OPT(TOK_BASE, obj->base) &&
         CHECK_TREE_NODE_OPT(TOK_IDX, obj->idx) &&
         CHECK_TREE_NODE_OPT(TOK_STEP, obj->step) &&
         CHECK_TREE_NODE_OPT(TOK_OFFSET, obj->offset) &&
         CHECK_TREE_NODE_OPT(TOK_ORIG, obj->orig) &&
         CHECK_TREE_NODE_OPT(TOK_TAG, obj->tag);
}
...
void tree_node_finder::operator()(const target_mem_ref461* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_TREE_NODE_OPT(TOK_TYPE, obj->type) &&
         ...
         CHECK_TREE_NODE_OPT(TOK_IDX, obj->idx) &&
         CHECK_TREE_NODE_OPT(TOK_STEP, obj->step) &&
         CHECK_TREE_NODE_OPT(TOK_IDX2, obj->idx2) &&
         CHECK_TREE_NODE_OPT(TOK_OFFSET, obj->offset);
}
...
void tree_node_finder::operator()(const bloc* obj, unsigned int & mask)
{
   ...
   find_res = find_res && CHECK_VALUE_OPT(TOK_HPL, obj->hpl) &&
         CHECK_VALUE_OPT(TOK_LOOP_ID, obj->loop_id) &&
         CHECK_VALUE_OPT(TOK_TRUE_EDGE, obj->true_edge) &&
         CHECK_VALUE_OPT(TOK_FALSE_EDGE, obj->false_edge);
