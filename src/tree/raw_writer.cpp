...
/**
 * Macro which writes on an output stream a string with its lenght. IDENTIFIER_NODE case
 */
#define WRITE_STRGLNGT_IDENTIFIER(os, field) os << " " << "strg: \"" << field << "\" lngt: " << field.size()

/**
 * Macro which writes on an output stream a string with its lenght. STRING_CST case
 */
#define WRITE_STRGLNGT_STRING(os, field) os << " " << "strg: \"" << field << "\" lngt: " << field.size()+1

/**
 * Macro which writes on an output stream the srcp fields.
 */
#define WRITE_SRCP(os, include_name, line_number, column_number) os << " " << "srcp: \"" << include_name << "\":" << line_number << ":" << column_number
...
{
   mask = NO_VISIT;
   if (obj->line_number || obj->column_number || obj->include_name != "")
