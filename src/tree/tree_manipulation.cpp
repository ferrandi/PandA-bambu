...
/// Constructor
tree_manipulation::tree_manipulation(const tree_managerRef _TreeM,
    const ParameterConstRef _parameters)
    : TreeM(_TreeM)
    , reuse(_parameters->IsParameter("reuse_gimple")
              ? _parameters->GetParameter<bool>("reuse_gimple")
              : true)
...
tree_nodeRef tree_manipulation::create_unary_operation(
    ...
    kind operation_kind) const
{
    ...
    THROW_ASSERT(type->get_kind() == tree_reindex_K,
        "Type node is not a tree reindex");
    THROW_ASSERT(op->get_kind() == tree_reindex_K,
        "Operator node is not a tree reindex");
    ...
    PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
        "Created node " + STR(GET_INDEX_NODE(return_tree_reindex)) + " (" + GET_NODE(return_tree_reindex)->get_kind_text() + ")");
...
tree_nodeRef tree_manipulation::create_binary_operation(
    const tree_nodeRef& type,
    ...
    kind operation_kind) const
{
    ...
    THROW_ASSERT(type->get_kind() == tree_reindex_K,
        "Type node is not a tree reindex");
    THROW_ASSERT(op0->get_kind() == tree_reindex_K,
        "Operator 0 node is not a tree reindex");
    THROW_ASSERT(op1->get_kind() == tree_reindex_K,
        "Operator 1 node is not a tree reindex");
    THROW_ASSERT(!srcp.empty(), "It requires a non empty string");
    ...
    PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
        "Created node " + STR(GET_INDEX_NODE(node_ref)) + " (" + GET_NODE(node_ref)->get_kind_text() + ")");
...
tree_nodeRef tree_manipulation::create_ternary_operation(
    const tree_nodeRef& type,
    const tree_nodeRef& op0,
    const tree_nodeRef& op1,
    ...
    kind operation_kind) const
{
    /// Check if the tree_node given are tree_reindex
    THROW_ASSERT(type->get_kind() == tree_reindex_K,
        "Type is not a tree reindex");
    THROW_ASSERT(op0->get_kind() == tree_reindex_K,
        "Operand 0 is not a tree reindex");
    THROW_ASSERT(op1->get_kind() == tree_reindex_K,
        "Operand 1 is not a tree reindex");
    THROW_ASSERT(op2->get_kind() == tree_reindex_K,
        "Operand 3 is not a tree reindex");
    THROW_ASSERT(!srcp.empty(), "It requires a non empty string");
...
tree_nodeRef tree_manipulation::create_quaternary_operation(
    const tree_nodeRef& type,
    const tree_nodeRef& op0,
    const tree_nodeRef& op1,
    const tree_nodeRef& op2,
    ...
    kind operation_kind) const
{
    ...
    THROW_ASSERT(type->get_kind() == tree_reindex_K,
        "Type node is not a tree reindex");
    THROW_ASSERT(op0->get_kind() == tree_reindex_K,
        "Operand 0 node is not a tree reindex");
    THROW_ASSERT(op1->get_kind() == tree_reindex_K,
        "Operand 1 node is not a tree reindex");
    THROW_ASSERT(op2->get_kind() == tree_reindex_K,
        "Operand 2 node is not a tree reindex");
    THROW_ASSERT(op3->get_kind() == tree_reindex_K,
...
tree_nodeRef tree_manipulation::CreateIntegerCst(
...
tree_nodeRef tree_manipulation::create_identifier_node(
    const std::string& strg) const
{
    ...
    if (!node_nid) {
        ...
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Created node " + STR(GET_INDEX_NODE(node_ref)) + " (" + GET_NODE(node_ref)->get_kind_text() + " " + strg + ")");
    } else {
        node_ref = TreeM->GetTreeReindex(node_nid);
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Found   node " + STR(GET_INDEX_NODE(node_ref)) + " (" + GET_NODE(node_ref)->get_kind_text() + " " + strg + ")");
    }
    return node_ref;
}
    ...
    bool addr_not_taken) const
{
    /// Check if the tree_node given are tree_reindex
    THROW_ASSERT(type->get_kind() == tree_reindex_K,
        "Node is not a tree reindex");
    THROW_ASSERT(scpe->get_kind() == tree_reindex_K,
        "Node is not a tree reindex");
    THROW_ASSERT(size->get_kind() == tree_reindex_K,
        "Node is not a tree reindex");
    ...
    if (name) {
        THROW_ASSERT(name->get_kind() == tree_reindex_K,
            "Node is not a tree reindex");
    ...
    if (smt_ann) {
        THROW_ASSERT(smt_ann->get_kind() == tree_reindex_K,
            "Node is not a tree reindex");
    ...
    if (init) {
        THROW_ASSERT(init->get_kind() == tree_reindex_K,
            "Node is not a tree reindex");
...
tree_nodeRef tree_manipulation::create_translation_unit_decl() const
{
    ...
    if (!translation_unit_decl_nid) {
        ...
        this->TreeM->create_tree_node(translation_unit_decl_nid,
            translation_unit_decl_K, IR_schema);
        translation_unit_decl_node = TreeM->GetTreeReindex(translation_unit_decl_nid);
        PRINT_DBG_MEX(
            DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Created node " + STR(GET_INDEX_NODE(translation_unit_decl_node)) + " (" + GET_NODE(translation_unit_decl_node)->get_kind_text() + ")");
    } else {
        translation_unit_decl_node = TreeM->GetTreeReindex(translation_unit_decl_nid);
        PRINT_DBG_MEX(
            DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Found   node " + STR(GET_INDEX_NODE(translation_unit_decl_node)) + " (" + GET_NODE(translation_unit_decl_node)->get_kind_text() + ")");
    }
...
tree_nodeRef tree_manipulation::create_void_type() const
{
    ...
    if (!void_type_nid) {
        ...
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Created node " + STR(void_type_nid) + " (void_type)");
        ...
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Created node " + STR(GET_INDEX_NODE(void_node)) + " (" + GET_NODE(void_node)->get_kind_text() + " void)");
    } else {
        void_node = TreeM->GetTreeReindex(void_type_nid);
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Found   node " + STR(GET_INDEX_NODE(void_node)) + " (" + GET_NODE(void_node)->get_kind_text() + " void)");
    }
...
tree_nodeRef tree_manipulation::create_bit_size_type() const
{
    ...
    if (!bit_size_type_nid) {
        ...
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Created node " + STR(GET_INDEX_NODE(bit_size_node)) + " (" + GET_NODE(bit_size_node)->get_kind_text() + " bit_size)");
        ...
        bit_size_node = TreeM->GetTreeReindex(bit_size_type_nid);
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Found   node " + STR(GET_INDEX_NODE(bit_size_node)) + " (" + GET_NODE(bit_size_node)->get_kind_text() + " bit_size)");
    }
...
tree_nodeRef tree_manipulation::create_size_type() const
{
    ...
    if (!size_type_nid) {
        ...
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Created node " + STR(GET_INDEX_NODE(size_node)) + " (" + GET_NODE(size_node)->get_kind_text() + " bit_size)");
        ...
        size_node = TreeM->GetTreeReindex(size_type_nid);
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Found   node " + STR(GET_INDEX_NODE(size_node)) + " (" + GET_NODE(size_node)->get_kind_text() + " bit_size)");
    }
    return size_node;
}
...
tree_nodeRef tree_manipulation::create_boolean_type() const
{
    ...
    if (!type_decl_nid) {
        ...
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Created node " + STR(GET_INDEX_NODE(boolean_type_node)) + " (" + GET_NODE(boolean_type_node)->get_kind_text() + " boolean)");
        ...
        boolean_type_node = TreeM->GetTreeReindex(boolean_type_nid);
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Found   node " + STR(GET_INDEX_NODE(boolean_type_node)) + " (" + GET_NODE(boolean_type_node)->get_kind_text() + " boolean)");
    }
...
tree_nodeRef tree_manipulation::create_default_unsigned_integer_type() const
{
    ...
    if (!type_decl_nid) {
        ...
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Created node " + STR(GET_INDEX_NODE(integer_type_node)) + " (" + GET_NODE(integer_type_node)->get_kind_text() + " unsigned_int)");
        ...
        integer_type_node = TreeM->GetTreeReindex(integer_type_nid);
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Found   node " + STR(GET_INDEX_NODE(integer_type_node)) + " (" + GET_NODE(integer_type_node)->get_kind_text() + " unsigned int)");
    }
...
tree_nodeRef tree_manipulation::CreateDefaultUnsignedLongLongInt() const
{
    ...
    if (!type_decl_nid) {
        ...
        integer_type_node = TreeM->GetTreeReindex(integer_type_nid);
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Created node " + STR(GET_INDEX_NODE(integer_type_node)) + " (" + GET_NODE(integer_type_node)->get_kind_text() + " unsigned long long int int)");

        CreateIntegerCst(bit_size_node, SIZE_VALUE_UNSIGNED_LONG_LONG_INT,
            size_node_nid);
        CreateIntegerCst(integer_type_node, MIN_VALUE_UNSIGNED_LONG_LONG_INT,
            min_node_nid);
        CreateIntegerCst(integer_type_node, MAX_VALUE_UNSIGNED_LONG_LONG_INT,
            max_node_nid);
    } else {
        IR_schema.clear();
        IR_schema[TOK(TOK_NAME)] = STR(type_decl_nid);
        ...
        INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Found  node " + STR(GET_INDEX_NODE(integer_type_node)) + " (" + GET_NODE(integer_type_node)->get_kind_text() + " unsigned long long int)");
    }
    return integer_type_node;
}
...
tree_nodeRef tree_manipulation::create_default_integer_type() const
{
    ...
    if (!type_decl_nid) {
        ...
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Created node " + STR(type_decl_nid) + " (type_decl int)");
        ...
        integer_type_node = TreeM->GetTreeReindex(integer_type_nid);
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Created node " + STR(GET_INDEX_NODE(integer_type_node)) + " (" + GET_NODE(integer_type_node)->get_kind_text() + " int)");
        ...
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "Found   node " + STR(GET_INDEX_NODE(integer_type_node)) + " (" + GET_NODE(integer_type_node)->get_kind_text() + " int)");
    }
    return integer_type_node;
}

/// Create a pointer type
tree_nodeRef tree_manipulation::create_pointer_type(
    const tree_nodeConstRef ptd) const
{
...
tree_nodeRef tree_manipulation::create_integer_type_with_prec(
    unsigned int prec,
    bool unsigned_p) const
{
    ...
    if (!integer_type_nid) {
        ...
        tree_nodeRef min_node = TreeM->CreateUniqueIntegerCst(
            (unsigned_p ? 0 : (1LL << (prec - 1))), integer_type_nid);
        IR_schema[TOK(TOK_MIN)] = STR(GET_INDEX_NODE(min_node));
        tree_nodeRef max_node = TreeM->CreateUniqueIntegerCst(
            (unsigned_p ? (~0LL) : static_cast<long long int>((~0ULL) >> 1)),
            integer_type_nid);
...
{
    INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
        "-->Creating ssa starting from var " + (var ? var->ToString() : "") + " and type " + (type ? type->ToString() : ""));
    THROW_ASSERT(!var || var->get_kind() == tree_reindex_K,
        "Var node is not a tree reindex");
    ...
    if (var and GetPointer<ssa_name>(curr_node)->CGetUseStmts().empty()) {
        THROW_ASSERT(GET_CONST_NODE(var)->get_kind() == var_decl_K or GET_CONST_NODE(var)->get_kind() == parm_decl_K,
            GET_CONST_NODE(var)->get_kind_text());
        std::map<TreeVocabularyTokenTypes_TokenEnum, std::string>
            gimple_nop_IR_schema;
        ...
        this->TreeM->create_tree_node(gimple_nop_node_id, gimple_nop_K,
            gimple_nop_IR_schema);
...
tree_nodeRef tree_manipulation::create_gimple_modify_stmt(
    const tree_nodeRef op0,
    const tree_nodeRef op1,
    ...
    const unsigned int bb_index) const
{
    ...
    INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
        "---Created node " + STR(node_ref));

    return node_ref;
}

tree_nodeRef tree_manipulation::CreateGimpleAssign(
    const tree_nodeRef type,
    const tree_nodeRef op,
...
/// GIMPLE_CALL
tree_nodeRef tree_manipulation::create_gimple_call(
    const tree_nodeConstRef called_function,
...
tree_nodeRef tree_manipulation::create_gimple_cond(
...
tree_nodeRef tree_manipulation::create_gimple_return(
    const tree_nodeRef& type,
    const tree_nodeRef& expr,
    std::string srcp,
    unsigned int bb_index) const
...
/// Create a gimple_phi
tree_nodeRef tree_manipulation::create_phi_node(
    tree_nodeRef& ssa_res,
    ...
    bool virtual_flag) const
{
    ...
    if (sn_ref)
        ...
        const tree_nodeRef ssa_res_type_node = tree_helper::get_type_node(
            GET_NODE(list_of_def_edge.begin()->first), ssa_res_type_index);
    ...
    PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
        "Created node " + STR(GET_INDEX_NODE(phi_stmt)) + " (" + GET_NODE(phi_stmt)->get_kind_text() + ")");
...
blocRef tree_manipulation::create_basic_block(
    ...
    std::vector<tree_nodeRef> phi) const
{
    ...
    for (auto pred : predecessors) {
        ...
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "   Added predecessor " + STR(pred));
    }
    /// Insert the list of successors
    for (auto suc : successors) {
        new_bb->list_of_succ.push_back(suc);
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "   Added successor   " + STR(suc));
    }
    /// Insert the list of statement
    for (auto tn : stmt) {
        new_bb->PushBack(tn);
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "   Added statement   " + STR(GET_INDEX_NODE(tn)) + " (" + GET_NODE(tn)->get_kind_text() + ").");
    }
    /// Insert the list of phi node
    for (auto tn : phi) {
        new_bb->AddPhi(tn);
        PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "   Added phi   " + STR(GET_INDEX_NODE(tn)) + " (" + GET_NODE(tn)->get_kind_text() + ").");
    }
...
void tree_manipulation::bb_add_stmts(
    blocRef& bb,
    const std::vector<tree_nodeRef>& stmts) const
{
    /// Insert the list of statement
    for (auto tn : stmts) {
        bb_add_stmt(bb, tn);
...
void tree_manipulation::bb_add_stmt(blocRef& bb,
    const tree_nodeRef& stmt) const
{
    /// Insert the list of statement
    bb->PushBack(stmt);
    PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
        "   Added statement   " + STR(GET_INDEX_NODE(stmt)) + " (" + GET_NODE(stmt)->get_kind_text() + ").");
}

/// Add a list of successors in basic block bb
void tree_manipulation::bb_add_successors(
    blocRef& bb,
    const std::vector<unsigned int>& successors) const
{
...
void tree_manipulation::bb_add_successor(blocRef& bb,
    const unsigned int& successor) const
{
    bb->add_succ(successor);
}

/// Add a list of predecessors in basic block bb
void tree_manipulation::bb_add_predecessors(
    blocRef& bb,
    const std::vector<unsigned int>& predecessors) const
{
...
void tree_manipulation::bb_add_predecessor(
    blocRef& bb,
    const unsigned int& predecessor) const
{
    bb->add_pred(predecessor);
}

/// Remove a list of successors from basic block bb
void tree_manipulation::bb_remove_successors(
    blocRef& bb,
    const std::vector<unsigned int>& successors) const
{
    for (auto successor : successors) {
...
void tree_manipulation::bb_remove_successor(
    blocRef& bb,
    const unsigned int& successor) const
{
...
void tree_manipulation::bb_remove_predecessors(
    blocRef& bb,
    const std::vector<unsigned int>& predecessors) const
{
    for (auto predecessor : predecessors) {
...
void tree_manipulation::bb_remove_predecessor(
    blocRef& bb,
    const unsigned int& predecessor) const
{
...
void tree_manipulation::bb_set_false_edge(
    blocRef& bb,
    const unsigned int& false_edge_index) const
{
    bb->false_edge = false_edge_index;
}

/// Set the true edge of basic block bb
void tree_manipulation::bb_set_true_edge(
    blocRef& bb,
    const unsigned int& true_edge_index) const
{
    bb->true_edge = true_edge_index;
}

void tree_manipulation::create_label(
    const blocRef block,
    const unsigned int function_decl_nid) const
{
    std::map<TreeVocabularyTokenTypes_TokenEnum, std::string> IR_schema;
    ...
    if (block->CGetStmtList().size()) {
        ...
        if (le) {
            ...
            TreeM->create_tree_node(label_decl_name_nid, identifier_node_K,
                IR_schema);
            IR_schema.clear();
    ...
    if (!label_void_type_name_nid) {
        ...
        if (!label_type_nid) {
            ...
            THROW_ASSERT(
                label_void_type_name_nid == GET_INDEX_NODE(GetPointer<void_type>(TreeM->get_tree_node_const(label_type_nid))->name),
...
tree_nodeRef tree_manipulation::create_function_decl(
    const std::string& function_name,
    const tree_nodeRef& scpe,
    const std::vector<tree_nodeRef>& argsT,
    const tree_nodeRef& returnType,
    std::string srcp,
    bool with_body) const
{
    ...
    for(auto par_type : boost::adaptors::reverse(argsT))
    {
    ...
    for(auto par_type : argsT)
    {
       const std::string p_name = "_P" + boost::lexical_cast<std::string>(Pindex);
       const auto p_identifier = create_identifier_node(p_name);
       const auto p_decl = create_parm_decl(p_identifier, par_type, function_decl_res, GetPointer<type_node>(GET_NODE(par_type))->size,
             par_type, tree_nodeRef(), tree_nodeRef(),
             srcp, GetPointer<const type_node>(GET_NODE(par_type))->algn,
             1, false, false);
       fd->AddArg(p_decl);
       ++Pindex;
    }
...
tree_nodeRef tree_manipulation::CreateOrExpr(
    const tree_nodeConstRef first_condition,
    const tree_nodeConstRef second_condition,
    const blocRef block) const
{
    if (block and reuse) {
    ...
    std::map<TreeVocabularyTokenTypes_TokenEnum, std::string> ssa_schema,
        truth_or_expr_schema, gimple_assign_schema;
    /// Create the or expr
    ...
    TreeM->create_tree_node(truth_or_expr_id, truth_or_expr_K,
        truth_or_expr_schema);
    ...
    TreeM->create_tree_node(gimple_assign_id, gimple_assign_K,
        gimple_assign_schema);
    if (block) {
        block->PushBack(TreeM->GetTreeReindex(gimple_assign_id));
    } else {
        GetPointer<ssa_name>(TreeM->get_tree_node_const(ssa_node_nid))
            ->SetDefStmt(TreeM->GetTreeReindex(gimple_assign_id));
    }
    return TreeM->GetTreeReindex(ssa_node_nid);
}

tree_nodeRef tree_manipulation::CreateAndExpr(
    const tree_nodeConstRef first_condition,
    const tree_nodeConstRef second_condition,
    const blocRef block) const
{
    ...
    std::map<TreeVocabularyTokenTypes_TokenEnum, std::string> ssa_schema,
        truth_and_expr_schema, gimple_assign_schema;
    /// Create the and expr
    ...
    TreeM->create_tree_node(truth_and_expr_id, truth_and_expr_K,
        truth_and_expr_schema);
    ...
    const blocRef block) const
{
    ...
    std::map<TreeVocabularyTokenTypes_TokenEnum, std::string> ssa_schema,
        truth_not_expr_schema, gimple_assign_schema;
    /// Create the not expr
    ...
    TreeM->create_tree_node(truth_not_expr_id, truth_not_expr_K,
        truth_not_expr_schema);
...
{
    INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
        "-->Extracting condition from " + condition->ToString());
    ...
    if (GET_NODE(gc->op0)->get_kind() == ssa_name_K || GetPointer<cst_node>(GET_NODE(gc->op0)) != nullptr) {
        ...
        INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "---Created statement " + cond1_created_stmt->ToString());
        /// and then add to the bb1 statement list
        if (block)
            block->PushBack(cond1_created_stmt);
        else {
            GetPointer<ssa_name>(TreeM->get_tree_node_const(ret))
                ->SetDefStmt(TreeM->GetTreeReindex(cond1_gimple_stmt_id));
        }
        INDENT_DBG_MEX(
            DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
            "<--Condition created is " + TreeM->GetTreeReindex(ret)->ToString());
        return TreeM->GetTreeReindex(ret);
    }
}

tree_nodeRef tree_manipulation::CreateNopExpr(
    const tree_nodeConstRef operand,
    const tree_nodeConstRef type) const
{
    ...
    if (not ssa_operand and not int_cst_operand) {
        ...
        THROW_UNREACHABLE(
            "Cannot create nop expr from something that is not a ssa: " + operand->ToString());
        return tree_nodeRef();
    }
...
tree_nodeRef tree_manipulation::CreateUnsigned(
    const tree_nodeConstRef signed_type) const
{
    ...
    THROW_ASSERT(not int_signed_type->unsigned_flag,
        signed_type->ToString() + " is not signed");
    ...
    const auto max = [&]() -> tree_nodeRef {
        ...
        max_schema[TOK(TOK_VALUE)] = STR(
            (2 * -(tree_helper::get_integer_cst_value(GetPointer<const integer_cst>(GET_NODE(int_signed_type->min))))) - 1);
...
tree_nodeRef tree_manipulation::CreateEqExpr(
    const tree_nodeConstRef first_operand,
    ...
    const blocRef block) const
{
    ...
    std::map<TreeVocabularyTokenTypes_TokenEnum, std::string> ssa_schema,
        eq_expr_schema, gimple_assign_schema;
    /// Create the eq expr
...
tree_nodeRef tree_manipulation::CreateCallExpr(
    const tree_nodeConstRef called_function,
    const std::vector<tree_nodeRef>& args,
    const std::string& srcp) const
{
...
tree_nodeRef tree_manipulation::CreateGimpleAssignAddrExpr(
...
tree_nodeRef tree_manipulation::CreateVectorBooleanType(
    const unsigned int number_of_elements) const
{
    const auto boolean_type = create_boolean_type();
    const auto size = TreeM->CreateUniqueIntegerCst(number_of_elements,
        create_size_type()->index);
