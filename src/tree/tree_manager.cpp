...
void tree_manager::AddTreeNode(unsigned int i, tree_nodeRef curr)
{
...
unsigned int tree_manager::function_index(std::string function_name) const
{
   ...
   for(std::map<unsigned int, tree_nodeRef>::const_iterator it = function_decl_nodes.begin(); it != function_decl_nodes.end(); ++it)
   {
      ...
      if(name == function_name || function_name == std::string("-") || (simple_name != "" && function_name == simple_name))
      {
   ...
   switch (curr_tn->get_kind())
   {
      ...
      case ssa_name_K:
      {
         ...
         if(sn->CGetDefStmts().size()==1)
         {
            ...
            if((Param->getOption<bool>(OPT_compare_models) or Param->getOption<bool>(OPT_normalize_models) or Param->getOption<bool>(OPT_compare_measure_regions) or Param->isOption(OPT_hand_mapping)) and (GET_NODE(gm->op1)->get_kind() == call_expr_K || GET_NODE(gm->op1)->get_kind() == aggr_init_expr_K))
            {
               ...
               if(curr_block == def_block)
               {
                  ...
                  for(copy_block_it = copy_bloc_to_stmt.begin(); copy_block_it != copy_bloc_to_stmt.end(); ++copy_block_it)
                  {
                     ...
                     if(copy_block != curr_block)
                     {
                        ...
                        INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "Copying statement " + STR(GET_INDEX_NODE(sn->CGetDefStmt())) + " in block " + STR(copy_block_it->first) + " (!= " + STR((stmt_to_bloc.find(GET_INDEX_NODE(sn->CGetDefStmt())))->second) + ")");
...
void tree_manager::ReplaceTreeNode(const tree_nodeRef stmt, const tree_nodeRef old_node, tree_nodeRef new_node)
{
...
void tree_manager::RecursiveReplaceTreeNode(tree_nodeRef & tn, const tree_nodeRef old_node, tree_nodeRef new_node, const tree_nodeRef stmt, const bool definition)
{
   ...
   THROW_ASSERT(tn->get_kind() == tree_reindex_K, "Node is not a tree reindex");
   tree_nodeRef curr_tn= GET_NODE(tn);
   INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, function_debug_level, "-->Replacing " + old_node->ToString() + "(" + STR(GET_INDEX_NODE(old_node)) + ") with " + new_node->ToString() + "(" + STR(GET_INDEX_NODE(new_node)) + ") starting from node " + STR(GET_INDEX_NODE(tn)) + "(" + STR(&tn) + "): " + tn->ToString() + ")");
   if(GET_INDEX_NODE(tn) == GET_INDEX_NODE(old_node))
   {
      ...
      tn = new_node;
      INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, function_debug_level, "<--Replaced " + old_node->ToString() + "(" + STR(GET_INDEX_NODE(old_node)) + ") with " + new_node->ToString() + "(" + STR(GET_INDEX_NODE(new_node)) + ") New statement " + STR(GET_INDEX_NODE(tn)) + ": " + tn->ToString() + ")");
      return;
   }
...
void tree_manager::erase_usage_info(tree_nodeRef tn, tree_nodeRef stmt)
{
...
void tree_manager::insert_usage_info(tree_nodeRef tn, tree_nodeRef stmt)
{
...
void tree_manager::merge_tree_managers(const tree_managerRef source_tree_manager)
{
   ...
   for (const auto ti : tree_nodes)
   {
      ...
      if(not dn)
      {
         ...
         {
            THROW_ASSERT(global_type_symbol_table.find(symbol_name) == global_type_symbol_table.end(), "duplicated symbol in global_type_symbol_table: "+global_type_symbol_table.find(symbol_name)->first+ " " + STR(global_type_symbol_table.find(symbol_name)->second) + " " + STR(ti.first));
            INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
      ...
      if(dn)
      {
         ...
         else
         {
            ...
            if((GetPointer<function_decl>(tn) and GetPointer<function_decl>(tn)->static_flag) or
                  (GetPointer<var_decl>(tn) and (GetPointer<var_decl>(tn)->static_flag or GetPointer<var_decl>(tn)->static_static_flag))
                  )
            {
               INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "---Static declaration");
               THROW_ASSERT((GetPointer<function_decl>(tn) and GetPointer<function_decl>(tn)->body) or (GetPointer<var_decl>(tn) and static_symbol_table.find(symbol_name+"-"+symbol_scope) == static_symbol_table.end()) or (GetPointer<function_decl>(tn) and not GetPointer<function_decl>(tn)->body and static_function_header_symbol_table.find(symbol_name+"-"+symbol_scope) == static_function_header_symbol_table.end()), "duplicated static symbol in the current tree_manager: "+symbol_name+"-"+symbol_scope+" " + STR(ti.first));
            ...
            {
              // THROW_ASSERT(global_decl_symbol_table.find(symbol_name+"-"+symbol_scope) == global_decl_symbol_table.end(), "duplicated symbol in global_decl_symbol_table: "+global_decl_symbol_table.find(symbol_name+"-"+symbol_scope)->first + " == " + boost::lexical_cast<std::string>(ti.first));
              continue;
            }
   ...
   for(const auto& ti_source : source_tree_nodes)
   {
      ...
      if (not dn)
      {
         ...
         else
         {
            ...
            if(tn->get_kind() == record_type_K and GetPointer<record_type>(tn)->unql and GetPointer<record_type>(tn)->qual == TreeVocabularyTokenTypes_TokenEnum::FIRST_TOKEN)
            {
               std::unordered_map<std::string, unsigned int>::const_iterator gst_it = global_type_symbol_table.find("u struct " + symbol_name);
            ...
            {
               std::unordered_map<std::string, unsigned int>::const_iterator gst_it = global_type_symbol_table.find("u union " + symbol_name);
               if(gst_it != global_type_symbol_table.end())
               {
   ...
   for(auto ti_source : source_tree_nodes)
   {
      ...
      if(dn)
      {
         ...
         else
         {
            ...
            std::unordered_map<std::string, unsigned int>::const_iterator gst_it =  global_decl_symbol_table.find(symbol_name+"-"+symbol_scope);
...
//                     )
                  ...
                  (
                     ...
                     (
                        ...
                        ||
                        GetPointer<var_decl>(tree_nodes.find(gst_it->second)->second)->extern_flag
                     )
                     and
                  ...
                  (
                     ...
                     GetPointer<function_decl>(tree_nodes.find(gst_it->second)->second)
                     and
                     (GetPointer<function_decl>(tree_nodes.find(gst_it->second)->second))->include_name == "<built-in>"
                     and
                     (GetPointer<function_decl>(tn))->include_name.size()
                     and
...
bool tree_manager::check_for_decl(const tree_nodeRef tn, const tree_managerRef TM_this, std::string & symbol_name, std::string & symbol_scope, unsigned int ASSERT_PARAMETER(node_id), const std::unordered_map<unsigned int, std::string> &global_type_unql_symbol_table)
{
...
bool tree_manager::check_for_type(const tree_nodeRef tn, const tree_managerRef TM, std::string & symbol_name, std::string &symbol_scope, const std::unordered_map<std::string, unsigned int> &global_type_symbol_table, unsigned int DEBUG_PARAMETER(node_id))
{
   ...
   if(global_type_symbol_table.find(symbol_name) != global_type_symbol_table.end())
      return true;
