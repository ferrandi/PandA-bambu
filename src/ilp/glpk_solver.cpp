   ...
   for(int i = 0; i < nc; i++)
   {
      ...
      switch (glp_get_col_type(lp, var))
      {
         ...
         case GLP_DB:
         {
            if((lower_bounds.find(i) != lower_bounds.end())&&(upper_bounds.find(i) != upper_bounds.end()))
            {
               THROW_ASSERT(lower_bounds[i] <= upper_bounds[i], "Error in bound of variable " + boost::lexical_cast<std::string>(i) + " : " + boost::lexical_cast<std::string>(lower_bounds[i]) + " " + boost::lexical_cast<std::string>(glp_get_col_ub(lp, var)));
               glp_set_col_bnds(lp, var, GLP_DB, lower_bounds[i], upper_bounds[i]);
            }
            else
            {
