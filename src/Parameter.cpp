...
#include "config_I386_CLANG40_VERSION.hpp"
...
void Parameter::PrintFullHeader(std::ostream & os) const
{
   ...
   os << "********************************************************************************" << std::endl;
   os << "                Copyright (c) 2004-2017 Politecnico di Milano" << std::endl;
   ...
   switch (next_option)
   {
      ...
      case 'd':
      {
         ...
         if(std::string(optarg_param) == "umpversion")
         {
            ...
            {
               PRINT_OUT_MEX(OUTPUT_LEVEL_NONE, 0, I386_CLANG40_VERSION);
            }
#endif
...
#endif
#if (HAVE_I386_CLANG40_COMPILER && HAVE_I386_CLANG40_MX32)
                   || getOption<GccWrapper_CompilerTarget>(OPT_default_compiler) == GccWrapper_CompilerTarget::CT_I386_CLANG40
#endif
                  )
...
#endif
#if HAVE_I386_CLANG40_COMPILER
         if(std::string(optarg_param) == "I386_CLANG40")
         {
