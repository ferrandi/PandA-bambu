...
operation::operation() :
   commutative(false),
   bounded(true),
   primary_inputs_registered(false)
{
}
...
void operation::xload(const xml_element* Enode, const technology_nodeRef fu, const ParameterConstRef Param, const target_deviceRef device)
{
   ...
   if(CE_XVM(commutative, Enode)) LOAD_XVM(commutative, Enode);
   ...
   if(CE_XVM(primary_inputs_registered, Enode)) LOAD_XVM(primary_inputs_registered, Enode);
   ...
   if(CE_XVM(pipe_parameters, Enode)) LOAD_XVM(pipe_parameters, Enode);
   if(CE_XVM(portsize_parameters, Enode)) LOAD_XVM(portsize_parameters, Enode);
   ...
   if(CE_XVM(execution_time, Enode)) LOAD_XVM(execution_time, Enode);
   if(CE_XVM(initiation_time, Enode))
      LOAD_XVM(initiation_time, Enode);
   if(CE_XVM(cycles, Enode)) LOAD_XVM(cycles, Enode);
   if(CE_XVM(stage_period, Enode)) LOAD_XVM(stage_period, Enode);
   bool synthesis_dependent = false;
   if (CE_XVM(synthesis_dependent, Enode)) LOAD_XVM(synthesis_dependent, Enode);
...
void operation::xwrite(xml_element* rootnode, const technology_nodeRef, const ParameterConstRef, const TargetDevice_Type /*type*/)
{
  ...
  ///commutative property
  if(commutative) WRITE_XVM(commutative, Enode);
  if(!bounded) WRITE_XVM(bounded, Enode);
  ...
  if(supported_types_string != "") WRITE_XNVM(supported_types, supported_types_string, Enode);
  if(pipe_parameters != "") WRITE_XVM(pipe_parameters, Enode);
  if(portsize_parameters != "") WRITE_XVM(portsize_parameters, Enode);
...
void functional_unit::gload(const std::string& definition, const std::string& fu_name, technology_nodeRef
#if HAVE_CIRCUIT_BUILT
   owner
#endif
...
void functional_unit::xload(const xml_element* Enode, const technology_nodeRef fu, const ParameterConstRef Param, const target_deviceRef device)
{
   ...
   for (const auto & iter_int : list_int)
   {
      ...
      else if (EnodeC->get_name() == GET_CLASS_NAME(characterizing_constant_value))
      {
          ...
          */
         const xml_text_node* text = EnodeC->get_child_text();
         if (!text) THROW_ERROR ("characterizing_constant_value is missing for " + EnodeC->get_name());
         if(text->get_content() == "") THROW_ERROR ("characterizing_constant_value is missing for " + EnodeC->get_name());
      ...
      else if (EnodeC->get_name() == GET_CLASS_NAME(memory_type))
      {
          ...
          */
         const xml_text_node* text = EnodeC->get_child_text();
         if (!text) THROW_ERROR ("memory_type is missing for " + EnodeC->get_name());
         if(text->get_content() == "") THROW_ERROR ("memory_type is missing for " + EnodeC->get_name());
      ...
      else if (EnodeC->get_name() == GET_CLASS_NAME(channels_type))
      {
          ...
          */
         const xml_text_node* text = EnodeC->get_child_text();
         if (!text) THROW_ERROR ("channels_type is missing for " + EnodeC->get_name());
         if(text->get_content() == "") THROW_ERROR ("channels_type is missing for " + EnodeC->get_name());
      ...
      else if (EnodeC->get_name() == GET_CLASS_NAME(memory_ctrl_type))
      {
          ...
          */
         const xml_text_node* text = EnodeC->get_child_text();
         if (!text) THROW_ERROR ("memory_ctrl_type is missing for " + EnodeC->get_name());
         if(text->get_content() == "") THROW_ERROR ("memory_ctrl_type is missing for " + EnodeC->get_name());
      ...
      else if (EnodeC->get_name() == GET_CLASS_NAME(bram_load_latency))
      {
          ...
          */
         const xml_text_node* text = EnodeC->get_child_text();
         if (!text) THROW_ERROR ("bram_load_latency is missing for " + EnodeC->get_name());
         if(text->get_content() == "") THROW_ERROR ("bram_load_latency is missing for " + EnodeC->get_name());
      ...
      else if (EnodeC->get_name() == GET_CLASS_NAME(component_timing_alias))
      {
          ...
          */
         const xml_text_node* text = EnodeC->get_child_text();
         if (!text) THROW_ERROR ("component_timing_alias is missing for " + EnodeC->get_name());
         if(text->get_content() == "") THROW_ERROR ("component_timing_alias is missing for " + EnodeC->get_name());
...
functional_unit_template::functional_unit_template():
    FU(new functional_unit),
    no_constant_characterization(false)
{

}

functional_unit_template::functional_unit_template(const xml_nodeRef XML_description):
    FU(new functional_unit(XML_description)),
    no_constant_characterization(false)
{
