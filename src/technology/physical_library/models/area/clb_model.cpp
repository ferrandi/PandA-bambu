...
clb_model::clb_model(const ParameterConstRef _Param) :
      area_model(_Param),
...
clb_model::clb_model(const ParameterConstRef _Param, const double &_area_) :
      area_model(_Param),
      area(_area_)
{
}
...
{
   if (used_resources.find(LUT_FF_PAIRS) != used_resources.end()) os << "LUT/FF pairs: " << used_resources.find(LUT_FF_PAIRS)->second << std::endl;
}
