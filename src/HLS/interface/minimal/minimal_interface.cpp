...
void minimal_interface::build_wrapper(structural_objectRef wrappedObj,
                          ...
                          structural_managerRef SM_minimal_interface)
{
   ...
   bool Has_intern_shared_data =
      ...
      (HLSMgr->hasToBeInterfaced(funId) and top_function_ids.find(funId) == top_function_ids.end()) ||
   ...
   if(with_slave)
   {
      portsToConstant.insert(
               wrappedObj->find_member("Sin_DataRdy", port_o_K, wrappedObj));
      portsToConstant.insert(
               wrappedObj->find_member("Sin_Rdata_ram", port_o_K, wrappedObj));
   }
   if(with_master)
   {
      portsToConstant.insert(
               wrappedObj->find_member("Min_oe_ram", port_o_K, wrappedObj));
      portsToConstant.insert(
               wrappedObj->find_member("Min_we_ram", port_o_K, wrappedObj));
      portsToConstant.insert(
               wrappedObj->find_member("Min_addr_ram", port_o_K, wrappedObj));
      portsToConstant.insert(
               wrappedObj->find_member("Min_Wdata_ram", port_o_K, wrappedObj));
      portsToConstant.insert(
               wrappedObj->find_member("Min_data_ram_size", port_o_K, wrappedObj));
   }
   ...
   if(!Has_intern_shared_data)
   {
      if(!Has_extern_allocated_data)
      {
         if(!with_master && with_slave)
         {
            /// slave INs
            portsToConstant.insert(
                     wrappedObj->find_member("S_oe_ram", port_o_K, wrappedObj));
            portsToConstant.insert(
                     wrappedObj->find_member("S_we_ram", port_o_K, wrappedObj));
            portsToConstant.insert(
                     wrappedObj->find_member("S_addr_ram", port_o_K, wrappedObj));
            portsToConstant.insert(
                     wrappedObj->find_member("S_Wdata_ram", port_o_K, wrappedObj));
            portsToConstant.insert(
                     wrappedObj->find_member("S_data_ram_size", port_o_K, wrappedObj));
            /// slave OUTs
            portsToSkip.insert(
      ...
      else
      {
         if(with_master && !Has_unknown_addresses)
         {
            ...
            if(parameters->getOption<MemoryAllocation_ChannelsType>(OPT_channels_type) == MemoryAllocation_ChannelsType::MEM_ACC_11 or
                  parameters->getOption<MemoryAllocation_ChannelsType>(OPT_channels_type) == MemoryAllocation_ChannelsType::MEM_ACC_N1 )
            {
            ...
            if(!with_slave)
            {
               ...
               if (null_values.find(GET_TYPE_SIZE(port1)) == null_values.end())
               {
                  structural_objectRef const_obj =
                        SM_minimal_interface->add_constant(
                           "null_value_" + STR(GET_TYPE_SIZE(port1)),
                           interfaceObj, port1->get_typeRef(), STR(0));
   ...
   for (unsigned int i = 0;
        i < GetPointer<module>(wrappedObj)->get_in_port_size(); ++i)
   {
      structural_objectRef port_in =
            GetPointer<module>(wrappedObj)->get_in_port(i);
      if (portsToSkip.find(port_in) == portsToSkip.end() &&
            portsToConnect.find(port_in) == portsToConnect.end() &&
            portsToConstant.find(port_in) == portsToConstant.end())
      {
         structural_objectRef ext_port = interfaceObj->find_member(GetPointer<port_o>(port_in)->get_id(), port_o_K, interfaceObj);

         if(!ext_port)
         {
            if(port_in->get_kind() == port_vector_o_K)
               ext_port= SM_minimal_interface->add_port_vector(
                                               GetPointer<port_o>(port_in)->get_id(), port_o::IN,
                                               GetPointer<port_o>(port_in)->get_ports_size(),
                                               interfaceObj, port_in->get_typeRef());
            else
               ext_port= SM_minimal_interface->add_port(
                                            GetPointer<port_o>(port_in)->get_id(), port_o::IN,
                                            interfaceObj, port_in->get_typeRef());
         }
      ...
      else if(portsToConstant.find(port_in) != portsToConstant.end())
      {
         if (null_values.find(GET_TYPE_SIZE(port_in)) == null_values.end())
         {
            structural_objectRef const_obj =
                  SM_minimal_interface->add_constant(
                     "null_value_" + STR(GET_TYPE_SIZE(port_in)),
                     interfaceObj, port_in->get_typeRef(), STR(0));
   ...
   // out ports
   for (unsigned int i = 0;
        i < GetPointer<module>(wrappedObj)->get_out_port_size(); ++i)
   {
      structural_objectRef port_out =
            GetPointer<module>(wrappedObj)->get_out_port(i);
      if (portsToSkip.find(port_out) == portsToSkip.end() && portsToConnect.find(port_out) == portsToConnect.end())
      {
         ...
         if(port_out->get_kind() == port_vector_o_K)
            ext_port = SM_minimal_interface->add_port_vector(
                                               GetPointer<port_o>(port_out)->get_id(), port_o::OUT,
   ...
   // in-out ports
   for (unsigned int i = 0;
        i < GetPointer<module>(wrappedObj)->get_in_out_port_size(); ++i)
   {
      structural_objectRef port_in_out =
         GetPointer<module>(wrappedObj)->get_in_out_port(i);
      structural_objectRef ext_port;
      if(port_in_out->get_kind() == port_vector_o_K)
         ext_port = SM_minimal_interface->add_port_vector(
                                            GetPointer<port_o>(port_in_out)->get_id(), port_o::IO,
