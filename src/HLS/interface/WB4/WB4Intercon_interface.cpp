...
WB4Intercon_interface::exec()
{
   ...
   structural_managerRef SM_wb4_interconnected =
...
static unsigned int get_data_bus_bitsize(const hlsRef HLS, const HLS_managerRef HLSMgr)
{
   ...
   std::map<unsigned int, memory_symbolRef> parameters =
				...
				const HLS_managerRef HLSMgr)
{
   ...
   memory::add_memory_parameter(
       SM, WB_BASE_ADDRESS "_" + functionName + "_interconnected", STR(0));
    ...
    const HLS_managerRef HLSMgr)
{
   ...
   structural_objectRef interconnect =
   ...
   for (unsigned int i = 0; i < interconPortsNumber; ++i)
   {
      ...
      if (portId != CLOCK_PORT_NAME && portId != RESET_PORT_NAME)
      {
         ...
         SM->add_connection(destPort,
                            portPtr->get_port(portPtr->get_ports_size() - 1));
      }
   }
   ...
   baseAddressFile
         << std::bitset<8 * sizeof(unsigned int)>(HLSMgr->Rmem->get_first_address(HLS->functionId))
         << '\n'
         << std::bitset<8 * sizeof(unsigned int)>(
             HLSMgr->Rmem->get_last_address(HLS->functionId, HLSMgr))
         << '\n';
   ...
   structural_type_descriptorRef b_type =
      structural_type_descriptorRef(new structural_type_descriptor("bool", 1));
   structural_objectRef irqPort =
         SM->add_port_vector(WB_IRQ_PORT_NAME, port_o::OUT, static_cast<unsigned int>(slaves.size()), interfaceObj, b_type);

   unsigned int idx = 0;
   for (std::vector<structural_objectRef>::const_iterator
	      itr = slaves.begin(), end = slaves.end(); itr != end; ++itr, ++idx)
   {
	 ...
	 std::string portId = port->get_id();
	 structural_objectRef destPort =
	       interconnect->find_member(portId, port_o_K, interconnect);
	 if (destPort && portId != CLOCK_PORT_NAME && portId != RESET_PORT_NAME)
	 {
	    structural_objectRef signal =
		  SM->add_sign(currentModule->get_id() + portId,
	    ...
	    SM->add_connection(signal,
			       destPortPtr->get_port(destPortPtr->get_ports_size() - 1));
	 }
      }
	 ...
	 if (destPort)
	 {
      ...
      structural_objectRef irqSignal =
            SM->add_sign(STR(WB_IRQ_PORT_NAME) + "_" + STR(idx) + "_int",
                         SM->get_circ(), b_type);

      SM->add_connection((*itr)->find_member(WB_IRQ_PORT_NAME, port_o_K, *itr),
			 irqSignal);
      SM->add_connection(GetPointer<port_o>(irqPort)->get_port(idx),
			 irqSignal);
   }
