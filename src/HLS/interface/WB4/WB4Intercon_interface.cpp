...
WB4Intercon_interface::WB4Intercon_interface(const ParameterConstRef P, const HLS_managerRef HLSManager,
                                             unsigned int functionId, const DesignFlowManager& _design_flow_manager)
...
void WB4Intercon_interface::exec()
{
   ...
   memory::propagate_memory_parameters(wrappedObj, SM_wb4_interconnected);
                         ...
                         const hlsRef HLS, const HLS_managerRef HLSMgr)
{
   ...
   const ir_managerRef TM = HLSMgr->get_ir_manager();
   ...
   auto* interconModule = GetPointer<module_o>(interconnect);
   ...
   const auto topFunctionName = ir_helper::GetFunctionName(HLSMgr->get_ir_manager()->GetIRNode(HLS->functionId));
   ...
   {
      if(HLSMgr->isOmpLambdaFunction(itr)) /// omp lambda function should not be considered
      {
         continue;
      }
      std::string functionName = ir_helper::GetFunctionName(TM->GetIRNode(itr));
   ...
   {
      // Input ports
      auto* currentModule = GetPointer<module_o>(*itr);
