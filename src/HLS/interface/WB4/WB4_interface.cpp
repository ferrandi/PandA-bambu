...
void WB4_interface::build_WB4_complete_logic(structural_managerRef SM, structural_objectRef wrappedObj, structural_objectRef interfaceObj)
{
   ...
   if(HLSMgr->Rmem->get_max_address() > HLS->Param->getOption<unsigned int>(OPT_base_address))
   ...
   if(is_master)
   {
      ...
      if(is_slave) // add the control logic for the multiplexers
      {
         ...
         constBaseAddress->set_parameter("value", baseAddressParameter + " + " + STR(HLSMgr->Rmem->get_first_address(HLS->functionId)));
         ...
         constMaxAddress->set_parameter("value", baseAddressParameter + " + " + STR(HLSMgr->Rmem->get_last_address(HLS->functionId, HLSMgr)));
         ...
         if(!only_mm_parameters_allocated)
         {
            ...
            bool Has_unknown_addresses = HLSMgr->Rmem->has_unknown_addresses() && HLS->Param->getOption<MemoryAllocation_Policy>(OPT_memory_allocation_policy) != MemoryAllocation_Policy::ALL_BRAM &&
   ...
   if(is_slave)
   {
      ...
      GetPointer<module>(addressFilterAddrIS)->set_parameter("MAX_ADDRESS", baseAddressParameter + " + " + STR(HLSMgr->Rmem->get_last_address(HLS->functionId, HLSMgr)));
