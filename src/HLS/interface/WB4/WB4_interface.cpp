...
DesignFlowStep_Status WB4_interface::InternalExec()
{
   ...
   structural_managerRef SM_wb4_interface =
      structural_managerRef(new structural_manager(parameters));
   structural_type_descriptorRef module_type = structural_type_descriptorRef(
      new structural_type_descriptor(module_name));
   ...
   GetPointer<module>(interfaceObj)->set_description(
      "WB4 interface for top component: " + wrappedObj->get_typeRef()->id_type);
...
void WB4_interface::build_WB4_bus_interface(structural_managerRef SM)
{
   ...
   structural_type_descriptorRef b_type =
      structural_type_descriptorRef(new structural_type_descriptor("bool", 1));
   ...
   structural_type_descriptorRef sel_type =
      structural_type_descriptorRef(
         new structural_type_descriptor("bool", data_bus_bitsize/8));
   structural_type_descriptorRef addr_type =
      structural_type_descriptorRef(
         new structural_type_descriptor(
            "bool", addr_bus_bitsize));
   structural_type_descriptorRef data_type = structural_type_descriptorRef(
      new structural_type_descriptor(
         "bool", data_bus_bitsize));

   // Parameters
   std::string functionName =
         tree_helper::name_function(HLSMgr->get_tree_manager(), HLS->functionId);
   memory::add_memory_parameter(
       SM, STR(WB_BASE_ADDRESS) + "_" + functionName, STR(0));
...
{
   structural_objectRef signA =
         GetPointer<port_o>(portA)->get_connected_signal();
   structural_objectRef signB =
         GetPointer<port_o>(portB)->get_connected_signal();
...
void WB4_interface::build_WB4_complete_logic(structural_managerRef SM, structural_objectRef wrappedObj, structural_objectRef interfaceObj)
{
   ...
   if (S_data_ram_size_port && S_Wdata_ram_port &&
         S_addr_ram_port && S_we_ram_port && S_oe_ram_port &&
         Sout_DataRdy_port && Sout_Rdata_ram_port)
   {
      is_slave = true;
   }
   else if (!S_data_ram_size_port && !S_Wdata_ram_port &&
         !S_addr_ram_port && !S_we_ram_port && !S_oe_ram_port &&
         !Sout_DataRdy_port && !Sout_Rdata_ram_port)
   {
      is_slave = false;
   ...
   if (Mout_data_ram_size_port && Mout_Wdata_ram_port &&
         Mout_addr_ram_port && Mout_we_ram_port && Mout_oe_ram_port &&
         M_DataRdy_port && M_Rdata_ram_port)
   {
      is_master = true;
   }
   else if (!Mout_data_ram_size_port && !Mout_Wdata_ram_port &&
         !Mout_addr_ram_port && !Mout_we_ram_port && !Mout_oe_ram_port &&
         !M_DataRdy_port && !M_Rdata_ram_port)
   {
      is_master = false;
   ...
   SM->add_connection(
      interfaceObj->find_member(CLOCK_PORT_NAME, port_o_K, interfaceObj),
      wrappedObj->find_member(CLOCK_PORT_NAME, port_o_K, wrappedObj));

   SM->add_connection(
      interfaceObj->find_member(RESET_PORT_NAME, port_o_K, interfaceObj),
      wrappedObj->find_member(RESET_PORT_NAME, port_o_K, wrappedObj));

   connect_with_signal_name(SM, wrappedObj, DONE_PORT_NAME,
                            interfaceObj, WB_IRQ_PORT_NAME,
   ...
   structural_type_descriptorRef addr_type = structural_type_descriptorRef
      (new structural_type_descriptor("bool", addr_bus_bitsize));

   structural_type_descriptorRef data_type = structural_type_descriptorRef
      (new structural_type_descriptor("bool", data_bus_bitsize));

   structural_type_descriptorRef sel_type = structural_type_descriptorRef
      (new structural_type_descriptor("bool", data_bus_bitsize/8));
   ...
   if (is_master)
   {
      ...
      connect_with_signal(SM, Mout_we_ram_port,
            interfaceObj->find_member(WB_WEOM_PORT_NAME, port_o_K, interfaceObj));

      structural_objectRef orGateReWe = SM->add_module_from_technology_library(
         "orGateReWe", OR_GATE_STD, LIBRARY_STD, interfaceObj,
         HLS->HLS_T->get_technology_manager());
      ...
      structural_objectRef notGateSTB = SM->add_module_from_technology_library(
         "notGateSTB", NOT_GATE_STD, LIBRARY_STD, interfaceObj,
         HLS->HLS_T->get_technology_manager());
      ...
      structural_objectRef andGateSTB = SM->add_module_from_technology_library(
         "andGateSTB", AND_GATE_STD, LIBRARY_STD, interfaceObj,
         HLS->HLS_T->get_technology_manager());
      ...
      connect_with_signal_name(SM, notGateSTB->find_member("out1", port_o_K, notGateSTB),
         in_portAndSTB->get_port(1), "NotAckIM");
      ...
      if (is_slave) // add the control logic for the multiplexers
      {
         ...
         structural_objectRef addressRangeChecker = SM->add_module_from_technology_library
            ("addressRangeChecker", ADDRESS_RANGE_CHECKER_WB, WBLIBRARY, interfaceObj, HLS->HLS_T->get_technology_manager());

         structural_objectRef arcAddress =
            addressRangeChecker->find_member("address", port_o_K, addressRangeChecker);
         GetPointer<port_o>(arcAddress)->set_type(addr_type);
         connect_with_signal(SM, Mout_addr_ram_port, arcAddress);

         structural_objectRef arcBase =
            addressRangeChecker->find_member("base", port_o_K, addressRangeChecker);
         ...
         structural_objectRef signNotInternal =
            SM->add_sign("notInternal", SM->get_circ(), internalNotGate->find_member("out1", port_o_K, internalNotGate)->get_typeRef());
         ...
         if(!only_mm_parameters_allocated)
         {
            ...
            bool Has_unknown_addresses = HLSMgr->Rmem->has_unknown_addresses() &&
                                         HLS->Param->getOption<MemoryAllocation_Policy>(OPT_memory_allocation_policy) != MemoryAllocation_Policy::ALL_BRAM &&
            ...
            else
            {
               ...
               structural_objectRef internalNotGate_delayed = SM->add_module_from_technology_library(
                                                                 "internalNotGate_delayed", NOT_INTERNAL_COMPONENT, WBLIBRARY, interfaceObj,
                                                                 HLS->HLS_T->get_technology_manager());
               ...
               structural_objectRef orGateControl_delayed = SM->add_module_from_technology_library(
                                                               "orGateControl_delayed", OR_GATE_STD, LIBRARY_STD, interfaceObj,
                                                               HLS->HLS_T->get_technology_manager());
               ...
               structural_objectRef signNotInternal_delayed =
                     SM->add_sign("notInternal_delayed", SM->get_circ(),
                                  in_portControl_delayed->get_port(0)->get_typeRef());
               ...
               signControl_delayed = SM->add_sign("control_delayed", SM->get_circ(),
                                                  in_portControl->get_port(0)->get_typeRef());
         ...
         structural_objectRef andGateStbCyc = SM->add_module_from_technology_library(
            "andGateStbCyc", AND_GATE_STD, LIBRARY_STD, interfaceObj,
            HLS->HLS_T->get_technology_manager());
         ...
         connect_with_signal_name(SM, orGateReWe->find_member("out1", port_o_K, orGateReWe),
            in_portStbCyc->get_port(0), "Mout_we_or_oe");
      ...
      {
         connect_with_signal_name(SM, orGateReWe->find_member("out1", port_o_K, orGateReWe),
            interfaceObj->find_member(WB_CYCOM_PORT_NAME, port_o_K, interfaceObj), "Mout_we_or_oe");
         connect_with_signal_name(SM, orGateReWe->find_member("out1", port_o_K, orGateReWe),
             in_portAndSTB->get_port(0), "Mout_we_or_oe");
      }
   ...
   if (is_slave)
   {
      ...
      structural_objectRef andGateWeICS = SM->add_module_from_technology_library(
         "andGateWeICS", AND_GATE_STD, LIBRARY_STD, interfaceObj,
         HLS->HLS_T->get_technology_manager());
      ...
      structural_objectRef weISNotGate = SM->add_module_from_technology_library(
         "weISNotGate", NOT_GATE_STD, LIBRARY_STD, interfaceObj,
         HLS->HLS_T->get_technology_manager());
      SM->add_connection(
         interfaceObj->find_member(WB_WEIS_PORT_NAME, port_o_K, interfaceObj),
      ...
      if (is_master)
      {
         ...
         else
         {
            ...
            structural_objectRef wDataMux = SM->add_module_from_technology_library(
                                               "wDataMux", MUX_GATE_STD,
                                               HLS->HLS_T->get_technology_manager()->get_library(MUX_GATE_STD),
                                               interfaceObj, HLS->HLS_T->get_technology_manager());

            structural_objectRef wDataMuxIn1 =
                  wDataMux->find_member("in1", port_o_K, wDataMux);
            GetPointer<port_o>(wDataMuxIn1)->set_type(data_type);

            structural_objectRef wDataMuxIn2 =
                  wDataMux->find_member("in2", port_o_K, wDataMux);
            GetPointer<port_o>(wDataMuxIn2)->set_type(data_type);

            structural_objectRef wDataMuxOut1 =
                  wDataMux->find_member("out1", port_o_K, wDataMux);
      ...
      if (is_slave)
      {
         ...
         else
         {
            ...
            structural_objectRef rDataMux = SM->add_module_from_technology_library(
                                               "rDataMux", MUX_GATE_STD,
                                               HLS->HLS_T->get_technology_manager()->get_library(MUX_GATE_STD),
                                               interfaceObj, HLS->HLS_T->get_technology_manager());

            structural_objectRef rDataMuxIn1 =
                  rDataMux->find_member("in1", port_o_K, rDataMux);
            GetPointer<port_o>(rDataMuxIn1)->set_type(data_type);

            structural_objectRef rDataMuxIn2 =
                  rDataMux->find_member("in2", port_o_K, rDataMux);
            GetPointer<port_o>(rDataMuxIn2)->set_type(data_type);

            structural_objectRef rDataMuxOut1 =
                  rDataMux->find_member("out1", port_o_K, rDataMux);
   ...
   if (is_master)
   {
      ...
      structural_objectRef dataRamSizeConverter = SM->add_module_from_technology_library(
            "dataRamSizeConverter", DATARAMSIZE_CONVERTER_WB, WBLIBRARY,
            interfaceObj, HLS->HLS_T->get_technology_manager());

      structural_objectRef dRSCIn =
         dataRamSizeConverter->find_member("dataRamSize", port_o_K, dataRamSizeConverter);
      GetPointer<port_o>(dRSCIn)->set_type(drs_type);

      structural_objectRef dRSCOut =
         dataRamSizeConverter->find_member("sel", port_o_K, dataRamSizeConverter);
   ...
   if (is_slave)
   {
      ...
      structural_objectRef selConverter = SM->add_module_from_technology_library(
            "selConverter", SEL_CONVERTER_WB, WBLIBRARY,
            interfaceObj, HLS->HLS_T->get_technology_manager());

      structural_objectRef selCOut =
         selConverter->find_member("dataRamSize", port_o_K, selConverter);
      GetPointer<port_o>(selCOut)->set_type(drs_type);

      structural_objectRef selCIn =
         selConverter->find_member("sel", port_o_K, selConverter);
      GetPointer<port_o>(selCIn)->set_type(sel_type);

      SM->add_connection(
         interfaceObj->find_member(WB_SELIS_PORT_NAME, port_o_K, interfaceObj),
         selConverter->find_member("sel",port_o_K, selConverter));
      ...
      if (is_master)
      {
         ...
         else
         {
            ...
            structural_objectRef dRSMuxIn1 =
                  dRSMux->find_member("in1", port_o_K, dRSMux);
            GetPointer<port_o>(dRSMuxIn1)->set_type(drs_type);
            structural_objectRef dRSMuxIn2 =
                  dRSMux->find_member("in2", port_o_K, dRSMux);
            GetPointer<port_o>(dRSMuxIn2)->set_type(drs_type);
            structural_objectRef dRSMuxOut1 =
                  dRSMux->find_member("out1", port_o_K, dRSMux);
   ...
   if (is_slave)
   {
      ...
      structural_objectRef afAddrISIn1 =
         addressFilterAddrIS->find_member("in1", port_o_K, addressFilterAddrIS);
      GetPointer<port_o>(afAddrISIn1)->set_type(addr_type);

      structural_objectRef afAddrISOut1 =
         addressFilterAddrIS->find_member("out1", port_o_K, addressFilterAddrIS);
      ...
      if (is_master)
      {
         ...
         else
         {
            ...
            structural_objectRef addrMux = SM->add_module_from_technology_library(
                                              "addrMux", MUX_GATE_STD,
                                              HLS->HLS_T->get_technology_manager()->get_library(MUX_GATE_STD),
                                              interfaceObj, HLS->HLS_T->get_technology_manager());

            structural_objectRef addrMuxIn1 =
                  addrMux->find_member("in1", port_o_K, addrMux);
            GetPointer<port_o>(addrMuxIn1)->set_type(addr_type);

            structural_objectRef addrMuxIn2 =
                  addrMux->find_member("in2", port_o_K, addrMux);
            GetPointer<port_o>(addrMuxIn2)->set_type(addr_type);

            structural_objectRef addrMuxOut1 =
                  addrMux->find_member("out1", port_o_K, addrMux);
   ...
   if (is_slave)
   {
      ...
      structural_objectRef andGateAckOS = SM->add_module_from_technology_library(
         "andGateAckOS", AND_GATE_STD, LIBRARY_STD, interfaceObj,
         HLS->HLS_T->get_technology_manager());
      ...
      SM->add_connection(
         interfaceObj->find_member(WB_CYCIS_PORT_NAME, port_o_K, interfaceObj),
         in_portAndAckOS->get_port(2));

      SM->add_connection(
         interfaceObj->find_member(WB_ACKOS_PORT_NAME, port_o_K, interfaceObj),
      ...
      if (is_master)
      {
         ...
         else
         {
            ...
            structural_objectRef dataRdyDemux =
                  SM->add_module_from_technology_library(
                     "dataRdyDemux", DEMUX_GATE_STD,
                     HLS->HLS_T->get_technology_manager()->get_library(DEMUX_GATE_STD),
                     interfaceObj, HLS->HLS_T->get_technology_manager());

            connect_with_signal_name(SM,
                                     dataRdyDemux->find_member("out2", port_o_K, dataRdyDemux),
                                     in_portAndAckOS->get_port(0), "Sout_DataRdyToWb");
