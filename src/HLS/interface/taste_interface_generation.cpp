...
///   auto taste_interface_module = GetPointer<const module>(taste_interface_circuit);
   ...
   GetPointer<module>(taste_interface_circuit)->set_copyright("Copyright (C) 2004-2015 Politecnico di Milano");
   ...
   THROW_ASSERT(HLSMgr->get_HLS(HLSMgr->get_tree_manager()->function_index(STR_CST_taste_endianess_check + function_name))->STG->get_number_of_states() == 1, "Number of states of " STR_CST_taste_endianess_check + function_name + " is " + STR(HLSMgr->get_HLS(HLSMgr->get_tree_manager()->function_index(STR_CST_taste_endianess_check + function_name))->STG->get_number_of_states()));
...
void TasteInterfaceGeneration::ComputeRelationships(DesignFlowStepSet & relationship, const DesignFlowStep::RelationshipType relationship_type)
{
   ...
   if(relationship_type == DesignFlowStep::DEPENDENCE_RELATIONSHIP)
   {
      ...
      const auto reg_status_step = reg_status_vertex != NULL_VERTEX ? design_flow_manager.lock()->CGetDesignFlowGraph()->CGetDesignFlowStepInfo(reg_status_vertex)->design_flow_step : hls_flow_step_factory->CreateHLSFlowStep(HLSFlowStep_Type::ADD_LIBRARY, reg_status_function_id, HLSFlowStepSpecializationConstRef(new AddLibrarySpecialization(true)));
      ...
      const auto endianess_inversion_step = endianess_inversion_vertex != NULL_VERTEX ? design_flow_manager.lock()->CGetDesignFlowGraph()->CGetDesignFlowStepInfo(endianess_inversion_vertex)->design_flow_step : hls_flow_step_factory->CreateHLSFlowStep(HLSFlowStep_Type::ADD_LIBRARY, endianess_inversion_function_id, HLSFlowStepSpecializationConstRef(new AddLibrarySpecialization(true)));
      ...
      const auto endianess_check_step = endianess_check_vertex != NULL_VERTEX ? design_flow_manager.lock()->CGetDesignFlowGraph()->CGetDesignFlowStepInfo(endianess_check_vertex)->design_flow_step : hls_flow_step_factory->CreateHLSFlowStep(HLSFlowStep_Type::ADD_LIBRARY, endianess_check_function_id, HLSFlowStepSpecializationConstRef(new AddLibrarySpecialization(true)));
      ...
      const auto output_multiplexer_step = output_multiplexer_vertex != NULL_VERTEX ? design_flow_manager.lock()->CGetDesignFlowGraph()->CGetDesignFlowStepInfo(output_multiplexer_vertex)->design_flow_step : hls_flow_step_factory->CreateHLSFlowStep(HLSFlowStep_Type::ADD_LIBRARY, output_multiplexer_function_id, HLSFlowStepSpecializationConstRef(new AddLibrarySpecialization(true)));
      ...
      if(memory_enabling_function_id)
      {
         ...
         const auto memory_enabling_step = memory_enabling_vertex != NULL_VERTEX ? design_flow_manager.lock()->CGetDesignFlowGraph()->CGetDesignFlowStepInfo(memory_enabling_vertex)->design_flow_step : hls_flow_step_factory->CreateHLSFlowStep(HLSFlowStep_Type::ADD_LIBRARY, memory_enabling_function_id, HLSFlowStepSpecializationConstRef(new AddLibrarySpecialization(true)));
         ...
         const auto data_size_step = data_size_vertex != NULL_VERTEX ? design_flow_manager.lock()->CGetDesignFlowGraph()->CGetDesignFlowStepInfo(data_size_vertex)->design_flow_step : hls_flow_step_factory->CreateHLSFlowStep(HLSFlowStep_Type::ADD_LIBRARY, data_size_function_id, HLSFlowStepSpecializationConstRef(new AddLibrarySpecialization(true)));
         ...
         const auto address_translation_step = address_translation_vertex != NULL_VERTEX ? design_flow_manager.lock()->CGetDesignFlowGraph()->CGetDesignFlowStepInfo(address_translation_vertex)->design_flow_step : hls_flow_step_factory->CreateHLSFlowStep(HLSFlowStep_Type::ADD_LIBRARY, address_translation_function_id, HLSFlowStepSpecializationConstRef(new AddLibrarySpecialization(true)));
