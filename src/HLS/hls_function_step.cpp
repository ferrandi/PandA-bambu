...
bool HLSFunctionStep::HasToBeExecuted() const
{
   ...
   for(boost::tie(ie, ie_end) = boost::in_edges(current_step, *design_flow_graph); ie != ie_end; ie++)
   {
      ...
      if(pre_info->status != DesignFlowStep_Status::SUCCESS and pre_info->status != DesignFlowStep_Status::EMPTY) continue;
      if(!GetPointer<HLSFunctionStep>(pre_info->design_flow_step)) continue;
      if(GetPointer<HLSFunctionStep>(pre_info->design_flow_step)->funId != funId) continue;
      return true;
   }
   return false;
}
