                                            ...
                                            const std::vector<ModuleGenerator::parameter>& /* _ports_inout */)
{
   ...
   out << "always @(*)\n";
   out << "  started0 = (started | " << _ports_in[i_start].name << ") & !" << _ports_in[i_empty_n].name << ";\n";
   ...
   out << "always @(*)\n";
   out << "begin\n";
   out << "  " << _ports_out[o_done].name << "0 = (" << _ports_in[i_start].name << " & " << _ports_in[i_empty_n].name
       << ") | (started & " << _ports_in[i_empty_n].name << ");\n";
   out << "end\n\n";

   out << "assign " << _ports_out[o_done].name << " = " << _ports_out[o_done].name << "0;\n";
   out << "assign " << _ports_out[o_read].name << " = " << _ports_out[o_done].name << "0;\n";
}