 ...
 */
/**
 * @file ReadWriteMAXIModuleGenerator.cpp
 * @brief
 *
 *
 *
...
#include "ReadWriteMAXIModuleGenerator.hpp"
...
ReadWriteMAXIModuleGenerator::ReadWriteMAXIModuleGenerator(const HLS_managerRef& _HLSMgr) : Registrar(_HLSMgr)
{
}

void ReadWriteMAXIModuleGenerator::InternalExec(std::ostream& out, const module* /* mod */,
...
always @(*) begin
  ...
  case (_present_state)
   ...
   out << "      if(" << _ports_in[RVALID].name << ") begin\n";
   out << "        if(" << _ports_in[RLAST].name << ") begin\n";
   out << "          next_acc_rdata = next_acc_rdata + (" << _ports_in[RDATA].name << " & (~read_mask));\n";
