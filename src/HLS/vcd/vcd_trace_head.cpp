...
{
   THROW_ASSERT(not i.exec_states.empty(), "no executing states for operation " +
         STR(i.stg_fun_id) + "_" + STR(i.op_id));
...
{
   THROW_ASSERT(not i.start_states.empty(), "no starting states for operation " +
         STR(i.stg_fun_id) + "_" + STR(i.op_id));
...
{
   THROW_ASSERT(not i.end_states.empty(), "no ending states for operation " +
         STR(i.stg_fun_id) + "_" + STR(i.op_id));
...
vcd_trace_head::vcd_trace_head(
   ...
   state(uninitialized), failed(fail_none),
   one_hot_fsm_encoding(_one_hot_fsm_encoding), op_info(op),
   ...
   HLSMgr(_HLSMgr), TM(_TM), initial_state_id(init_state_id),
...
void vcd_trace_head::set_consecutive_state_executions()
{
   THROW_ASSERT((fsm_ss_it->duration == std::numeric_limits<decltype(fsm_ss_it->duration)>::max()) ||
         ...
         "timestamp = " + STR(fsm_ss_it->time_stamp) + "\n"
         "op_start_time = " + STR(op_start_time) + "\n"
         "duration = " + STR(fsm_ss_it->duration) + "\n"
   ...
   if (fsm_ss_it->duration == std::numeric_limits<decltype(fsm_ss_it->duration)>::max())
   {
      if (start_state_is_initial and fsm_has_a_single_state and (sp_var_it != sp_var_end))
      {
         consecutive_state_executions =
            (sp_var_it->time_stamp - op_start_time) / clock_period;
      }
      else
      {
         consecutive_state_executions =
            std::numeric_limits<decltype(consecutive_state_executions)>::max();
      }
   }
   else
   {
      consecutive_state_executions =
         (fsm_ss_it->duration + fsm_ss_it->time_stamp - op_start_time) / clock_period;
   }
...
void vcd_trace_head::unbounded_find_end_time()
{
   if (exec_times_in_current_state == consecutive_state_executions - 1)
   {
      ...
      THROW_ASSERT(is_end(*fsm_es_it, op_info, one_hot_fsm_encoding), "unbounded operation " +
            STR(op_info.stg_fun_id) + "_" + STR(op_info.op_id) +
            " with starting state " + fsm_es_it->value +
            " which is not ending\n");
      THROW_ASSERT(not is_phi,
      "\n/--------------------------------------------------------------------\n"
      "|  operation: " + STR(op_info.stg_fun_id) + "_" + STR(op_info.op_id) + "\n"
      ...
      if (is_exec(*std::next(fsm_es_it), op_info, one_hot_fsm_encoding) and not is_start(*std::next(fsm_es_it), op_info, one_hot_fsm_encoding))
      {
         ...
         THROW_ASSERT(is_end(*fsm_es_it, op_info, one_hot_fsm_encoding), "dummy state of unbounded operation " +
            STR(op_info.stg_fun_id) + "_" + STR(op_info.op_id) + " must be ending state\n");
      }
...
void vcd_trace_head::update()
{
   ...
   const auto is_starting_state = [& i, onehot](const sig_variation & s)
   {
      return is_start(s, i, onehot);
   };
         ...
         if (state_info->is_duplicated and not state_info->isOriginalState and not state_info->all_paths)
         {
            for(const auto def_edge : gp->CGetDefEdgesList())
            {
               if (state_info->moved_op_def_set.find(def_edge.first->index) !=
                     ...
                     or
                     state_info->moved_op_use_set.find(op_info.ssa_name_node_id) !=
...
void vcd_trace_head::advance()
{
   ...
   const auto var_later_or_equal_than_end = [& end] (const sig_variation & s)
   {
