...
void vcd_utility::ComputeRelationships
(DesignFlowStepSet & relationship, const DesignFlowStep::RelationshipType relationship_type)
{
      ...
      case DEPENDENCE_RELATIONSHIP:
         {
            ...
            ret.insert(std::make_tuple(HLSFlowStep_Type::TESTBENCH_GENERATION,
                  HLSFlowStepSpecializationConstRef(),
                  HLSFlowStep_Relationship::TOP_FUNCTION));
            ret.insert(std::make_tuple(HLSFlowStep_Type::VCD_SIGNAL_SELECTION,
                  HLSFlowStepSpecializationConstRef(),
                  HLSFlowStep_Relationship::TOP_FUNCTION));
            break;
         }
...
unsigned long long vcd_utility::GetClockPeriod(const vcd_parser::vcd_trace_t & vcd_trace) const
{
   ...
   const auto same_value = [clock_var_beg](const sig_variation & v)
   {
...
DesignFlowStep_Status vcd_utility::Exec()
{
   ...
   std::string vcd_filename =
      parameters->getOption<std::string>(OPT_output_directory) + "/simulation/test.vcd";
   ...
   vcd_parser::vcd_trace_t vcd_trace =
      vcd_parser.parse_vcd(vcd_filename, HLSMgr->RDiscr->selected_vcd_signals);
   ...
   std::string discrepancy_data_filename =
      ...
      HLSMgr->CGetFunctionBehavior(root_function)->CGetBehavioralHelper()->get_function_name() +
      "_discrepancy.data";
   ...
   INDENT_OUT_MEX(OUTPUT_LEVEL_VERBOSE, output_level,
      "Parsed C trace file " + discrepancy_data_filename + " in " + print_cpu_time(ctrace_parse_time) + " seconds");
#endif
   ...
   for (const auto & c : c_op_trace)
   {
      ...
      THROW_ASSERT(HLSMgr->RDiscr->opid_to_outsignal.find(op_info.op_id) !=
            HLSMgr->RDiscr->opid_to_outsignal.end(),
      ...
      for (const std::string& scope : scope_set)
      {
         ...
         const std::list<sig_variation> & op_out_vars =
          ...
          */
         const StateTransitionGraphManagerConstRef stg_man =
            HLSMgr->get_HLS(op_info.stg_fun_id)->STG;
         ...
         const unsigned int initial_state_id =
            stg->CGetStateTransitionGraphInfo()->vertex_to_state_id.at(first_state);
   ...
   INDENT_OUT_MEX(OUTPUT_LEVEL_VERBOSE, output_level,
         "DISCREPANCY CHECKS: " + STR(Discr->n_checked_operations) +
         "/" + STR(Discr->n_total_operations));
   ...
   for (const auto & id_to_scope_to_head : op_id_to_scope_to_vcd_head)
   {
      for (const auto & t : id_to_scope_to_head.second)
      {
         ...
         suspended_op_never_ends =
            t.second.state == vcd_trace_head::suspended and
      ...
      const std::string& c_val)
{
   ...
   {
      THROW_ASSERT(c_val.length() % t.op_info.vec_base_bitsize == 0,
            "ssa_name = " + STR(t.op_info.ssa_name) + "\n"
            "ssa node id = " + STR(t.op_info.ssa_name_node_id) + "\n"
            "c_val.length() = " + STR(c_val.length()) + "\n"
            "vec_base_bitsize = " + STR(t.op_info.vec_base_bitsize) + "\n");

      THROW_ASSERT(vcd_val.length() >= c_val.length(),
            "ssa_name = " + STR(t.op_info.ssa_name) + "\n"
            "ssa node id = " + STR(t.op_info.ssa_name_node_id) + "\n"
            "c_val.length() = " + STR(c_val.length()) + "\n"
            "vcd signal = " + t.fullsigname + "\n"
            "vcd length = " + STR(vcd_val.length()) + "\n");
      ...
      const unsigned int el_idx)
{
   ...
   THROW_ASSERT(c_val.length() >= first_c_bit + c_elem_size,
         "ssa_name = " + STR(t.op_info.ssa_name) + "\n"
         "ssa node id = " + STR(t.op_info.ssa_name_node_id) + "\n"
         "c_val.length() = " + STR(c_val.length()) + "\n"
         "first_c_bit = " + STR(first_c_bit) + "\n"
         "c_elem_size = " + STR(c_elem_size) + "\n");

   THROW_ASSERT(t.out_var_it->value.length() > first_c_bit,
         "ssa_name = " + STR(t.op_info.ssa_name) + "\n"
         "ssa node id = " + STR(t.op_info.ssa_name_node_id) + "\n"
         "c_val.length() = " + STR(c_val.length()) + "\n"
         "first_c_bit = " + STR(first_c_bit) + "\n"
   ...
   {
      THROW_ASSERT(t.out_var_it->value.length() >= c_val.length(),
            "ssa_name = " + STR(t.op_info.ssa_name) + "\n"
            "ssa node id = " + STR(t.op_info.ssa_name_node_id) + "\n"
            "c_val.length() = " + STR(c_val.length()) + "\n"
            "first_c_bit = " + STR(first_c_bit) + "\n"
      ...
      is_mismatch = detect_mismatch_simple(t, c_context, c_val, el_idx,
            first_c_bit, part_size);

      is_mismatch |= detect_mismatch_simple(t, c_context, c_val, el_idx,
            first_c_bit + part_size, part_size);
   }
   else
   {
      is_mismatch = detect_mismatch_simple(t, c_context, c_val, el_idx,
            first_c_bit, c_elem_size);
   }
   return is_mismatch;
}
      ...
      const std::string::size_type c_size)
{
   ...
   {
      THROW_ASSERT(not (t.op_info.type & DISCR_REAL) and
            not (t.op_info.type & DISCR_COMPLEX),
            "address ssa cannot be real or complex: "
            "real = " + STR(static_cast<bool>(t.op_info.type & DISCR_VECTOR)) +
       ...
       */
      discrepancy_found = detect_address_mismatch(t.op_info, c_context,
            resized_c_val, resized_vcd_val, base_index);
   }
      ...
      const unsigned int base_index)
{
   ...
   if (parameters->isOption(OPT_discrepancy_permissive_ptrs) and
         parameters->getOption<bool>(OPT_discrepancy_permissive_ptrs) and
         (t.op_info.type & DISCR_ADDR))
   {
   ...
   const
{
   ...
   THROW_ASSERT(context_it != Discr->c_addr_map.end(),
   ...
   THROW_ASSERT(memory_area_bitsize % 8 == 0,
         "bitsize of a variable in memory must be mutliple of 8 --> is " + STR(memory_area_bitsize));
      ...
      if (ssa->use_set->is_fully_resolved())
      {
         ...
         for (const tree_nodeRef & pointed_var_decl_id : ssa->use_set->variables)
         {
            ...
            THROW_ASSERT(Discr->c_addr_map.find(c_context) != Discr->c_addr_map.end(),
                  "no address map found for context " + STR(c_context));
            if (Discr->c_addr_map.at(c_context).find(base_index) !=
   ...
   const
{
   ...
   {
      THROW_ASSERT(c_val.length() <= vcd_val.length(),
            "ssa_name = " + t.op_info.ssa_name + "\n"
            "vcd signal = " + t.fullsigname + "\n"
            "c_val.length() = " + STR(c_val.length()) + "\n"
...
void vcd_utility::print_discrepancy(const DiscrepancyLog & l, const int verbosity) const
{
   std::string out_msg =
         "\n/--------------------------------------------------------------------\n"
         "|  ERROR in signal " + l.fullsigname + "\n"
         "|  value in vcd: " + l.vcd_val + "\n"
         "|  value in C:   " + l.c_val + "\n"
         "|  bitvalue: " + STR(l.bitsize) + "\n"
         "|  operation id: " + STR(l.fun_id) + "_" + STR(l.op_id) + "\n"
         "|  in function: " + l.fu_name + "\n"
         "|  in context: " + STR(l.context) + "\n"
         "|  operation: " + l.stmt_string + "\n"
         "|  discrepancy time = " + STR(l.op_end_time) + "\n"
         "|  failed operation starts at time " + STR(l.op_start_time) + "\n"
         "|  when fsm state is " + compute_fsm_state_from_vcd_string(l.op_start_state) + "\n";
   ...
   if (l.type & DISCR_ADDR)
   {
      ...
      THROW_ASSERT(context_it != Discr->c_addr_map.end(),
            "context " + STR(l.context) + " not find");
      ...
      if (var_id_to_base_address_it != context_it->second.end())
      {
         ...
         uint64_t c_addr_offset =  c_addr - c_base_addr;
         THROW_ASSERT(c_addr >= c_base_addr,
               "discrepancies for out of range addresses should not be raised:"
               " c_addr = " + STR(c_addr) + " c_base_addr = " + STR(c_base_addr));
         out_msg += "|  address offset in C: " + STR(c_addr_offset) + "\n";
#if HAVE_ASSERTS
         const uint64_t memory_area_bitsize = tree_helper::size(TM, l.base_index);
         THROW_ASSERT(memory_area_bitsize % 8 == 0,
               "bitsize of a variable in memory must be mutliple of 8 --> is " +
               STR(memory_area_bitsize) + ": this should be catched by an assertion earlier");
#endif
      }
      else
      {
...
void vcd_utility::print_failed_vcd_head(const vcd_trace_head & t, const int verbosity) const
{
   ...
   switch(t.failed)
   {
      ...
      {
         out_msg +=
            "|  operation " + STR(t.op_info.stg_fun_id) + "_" + STR(t.op_info.op_id) + "\n"
            "|  is scheduled in the initial state S_0 of a function which does not start\n"
            "|  failed operation starts at time " + STR(t.op_start_time) + "\n"
            "|  when fsm state is " + state + "\n";
         break;
      }
      case vcd_trace_head::no_start_state:
      {
         out_msg +=
            "|  cannot find next start state for operation " + STR(t.op_info.stg_fun_id) + "_" + STR(t.op_info.op_id) + "\n"
            "|  previous execution started at time  " + STR(t.op_start_time) + "\n"
            "|  when fsm state was " + state + "\n"
            "|  and ended at time " + STR(t.op_end_time) + "\n";
         break;
      }
      case vcd_trace_head::no_end_state:
      {
         out_msg +=
            "|  cannot find next end state for operation " + STR(t.op_info.stg_fun_id) + "_" + STR(t.op_info.op_id) + "\n"
