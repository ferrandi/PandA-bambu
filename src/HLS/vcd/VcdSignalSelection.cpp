...
(
   ...
   const DesignFlowManagerConstRef _design_flow_manager
    ...
    (
      ...
      HLSFlowStep_Type::VCD_SIGNAL_SELECTION
    ),
...
{
   return tree_helper::is_a_pointer(TM, type_index) or
         tree_helper::is_an_array(TM, type_index) or
      ...
      case DEPENDENCE_RELATIONSHIP:
         {
            ...
            ret.insert(std::make_tuple(HLSFlowStep_Type::GENERATE_HDL,
                  HLSFlowStepSpecializationConstRef(),
...
void VcdSignalSelection::SelectInitialAddrParam(
   const std::set<unsigned int> & reached_body_fun_ids,
...
void VcdSignalSelection::InitialSsaIsAddress(
   ...
   const std::unordered_set<unsigned int> & addr_fun_ids,
   const std::unordered_map<unsigned int, std::unordered_set<unsigned int> > & call_id_to_called_id)
{
    ...
    */
   THROW_ASSERT(
         rhs_kind != component_ref_K and
         rhs_kind != array_ref_K and
         rhs_kind != target_mem_ref_K and
...
{
   THROW_ASSERT(tn->get_kind() == gimple_phi_K,
         tn->ToString() + " is of kind " + tree_node::GetString(tn->get_kind()));
   ...
   if (std::find_if(phi_it, phi_end, is_address) != phi_end)
   {
      THROW_ASSERT(GET_NODE(phi.res)->get_kind() == ssa_name_K,
         "phi node id: " + STR(tn->index) +
         " result node id: " + STR(GET_NODE(phi.res)->index) + "\n");
   ...
   const std::unordered_map<unsigned int, std::unordered_set<unsigned int> > & call_id_to_called_id)
{
   for (const auto fid : reached_body_fun_ids)
   {
      ...
      for (boost::tie(vi, vi_end) = boost::vertices(*op_graph); vi != vi_end; vi++)
      {
          ...
          */
         if (tree_helper::is_a_complex(TM, assigned_tree_node_id) or
               tree_helper::is_real(TM, assigned_tree_node_id))
            continue;
         /*
...
void VcdSignalSelection::SingleStepPropagateParamToSsa(
...
void VcdSignalSelection::PropagateAddrParamToSsa(
   const std::unordered_map<unsigned int, TreeNodeSet> & address_parameters,
   const std::set<unsigned int> & reached_body_fun_ids)
{
   for (const auto fid : reached_body_fun_ids)
   {
...
void VcdSignalSelection::SingleStepPropagateAddrSsa(const tree_nodeRef & curr_tn)
{
   ...
   if (tn->get_kind() == gimple_assign_K)
   {
      ...
      if (ssa_node->get_kind() == ssa_name_K)
      {
         ...
         const bool rhs_is_comparison =
            rhs_kind == lt_expr_K        || rhs_kind == le_expr_K ||
            rhs_kind == gt_expr_K        || rhs_kind == ge_expr_K ||
            rhs_kind == eq_expr_K        || rhs_kind == ne_expr_K ||
            rhs_kind == unordered_expr_K || rhs_kind == ordered_expr_K ||
            rhs_kind == unlt_expr_K      || rhs_kind == unle_expr_K ||
            rhs_kind == ungt_expr_K      || rhs_kind == unge_expr_K ||
            rhs_kind == uneq_expr_K      || rhs_kind == ltgt_expr_K;
         ...
         bool rhs_is_load_candidate = (rhs_kind == bit_field_ref_K && !is_a_vector_bitfield) ||
            rhs_kind == component_ref_K            || rhs_kind == indirect_ref_K ||
            rhs_kind == misaligned_indirect_ref_K  || rhs_kind == mem_ref_K ||
...
void VcdSignalSelection::DetectInvalidReturns(
   const std::set<unsigned int> & reached_body_functions,
...
void VcdSignalSelection::CrossPropagateAddrSsa(
   ...
   const std::set<unsigned int> & reached_body_functions,
   const std::unordered_set<unsigned int> & addr_fun_ids,
   const std::unordered_map<unsigned int, std::unordered_set<unsigned int> > & fu_id_to_call_ids,
   const std::unordered_map<unsigned int, std::unordered_set<unsigned int> > & call_id_to_called_id)
{
   for (const unsigned int caller_fun_id : reached_body_functions)
   {
      ...
      {
         THROW_ASSERT(call_id_to_called_id.find(callid) != call_id_to_called_id.end(),
               "callid = " + STR(callid));
         for (const unsigned int called_id : call_id_to_called_id.at(callid))
         {
            ...
            const BehavioralHelperConstRef called_BH =
            ...
            if (call_node->get_kind() == gimple_assign_K)
            {
               ...
               if (ssa_node->get_kind() == ssa_name_K and (rhs->get_kind() == call_expr_K || rhs->get_kind() == aggr_init_expr_K))
               {
                   ...
                   */
                  if (called_BH->get_function_name() == "open" and
                        called_BH->is_operating_system_function(called_id))
                  {
                     Discr->ssa_to_skip.insert(ssa_node);
                  }
               }
                  ...
                  op_graph->CGetOpGraphInfo()->tree_node_to_operation.end())
            {
               THROW_WARNING("cannot find call for interprocedural address propagation:\n\t"
                     "caller id = " + STR(caller_fun_id) + "\n\t"
                     "caller name = " +  BH->get_function_name() + "\n\t"
                     "called id = " + STR(called_id)  + "\n\t"
                     "called name = " + called_BH->get_function_name() + "\n\t"
            ...
            THROW_ASSERT(callopinfo->called.size() == 1, "call id " + STR(callid) +
                  " called.size() = " + STR(callopinfo->called.size()));
            /*
            ...
            if (called_id == direct_called_id)
            {
               ...
               THROW_ASSERT(called_BH->is_var_args() or
                  ...
                  "call id " + STR(callid) + ", called id " + STR(called_id) + "\n"
                  "list_of_args.size() = " + STR(fu_dec->list_of_args.size()) + ", "
                  "actual_parameters.size() = " + STR(callopinfo->actual_parameters.size()) + "\n");
...
void VcdSignalSelection::SelectInternalSignals(
      std::unordered_map<unsigned int, std::unordered_set<std::string> > & fun_id_to_sig_names)
   const
{
   ...
   for (const unsigned int f_id : fun_with_body)
   {
      ...
      for (boost::tie(op_vi, op_vi_end) = boost::vertices(*op_graph); op_vi != op_vi_end; op_vi++)
      {
         ...
         if (op_node->get_kind() == gimple_assign_K)
         {
            ...
            std::string to_select = "out_" + fu_bind->get_fu_name(*op_vi) +
               "_i" + STR(fu_bind->get_index(*op_vi)) +  "_";

            if (alloc_info->is_direct_access_memory_unit(fu_type_id) and
                  alloc_info->is_memory_unit(fu_type_id))
            {
            ...
            else if ((alloc_info->is_direct_proxy_memory_unit(fu_type_id) and
                  alloc_info->is_proxy_memory_unit(fu_type_id)) or
         ...
         else if (op_node->get_kind() == gimple_phi_K)
         {
            ...
            if (not phi->virtual_flag)
            {
               ...
               THROW_ASSERT(storage_val_info->is_a_storage_value(nullptr, assigned_tree_node_id),
                     " variable " + HLSMgr->CGetFunctionBehavior(f_id)->CGetBehavioralHelper()->PrintVariable(assigned_tree_node_id) +
...
DesignFlowStep_Status VcdSignalSelection::Exec()
{
   ...
   boost::depth_first_visit(*cg, CGMan->GetVertex(root_function), csc_vis,
         boost::make_iterator_property_map(csc_color.begin(), boost::get(boost::vertex_index_t(), *cg), boost::white_color));
   ...
   boost::depth_first_visit(Discr->DiscrepancyCallGraph, Discr->unfolded_root_v, sig_sel_v,
      boost::make_iterator_property_map(sig_sel_color.begin(),
         boost::get(boost::vertex_index_t(), Discr->DiscrepancyCallGraph), boost::white_color));
   ...
   for (const auto vs : Discr->unfolded_v_to_scope)
   {
      const unsigned int f_id =
         Cget_node_info<UnfoldedFunctionInfo>(vs.first, Discr->DiscrepancyCallGraph)->f_id;
      ...
      THROW_ASSERT(not controller_scope.empty() or datapath_scope.empty(),
         "controller_scope size " + STR(controller_scope.size()) +
         " datapath_scope size " + STR(datapath_scope.size()));
      ...
      if (parameters->isOption(OPT_discrepancy_only))
      {
         const auto discrepancy_functions =
            parameters->getOption<const CustomSet<std::string> >(OPT_discrepancy_only);
