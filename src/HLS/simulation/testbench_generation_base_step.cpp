                  ...
                  const auto manage_pidout = [&](const std::string& portID) {
                     ...
                     std::string mem_aggregated;
                     {
                        ...
                        for(unsigned int bitsize_index = 0; bitsize_index < bitsize; bitsize_index = bitsize_index + 8)
                        {
                           if(bitsize_index)
                              mem_aggregated += ", ";
                           mem_aggregated += "_bambu_testbench_mem_[paddr" +port_name.substr(0, port_name.size() - 1) + "0 + " + STR((bitsize - bitsize_index) / 8 - 1) + " - base_addr + " + port_addr->get_id() + "*" + STR(GetPointer<port_o>(port_addr)->get_port_alignment()) + "]";
                        }
                        mem_aggregated += "}";
                     }
