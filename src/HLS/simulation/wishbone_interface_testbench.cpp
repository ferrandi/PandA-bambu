...
void WishboneInterfaceTestbench::write_call(bool hasMultiIrq) const
{
   ...
   for (auto const function_parameter : function_parameters)
   {
      ...
      std::string variableName =
         (var == behavioral_helper->GetFunctionReturnType(topFunctionId)) ?
         RETURN_PORT_NAME :
...
void WishboneInterfaceTestbench::write_memory_handler() const
{
   ...
   for (unsigned int j = 0; j < dataBusByteSize; ++j) {
      ...
      for (unsigned int i = 0; i <= j; ++i) {
         writer->write("_bambu_testbench_mem_[(addr_om - base_addr) + "
                       + STR(i) + "] = dat_om["
                       + STR((i * 8) + 7) + ":"
                       + STR((i * 8)) + "];\n");
      }
   ...
   for (unsigned int j = 0; j < dataBusByteSize; ++j) {
      ...
      for (unsigned int i = 0; i <= j; ++i) {
         writer->write("dat_im["
                       + STR((i * 8) + 7) + ":"
                       + STR((i * 8)) + "] = _bambu_testbench_mem_[(addr_om - base_addr) + "
                       + STR(i) + "];\n");
      }
      writer->write(STR(STD_CLOSING_CHAR));
      writer->write("end\n");
...
void WishboneInterfaceTestbench::write_wishbone_output_signal_declaration(
      bool & withMemory,
      bool & hasMultiIrq)
   const
{
...
void WishboneInterfaceTestbench::write_signals(
