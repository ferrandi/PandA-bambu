   ...
   parameters(_Param)
{
      ...
      try
      {
         ...
         if (parser)
         {
            ...
            for (auto & iter : list)
            {
               ...
               {
                  if (CE_XVM(function_name, Enode)) LOAD_XVM(function_name, Enode);
                  if (function_name == fun_name) constraint_node = Enode;
               }
               else /* general constraints */
               {
                  if (!CE_XVM(function_name, Enode)) constraint_node = Enode;
               }
            }
         }
         if (constraint_node)
   ...
   if (parameters->isOption(OPT_constraints_file))
   {
      ...
      read_HLS_constraints_File(parameters->getOption<std::string>(OPT_constraints_file).c_str());
...
void HLS_constraints::xload(const xml_element* Enode)
{
   ...
   for (const auto & iter : list)
   {
      ...
      if(EnodeC->get_name() == "tech_constraints")
      {
         ...
         THROW_ASSERT(fu_name != "", "bad formed xml file: fu_name expected in a tech_constraints");
         if(CE_XVM(fu_library, EnodeC)) LOAD_XVM(fu_library, EnodeC);
         LOAD_XVM(n, EnodeC);
      ...
      else if(EnodeC->get_name() == "binding_constraints")
      {
         ...
         THROW_ASSERT(vertex_name != "", "bad formed xml file: vertex_name expected in a binding_constraints");
         LOAD_XVM(fu_name, EnodeC);
         THROW_ASSERT(fu_name != "", "bad formed xml file: fu_name expected in a binding_constraints");
         if(CE_XVM(fu_library, EnodeC)) LOAD_XVM(fu_library, EnodeC);
      ...
      else if(EnodeC->get_name() == "scheduling_constraints")
      {
         ...
         int priority=0;
         LOAD_XVM(vertex_name, EnodeC);
         THROW_ASSERT(vertex_name != "", "bad formed xml file: vertex_name expected in a scheduling_constraints");
         LOAD_XVM(priority, EnodeC);
         scheduling_constraints[vertex_name] = priority;
      }
   }
   if(CE_XVM(clock_period, Enode)) LOAD_XVM(clock_period, Enode);
   if(CE_XVM(registers, Enode)) LOAD_XVM(registers, Enode);
}

void HLS_constraints::xwriteFUConstraints(xml_element* Enode,
                                          const std::string& fu_name, const std::string& fu_library, unsigned int n,
                                          bool forDump)
{
   if(n == INFINITE_UINT and !forDump) return;
   xml_element* EnodeC = Enode->add_child_element("tech_constraints");
   WRITE_XVM(fu_name, EnodeC);
   if(fu_library != LIBRARY_STD) WRITE_XVM(fu_library, EnodeC);
   WRITE_XVM(n, EnodeC);
}
...
void HLS_constraints::xwriteHLSConstraints(xml_element* Enode, bool forDump)
{
   ...
   if(registers != INFINITE_UINT or forDump) WRITE_XVM(registers, Enode);
}
...
void HLS_constraints::xwrite(xml_element* rootnode)
{
  ...
  std::unordered_map<std::string, std::pair<std::string, std::pair<std::string, unsigned int> > >::const_iterator bi_end = binding_constraints.end();
  for (std::unordered_map<std::string, std::pair<std::string, std::pair<std::string, unsigned int> > >::const_iterator bi = binding_constraints.begin(); bi != bi_end; ++bi)
  {
  ...
  CustomMap<std::string, int>::const_iterator sch_end = scheduling_constraints.end();
  for (CustomMap<std::string, int>::const_iterator sch = scheduling_constraints.begin(); sch != sch_end; ++sch)
  {
