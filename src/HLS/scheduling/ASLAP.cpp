...
ASLAP::ASLAP(const HLS_managerConstRef _hls_manager, const hlsRef HLS, const bool _speculation, const OpVertexSet & operations, const ParameterConstRef _parameters, unsigned int _ctrl_step_multiplier) :
   ASAP(ScheduleRef(new Schedule(_hls_manager, _hls_manager->CGetFunctionBehavior(HLS->functionId)->CGetBehavioralHelper()->get_function_index(), _hls_manager->CGetFunctionBehavior(HLS->functionId)->CGetOpGraph(FunctionBehavior::FLSAODG), _parameters))),
   ALAP(ScheduleRef(new Schedule(_hls_manager, _hls_manager->CGetFunctionBehavior(HLS->functionId)->CGetBehavioralHelper()->get_function_index(), _hls_manager->CGetFunctionBehavior(HLS->functionId)->CGetOpGraph(FunctionBehavior::FLSAODG), _parameters))),
   min_tot_csteps(0u),
   max_tot_csteps(0u),
