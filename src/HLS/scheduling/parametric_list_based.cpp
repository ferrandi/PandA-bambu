...
ParametricListBasedSpecialization::ParametricListBasedSpecialization(const ParametricListBased_Metric _parametric_list_based_metric) :
...
parametric_list_based::parametric_list_based(const ParameterConstRef _parameters, const HLS_managerRef _HLSMgr, unsigned int _funId, const DesignFlowManagerConstRef _design_flow_manager, const HLSFlowStepSpecializationConstRef _hls_flow_step_specialization) :
   Scheduling(_parameters, _HLSMgr, _funId, _design_flow_manager, HLSFlowStep_Type::LIST_BASED_SCHEDULING, _hls_flow_step_specialization ? _hls_flow_step_specialization : HLSFlowStepSpecializationConstRef(new ParametricListBasedSpecialization(static_cast<ParametricListBased_Metric>(_parameters->getOption<unsigned int>(OPT_scheduling_priority))))),
      ...
      do
      {
         ...
         while(ready_resources.size())
         {
            ...
            while(queue.size())
            {
               ...
               {
                  auto insertResult = postponed_resources.insert(make_pair(fu_type, OpVertexSet(flow_graph)));
                  insertResult.first->second.insert(current_vertex);
               ...
               if (!HLS->allocation_information->is_operation_bounded(flow_graph, current_vertex, fu_type) && (unbounded || is_live || store_unbounded_check))
               {
                  auto insertResult = black_list.insert(make_pair(fu_type, OpVertexSet(flow_graph)));
                  insertResult.first->second.insert(current_vertex);
               ...
               if(current_ASAP.find(current_vertex) != current_ASAP.end() and current_ASAP.find(current_vertex)->second > current_cycle)
               {
               ...
               compute_starting_ending_time_asap(current_vertex, fu_type, current_cycle, current_starting_time, current_ending_time, current_stage_period, cannot_be_chained, res_binding, schedule, phi_extra_time, setup_hold_time, local_connection_map);
               ...
               if(is_pipelined and (current_starting_time > current_cycle_starting_time)  and ((current_stage_period+current_starting_time+setup_hold_time+phi_extra_time+scheduling_mux_margins>(current_cycle_ending_time) || unbounded)))
               {
                  PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "                  Pipelined unit cannot be chained: starting time is " + boost::lexical_cast<std::string>(current_starting_time) + " stage period is " + boost::lexical_cast<std::string>(current_stage_period)) ;
                  auto insertResult = black_list.insert(make_pair(fu_type, OpVertexSet(flow_graph)));
                  insertResult.first->second.insert(current_vertex);
                  continue;
               }
               else if(current_starting_time >= (current_cycle_ending_time) || ((!is_pipelined && n_cycles == 0 && current_starting_time > (current_cycle_starting_time)) && current_ending_time+setup_hold_time+phi_extra_time+scheduling_mux_margins > current_cycle_ending_time))
               {
                  PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "                  It cannot be chained: starting time is " + boost::lexical_cast<std::string>(current_starting_time) + " ending time is " + boost::lexical_cast<std::string>(current_ending_time)) ;
                  auto insertResult = black_list.insert(make_pair(fu_type, OpVertexSet(flow_graph)));
                  insertResult.first->second.insert(current_vertex);
                  continue;
               }
               else if ((unbounded  || cstep_has_RET_conflict /*|| current_starting_time > (current_cycle_starting_time)*/) && (GET_TYPE(flow_graph,current_vertex) & TYPE_RET))
               {
               ...
               {
                  PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "                  READ_COND or MULTI_READ_COND cannot be chained with non-direct operations -> starting time " + boost::lexical_cast<std::string>(current_starting_time) + " ending time: " + boost::lexical_cast<std::string>(current_ending_time));
                  auto insertResult = black_list.insert(make_pair(fu_type, OpVertexSet(flow_graph)));
                  insertResult.first->second.insert(current_vertex);
                  continue;
               }*/
               ...
               else if ((current_starting_time > (current_cycle_starting_time)) && !HLS->Param->getOption<bool>(OPT_chaining))
               {
               ...
               else if((!is_pipelined && n_cycles > 0 && current_starting_time > (current_cycle_starting_time)) && current_ending_time-(n_cycles-1)*clock_cycle+setup_hold_time+phi_extra_time+scheduling_mux_margins > current_cycle_ending_time)
               {
               ...
               else if(current_ending_time+setup_hold_time+phi_extra_time+scheduling_mux_margins > current_cycle_ending_time && unbounded)
               {
               ...
               else if((GET_TYPE(flow_graph, current_vertex) & (TYPE_LOAD | TYPE_STORE)) && !HLS->allocation_information->is_direct_access_memory_unit(fu_type) && unbounded)
               {
               ...
               if(schedulable)
               {
                  ...
                  THROW_ASSERT(floor(ending_time[current_vertex]/clock_cycle + 0.0001) >= from_strongtype_cast<double>(current_cycle), GET_NAME(flow_graph, current_vertex) + " " + STR(ending_time[current_vertex]) + " " + STR(from_strongtype_cast<double>(current_cycle)));
...
void parametric_list_based::compute_starting_ending_time_asap(const vertex v, const unsigned int fu_type, const ControlStep cs, double & current_starting_time, double & current_ending_time, double &stage_period, bool &cannot_be_chained, fu_bindingRef res_binding, const ScheduleConstRef schedule, double &phi_extra_time, double setup_hold_time, CustomMap<std::pair<unsigned int, unsigned int>, double> & local_connection_map)
{
   ...
   INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "<--Operation " + GET_NAME(flow_graph, v) + " starts at " + STR(current_starting_time) + " and ends at " + STR(current_ending_time) + " execution time " + STR(op_execution_time) + " stage period " + STR(stage_period) + " phi_extra_time " + STR(phi_extra_time));
}

void parametric_list_based::compute_exec_stage_time(const unsigned int fu_type, double &stage_period, const ControlStep cs, const OpGraphConstRef op_graph, vertex v, double &op_execution_time, double &phi_extra_time, double current_starting_time, double setup_hold_time)
{
...
void parametric_list_based::compute_starting_ending_time_alap(vertex v, const unsigned int fu_type, const ControlStep cs, double & current_starting_time, double & max_ending_time, double &op_execution_time, double &stage_period, unsigned int &n_cycles, bool &cannot_be_chained, fu_bindingRef res_binding, const ScheduleConstRef schedule, double &phi_extra_time, double setup_hold_time, CustomMap<std::pair<unsigned int, unsigned int>, double> & local_connection_map)
{
   InEdgeIterator ei, ei_end;
   current_starting_time = from_strongtype_cast<double>(cs) * clock_cycle;
   ...
   PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "                  Operation " + GET_NAME(flow_graph, v) + " starts at " + boost::lexical_cast<std::string>(current_starting_time) + " and should end before of " + boost::lexical_cast<std::string>(max_ending_time) + " execution time " + STR(op_execution_time) + " stage period " + STR(stage_period));
}
...
{
   INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "---Checking if operation " + GET_NAME(flow_graph, current_vertex) + " is schedulable in this step on " + boost::lexical_cast<std::string>(fu_type) + " - starting_time is " + boost::lexical_cast<std::string>(current_starting_time));
...
void parametric_list_based::do_balanced_scheduling(std::deque<vertex> &sub_levels, const ScheduleRef schedule, const fu_bindingRef res_binding, const double setup_hold_time, const double scheduling_mux_margins, const OpGraphConstRef opDFG)
{
   ...
   for(std::deque<vertex>::const_reverse_iterator rit = sub_levels.rbegin(); rit != sub_levels.rend(); ++rit)
   {
      ...
      for(boost::tie(ei, ei_end) = boost::in_edges(candidate_v, *opDFG); ei != ei_end; ++ei)
      {
         ...
         auto insertResult = DMAP.insert(make_pair(candidate_v, OpVertexSet(opDFG)));
      ...
      while(T_area[time] < average_load && ptime >= min_cycle)
      {
         while(T.find(ptime) != T.end())
         {
            ...
            for(std::deque<vertex>::const_iterator ptime_it = T.find(ptime)->second.begin(); ptime_it != ptime_it_end; ++ptime_it)
            {
               ...
               unsigned int fu_type = res_binding->get_assign(candidate_v);
               if(HLS->allocation_information->get_number_fu(fu_type) != INFINITE_UINT && !HLS->allocation_information->is_vertex_bounded(fu_type)) continue;
               /// only 1 unbounded operation is allowed in a control step
               if(!HLS->allocation_information->is_operation_bounded(opDFG, candidate_v, fu_type)) continue; /// unbounded operations could not be moved
               ...
               CustomMap<std::pair<unsigned int, unsigned int>, double> local_connection_map;
               update_starting_ending_time(candidate_v, res_binding, opDFG, schedule);
               compute_starting_ending_time_alap(candidate_v, fu_type, time, candidate_starting_time, max_ending_time, candidate_op_execution_time, candidate_stage_period, n_cycles, cannot_be_chained, res_binding, schedule, phi_extra_time, setup_hold_time, local_connection_map);

               if ((candidate_starting_time > (from_strongtype_cast<double>(time) * clock_cycle)) && !HLS->Param->getOption<bool>(OPT_chaining)) continue; /// Chaining is possible but not allowed

               if ((candidate_starting_time > (from_strongtype_cast<double>(time) * clock_cycle)) && cannot_be_chained) continue; /// Chaining with STOREs/LOADs is not possible

               bool is_pipelined = HLS->allocation_information->get_initiation_time(fu_type, candidate_v) != 0;
               if(is_pipelined &&
                     (candidate_starting_time+setup_hold_time+phi_extra_time+scheduling_mux_margins+candidate_stage_period)>((from_strongtype_cast<double>(time) + 1) * clock_cycle)) continue; /// pipelined operation does not fit at the begin

               if(is_pipelined &&
                     (max_ending_time-setup_hold_time-phi_extra_time-scheduling_mux_margins) < (from_strongtype_cast<double>(time + n_cycles - 1) * clock_cycle + candidate_stage_period)) continue; /// pipelined operation does not fit at the end

               if(!is_pipelined &&
                     (candidate_op_execution_time+setup_hold_time+phi_extra_time+scheduling_mux_margins) > clock_cycle && has_unbounded) continue; /// Multi-cycles operations cannot be scheduled together with unbounded operations

               if(!is_pipelined &&
                  candidate_starting_time+setup_hold_time+phi_extra_time+scheduling_mux_margins+candidate_op_execution_time > max_ending_time) continue; /// operation does not fit at the end

               if(!is_pipelined &&
                     setup_hold_time+phi_extra_time+candidate_op_execution_time < clock_cycle &&
                     setup_hold_time+phi_extra_time+scheduling_mux_margins+candidate_starting_time+candidate_op_execution_time > (from_strongtype_cast<double>(time) + 1) * clock_cycle) continue; /// operation does not fit in the cycle
               ...
               if(candidate_ending_time> (from_strongtype_cast<double>(max_cycle)+1)*clock_cycle) continue;
...
void parametric_list_based::do_balanced_scheduling1(std::deque<vertex> &sub_levels, const ScheduleRef schedule, const fu_bindingRef res_binding, const double setup_hold_time, const double scheduling_mux_margins, const OpGraphConstRef opDFG, bool seen_cstep_has_RET_conflict)
{
   //#define SLACK_BASED
      ...
      while(ptime>=min_cycle)
      {
         while(T.find(ptime) != T.end())
         {
            ...
            for(std::deque<vertex>::const_iterator ptime_it = T.find(ptime)->second.begin(); ptime_it != ptime_it_end; ++ptime_it)
            {
               ...
               {
                  PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "   no room for fu_type at time time " + HLS->allocation_information->get_fu_name(fu_type).first + " obj=" + STR(T_obj[fu_type][time]) + " tot_obj="+STR(total_obj[fu_type]/ from_strongtype_cast<double>(cycles)) + " cycles="+ STR(cycles) + " normalized_area="+STR(normalized_area));
               ...
               bool is_pipelined = HLS->allocation_information->get_initiation_time(fu_type, candidate_v) != 0;
               if(is_pipelined &&
                     (candidate_starting_time+setup_hold_time+phi_extra_time+scheduling_mux_margins+candidate_stage_period)> time_clock_cycle_ending)
               {
               ...
               if(is_pipelined &&
                     (max_ending_time-setup_hold_time-phi_extra_time-scheduling_mux_margins) < ((from_strongtype_cast<double>(time) + n_cycles - 1)* clock_cycle+candidate_stage_period))
               {
               ...
               if(!is_pipelined &&
                     (candidate_op_execution_time+setup_hold_time+phi_extra_time+scheduling_mux_margins) > clock_cycle && has_unbounded)
               {
                  PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "   Multi-cycles operations cannot be scheduled together with unbounded operations");
               ...
               if(!is_pipelined &&
                  candidate_starting_time+setup_hold_time+phi_extra_time+scheduling_mux_margins+candidate_op_execution_time > max_ending_time)
               {
               ...
               if(!is_pipelined &&
                     setup_hold_time+phi_extra_time+scheduling_mux_margins+candidate_op_execution_time < clock_cycle &&
...
#else
               double curr_area = 1;
               if((T_obj[fu_type][time]+curr_area) > ceil(total_obj[fu_type] / from_strongtype_cast<double>(cycles))
                     && normalized_area>1.5)
               {
                  PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "  No room for fu_type at time time " + HLS->allocation_information->get_fu_name(fu_type).first + " obj=" + STR(T_obj[fu_type][time]+curr_area) + " tot_obj="+STR((total_obj[fu_type] / from_strongtype_cast<double>(cycles))) + " normalized_area="+STR(normalized_area));
                  continue; /// no room for fu_type at time time
               }
#endif
               ...
               PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "  candidate ending time="+STR(candidate_ending_time)+" candidate starting time="+STR(candidate_starting_time)+ " functional unit " + HLS->allocation_information->get_fu_name(fu_type).first);
...
const std::unordered_set<std::tuple<HLSFlowStep_Type, HLSFlowStepSpecializationConstRef, HLSFlowStep_Relationship> > parametric_list_based::ComputeHLSRelationships(const DesignFlowStep::RelationshipType relationship_type) const
{
