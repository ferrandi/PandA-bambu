...
class mux_conn : public connection_obj
{
   ...
   public:
       ...
       */
      std::vector<std::pair<generic_objRef,unsigned int> > get_mux_tree() const { return mux_tree; }
       ...
       */
      unsigned int get_mux_tree_size() const { return static_cast<unsigned int>(mux_tree.size()); }
