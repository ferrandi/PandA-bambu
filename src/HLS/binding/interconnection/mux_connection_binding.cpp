...
{
   if(algn == 8) return 0;
   else if(algn == 16) return 1;
   else if(algn == 32) return 2;
   else if(algn == 64) return 3;
   else if(algn == 128) return 4;
   else if(algn == 256) return 5;
   else return 0;
}
...
mux_connection_binding::mux_connection_binding(const ParameterConstRef _parameters, const HLS_managerRef _HLSMgr, unsigned int _funId, const DesignFlowManagerConstRef _design_flow_manager) :
   conn_binding_creator(_parameters, _HLSMgr, _funId, _design_flow_manager, HLSFlowStep_Type::MUX_INTERCONNECTION_BINDING),
   id(0),
   cur_phi_tree_var(0),
   is_PC(false)
{
...
generic_objRef mux_connection_binding::dynamic_multidimensional_array_handler(array_ref * ar, const vertex& op, const OpGraphConstRef data, unsigned int &base_address_index, std::vector<unsigned int> &recursive_indexes_values, std::vector<unsigned int> &dims, generic_objRef &global_adder, const bool is_not_a_phi)
{
   ...
   if(GET_NODE(ar->op0)->get_kind()==array_ref_K)
   {
      ...
      if(recursive_indexes_values.size()>0)
      {
         ...
         HLS->Rconn->add_component(UI_ADDER_STD);
      ...
      for(unsigned int i=0; i<recursive_indexes_values.size(); i++)
      {
         ...
         HLS->Rconn->add_component(UI_CONST_MULTIPLIER_STD);
...
void mux_connection_binding::create_single_conn(const OpGraphConstRef data, const vertex& op, generic_objRef fu_obj_src,
                                      generic_objRef fu_obj, unsigned int port_num, unsigned int port_index, unsigned int tree_var, unsigned int precision, bool is_not_a_phi)
{
   ...
   for(std::set<vertex>::const_iterator rs_it = running_states.begin(); rs_it_end != rs_it; ++rs_it)
   {
      ...
      if(GetPointer<register_obj>(fu_obj) && !is_not_a_phi)
      {
         ...
         if(state_info && state_info->is_duplicated && !state_info->all_paths)
         {
            ...
            for(const auto def_edge : gp->CGetDefEdgesList())
            {
               ...
               else if(state_info->isOriginalState && bbID == state_info->sourceBb) continue;
               else if(state_info->moved_op_def_set.find(cur_phi_tree_var) != state_info->moved_op_def_set.end()) continue;
               else if(def_edge.first->index != cur_phi_tree_var) continue;
               found_branch = true;
               break;
            }
            if(!found_branch) continue;
         }
      ...
      else
      {
         ...
         PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "       - add data transfer from " << fu_obj_src->get_string() << " to " << fu_obj->get_string() << " port " << boost::lexical_cast<std::string>(port_num) << ":" << boost::lexical_cast<std::string>(port_index)  << " in state " << HLS->Rliv->get_name(state) + (tree_var ? (" for " + HLSMgr->CGetFunctionBehavior(funId)->CGetBehavioralHelper()->PrintVariable(tree_var)) : ""));
      }
   }
}
         ...
         case array_ref_K:
         {
            ...
            if(GET_NODE(ar->op0)->get_kind() == array_ref_K) ///dynamic multidimensional array case
            {
               ...
               if(alignment)
               {
                  ...
                  HLS->Rconn->add_component(UI_ALIGN_ADDER_STD);
               }
               else
                  HLS->Rconn->add_component(UI_ADDER_STD);
         ...
         case target_mem_ref_K:
         {
            ...
            if(idx_index)
            {
               if(step_index)
               {
                  ...
                  HLS->Rconn->add_component(UI_MULTIPLIER_STD);
      ...
      if(!is_not_a_phi)
      {
         ...
         if(state_info && state_info->is_duplicated && !state_info->all_paths)
         {
            ...
            for(const auto def_edge : gp->CGetDefEdgesList())
            {
               ...
               else if(state_info->moved_op_use_set.find(gp->res->index) != state_info->moved_op_use_set.end()) continue;
               else if(def_edge.first->index != cur_phi_tree_var) continue;
               found_branch = true;
               break;
            }
      ...
      for(std::set<vertex>::const_iterator s_in_it = states_in.begin(); s_in_it != s_in_it_end; ++s_in_it)
      {
         ...
         if(!is_not_a_phi)
         {
            ...
            if(tree_helper::is_parameter(TreeM, tree_var))
            {
               ...
               PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "       - add data transfer from primary input " << fu_src_obj->get_string() << " to " << fu_obj->get_string() << " port " << boost::lexical_cast<std::string>(port_num) << ":" << boost::lexical_cast<std::string>(port_index) << " from state " << HLS->Rliv->get_name(src_state) + " to state " + HLS->Rliv->get_name(state) + " for " + HLSMgr->CGetFunctionBehavior(funId)->CGetBehavioralHelper()->PrintVariable(tree_var));
            }
            else
            {
               ...
               if((GET_TYPE(data, def_op) & TYPE_PHI)==0)
               {
                  if(def_op_ending_states.find(src_state) != def_op_ending_states.end())
                  {
                     ...
                     HLS->Rconn->add_data_transfer(fu_src_obj, fu_obj, port_num, port_index, data_transfer(tree_var, precision, src_state, state, op));
                     PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "       - add data transfer from " << fu_src_obj->get_string() << " to " << fu_obj->get_string() << " port " << boost::lexical_cast<std::string>(port_num) << ":" << boost::lexical_cast<std::string>(port_index) << " from state " << HLS->Rliv->get_name(src_state) + " to state " + HLS->Rliv->get_name(state) + " for " + HLSMgr->CGetFunctionBehavior(funId)->CGetBehavioralHelper()->PrintVariable(tree_var));
                  }
                  else if (HLS->storage_value_information->is_a_storage_value(src_state, tree_var))
                  {
                     ...
                     PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "       - register: " << r_index << " from " << HLS->Rliv->get_name(src_state) + " to state " + HLS->Rliv->get_name(state) + " for " + HLSMgr->CGetFunctionBehavior(funId)->CGetBehavioralHelper()->PrintVariable(tree_var));
                     ...
                     if(reg_obj != fu_obj)
                     {
                        ...
                        PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "       - add data transfer from " << reg_obj->get_string() << " to " << fu_obj->get_string() << " port " << boost::lexical_cast<std::string>(port_num) << ":" << boost::lexical_cast<std::string>(port_index) << " from state " << HLS->Rliv->get_name(src_state) + " to state " + HLS->Rliv->get_name(state) + " for " + HLSMgr->CGetFunctionBehavior(funId)->CGetBehavioralHelper()->PrintVariable(tree_var));
                     }
                  }
                  else
                  {
         ...
         else
         {
            ...
            if(tree_helper::is_parameter(TreeM, tree_var))
            {
               ...
               PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "       - add data transfer from primary input " << fu_src_obj->get_string() << " to " << fu_obj->get_string() << " port " << boost::lexical_cast<std::string>(port_num) << ":" << boost::lexical_cast<std::string>(port_index) << " from state " << HLS->Rliv->get_name(state) + " to state " + HLS->Rliv->get_name(tgt_state) + " for " + HLSMgr->CGetFunctionBehavior(funId)->CGetBehavioralHelper()->PrintVariable(tree_var));
            }
            else
            {
               vertex def_op = HLS->Rliv->get_op_where_defined(tree_var);
               ...
               if((GET_TYPE(data, def_op) & TYPE_PHI)==0)
               {
                  if(def_op_ending_states.find(state) != def_op_ending_states.end())
                  {
                     const generic_objRef fu_src_obj = HLS->Rfu->get(def_op);
                     HLS->Rconn->add_data_transfer(fu_src_obj, fu_obj, port_num, port_index, data_transfer(tree_var, precision, state, tgt_state, op));
                     PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "       - add data transfer from " << fu_src_obj->get_string() << " to " << fu_obj->get_string() << " port " << boost::lexical_cast<std::string>(port_num) << ":" << boost::lexical_cast<std::string>(port_index)  << " from state " << HLS->Rliv->get_name(state) + " to state " + HLS->Rliv->get_name(tgt_state) + " for " + HLSMgr->CGetFunctionBehavior(funId)->CGetBehavioralHelper()->PrintVariable(tree_var));
                  }
                  else if (HLS->storage_value_information->is_a_storage_value(state, tree_var))
                  {
                     ...
                     unsigned int r_index = HLS->Rreg->get_register(storage_value);
                     PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "       - register: " << r_index << " from " << HLS->Rliv->get_name(state) + " to state " + HLS->Rliv->get_name(tgt_state) + " for " + HLSMgr->CGetFunctionBehavior(funId)->CGetBehavioralHelper()->PrintVariable(tree_var));
                     reg_obj = HLS->Rreg->get(r_index);
                     if(reg_obj != fu_obj)
                     {
                        HLS->Rconn->add_data_transfer(reg_obj, fu_obj, port_num, port_index, data_transfer(tree_var, precision, state, tgt_state, op));
                        PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "       - add data transfer from " << reg_obj->get_string() << " to " << fu_obj->get_string() << " port " << boost::lexical_cast<std::string>(port_num) << ":" << boost::lexical_cast<std::string>(port_index) << " from state " << HLS->Rliv->get_name(state) + " to state " + HLS->Rliv->get_name(tgt_state) + " for " + HLSMgr->CGetFunctionBehavior(funId)->CGetBehavioralHelper()->PrintVariable(tree_var));
                     }
                  }
                  else
                  {
                     THROW_UNREACHABLE("not expected from " + HLS->Rliv->get_name(state) + " to " + HLS->Rliv->get_name(tgt_state) + " " + HLSMgr->get_tree_manager()->get_tree_node_const(tree_var)->ToString());
                       ...
                       !state_info->all_paths &&
                       def_op_ending_states.find(state) != def_op_ending_states.end() &&
                       std::find(state_info->moved_exec_op.begin(), state_info->moved_exec_op.end(), op)== state_info->moved_exec_op.end())
               {
                  ...
                  for(const auto def_edge : gp->CGetDefEdgesList())
                  {
                     ...
                     if(bbID != state_info->sourceBb) continue;
...
void mux_connection_binding::add_conversion(unsigned int num, unsigned int size_tree_var, VertexIterator op, unsigned int form_par_type, unsigned int port_index, const generic_objRef fu_obj, const OpGraphConstRef data, const tree_managerRef TreeM, unsigned int tree_var, std::vector<HLS_manager::io_binding_type> &var_read, unsigned int size_form_par)
{
   ...
   {
      generic_objRef conv_port = generic_objRef(new iu_conv_conn_obj("iu_conv_conn_obj_"+STR(id++)));
   ...
   {
      generic_objRef conv_port = generic_objRef(new ui_conv_conn_obj("ui_conv_conn_obj_"+STR(id++)));
   ...
   else if(size_form_par != size_tree_var)
   {
      ...
      if(parameters->isOption(OPT_soft_float) && parameters->getOption<bool>(OPT_soft_float))
      {
         ...
         HLS->Rconn->add_component(AllocationInformation::extract_bambu_provided_name(size_tree_var, size_form_par, HLSMgr, current_fu));
      }
      else
         HLS->Rconn->add_component(FFDATA_CONVERTER_STD);
      HLS->Rconn->add_sparse_logic(conv_port);
      unsigned int in_bitsize = size_form_par;
...
void mux_connection_binding::create_connections()
{
   ...
   for(boost::tie(op, opend) = boost::vertices(*data); op != opend; op++)
   {
      ...
      if(GET_TYPE(data,*op) & TYPE_VPHI) continue; /// virtual phis are skipped
            ...
            if (HLS->allocation_information->is_direct_access_memory_unit(fu) || HLS->allocation_information->is_indirect_access_memory_unit(fu)) ///MEMORY REFERENCES
            {
               ...
               if (GET_TYPE(data, *op) & TYPE_STORE)
               {
                  ...
                  if(size_var)
                  {
                     ...
                     determine_connection(*op, var_read[0], conv_port, 0, 0, data, var_bitsize);
            ...
            switch(node->get_kind())
            {
               ...
               case gimple_assign_K:
               {
                  ...
                  if(GET_NODE(gm->op1)->get_kind() == call_expr_K || GET_NODE(gm->op1)->get_kind() == aggr_init_expr_K)
                  {
                     ...
                     THROW_ASSERT(fd && (tree_helper::print_function_name(TreeM, fd) == "memcpy" || tree_helper::print_function_name(TreeM, fd) == "__builtin_memcpy"), "expected a memcpy call");
   #endif
   ...
   #ifndef NDEBUG
                     function_decl * fd = GetPointer<function_decl>(GET_NODE(GetPointer<addr_expr>(cefn)->op));
                     THROW_ASSERT(fd && (tree_helper::print_function_name(TreeM, fd) == "memset" || tree_helper::print_function_name(TreeM, fd) == "__builtin_memset"), "expected a memcpy call");
   #endif
                     const std::vector<tree_nodeRef> & args = ce->args;
                     unsigned int num = 0;
                     std::vector<tree_nodeRef>::const_iterator arg_end = args.end();
               ...
               if(tree_var && HLSMgr->Rmem->is_actual_parm_loaded(tree_var))
                  THROW_ERROR("LOADING of actual parameter not yet implemented");
                       ...
                       ((tree_helper::is_int(TreeM, tree_var) && (tree_helper::is_unsigned(TreeM, form_par_type) or tree_helper::is_bool(TreeM, form_par_type))) ||
                        ((tree_helper::is_unsigned(TreeM, tree_var) or (tree_helper::is_bool(TreeM, form_par_type)))&& tree_helper::is_int(TreeM, form_par_type)) ||
                        (tree_helper::is_real(TreeM, tree_var) && tree_helper::is_real(TreeM, form_par_type))
                        ))
               {
                  add_conversion(port_num, size_tree_var, op, form_par_type, port_index, fu_obj, data, TreeM, tree_var, var_read, size_form_par);
               }
               else if(first_valid_id && tree_var && first_valid_id != tree_var && form_par_type == 0 &&
                       data->CGetOpNodeInfo(*op)->GetOperation() != "rshift_expr" && data->CGetOpNodeInfo(*op)->GetOperation() != "lshift_expr" &&
                       ((tree_helper::is_int(TreeM, tree_var) && tree_helper::is_unsigned(TreeM, first_valid_id)) ||
                        (tree_helper::is_unsigned(TreeM, tree_var) && tree_helper::is_int(TreeM, first_valid_id))
                        ))
               {
      ...
      if(GET_TYPE(data,*op) & TYPE_PHI)
      {
         ...
         for(std::set<vertex>::const_iterator e_it = ending_states.begin(); e_it_end != e_it; ++e_it)
         {
            ...
            for(const auto def_edge : gp->CGetDefEdgesList())
            {
               ...
               PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "Pre-Managing phi operation2 "  + GET_NAME(data, *op) + " ending in state " + HLS->Rliv->get_name(*e_it) + (cur_phi_tree_var ? " for variable " + behavioral_helper->PrintVariable(cur_phi_tree_var) : ""));
               ...
               PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "Managing phi operation "  + GET_NAME(data, *op) + " ending in state " + HLS->Rliv->get_name(*e_it) + (cur_phi_tree_var ? " for variable " + behavioral_helper->PrintVariable(cur_phi_tree_var) : ""));
               if(HLS->storage_value_information->is_a_storage_value(*e_it, var_written))
               {
                  ...
                  if(in_bitsize != out_bitsize)
                  {
                     if(tree_helper::is_unsigned(TreeM, var_written) || tree_helper::is_a_pointer(TreeM, var_written) || tree_helper::is_bool(TreeM, var_written))
                     {
                        ...
                        HLS->Rconn->add_component(ASSIGN_UNSIGNED_STD);
                        HLS->Rconn->add_sparse_logic(conv_port);
                        ...
                        if(phi_postponed)
                        {
                           if (HLS->Rliv->has_state_out(*e_it, *op, var_written))
                           {
                              ...
                              for(std::set<vertex>::const_iterator s_out_it = states_out.begin(); s_out_it != s_out_it_end; ++s_out_it)
                              {
                                 ...
                                 PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "       - add data transfer from " << fu_src_obj->get_string() << " to " << conv_port->get_string() << " port 0:0 from state " << HLS->Rliv->get_name(*e_it) + " to state " + HLS->Rliv->get_name(*s_out_it) + " for " + HLSMgr->CGetFunctionBehavior(funId)->CGetBehavioralHelper()->PrintVariable(cur_phi_tree_var));
                     ...
                     {
                        generic_objRef conv_port = generic_objRef(new i_assign_conn_obj("i_assign_conn_obj_"+STR(id++)));
                     ...
                     else if(tree_helper::is_real(TreeM, var_written))
                     {
                        ...
                        HLS->Rconn->add_component(ASSIGN_REAL_STD);
                        HLS->Rconn->add_sparse_logic(conv_port);
...
unsigned int mux_connection_binding::mux_interconnection()
{
   ...
   for(const auto connection : connection_to_unit)
   {
      ...
      PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level, "Unit: " + unit->get_string() + "(" + boost::lexical_cast<std::string>(operand) + ":" + boost::lexical_cast<std::string>(port_index) + "): " + boost::lexical_cast<std::string>(connection.second.size()) + " connections");
...
unsigned int mux_connection_binding::input_logic(const conn_binding::ConnectionSources & srcs, const generic_objRef tgt, unsigned int op, unsigned int port_index, unsigned int iteration)
{
   ...
   if (srcs.size() == 1)
   {
      ...
      HLS->Rconn->add_connection(op1, tgt, op, port_index, conn_obj);
      PRINT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "  - Direct connection between " + op1->get_string() +
                    " and " + tgt->get_string() + "(" + STR(op) + ":" + STR(port_index) + ")");
      return 0;
   }
   ...
   if (to_allocate.size() > 1)
   {
      ...
      do
      {
         ...
         for (v = obj2var[first].begin(); v != obj2var[first].end(); ++v)
         {
            ...
            if (std::find(src_mux_tree[var2src[*v]].begin(),
...
unsigned int mux_connection_binding::swap_p(const OpGraphConstRef data, vertex op, unsigned int num, std::vector<HLS_manager::io_binding_type> &vars_read, const BehavioralHelperConstRef behavioral_helper, const tree_managerRef TreeM)
{
   ...
   if(operation == STOK(TOK_PLUS_EXPR)
...
void mux_connection_binding::connect_array_index(unsigned int tree_index, generic_objRef fu_obj, unsigned int port_num, unsigned int port_index, unsigned int bus_addr_bitsize, const OpGraphConstRef data, const vertex &op)
{
   ...
   if(tree_helper::is_int(TreeM, tree_index))
   {
      ...
      determine_connection(op, HLS_manager::io_binding_type(tree_index, 0), conv_port, 0, 0, data, bus_addr_bitsize);
