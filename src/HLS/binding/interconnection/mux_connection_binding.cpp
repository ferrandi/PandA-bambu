   ...
   if(!right_addr_expr && is_load_store && HLS->allocation_information->is_direct_access_memory_unit(fu_type))
   {
      ...
      if(var && HLSMgr->Rmem->is_private_memory(var))
      {
         unsigned long long int max_addr =
             HLSMgr->Rmem->get_base_address(var, HLS->functionId) + tree_helper::Size(TreeM->GetTreeNode(var)) / 8;
      ...
      switch(tn->get_kind())
      {
         case addr_expr_K:
         {
            ...
            if(gm && gm->temporary_address)
            {
               ...
               {
                  unsigned long long int max_addr =
                      HLSMgr->Rmem->get_base_address(ref_var->index, HLS->functionId) + tree_helper::Size(ref_var) / 8;
