...
//-----------------------------------------------------------------------------------------------
    ...
    module_binding_check(unsigned int _fu_prec, double _area_resource, const hlsRef _HLS, const HLS_managerRef _HLSMgr, const std::unordered_map<vertex,double> & _slack_time, const std::unordered_map<vertex,double> &_starting_time, double _controller_delay, module_register_binding_spec & _tree_index_dsets) : fu_prec(_fu_prec), area_resource(_area_resource), tree_index_dsets(_tree_index_dsets), HLS(_HLS), HLSMgr(_HLSMgr), slack_time(_slack_time), starting_time(_starting_time), controller_delay(_controller_delay), is_disabled_slack_based_binding(false) {}
    ...
    bool check_no_mux_needed(C_vertex& rep, C_vertex& other) override
    {
       ...
       for (decltype(port_number) i=0 ; i< port_number; i++)
       {
          ...
          if(port_inputs.size()>1) return false;
...
{
      module_binding_check_no_filter(unsigned int _fu_prec, double _area_resource, const hlsRef _HLS, const HLS_managerRef _HLSMgr, const std::unordered_map<vertex,double> & _slack_time, const std::unordered_map<vertex,double> &_starting_time, double _controller_delay, module_register_binding_spec & _tree_index_dsets) :  module_binding_check<vertex_type>(_fu_prec, _area_resource, _HLS, _HLSMgr, _slack_time, _starting_time, _controller_delay, _tree_index_dsets) {module_binding_check<vertex_type>::is_disabled_slack_based_binding=true;}
