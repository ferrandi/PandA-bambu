...
*/
   ...
   {
      double resource_area = allocation_information->compute_normalized_area(fu_s1);
      double exec_time = allocation_information->get_worst_execution_time(fu_s1) - allocation_information->get_correction_time(fu_s1, fsdg->CGetOpNodeInfo(v)->GetOperation(), static_cast<unsigned>(fsdg->CGetOpNodeInfo(v)->GetVariables(FunctionBehavior_VariableType::SCALAR, FunctionBehavior_VariableAccessType::USE).size()));
      // double stage_time = allocation_information->get_worst_stage_period(fu_s1);
