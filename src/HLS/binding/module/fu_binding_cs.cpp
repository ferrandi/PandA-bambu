...
void fu_binding_cs::instantiate_component_kernel(const HLS_managerRef HLSMgr, const hlsRef HLS,
                                                 structural_objectRef clock_port, structural_objectRef reset_port)
{
   ...
   structural_objectRef start_port_scheduler =
       scheduler_mod->find_member(STR(START_PORT_NAME) + "_task", port_o_K, scheduler_mod);
   ...
   structural_objectRef task_pool_end_scheduler =
       scheduler_mod->find_member(STR(TASKS_POOL_END), port_o_K, scheduler_mod);
...
void fu_binding_cs::connect_selector(const hlsRef HLS)
{
   ...
   for(unsigned int i = 0; i < GetPointer<module>(circuit)->get_internal_objects_size(); i++)
   {
      ...
      const structural_objectRef port_selector_module =
          curr_gate->find_member(STR(SELECTOR_REGISTER_FILE), port_o_K, curr_gate);
...
void fu_binding_cs::connect_selector_kernel(const hlsRef HLS)
{
   ...
   structural_objectRef port_selector =
       scheduler_mod->find_member(STR(SELECTOR_REGISTER_FILE), port_o_K, scheduler_mod);
   port_selector->type_resize(static_cast<unsigned>(num_slots));
   structural_objectRef selector_regFile_scheduler =
       scheduler_mod->find_member(STR(SELECTOR_REGISTER_FILE), port_o_K, scheduler_mod);
   structural_objectRef selector_regFile_datapath =
       circuit->find_member(STR(SELECTOR_REGISTER_FILE), port_o_K, circuit);
   structural_objectRef selector_regFile_sign =
       SM->add_sign(STR(SELECTOR_REGISTER_FILE) + "_signal", circuit, port_type);
...
void fu_binding_cs::manage_memory_ports_parallel_chained(const HLS_managerRef HLSMgr, const structural_managerRef SM,
                                              ...
                                              unsigned int& _unique_id)
{
   ...
   for(unsigned int j = 0; j < GetPointer<module>(scheduler)->get_in_port_size();
       j++) // connect input scheduler with datapath input
   {
      ...
      {
         cir_port =
             circuit->find_member(port_name.erase(0, 3), port_i->get_kind(), circuit); // erase IN_ from port name
   ...
   for(const auto& memory_module : memory_modules) // connect scheduler with memory_modules
   {
      ...
      for(unsigned int j = 0; j < GetPointer<const module>(memory_module)->get_out_port_size();
          j++) // connect memory_modules out with scheduler by jms
      {
            ...
            !GetPointer<port_o>(port_i)->get_is_extern())
         {
            ...
            if(std::find(primary_outs[sche_port].begin(), primary_outs[sche_port].end(), port_i) ==
               primary_outs[sche_port].end())
            {
   ...
   for(unsigned int j = 0; j < GetPointer<module>(scheduler)->get_out_port_size();
       j++) // connect scheduler out with datapath out
   {
      ...
      {
         cir_port =
             circuit->find_member(port_name.erase(0, 4), port_i->get_kind(), circuit); // erase OUT_ from port name
         THROW_ASSERT(!cir_port || GetPointer<port_o>(cir_port), "should be a port or null");
         if(!cir_port)
         {
            if(port_i->get_kind() == port_vector_o_K)
   ...
   for(const auto& memory_module : memory_modules)
   {
      for(unsigned int j = 0; j < GetPointer<const module>(memory_module)->get_in_port_size(); j++)
      {
            ...
            GetPointer<port_o>(port_i)->get_is_extern())
         {
            ...
            {
               cir_port =
                   SM->add_port_vector(port_name, port_o::IN, parameters->getOption<unsigned int>(OPT_channels_number),
                                       circuit, port_i->get_typeRef());
               port_o::fix_port_properties(port_i, cir_port);
            }
            if(port_i->get_kind() == port_vector_o_K) // connecting a port vector
            {
               SM->add_connection(GetPointer<port_o>(cir_port)->get_port(0),
                                  GetPointer<port_o>(port_i)->get_port(0)); // connect first port
               if(GetPointer<port_o>(port_i)->get_ports_size() > 1)         // More than 1 channel
               {
                  for(unsigned int num_chan = 1; num_chan < parameters->getOption<unsigned int>(OPT_channels_number);
                      num_chan++)
                  {
                     SM->add_connection(
                         GetPointer<port_o>(cir_port)->get_port(num_chan),
                         GetPointer<port_o>(port_i)->get_port(num_chan)); // connect other port one by one
                  }
               }
            }
            else
      ...
      for(unsigned int j = 0; j < GetPointer<const module>(memory_module)->get_out_port_size(); j++)
      {
            ...
            !GetPointer<port_o>(port_i)->get_is_extern())
         {
            ...
            {
               cir_port =
                   SM->add_port_vector(port_name, port_o::OUT, parameters->getOption<unsigned int>(OPT_channels_number),
                                       circuit, port_i->get_typeRef());
               port_o::fix_port_properties(port_i, cir_port);
            }
            if(port_i->get_kind() == port_vector_o_K) // connecting a port vector
            {
               if(std::find(primary_outs[GetPointer<port_o>(cir_port)->get_port(0)].begin(),
                            primary_outs[GetPointer<port_o>(cir_port)->get_port(0)].end(),
                            GetPointer<port_o>(port_i)->get_port(0)) ==
                  primary_outs[GetPointer<port_o>(cir_port)->get_port(0)].end())
               {
                  primary_outs[GetPointer<port_o>(cir_port)->get_port(0)].push_back(
                      GetPointer<port_o>(port_i)->get_port(0)); // merge first cell of vector
               }
               if(GetPointer<port_o>(port_i)->get_ports_size() > 1) // More than 1 channel
               {
                  for(unsigned int num_chan = 1; num_chan < parameters->getOption<unsigned int>(OPT_channels_number);
                  ...
                  {
                     SM->add_connection(
                         GetPointer<port_o>(port_i)->get_port(num_chan),
                         GetPointer<port_o>(cir_port)->get_port(num_chan)); // connect other port one by one
                  }
               }
            }
            else if(std::find(primary_outs[GetPointer<port_o>(cir_port)->get_port(0)].begin(),
                              primary_outs[GetPointer<port_o>(cir_port)->get_port(0)].end(),
                              port_i) == primary_outs[GetPointer<port_o>(cir_port)->get_port(0)].end())
            {
...
void fu_binding_cs::manage_extern_global_port(const HLS_managerRef HLSMgr, const hlsRef HLS,
                                              ...
                                              unsigned int _dir, structural_objectRef circuit, unsigned int num)
{
   ...
   else
   {
      auto omp_functions = GetPointer<OmpFunctions>(HLSMgr->Rfuns);
      if(omp_functions->kernel_functions.find(HLS->functionId) != omp_functions->kernel_functions.end() ||
         omp_functions->hierarchical_functions.find(HLS->functionId) != omp_functions->hierarchical_functions.end())
      {
         if(!GetPointer<port_o>(port_in)->get_is_memory())
         { // if not a memory port then use standard method
