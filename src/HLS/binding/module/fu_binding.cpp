...
fu_bindingRef fu_binding::create_fu_binding(const HLS_managerConstRef _HLSMgr, const unsigned int _function_id, const ParameterConstRef _parameters)
{
   if(_parameters->isOption(OPT_context_switch))
   {
      ...
      if(omp_functions->parallelized_functions.find(_function_id) != omp_functions->parallelized_functions.end()) found=true;
      if(omp_functions->atomic_functions.find(_function_id) != omp_functions->atomic_functions.end()) found=true;
      if(found)
...
{
   if (operations.find(std::make_pair(unit, index)) == operations.end()) return std::set<vertex>();
...
structural_objectRef fu_binding::add_gate
(
      const HLS_managerRef HLSMgr,
      const hlsRef HLS,
      const technology_nodeRef fu,
      const std::string& name,
      const std::set<vertex>& ops,
...
)
{
   ...
   THROW_ASSERT(curr_lib_instance,
   ...
   if(port_ck) SM->add_connection(clock_port, port_ck);
   structural_objectRef port_rst = curr_gate->find_member(RESET_PORT_NAME, port_o_K, curr_gate);
   if(port_rst) SM->add_connection(reset_port, port_rst);
   GetPointer<module>(circuit)->add_internal_object(curr_gate);

   return curr_gate;
}

void fu_binding::kill_proxy_memory_units(std::map<unsigned int, unsigned int> &memory_units, structural_objectRef curr_gate, std::map<unsigned int, std::set<structural_objectRef> > &var_call_sites_rel, std::map<unsigned int, unsigned int> &reverse_memory_units)
{
...
void fu_binding::kill_proxy_function_units(std::map<unsigned int, std::string> &wrapped_units, structural_objectRef curr_gate, std::map<std::string, std::set<structural_objectRef> > &fun_call_sites_rel, std::map<std::string, unsigned int> &reverse_wrapped_units)
{
...
void fu_binding::manage_killing_memory_proxies(std::map<unsigned int, structural_objectRef>&mem_obj, std::map<unsigned int, unsigned int> &reverse_memory_units, std::map<unsigned int, std::set<structural_objectRef> > &var_call_sites_rel, const structural_managerRef SM, const hlsRef HLS, unsigned int & _unique_id)
{
   const structural_objectRef circuit = SM->get_circ();
...
void fu_binding::manage_killing_function_proxies(std::map<unsigned int, structural_objectRef>&fun_obj, std::map<std::string, unsigned int> &reverse_function_units, std::map<std::string, std::set<structural_objectRef> > &fun_call_sites_rel, const structural_managerRef SM, const hlsRef HLS, unsigned int & _unique_id)
{
   const structural_objectRef circuit = SM->get_circ();
...
void fu_binding::add_to_SM(const HLS_managerRef HLSMgr, const hlsRef HLS, structural_objectRef clock_port, structural_objectRef reset_port)
{
   ...
   for (const auto & m : memory_units)
   {
      ...
      PRINT_DBG_MEX(DEBUG_LEVEL_PEDANTIC, debug_level,
         "Memory Unit: " + allocation_information->get_string_name(fu_type_id) +
         " for variable: " + HLSMgr->CGetFunctionBehavior(HLS->functionId)->CGetBehavioralHelper()->PrintVariable(var));
      ...
      for (unsigned int num = 0; num < n_iterations; num = num + n_channels)
      {
         ...
         specialize_memory_unit(HLSMgr, HLS, curr_gate, var, base_address, rangesize,
                                false,
                                memory_type == MEMORY_TYPE_SYNCHRONOUS_UNALIGNED && (channels_type == CHANNELS_TYPE_MEM_ACC_N1 || channels_type == CHANNELS_TYPE_MEM_ACC_NN),
   ...
   for(std::map<unsigned int, std::string>::iterator wu = wrapped_units.begin(); wu != wrapped_units.end(); ++wu)
   {
      ...
      if (added_memory_element) memory_modules.insert(curr_gate);
   ...
   if (top_function_ids.find(HLS->functionId) != top_function_ids.end() and cg_man->ExistsAddressedFunction())
   {
      ...
      structural_objectRef constBitZero =
            SM->add_module_from_technology_library(
                "constBitZero", CONSTANT_STD, LIBRARY_STD, circuit,
                HLS->HLS_T->get_technology_manager());
      structural_objectRef signBitZero = SM->add_sign(
          "bitZero", circuit,
          circuit->find_member(CLOCK_PORT_NAME, port_o_K, circuit)->get_typeRef());
      ...
      for (const auto Itr : addressed_functions)
      {
         ...
         if (HLSMgr->Rfuns->is_a_proxied_function(FUName)) continue;

         structural_objectRef FU =
               SM->add_module_from_technology_library(
                   FUName + "_i0", FUName,
                   WORK_LIBRARY,
         ...
         SM->add_connection(
             FU->find_member(CLOCK_PORT_NAME, port_o_K, FU),
             circuit->find_member(CLOCK_PORT_NAME, port_o_K, circuit));

         SM->add_connection(
             FU->find_member(RESET_PORT_NAME, port_o_K, FU),
         ...
         for(const auto additional_parameter : HLSMgr->CGetFunctionBehavior(Itr)->CGetBehavioralHelper()->get_parameters())
         {
            ...
            structural_objectRef constZeroParam =
                  SM->add_module_from_technology_library(
                      "zeroParam_" + FUName + "_" + parameterName,
                      CONSTANT_STD, LIBRARY_STD, circuit,
                      HLS->HLS_T->get_technology_manager());
            ...
            structural_objectRef signBitZeroParam = SM->add_sign(
                "signZeroParam_" + FUName + "_" + parameterName, SM->get_circ(),
...
void fu_binding::specialise_fu(const HLS_managerRef HLSMgr, const hlsRef HLS, structural_objectRef fu_obj, unsigned int fu, const std::set<vertex>& mapped_operations, unsigned int ar)
{
   ...
   if (ar)
   {
      ...
      bool Has_extern_allocated_data = ((HLSMgr->Rmem->get_memory_address()-HLSMgr->base_address)>0 and
                                       ...
                                       or
                                       (HLSMgr->Rmem->has_unknown_addresses() and
   ...
   else
   {
      ...
      for(std::set<vertex>::iterator op = mapped_operations.begin(); op != mapped_operations.end(); ++op)
      {
         ...
         if (memory_ctrl_type != "")
         {
            ...
            if(required_variables.find(0) == required_variables.end()) required_variables[0] = 0;
         ...
         else if(HLS->HLS_T->get_technology_manager()->get_library(allocation_information->get_fu_name(fu).first) != WORK_LIBRARY && HLS->HLS_T->get_technology_manager()->get_library(allocation_information->get_fu_name(fu).first) != PROXY_LIBRARY) //functions just synthesized shouldn't be customized
         {
            NP_functionalityRef np = fu_module->get_NP_functionality();
            ...
            for(unsigned int i = 0; i < vars.size(); i++)
            {
               ...
               if (tree_var == 0) continue;
               if(required_variables.find(i) == required_variables.end()) required_variables[i] = 0;
   ...
   for(unsigned int i = 0; i < fu_module->get_in_port_size(); i++)
   {
      ...
      if (port->get_id() == CLOCK_PORT_NAME || port->get_id() == RESET_PORT_NAME || port->get_id() == START_PORT_NAME ) ++offset;
      if(is_multiport && port->get_kind() == port_vector_o_K && GetPointer<port_o>(port)->get_ports_size() == 0)
         GetPointer<port_o>(port)->add_n_ports(static_cast<unsigned int>(max_n_ports), port);
...
void fu_binding::fill_array_ref_memory(std::ostream &init_file_a, std::ostream &init_file_b, unsigned int ar, long long int &vec_size, unsigned int &elts_size, const memoryRef mem, unsigned int bram_bitsize, bool is_memory_splitted, bool is_sds, module* fu_module)
{
   ...
   else if (GetPointer<integer_type>(array_type_node) ||
            GetPointer<real_type>(array_type_node) ||
            GetPointer<enumeral_type>(array_type_node) ||
            GetPointer<pointer_type>(array_type_node) ||
            GetPointer<reference_type>(array_type_node) ||
            GetPointer<record_type>(array_type_node) ||
            GetPointer<union_type>(array_type_node) ||
            GetPointer<complex_type>(array_type_node))
   {
   ...
   if(init_node &&
         (
            (GetPointer<constructor>(init_node) && GetPointer<constructor>(init_node)->list_of_idx_valu.size()) ||
            (GetPointer<string_cst>(init_node) && GetPointer<string_cst>(init_node)->strg != "") ||
...
tree_nodeRef
getFunctionType(tree_nodeRef  exp)
{
...
void
fu_binding::set_ports_are_swapped(vertex v, bool condition)
{
   if(condition)
