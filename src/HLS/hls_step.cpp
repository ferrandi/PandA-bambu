...
std::string HLS_step::EnumToName(const HLSFlowStep_Type hls_flow_step_type)
{
   switch(hls_flow_step_type)
   {
      ...
      case HLSFlowStep_Type::BUS_INTERFACE_GENERATION:
         return "BankedMemoryInterfaceCSGeneration";
      ...
      case HLSFlowStep_Type::C_TESTBENCH_GENERATION:
         return "CTestbenchGeneration";
      case HLSFlowStep_Type::HDL_TESTBENCH_GENERATION:
         return "HDLTestbenchGeneration";
                                    ...
                                    const DesignFlowStep::RelationshipType relationship_type)
{
   ...
   const auto memcpy_function =
       HLSMgr->Rmem && HLSMgr->Rmem->has_implicit_memcpy() ? IRM->GetFunction(MEMCPY)->index : 0;
   ...
   for(auto const& [step_type, step_spec, rel_type] : steps_to_be_created)
   {
      switch(rel_type)
      {
         case HLSFlowStep_Relationship::TOP_FUNCTION:
         {
            ...
            for(const auto top_function : CGM.GetRootFunctions())
            {
               const auto hls_step = design_flow_manager.GetDesignFlowStep(
                   HLSFunctionStep::ComputeSignature(step_type, step_spec, top_function));
         ...
         case HLSFlowStep_Relationship::ALL_FUNCTIONS:
         {
            ...
            {
               const auto hls_step = design_flow_manager.GetDesignFlowStep(
                   HLSFunctionStep::ComputeSignature(step_type, step_spec, memcpy_function));
               const auto design_flow_step =
                   hls_step != DesignFlowGraph::null_vertex() ?
                       DFG->CGetNodeInfo(hls_step)->design_flow_step :
         ...
         {
            const auto hls_step =
                design_flow_manager.GetDesignFlowStep(HLS_step::ComputeSignature(step_type, step_spec));
...
{
   return design_flow_manager.CGetDesignFlowStepFactory(DesignFlowStep::HLS);
}
