...
void TopEntityMemoryMapped::Initialize()
{
   ...
   bool is_wb4_root = is_root_function and
...
static void
propagateInterface(structural_managerRef SM, structural_objectRef wrappedObj, std::list<std::string > & ParametersName);
...
DesignFlowStep_Status TopEntityMemoryMapped::InternalExec()
{
   ...
   structural_type_descriptorRef module_type = structural_type_descriptorRef(
         new structural_type_descriptor(module_name + "_int"));
   ...
   GetPointer<module>(interfaceObj)->set_description(
   ...
   GetPointer<module>(interfaceObj)->set_authors(
   ...
   fu_binding::manage_memory_ports_parallel_chained(SM_mm_interface, AddedComponents, interfaceObj, HLS, unique_id);
...
connect_with_signal_name(
      structural_managerRef SM, structural_objectRef portA,
      ...
      structural_objectRef wrappedObj)
{
   ...
   structural_objectRef controlSignal =
   ...
   for(const auto& function_parameter : function_parameters)
   {
      ...
      if (function_parameter.first == BH->GetFunctionReturnType(HLS->functionId) ||
            function_parameter.first == HLS->functionId) continue;
      ...
      structural_objectRef memoryMappedRegister =
         SM_mm->add_module_from_technology_library(
               "mm_register_" + signalName, component_name,
      ...
      structural_objectRef parameterPort = wrappedObj->find_member(
            signalName, port_o_K, wrappedObj);
      // out1
      structural_objectRef port_out1 = memoryMappedRegister->find_member(
            "out1", port_o_K, memoryMappedRegister);
   ...
   if (!BH->GetFunctionReturnType(HLS->functionId)) return;
   ...
   structural_objectRef returnRegister =
      SM_mm->add_module_from_technology_library(
            "mm_register_" + STR(RETURN_PORT_NAME), component_name,
            HLS->HLS_T->get_technology_manager()->get_library(component_name),
            interfaceObj, HLS->HLS_T->get_technology_manager());
   if(multi_channel_bus)
   ...
   structural_objectRef innerReturnPort =
      wrappedObj->find_member(RETURN_PORT_NAME, port_o_K, wrappedObj);

   structural_objectRef outerReturnPort =
      interfaceObj->find_member(RETURN_PORT_NAME, port_o_K, interfaceObj);

   structural_objectRef returnInPort =
      returnRegister->find_member("in1", port_o_K, returnRegister);
   GetPointer<port_o>(returnInPort)->set_type(innerReturnPort->get_typeRef());

   structural_objectRef returnOutPort =
      returnRegister->find_member("out1", port_o_K, returnRegister);
   ...
   structural_objectRef donePortSignal =
      interfaceObj->find_member("DonePortSignal", signal_o_K, interfaceObj);
   SM_mm->add_connection(
         donePortSignal,
         returnRegister->find_member(DONE_PORT_NAME, port_o_K, returnRegister));

   HLS->Rfu->manage_module_ports(HLSMgr, HLS, SM_mm, returnRegister, 0);
}

   void
TopEntityMemoryMapped::insertStartDoneLogic(
      structural_managerRef SM_mm, structural_objectRef wrappedObj)
{
   structural_objectRef interfaceObj = SM_mm->get_circ();
   ...
   SM_mm->add_connection(
         donePortSignal,
         interfaceObj->find_member(DONE_PORT_NAME, port_o_K, interfaceObj));
   ...
   if (HLSMgr->CGetCallGraphManager()->ExistsAddressedFunction())
   {
      ...
      structural_objectRef notifyCaller =
         SM_mm->add_module_from_technology_library(
               "notifyCaller", component_name,
               HLS->HLS_T->get_technology_manager()->get_library(component_name),
               interfaceObj, HLS->HLS_T->get_technology_manager());
      if(multi_channel_bus)
      ...
      structural_objectRef NotifiedSignal =
         interfaceObj->find_member("Notified", signal_o_K, interfaceObj);
      SM_mm->add_connection(
            notifyCaller->find_member("notified", port_o_K, notifyCaller),
            NotifiedSignal);

      // Connect notify address signal and donePortSignal
      SM_mm->add_connection(
            donePortSignal,
            notifyCaller->find_member(DONE_PORT_NAME, port_o_K, notifyCaller));
      structural_objectRef notifyAddressSignal =
         interfaceObj->find_member("NotifyAddressSignal", signal_o_K, interfaceObj);
      SM_mm->add_connection(
            notifyAddressSignal,
            notifyCaller->find_member("notifyAddress", port_o_K, notifyCaller));
   }
}

   void
TopEntityMemoryMapped::insertStatusRegister(
      structural_managerRef SM_mm, structural_objectRef wrappedObj)
{
   bool multi_channel_bus = HLS->Param->getOption<MemoryAllocation_ChannelsType>(OPT_channels_type) == MemoryAllocation_ChannelsType::MEM_ACC_NN;
   structural_objectRef interfaceObj = SM_mm->get_circ();
   if (HLSMgr->CGetCallGraphManager()->ExistsAddressedFunction())
   {
      ...
      structural_objectRef statusRegister =
         SM_mm->add_module_from_technology_library(
               "StatusRegister", component_name,
               HLS->HLS_T->get_technology_manager()->get_library(component_name),
               interfaceObj, HLS->HLS_T->get_technology_manager());
      if(multi_channel_bus)
      ...
      structural_objectRef controlPort =
         statusRegister->find_member("control", port_o_K, statusRegister);
      structural_objectRef ControlSignal =
         SM_mm->add_sign("ControlSignal", interfaceObj, controlPort->get_typeRef());
      SM_mm->add_connection(ControlSignal, controlPort);

      structural_objectRef donePort =
         wrappedObj->find_member(DONE_PORT_NAME, port_o_K, wrappedObj);
      structural_objectRef donePortSignal =
         SM_mm->add_sign("DonePortSignal", interfaceObj, donePort->get_typeRef());
      SM_mm->add_connection(donePort, donePortSignal);
      SM_mm->add_connection(
            statusRegister->find_member(DONE_PORT_NAME, port_o_K, statusRegister),
            donePortSignal);

      structural_objectRef notifyAddressPort =
         statusRegister->find_member("notifyAddress", port_o_K, statusRegister);
      structural_objectRef notifyAddressSignal =
      ...
      structural_objectRef notifiedPort =
         statusRegister->find_member("notified", port_o_K, statusRegister);
      structural_objectRef NotifiedSignal =
         SM_mm->add_sign("Notified", interfaceObj, notifiedPort->get_typeRef());
      NotifiedSignal->set_type(notifiedPort->get_typeRef());
      SM_mm->add_connection(notifiedPort, NotifiedSignal);

      structural_objectRef startPort =
         statusRegister->find_member(START_PORT_NAME, port_o_K, statusRegister);
      structural_objectRef statusRegisterStart =
   ...
   void
TopEntityMemoryMapped::forwardPorts(structural_managerRef SM_mm,
      structural_objectRef wrappedObj)
{
   structural_objectRef interfaceObj = SM_mm->get_circ();
   ...
   // Start and done
   SM_mm->add_connection(
         interfaceObj->find_member(START_PORT_NAME, port_o_K, interfaceObj),
         wrappedObj->find_member(START_PORT_NAME, port_o_K, wrappedObj));
   SM_mm->add_connection(
         interfaceObj->find_member(DONE_PORT_NAME, port_o_K, interfaceObj),
         wrappedObj->find_member(DONE_PORT_NAME, port_o_K, wrappedObj));
   ...
   for (std::list<std::string>::const_iterator
         Itr = ParametersName.begin(), End = ParametersName.end();
         Itr != End; ++Itr)
   {
      structural_objectRef outerPort =
         interfaceObj->find_member(*Itr, port_o_K, interfaceObj);
      structural_objectRef innerPort =
         wrappedObj->find_member(*Itr, port_o_K, wrappedObj);
   ...
   static void
propagateInterface(structural_managerRef SM,
      structural_objectRef wrappedObj,
      std::list<std::string >  & ParametersName)
{
   ...
   for (unsigned int i = 0; i < wrappedModule->get_num_ports(); ++i)
   {
      ...
      if (portID != CLOCK_PORT_NAME && portID != RESET_PORT_NAME &&
            portID != START_PORT_NAME && portID != RETURN_PORT_NAME &&
            portID != DONE_PORT_NAME &&
            std::find(ParametersName.begin(), ParametersName.end(), portID) == ParametersName.end()) continue;

      SM->add_port(portObj->get_id(),
            portObj->get_port_direction(),
            interfaceObj,
            portObj->get_typeRef());
   }
...
static void
connect_with_signal_name(structural_managerRef SM, structural_objectRef portA, structural_objectRef portB, std::string signalName)
{
      ...
      structural_objectRef component)
{
   ...
   structural_objectRef port_ck = component->find_member(
         CLOCK_PORT_NAME, port_o_K, component);
   structural_objectRef clock = interfaceObj->find_member(
         CLOCK_PORT_NAME, port_o_K, interfaceObj);
   SM->add_connection(port_ck, clock);

   structural_objectRef port_rst = component->find_member(
         RESET_PORT_NAME, port_o_K, component);
   structural_objectRef reset = interfaceObj->find_member(
         RESET_PORT_NAME, port_o_K, interfaceObj);
