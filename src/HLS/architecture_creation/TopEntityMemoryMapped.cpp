...
DesignFlowStep_Status TopEntityMemoryMapped::InternalExec()
{
   ...
   memory::propagate_memory_parameters(wrappedObj, SM_mm_interface);

   HLS->top = SM_mm_interface;
   return DesignFlowStep_Status::SUCCESS;
}

void TopEntityMemoryMapped::resizing_IO(module_o* fu_module, unsigned int max_n_ports) const
{
...
void TopEntityMemoryMapped::allocate_parameters() const
{
   ...
   const unsigned int function_return = [&]() -> unsigned int {
      const auto return_type = ir_helper::GetFunctionReturnType(HLSMgr->get_ir_manager()->GetIRNode(HLS->functionId));
...
void TopEntityMemoryMapped::insertMemoryMappedRegister(structural_managerRef SM_mm, structural_objectRef wrappedObj)
{
   ...
   for(const auto& function_parameter : function_parameters)
   {
      ...
      if(function_parameter.first == function_return || function_parameter.first == HLS->functionId)
      {
         continue;
      }
      ...
      {
         resizing_IO(GetPointerS<module_o>(memoryMappedRegister), _channels_number);
      }
      GetPointerS<module_o>(memoryMappedRegister)
      ...
      if(!is_root_function)
      {
         ...
         const auto pMux = SM_mm->add_module_from_technology_library(
             "pMux_" + signalName, MUX2_GATE_STD, HLS->HLS_D->get_technology_manager()->get_library(MUX2_GATE_STD),
             interfaceObj, HLS->HLS_D->get_technology_manager());
   ...
   if(function_return)
   {
      const auto returnType = function_return;
      ...
      if(multi_channel_bus)
      {
         resizing_IO(GetPointer<module_o>(returnRegister), _channels_number);
      }
      GetPointer<module_o>(returnRegister)
...
void TopEntityMemoryMapped::insertStartDoneLogic(structural_managerRef SM_mm, structural_objectRef wrappedObj)
{
   ...
   if(!is_root_function && HLSMgr->CGetCallGraphManager().ExistsAddressedFunction())
   {
      ...
      notifyCaller->SetParameter("EMULATE_BRAM", emulate_bram ? "1" : "0");
      if(multi_channel_bus)
      {
         resizing_IO(GetPointerS<module_o>(notifyCaller), _channels_number);
      }
...
void TopEntityMemoryMapped::insertStatusRegister(structural_managerRef SM_mm, structural_objectRef wrappedObj)
{
   ...
   if(multi_channel_bus)
   {
      resizing_IO(GetPointer<module_o>(statusRegister), _channels_number);
   }
   GetPointerS<module_o>(statusRegister)
       ->SetParameter("ALLOCATED_ADDRESS",
                               ...
                               const std::list<std::string>& ParametersName)
{
   ...
   const auto wrappedModule = GetPointer<module_o>(wrappedObj);
...
static void setBusSizes(structural_objectRef component, const HLS_managerRef HLSMgr)
{
   const auto componentModule = GetPointerS<module_o>(component);
