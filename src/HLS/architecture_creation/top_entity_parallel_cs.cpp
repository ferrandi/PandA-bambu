...
top_entity_parallel_cs::top_entity_parallel_cs(const ParameterConstRef _parameters, const HLS_managerRef _HLSMgr,
                                               unsigned int _funId,
...
top_entity_parallel_cs::ComputeHLSRelationships(const DesignFlowStep::RelationshipType relationship_type) const
{
   ...
   switch(relationship_type)
   {
      ...
      {
         ret.insert(std::make_tuple(HLSFlowStep_Type::DATAPATH_CS_PARALLEL_CREATOR, HLSFlowStepSpecializationConstRef(),
...
DesignFlowStep_Status top_entity_parallel_cs::InternalExec()
{
   ...
   structural_objectRef controller_clock =
       controller_circuit->find_member(CLOCK_PORT_NAME, port_o_K, controller_circuit);
   ...
   structural_objectRef controller_reset =
       controller_circuit->find_member(RESET_PORT_NAME, port_o_K, controller_circuit);
   ...
   structural_objectRef controller_start =
       controller_circuit->find_member(START_PORT_NAME, port_o_K, controller_circuit);
   ...
   if(HLS->registered_done_port && (parameters->getOption<HLSFlowStep_Type>(OPT_controller_architecture) ==
                                        HLSFlowStep_Type::FSM_CONTROLLER_CREATOR or
                                    parameters->getOption<HLSFlowStep_Type>(OPT_controller_architecture) ==
                                        HLSFlowStep_Type::FSM_CS_CONTROLLER_CREATOR or
                                    parameters->getOption<HLSFlowStep_Type>(OPT_controller_architecture) ==
                                        HLSFlowStep_Type::PIPELINE_CONTROLLER_CREATOR))
   {
      ...
      structural_objectRef done_signal_in = SM->add_sign("done_delayed_REG_signal_in", circuit,
                                                         GetPointer<module>(delay_gate)->get_in_port(2)->get_typeRef());
      ...
      structural_objectRef done_signal_out = SM->add_sign(
          "done_delayed_REG_signal_out", circuit, GetPointer<module>(delay_gate)->get_out_port(0)->get_typeRef());
...
void top_entity_parallel_cs::connect_loop_iter(const structural_objectRef circuit, unsigned long long loopBW)
{
   ...
   if(n != 0)
   {
      ...
      structural_type_descriptorRef constant_type =
          structural_type_descriptorRef(new structural_type_descriptor("bool", loopBW));
      constant->set_type(constant_type);
      GetPointer<module>(SM->get_circ())->add_internal_object(constant);
      structural_objectRef controller_Loop_Iter =
          controller_circuit->find_member("LoopIteration", port_o_K, controller_circuit);
                                                        ...
                                                        unsigned long long loopBW)
{
   ...
   structural_objectRef controller_done_request =
       controller_circuit->find_member(STR(DONE_REQUEST) + "_accelerator", port_vector_o_K, controller_circuit);
   GetPointer<port_o>(controller_done_request)->add_n_ports(num_kernel, controller_done_request);
   structural_objectRef controller_done_port =
       controller_circuit->find_member(STR(DONE_PORT_NAME) + "_accelerator", port_vector_o_K, controller_circuit);
   GetPointer<port_o>(controller_done_port)->add_n_ports(num_kernel, controller_done_port);
   structural_objectRef controller_start_port =
       controller_circuit->find_member(STR(START_PORT_NAME) + "_accelerator", port_vector_o_K, controller_circuit);
   ...
   structural_objectRef controller_LoopIteration =
       controller_circuit->find_member("LoopIteration", port_o_K, controller_circuit);
...
void top_entity_parallel_cs::connect_port_parallel(const structural_objectRef circuit, unsigned long long loopBW)
{
   ...
   structural_objectRef controller_task_pool_end =
       controller_circuit->find_member(STR(TASKS_POOL_END), port_o_K, controller_circuit);
   structural_objectRef datapath_task_pool_end =
       datapath_circuit->find_member(STR(TASKS_POOL_END), port_o_K, datapath_circuit);
   ...
   structural_objectRef datapath_done_request =
       datapath_circuit->find_member(STR(DONE_REQUEST) + "_accelerator", port_vector_o_K, datapath_circuit);
   structural_objectRef controller_done_request =
       controller_circuit->find_member(STR(DONE_REQUEST) + "_accelerator", port_vector_o_K, controller_circuit);
   structural_objectRef done_request_sign =
       SM->add_sign_vector(STR(DONE_REQUEST) + "_accelerator" + "_signal", num_slots, circuit, bool_type);
   SM->add_connection(datapath_done_request, done_request_sign);
   SM->add_connection(done_request_sign, controller_done_request);

   structural_objectRef datapath_done_port =
       datapath_circuit->find_member(STR(DONE_PORT_NAME) + "_accelerator", port_vector_o_K, datapath_circuit);
   structural_objectRef controller_done_port =
       controller_circuit->find_member(STR(DONE_PORT_NAME) + "_accelerator", port_vector_o_K, controller_circuit);
   structural_objectRef done_port_sign =
       SM->add_sign_vector(STR(DONE_PORT_NAME) + "_accelerator" + "_signal", num_slots, circuit, bool_type);
   SM->add_connection(datapath_done_port, done_port_sign);
   SM->add_connection(done_port_sign, controller_done_port);

   structural_objectRef datapath_start_port =
       datapath_circuit->find_member(STR(START_PORT_NAME) + "_accelerator", port_vector_o_K, datapath_circuit);
   structural_objectRef controller_start_port =
       controller_circuit->find_member(STR(START_PORT_NAME) + "_accelerator", port_vector_o_K, controller_circuit);
   structural_objectRef done_start_sign =
       SM->add_sign_vector(STR(START_PORT_NAME) + "_accelerator" + "_signal", num_slots, circuit, bool_type);
