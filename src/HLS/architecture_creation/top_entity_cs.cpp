...
void top_entity_cs::add_context_switch_port()
{
   ...
   structural_objectRef datapath_suspension =
   ...
   structural_objectRef datapath_selector =
       datapath_circuit->find_member(STR(SELECTOR_REGISTER_FILE), port_o_K, datapath_circuit);
   SM->add_connection(datapath_selector, selector_obj);
   structural_objectRef controller_selector =
       controller_circuit->find_member(STR(SELECTOR_REGISTER_FILE), port_o_K, controller_circuit);
...
void top_entity_cs::add_context_switch_port_kernel()
{
   ...
   structural_objectRef datapath_done_request =
       datapath_circuit->find_member(STR(DONE_REQUEST), port_o_K, datapath_circuit);
   ...
   structural_objectRef datapath_done_port =
       datapath_circuit->find_member(STR(DONE_SCHEDULER), port_o_K, datapath_circuit);
   ...
   structural_objectRef datapath_start_port =
       datapath_circuit->find_member(STR(START_PORT_NAME) + "_task", port_o_K, datapath_circuit);
...
void top_entity_cs::add_input_register(structural_objectRef port_in, const std::string& port_prefix,
                                       ...
                                       structural_objectRef, structural_objectRef e_port)
{
   ...
   {
      register_file_selector_signal =
          SM->add_sign(STR(SELECTOR_REGISTER_FILE) + "_signal", circuit, register_file_selector_port->get_typeRef());
   }
   ...
   auto register_to_internal_port = SM->add_sign(
       port_prefix + "_from_reg", circuit, GetPointer<module>(register_file_module)->get_out_port(0)->get_typeRef());
