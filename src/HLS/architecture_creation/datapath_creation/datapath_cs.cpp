...
DesignFlowStep_Status datapath_CS::InternalExec()
{
   ...
   //if is kernel or sub module use register file
   ...
   if(n_elements==0)
   {
      ...
      if(port_ck) HLS->datapath->add_connection(clock, port_ck);
      structural_objectRef port_rst = dummy_gate->find_member(RESET_PORT_NAME, port_o_K, dummy_gate);
      if(port_rst) HLS->datapath->add_connection(reset, port_rst);
   }
