...
class AllocationInformation : public HLSFunctionIR
{
   private:
       ...
       */
      static
      technology_nodeRef get_fu(const std::string& fu_name, const HLS_managerConstRef HLSMgr);
   ...
   public:
      ...
      */
      ...
      static
      std::string extract_bambu_provided_name(unsigned int prec_in, unsigned int prec_out, const HLS_managerConstRef HLSMgr, technology_nodeRef &current_fu);
       ...
       * @return the minimum slack of the component estimated by scheduling
       */
      double getMinimumSlack() const { return minimumSlack; }

      /**
       * set the minimum slack  of the component estimated by scheduling
       * @param slack is the timing slack
       */
      void setMinimumSlack(const double slack) { minimumSlack = slack; }
      ...
      static
      bool can_be_asynchronous_ram(tree_managerConstRef TM, unsigned int var, unsigned int threshold, bool is_read_only_variable);
