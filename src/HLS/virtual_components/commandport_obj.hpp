...
class commandport_obj : public generic_obj
{
   ...
   public:
      ...
      {
         THROW_ASSERT (mode == OPERATION || mode == CONDITION || mode == SWITCH || MULTIIF || mode == UNBOUNDED, "Command mode not allowed into this constructor");
      }
      ...
      {
         THROW_ASSERT (mode == OPERATION || mode == CONDITION || mode == SWITCH || MULTIIF || mode == UNBOUNDED, "Command mode not allowed");
         return signal;
      }
       ...
       */
      unsigned int get_command_type() const { return mode; }

      const std::set< transition >& get_activations() const { return activations; }

      void add_activation(const transition& act) { activations.insert(act); }

      /**
       * Sets structural_object associated to this object
       * @param _SM is reference to structural_object to be associated
       */
      void set_controller_obj(const structural_objectRef& _SM) { controller_SM = _SM; }
      ...
      void set_phi_write_enable() {is_a_phi_write_enable = true;}

      bool get_phi_write_enable() {return is_a_phi_write_enable;}
};
