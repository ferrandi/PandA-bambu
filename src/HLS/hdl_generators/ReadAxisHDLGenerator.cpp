 ...
 */
/**
 * @file ReadAxisHDLGenerator.cpp
 * @brief Implementation of the HDL generator for an AXI-Stream read interface.
 *
...
#include "ReadAxisHDLGenerator.hpp"
...
ReadAxisHDLGenerator::ReadAxisHDLGenerator(const HLS_managerRef& _HLSMgr) : Registrar(_HLSMgr)
{
}

void ReadAxisHDLGenerator::InternalExec(std::ostream& out, structural_objectRef mod, unsigned int function_id,
                                        ...
                                        const std::vector<HDLGenerator::parameter>& /* _ports_inout */)
{
   ...
   out << "assign active = (started | " << _ports_in[i_start].name << ");\n"
       << "assign started_0 = active & ~" << _ports_in[i_tvalid].name << ";\n\n";
