 ...
 */
/**
 * @file TestbenchAcknowledgeHDLGenerator.cpp
 * @brief Implementation of the HDL generator for the testbench model of an acknowledge-based interface.
 *
 * @author Michele Fiorito <michele.fiorito@polimi.it>
 *
 */

#include "TestbenchAcknowledgeHDLGenerator.hpp"
...
TestbenchAcknowledgeHDLGenerator::TestbenchAcknowledgeHDLGenerator(const HLS_managerRef& _HLSMgr) : Registrar(_HLSMgr)
{
}
                                                    ...
                                                    const std::vector<HDLGenerator::parameter>& /* _ports_inout */)
{
   ...
   {
      const auto out_port = add_port(mod_cir, arg_name + in_suffix, port_o::OUT, 1U, true);
      const auto ack_port = add_port(mod_cir, arg_name + in_suffix + "_ack", port_o::IN, 0U);
   ...
   {
      const auto in_port = add_port(mod_cir, arg_name + out_suffix, port_o::IN, 1U, true);
      const auto ack_port = add_port(mod_cir, arg_name + out_suffix + "_ack", port_o::OUT, 0U);
