 ...
 */
/**
 * @file ReadHandshakeHDLGenerator.cpp
 * @brief Implementation of the HDL generator for a handshake-based read interface.
 *
...
#include "ReadHandshakeHDLGenerator.hpp"
...
ReadHandshakeHDLGenerator::ReadHandshakeHDLGenerator(const HLS_managerRef& _HLSMgr) : Registrar(_HLSMgr)
{
}

void ReadHandshakeHDLGenerator::InternalExec(std::ostream& out, structural_objectRef mod, unsigned int function_id,
                                             ...
                                             const std::vector<HDLGenerator::parameter>& /* _ports_inout */)
{
   ...
   out << "assign active = (started | " << _ports_in[i_start].name << ");\n"
       << "assign started_0 = active & ~" << _ports_in[i_vld].name << ";\n"
       << "assign done_0 = active & " << _ports_in[i_vld].name << ";\n\n";
