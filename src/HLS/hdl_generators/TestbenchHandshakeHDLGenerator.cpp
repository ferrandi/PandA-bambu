 ...
 */
/**
 * @file TestbenchHandshakeHDLGenerator.cpp
 * @brief Implementation of the HDL generator for the testbench model of a handshake-based interface.
 *
 * @author Michele Fiorito <michele.fiorito@polimi.it>
 *
 */

#include "TestbenchHandshakeHDLGenerator.hpp"
...
TestbenchHandshakeHDLGenerator::TestbenchHandshakeHDLGenerator(const HLS_managerRef& _HLSMgr) : Registrar(_HLSMgr)
{
}

void TestbenchHandshakeHDLGenerator::InternalExec(std::ostream& out, structural_objectRef mod_cir,
                                                  ...
                                                  const std::vector<HDLGenerator::parameter>& /* _ports_inout */)
{
   ...
   const auto arg_name = mod_cir->get_id().substr(sizeof("if_handshake_") - 1U, std::string::npos);
   ...
   if(if_dir == port_o::IN || if_dir == port_o::IO)
   {
      ...
      const auto empty_n_port = add_port(mod_cir, arg_name + in_suffix + "_vld", port_o::OUT, 0U, true);
      const auto read_port = add_port(mod_cir, arg_name + in_suffix + "_ack", port_o::IN, 0U, true);
   ...
   if(if_dir == port_o::OUT || if_dir == port_o::IO)
   {
      ...
      const auto full_n_port = add_port(mod_cir, arg_name + out_suffix + "_ack", port_o::OUT, 0U, true);
