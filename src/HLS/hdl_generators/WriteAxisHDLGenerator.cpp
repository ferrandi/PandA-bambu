 ...
 */
/**
 * @file WriteAxisHDLGenerator.cpp
 * @brief Implementation of the HDL generator for an AXI-Stream write interface.
 *
...
#include "WriteAxisHDLGenerator.hpp"
...
WriteAxisHDLGenerator::WriteAxisHDLGenerator(const HLS_managerRef& _HLSMgr) : Registrar(_HLSMgr)
{
}

void WriteAxisHDLGenerator::InternalExec(std::ostream& out, structural_objectRef mod, unsigned int function_id,
                                         ...
                                         const std::vector<HDLGenerator::parameter>& /* _ports_inout */)
{
   ...
   out << "assign active = (started | " << _ports_in[i_start].name << ");\n"
       << "assign started_0 = active & ~" << _ports_in[i_ready].name << ";\n\n";

   out << "assign " << _ports_out[o_done].name << " = active & (" << _ports_in[i_ready].name << "|(1&"
       << _ports_in[i_async].name << "));\n";
   out << "assign " << _ports_out[o_valid].name << " = active;\n";
   out << "assign " << _ports_out[o_out1].name << " = " << _ports_in[i_ready].name << ";\n";
   out << "assign " << _ports_out[o_tdata].name << " = " << _ports_in[i_in3].name << ";\n";
}
