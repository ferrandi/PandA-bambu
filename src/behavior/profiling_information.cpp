...
BBExecutions::BBExecutions(const BBGraphConstRef) :
   std::unordered_map<vertex, unsigned long long int>()
{}
#else
...
#if HAVE_UNORDERED
BBEdgeExecutions::BBEdgeExecutions(const BBGraphConstRef) :
   std::unordered_map<EdgeDescriptor, unsigned long long int>()
{}
#else
...
ProfilingInformation::ProfilingInformation(const BBGraphConstRef _bb_graph) :
   bb_executions(_bb_graph),
   edge_executions(_bb_graph)
{}
...
void ProfilingInformation::WriteToXml(xml_element * root, const BBGraphConstRef fcfg) const
{
   ...
   {
      ordered_edge_executions[std::pair<unsigned int, unsigned int>(fcfg->CGetBBNodeInfo(boost::source(edge_execution->first, *fcfg))->block->number, fcfg->CGetBBNodeInfo(boost::target(edge_execution->first, *fcfg))->block->number)]= edge_execution->second;
   }
