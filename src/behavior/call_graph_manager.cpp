...
void CallGraphManager::AddFunction(unsigned int new_function_id, const FunctionBehaviorRef fun_behavior)
{
   ...
   else
   {
      ...
      THROW_ASSERT(call_graph->GetCallGraphInfo()->behaviors.at(new_function_id) == fun_behavior,
	    ...
	    "prev: " + STR(call_graph->GetCallGraphInfo()->behaviors.at(new_function_id)) + "new: " + STR(fun_behavior));
   }
}

void CallGraphManager::AddCallPoint(
...
{
   INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level,
         "---Adding call with id: "+ STR(call_id) + " from fun id: " +
         STR(caller_id) + " to fun id: " + STR(called_id));
...
bool CallGraphManager::IsCallPoint(
      unsigned int caller_id,
      unsigned int called_id,
      unsigned int call_id,
      enum FunctionEdgeInfo::CallType call_type) const
{
   ...
   EdgeDescriptor e; bool found;
   boost::tie(e, found) = boost::edge(src, tgt, *CGetCallGraph());
   THROW_ASSERT(found, "call id " + STR(call_id) +
         " from function " + STR(caller_id) + " function " + STR(called_id) +
         " was not in the call graph");
   ...
   switch (call_type)
   {
      case FunctionEdgeInfo::CallType::direct_call:
         ...
         break;
      case FunctionEdgeInfo::CallType::indirect_call:
         res = functionEdgeInfo->indirect_call_points.find(call_id) !=
            functionEdgeInfo->indirect_call_points.end();
         INDENT_DBG_MEX(DEBUG_LEVEL_VERY_PEDANTIC, debug_level, "indirect call! present? "+ STR(res));
         break;
      case FunctionEdgeInfo::CallType::function_address:
         res = functionEdgeInfo->function_addresses.find(call_id) !=
...
void CallGraphManager::AddFunctionAndCallPoint(
   unsigned int caller_id,
   unsigned int called_id,
   unsigned int call_id,
...
void CallGraphManager::RemoveCallPoint(EdgeDescriptor e, const unsigned int callid)
{
   const unsigned int caller_id =
      Cget_node_info<FunctionInfo, CallGraph>(boost::source(e, *call_graph), *call_graph)->nodeID;
...
std::set<unsigned int> CallGraphManager::GetAddressedFunctions() const
{
   ...
   std::set_intersection(reached_body_functions.cbegin(), reached_body_functions.cend(),
         addressed_functions.cbegin(), addressed_functions.cend(),
...
CalledFunctionsVisitor::CalledFunctionsVisitor(const bool _allow_recursive_functions,
      ...
      std::set<unsigned int> & _body_functions,
      std::set<unsigned int> & _library_functions)
   : allow_recursive_functions(_allow_recursive_functions)
