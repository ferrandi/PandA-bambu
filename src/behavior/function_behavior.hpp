...
class FunctionBehavior
{
   ...
   public:
      ...
      */
       ...
       */
      void add_state_variable(unsigned int node_id) {state_variables.insert(node_id);}

      /**
       * return true if a variable is a state variable or not
       * @param node_id is the node id of the variable
       */
      bool is_a_state_variable(unsigned int node_id) const {return state_variables.find(node_id) != state_variables.end();}
...
class op_vertex_order_by_map : std::binary_function<vertex, vertex, bool>
{
   ...
   public:
       ...
       */
      op_vertex_order_by_map(const std::map<vertex, unsigned int> & ref_, const graph * g_) :
         ref(ref_),
             g(g_)
             {}
