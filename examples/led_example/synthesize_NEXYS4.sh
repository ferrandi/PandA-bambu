...
timeout 2h bambu $root_dir/led_example.c $root_dir/constraints_STD.xml $root_dir/IPs.xml --top-fname=led_example \
   --backend-sdc-extensions=$root_dir/Nexys4_Master.sdc -O3 --device-name=xc7a100t-1csg324 --clock-period=10 --connect-iob \
   ...
   --simulator=MODELSIM --evaluation=BACKEND --print-dot "$@"
return_value=$?
