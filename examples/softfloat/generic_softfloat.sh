...
BATCH_ARGS=("--no-iob" "--registered-inputs=top" "--panda-parameter=profile-top=1" "--device-name=TO_BE_OVERWRITTEN" "--simulate" "-s" "--max-ulp=0")
configuration="${device}_$(printf "%04.1f" $period)_$(echo $compiler | tr '[:upper:]' '[:lower:]')"
OUT_SUFFIX="${configuration}_softfloat"
