...
#if USE_CACHE == 1
#pragma HLS bus bank_number = 4 chunk_size = 4
#elif USE_AXI == 1
#pragma HLS bus bank_number = 4 chunk_size = 4 mode = m_axi
#elif BANK_NUMBER == 2
#pragma HLS bus bank_number = 2 chunk_size = 4 mode = m_axi
#elif BANK_NUMBER == 4
#pragma HLS bus bank_number = 4 chunk_size = 4
#elif BANK_NUMBER == 8
#pragma HLS bus bank_number = 8 chunk_size = 4
#endif
#if CACHE_64b_16d == 1
...
#endif
float __attribute__((noinline)) red(float* const a, float* b, float* c, int n)
{
   float sum = b[0];
   int i;
