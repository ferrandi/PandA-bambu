...
#if USE_CACHE == 1
#pragma HLS bus bank_number = 4 chunk_size = 512
#elif USE_AXI == 1
#pragma HLS bus bank_number = 4 chunk_size = 512 mode = m_axi
#elif BANK_NUMBER == 2
#pragma HLS bus bank_number = 2 chunk_size = 512 mode = m_axi
#elif BANK_NUMBER == 4
#pragma HLS bus bank_number = 4 chunk_size = 512
#elif BANK_NUMBER == 8
#pragma HLS bus bank_number = 8 chunk_size = 512
#endif
#if CACHE_64b_16d == 1
...
#endif
int __attribute__((noinline)) red(int* const a, int* b, int* c, int n)
{
   int sum = b[0];
   int i;
