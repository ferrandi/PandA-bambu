...
/// The first relevant change is in the initialization of coeffs variable done by exploiting constexpr described by the
/// c++14 standard. The second change is on the number of samples that can be handled by a function call. Here we assume
/// a single sample per function call.
...
class firFixedClass
{
   constexpr static short int compute_coeff(size_t index)
   {
      ...
      // generate sin function (time offset in 't' prevents divide by zero)
      float s = sin(2 * M_PI * fc * t + 1e-6f) / (2 * M_PI * fc * t + 1e-6f);
      // generate Hamming window
      float w = 0.53836 - 0.46164 * cos((2 * M_PI * (float)index) / ((float)(filter_len - 1)));
...
int main(void)
{
   ...
   out_fid =
       open("outputFixed.pcm", O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
   if(out_fid < 0)
   {
#ifndef NDEBUG
