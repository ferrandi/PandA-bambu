#!/bin/bash
ARGS="-c=--simulator=MODELSIM -c=--no-iob -c=--soft-float -c=--softfloat-norounding -c=-lm -c=--hls-div -c=--registered-inputs=top -c=--panda-parameter=profile-top=1 -c=--device=TO_BE_OVERWRITTEN -c=--evaluation"
