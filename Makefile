...
scan-build: run-configure
	...
	@scan-build$(SCAN_BUILD_VERSION) -v -v --use-cc=clang$(SCAN_BUILD_VERSION) --use-c++=clang++$(SCAN_BUILD_VERSION) --use-analyzer=clang$(SCAN_BUILD_VERSION) -o scan-build-report make --directory=build/src -j$(J)
...
distclean :
	...
	@find . -name test-driver -exec rm -f {} \;
	@find src -name Makefile.in -exec rm -f {} \;
	@find ext -name aclocal.m4 -exec rm -f {} \;
