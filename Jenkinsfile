    ...
    stage('build') {
      ...
      steps {
        withEnv(['PATH+EXTRACCACHE=/usr/lib/ccache']) {
          sh 'make -f Makefile.init J=20 && mkdir -p build && cd build  && ../configure --prefix=$WORKSPACE/panda-bin --enable-flopoco --enable-glpk --enable-release --enable-opt --with-clang10=/opt/llvm10/bin/clang --with-clang11=/opt/llvm11/bin/clang && nice -n 17 make -j20  install  '
        }
      }
    }
    stage('tests') {
