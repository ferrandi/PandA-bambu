  ...
  stages {
    ...
    stage('tests') {
      parallel {
        ...
        stage('VHDL') {
          steps {
           sh 'cd $WORKSPACE/panda_regressions/vhdl_tests && nice -n 17 $WORKSPACE/panda_regressions/panda_regression_hls.sh -j24 --bambu $WORKSPACE/panda-bin/bin/bambu --spider $WORKSPACE/panda-bin/bin/spider --junitdir="$WORKSPACE/test-reports" -t 120m -c="-wH" --name="_VHDL" --returnfail '
          }
        }
      }
    }
