...
namespace gbbs
{
   ...
   inline vertexSubsetData<data> edgeMapSparse(Graph& G, VS& indices, F& f, const flags fl)
   {
      ...
      if(should_output(fl))
      {
         ...
         auto p = [](std::tuple<uintE, data>& v) { return std::get<0>(v) != UINT_E_MAX; };
   ...
   template <class data /* data associated with vertices in the output vertex_subset */, class Graph /* graph type */,
             class VS /* vertex_subset type */, class F /* edgeMap struct */>
   inline vertexSubsetData<data> edgeMapSparseNoOutput(Graph& G, VS& indices, F& f, const flags fl)
   {
   ...
   struct block
   {
      ...
      void print()
      {
         std::cout << id << " " << block_num << "\n";
      }
   };

   template <class data /* data associated with vertices in the output vertex_subset */, class Graph /* graph type */,
             class VS /* vertex_subset type */, class F /* edgeMap struct */>
   inline vertexSubsetData<data> edgeMapBlocked(Graph& G, VS& indices, F& f, const flags fl)
   {
      if(fl & no_output)
      {
      ...
      parallel_for(0, indices.size(), [&](size_t i) { vertex_offs[i] = block_imap[i]; });
      ...
      parallel_for(
          ...
          [&](size_t i) {
             ...
             assert(vtx_id < n);
             auto neighbors =
                 (fl & in_edges) ? G.get_vertex(vtx_id).in_neighbors() : G.get_vertex(vtx_id).out_neighbors();
   ...
   struct emhelper
   {
      ...
      em_data_block* get_block_and_offset_for_group(size_t group_id)
      {
         ...
         else
         {
            ...
            if(offset + work_block_size > max_block_size)
            { // realloc
               block_ptr = gbbs::new_array_no_init<em_data_block>(1);
               vec.emplace_back(block_ptr);
               block_ptr->block_size = 0;
   ...
   template <class data /* data associated with vertices in the output vertex_subset */, class Graph /* graph type */,
             class VS /* vertex_subset type */, class F /* edgeMap struct */>
   inline vertexSubsetData<data> edgeMapChunked(Graph& G, VS& indices, F& f, const flags fl)
   {
      if(fl & no_output)
      {
         return edgeMapSparseNoOutput<data, Graph, VS, F>(G, indices, f, fl);
      ...
      // kEMBlockSize
      size_t edge_block_size_guess = parlay::num_blocks(outEdgeCount, num_workers() << 3);
      ...
      auto block_offsets = parlay::sequence<size_t>::from_function(all_blocks.size(),
                                                                   [&](size_t i) { return all_blocks[i]->block_size; });
