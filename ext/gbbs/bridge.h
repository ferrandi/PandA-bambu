...
namespace gbbs
{
   ...
   template <typename F>
   inline void parallel_for(size_t start, size_t end, long granularity, F f, bool conservative = false)
   {
      return parallel_for(start, end, f, granularity, conservative);
   }

   template <typename A, typename Af, typename Df, typename F>
   static void parallel_for_alloc(Af init_alloc, Df finish_alloc, long start, long end, F f, long granularity = 0,
                                  bool conservative = false);

#ifdef CILK
   // TODO try parallel_for_1
   template <typename A, typename Af, typename Df, typename F>
   inline void parallel_for_alloc(Af init_alloc, Df finish_alloc, long start, long end, F f, long granularity,
                                  bool conservative)
   {
...
#pragma omp parallel private(alloc)
      {
         ...
         init_alloc(alloc);
         parallel_for_1(
             start, end, [&](size_t i) { f(i, alloc); }, granularity, conservative);
   ...
   inline std::optional<E> fetch_and_add_threshold(E* a, EV b, EV max_v)
   {
      ...
      while(oldV <= max_v)
      {
         if(atomic_compare_and_swap(a, oldV, newV))
            return oldV;
         oldV = atomic_load(a);
         newV = oldV + b;
      }
   ...
   inline uint64_t hash_combine(uint64_t hash_value_1, uint64_t hash_value_2)
   {
      ...
      return hash_value_1 ^ (hash_value_2 + 0x9e3779b97f4a7c15 + (hash_value_1 << 6) + (hash_value_1 >> 2));
   }
...
namespace parlay
{
   ...
   size_t pack_index_out(Bool_Seq const& Fl, Out_Seq&& Out, flags fl = no_flag)
   {
      ...
      return pack_out(delayed_seq<Idx_Type>(Fl.size(), identity), Fl, std::forward<Out_Seq>(Out), fl);
   }
   ...
   auto map_with_index(Seq const& A, Func&& f, flags fl = no_flag) -> sequence<OT>
   {
   ...
   auto filter_index(In_Seq const& In, F f, flags fl = no_flag) -> sequence<typename In_Seq::value_type>
   {
      ...
      parlay::internal::sliced_for(n, _block_size, [&](size_t i, size_t s, size_t e) {
         parlay::internal::pack_serial_at(make_slice(In).cut(s, e), make_slice(Fl).cut(s, e),
   ...
   inline sequence<std::tuple<Idx_Type, D>> pack_index_and_data(F& f, size_t size)
   {
      auto id_seq = parlay::delayed_seq<std::tuple<Idx_Type, D>>(
          size, [&](size_t i) { return std::make_tuple((Idx_Type)i, std::get<1>(f[i])); });
   ...
   typename Seq::value_type kth_smallest(Seq const& s, size_t k, Compare less, random r = random())
   {
      using T = typename Seq::value_type;
      size_t n = s.size();
      T pivot = s[r[0] % n];
   ...
   typename Seq::value_type approximate_kth_smallest(Seq const& S, size_t k, Compare less, random r = random())
   {
      ...
      sequence<T> samples = sequence<T>::from_function(num_samples, [&](size_t i) -> T { return S[r[i] % n]; });
   ...
   inline int t_to_stringlen(long a)
   {
      return 21;
   }
   inline void type_to_string(char* s, long a)
   {
      sprintf(s, "%ld", a);
   }
   inline int t_to_stringlen(unsigned long a)
   {
      return 21;
   }
   inline void type_to_string(char* s, unsigned long a)
   {
      sprintf(s, "%lu", a);
   }
   inline uint t_to_stringlen(uint a)
   {
      return 12;
   }
   inline void type_to_string(char* s, uint a)
   {
      sprintf(s, "%u", a);
   }
   inline int t_to_stringlen(int a)
   {
      return 12;
   }
   inline void type_to_string(char* s, int a)
   {
      sprintf(s, "%d", a);
   }
   inline int t_to_stringlen(double a)
   {
      return 18;
   }
   inline int t_to_stringlen(char* a)
   {
      return strlen(a) + 1;
   }
   inline void type_to_string(char* s, char* a)
   {
      sprintf(s, "%s", a);
   }
   inline void type_to_string(char* s, double a)
   {
      sprintf(s, "%.11le", a);
   }
