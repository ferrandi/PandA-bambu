...
void random_shuffle_(slice<InIterator, InIterator> In, slice<OutIterator, OutIterator> Out, random r = random()) {

  size_t n = In.size();
  if (n < SEQ_THRESHOLD) {
    parallel_for(0, n, [&](size_t i) { assign_uninitialized(Out[i], In[i]); });
  ...
  std::tie(bucket_offsets, single) =
      count_sort<uninitialized_copy_tag>(make_slice(In), make_slice(Out), make_slice(get_pos), num_buckets);
...
sequence<_Integer> random_permutation(_Integer n, random r = random()) {
  auto id = sequence<_Integer>::from_function(n, [&](_Integer i) -> _Integer { return i; });
