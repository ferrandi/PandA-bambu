...
template<typename F>
inline void parallel_for(size_t start, size_t end, F f, long granularity = 0, bool conservative = false);
...
inline void parallel_for(size_t start, size_t end, F f, long granularity, bool conservative) {
  if (end > start) internal::get_default_scheduler().parfor(start, end, f, (size_t)granularity, conservative);
}
