...
typename Seq::value_type kth_smallest_old(Seq const &s, size_t k, Compare less, random r = random()) {
...
typename Seq::value_type approximate_kth_smallest(Seq const &S, size_t k, Compare less, random r = random()) {
  ...
  parlay::sequence<T> samples(num_samples, [&](size_t i) -> T { return S[r[i] % n]; });
  return sample_sort(samples, less)[k * num_samples / n];
}

template<class Iterator, class Compare>
auto kth_smallest(slice<Iterator, Iterator> S, size_t k, Compare less, random r = random()) {
  ...
  auto samples = parlay::sequence<T>::from_function(num_samples, [&](auto i) { return S[r[i] % n]; });
  auto s = sample_sort(make_slice(samples), less);
  auto a = s[std::max((k * num_samples / n), (size_t)sqrt_samples) - sqrt_samples];
