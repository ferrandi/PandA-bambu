...
void get_bucket_counts(slice<InIterator, InIterator> sA, slice<PivotIterator, PivotIterator> sB,
...
void seq_sort_(slice<InIterator, InIterator> In, slice<OutIterator, OutIterator> Out, const Compare& less,
               bool stable = false) {
  size_t l = In.size();
...
template<typename s_size_t = size_t, typename InIterator, typename OutIterator, typename Compare>
void sample_sort_inplace_(slice<InIterator, InIterator> In, slice<OutIterator, OutIterator> Out, const Compare& less) {
  ...
  if (n < QUICKSORT_THRESHOLD) {
    ...
    auto bucket_offsets = transpose_buckets<uninitialized_relocate_tag>(Tmp.begin(), Out.begin(), counts, n, block_size,
...
// be stable.
template<typename s_size_t = size_t, typename InIterator, typename OutIterator, typename Compare>
void sample_sort_(slice<InIterator, InIterator> In, slice<OutIterator, OutIterator> Out, const Compare& less,
                  bool stable = false) {
  using value_type = typename slice<InIterator, InIterator>::value_type;
  size_t n = In.size();
  ...
  if (n < QUICKSORT_THRESHOLD) {
    ...
    auto sample_set = sequence<value_type>::from_function(sample_set_size, [&](size_t i) { return In[hash64(i) % n]; });
    // sort the samples
    quicksort(sample_set.begin(), sample_set_size, less);

    // subselect samples at even stride
    auto pivots =
        sequence<value_type>::from_function(num_buckets - 1, [&](size_t i) { return sample_set[OVER_SAMPLE * i]; });
...
template<typename Iterator, typename Compare>
auto sample_sort(slice<Iterator, Iterator> A, const Compare& less, bool stable = false) {
  using value_type = typename slice<Iterator, Iterator>::value_type;
...
void sample_sort_inplace(slice<Iterator, Iterator> A, const Compare& less) {
  if (A.size() < (std::numeric_limits<unsigned int>::max)()) {
