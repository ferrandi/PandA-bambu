...
class file_map {
  ...
  ~file_map() { contents.clear(); }

  file_map(file_map&& other) noexcept : contents(std::move(other.contents)) { other.contents.clear(); }
  ...
  void swap(file_map& other) { std::swap(contents, other.contents); }
