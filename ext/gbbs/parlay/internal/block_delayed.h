...
struct flatten_iterator {
  ...
  flatten_iterator(in_iter_t in_iter, out_iter_t out_iter) : in_iter(in_iter), out_iter(out_iter) {
    // std::cout << "in flatten init: " << (*out_iter).end() - (*out_iter).begin() << std::endl;
  }
  flatten_iterator(out_iter_t out_iter) : in_iter((*out_iter).begin()), out_iter(out_iter) {}
};
...
static inline std::pair<size_t, size_t> num_blocks_and_size(size_t n) {
  return std::pair((n == 0) ? 0 : 1ul + ((n)-1ul) / (_block_size), _block_size);
}
...
auto make_iterators(Seq const &S) {
  ...
  return internal::tabulate(
      num_blocks,
      [&, bs = block_size](size_t i) {
        size_t start = i * bs;
...
auto scan_(Seq const &S, Monoid const &m, bool inclusive) {
  ...
  if (!(num_blocks == 1 && inclusive)) {
    auto sums = internal::map(iters, [&](auto iter) { return stream_delayed::reduce(m.f, m.identity, iter); });
    std::tie(offsets, total) = internal::scan(sums, m);
  } else
    offsets = sequence<T>(1, m.identity);
  auto iters2 = internal::tabulate(
      num_blocks, [&](size_t i) { return stream_delayed::scan(m.f, offsets[i], iters[i], inclusive); }, 1);
  assert(!(iters2.empty()));
...
auto reduce(block_delayed_sequence<IDS> const &A, Monoid const &m) {
  auto sums = internal::map(A.sub_ranges, [&](auto iter) { return stream_delayed::reduce(m.f, m.identity, iter); });
...
auto zip(Seq1 const &S1, Seq2 const &S2) {
  ...
  auto results = internal::tabulate(
      iters1.size(), [&](size_t i) { return stream_delayed::zip(iters1[i], iters2[i]); }, 1);
...
auto zip_with(Seq1 const &S1, Seq2 const &S2, F const &f) {
  ...
  auto iters2 = make_iterators(S2);
  auto results = internal::tabulate(
      iters1.size(), [&](size_t i) { return stream_delayed::zip_with(iters1[i], iters2[i], f); }, 1);
...
void apply(block_delayed_sequence<IDS> const &A, F const &f) {
  auto I = A.sub_ranges;
  parlay::parallel_for(
      0, I.size(), [&](size_t i) { stream_delayed::apply(I[i], f); }, 1);
}
...
void zip_apply(Seq1 const &s1, Seq2 const &s2, F const &f) {
  ...
  auto iters2 = make_iterators(s2);
  parlay::parallel_for(
      0, iters1.size(), [&](size_t i) { stream_delayed::zip_apply(iters1[i], iters2[i], f); }, 1);
}
...
auto map(block_delayed_sequence<IDS> const &A, F const &f) {
  auto I = A.sub_ranges;
  auto results = internal::tabulate(
      I.size(), [&](size_t i) { return stream_delayed::map(I[i], f); }, 1);
  return block_delayed_sequence(std::move(results), A.size());
}

template<typename Seq, typename F, typename std::enable_if_t<!is_delayed<Seq>::value, int> = 0>
auto map(Seq const &A, F const &f) {
  return internal::map(A, f);
}

template<typename Seq, typename std::enable_if_t<is_delayed<Seq>::value, int> = 0>
auto force(Seq A) {
  size_t n = A.size();
...
auto flatten(Seq &seq) {
  ...
  auto sizes = internal::map(seq, [](auto const &s) -> size_t { return s.size(); });
  ...
  auto results = internal::tabulate(
      num_blocks,
      [&, block_size = block_size](size_t i) {
        size_t start = i * block_size;
        size_t len = std::min(block_size, n - start);
        size_t j = (std::upper_bound(offsets.begin(), offsets.end(), start) - offsets.begin() - 1);
...
auto filter_map(Seq A, F const &f, G const &g) {
  ...
  if (num_blocks == 1) return stream_delayed::filter_map(iters[0], f, g);
  auto seqs = internal::tabulate(
      num_blocks, [&](size_t i) -> parlay::sequence<T> { return stream_delayed::filter_map(iters[i], f, g); }, 1);
  t.next("tabulate");
  auto sizes = internal::map(seqs, [&](parlay::sequence<T> const &x) { return x.size(); });
  auto [r, total] = internal::scan(sizes, addm<size_t>());
  parlay::sequence<T> out = parlay::sequence<T>::uninitialized(total);
  parlay::parallel_for(
      0, num_blocks,
      [&, ri = r.begin(), oi = out.begin()](size_t i) {
...
auto filter_op2(Seq A, F const &f) {
  ...
  auto num_blocks = iters.size();
  auto seqs = internal::tabulate(
      num_blocks, [&](size_t i) -> parlay::sequence<T> { return stream_delayed::filter_op(iters[i], f); }, 1);
  auto y = flatten(seqs);
  return std::pair(std::move(y), std::move(seqs));
}
