...
// needs to be made portable
    ...
    if constexpr (is_contiguous_iterator_v<oi>) __builtin_prefetch(((char*)k) + 64);
#endif
...
void seq_write_down_(InSeq In, OutIterator Out, KeySeq Keys, OffsetIterator offsets, size_t) {  // num_buckets) {
...
void seq_count_sort_(InS In, OutS Out, KeyS Keys, CountIterator counts, size_t num_buckets) {
...
sequence<size_t> seq_count_sort(slice<InIterator, InIterator> In, slice<OutIterator, OutIterator> Out,
...
std::pair<sequence<size_t>, bool> count_sort_(slice<InIterator, InIterator> In, slice<OutIterator, OutIterator> Out,
                                              slice<KeyIterator, KeyIterator> Keys, size_t num_buckets,
  ...
  if (n < SEQ_THRESHOLD || num_blocks == 1 || num_threads == 1) {
    return std::make_pair(seq_count_sort<assignment_tag>(In, Out, Keys, num_buckets), false);
  }
  ...
  parallel_for(
      ...
      [&](size_t i) {
        ...
        seq_count_(In.cut(start, end), make_slice(Keys).cut(start, end), counts.begin() + i * num_buckets, num_buckets);
      },
      1, is_nested);
  t.next("first loop");
  ...
  parallel_for(
      ...
      [&](size_t i) {
        size_t start = (std::min)(i * block_size, n);
        size_t end = (std::min)(start + block_size, n);
        seq_write_<assignment_tag>(In.cut(start, end), Keys.cut(start, end), dest_offsets.begin() + i * num_buckets,
                                   num_buckets);
      },
      1, is_nested);
  t.next("last loop");
...
auto group_by_small_int(slice<InIterator, InIterator> In, slice<KeyIterator, KeyIterator> Keys, size_t num_buckets) {
...
template<typename assignment_tag, typename InIterator, typename OutIterator, typename KeyIterator>
auto count_sort(slice<InIterator, InIterator> In, slice<OutIterator, OutIterator> Out,
                slice<KeyIterator, KeyIterator> Keys, size_t num_buckets, float parallelism = 1.0,
  ...
  if (n < max32 && num_buckets < max32)
    ...
    return count_sort_<assignment_tag, uint32_t>(In, Out, Keys, num_buckets, parallelism, skip_if_in_one);
  return count_sort_<assignment_tag, size_t>(In, Out, Keys, num_buckets, parallelism, skip_if_in_one);
}
