...
void seq_merge(slice<InIterator1, InIterator1> A, slice<InIterator2, InIterator2> B, slice<OutIterator, OutIterator> R,
               const BinaryOp &f) {
  size_t nA = A.size();
  size_t nB = B.size();
  size_t i = 0;
...
template<typename assignment_tag, typename InIterator1, typename InIterator2, typename OutIterator, typename BinaryOp>
void merge_into(slice<InIterator1, InIterator1> A, slice<InIterator2, InIterator2> B, slice<OutIterator, OutIterator> R,
                const BinaryOp &f, bool cons = false) {
  size_t nA = A.size();
  size_t nB = B.size();
  size_t nR = nA + nB;
  if (nR < _merge_base) {
    ...
    parallel_for(0, nB, [&](size_t i) { assign_dispatch(R[i], B[i], assignment_tag{}); });
  } else if (nB == 0) {
    parallel_for(0, nA, [&](size_t i) { assign_dispatch(R[i], A[i], assignment_tag{}); });
  } else {
    size_t mA = nA / 2;
    ...
    size_t mR = mA + mB;
    auto left = [&]() { merge_into<assignment_tag>(A.cut(0, mA), B.cut(0, mB), R.cut(0, mR), f, cons); };
    auto right = [&]() { merge_into<assignment_tag>(A.cut(mA, nA), B.cut(mB, nB), R.cut(mR, nR), f, cons); };
...
auto merge(slice<IteratorA, IteratorA> A, slice<IteratorB, IteratorB> B, const BinaryOp &f, bool cons = false) {
...
// into the resulting sequence
template<typename IteratorA, typename IteratorB, typename BinaryOp>
auto merge_move(slice<IteratorA, IteratorA> A, slice<IteratorB, IteratorB> B, const BinaryOp &f, bool cons = false) {
