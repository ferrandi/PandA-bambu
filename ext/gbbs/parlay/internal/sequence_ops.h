...
auto tabulate(size_t n, UnaryOp &&f, size_t granularity = 0) {
  return sequence<typename std::remove_reference<typename std::remove_cv<decltype(f(0))>::type>::type>::from_function(
      n, f, granularity);
}
...
auto map(R &&r, UnaryOp &&f, size_t granularity = 0) {
  return tabulate(
      parlay::size(r), [&f, it = std::begin(r)](size_t i) { return f(it[i]); }, granularity);
}
...
auto dmap(R &&r, UnaryOp &&f) {
  ...
  return delayed_tabulate(
      n, [r = std::forward<R>(r), f = std::forward<UnaryOp>(f)](size_t i) { return f(std::begin(r)[i]); });
}
...
auto delayed_map(R &r, UnaryOp &&f) {
  size_t n = parlay::size(r);
  return delayed_tabulate(n, [ri = std::begin(r), f = std::forward<UnaryOp>(f)](size_t i) { return f(ri[i]); });
}
...
template<typename Seq, typename Monoid>
auto reduce(Seq const &A, Monoid m, flags fl = no_flag) -> typename Seq::value_type {
  using T = typename Seq::value_type;
  size_t n = A.size();
  ...
  sequence<T> Sums(l);
  sliced_for(n, block_size, [&](size_t i, size_t s, size_t e) { Sums[i] = reduce_serial(make_slice(A).cut(s, e), m); });
...
auto scan_serial(In_Seq const &In, Out_Seq Out, Monoid const &m, typename In_Seq::value_type offset, flags fl,
                 bool out_uninitialized = false) -> typename In_Seq::value_type {
...
auto scan_(In_Seq const &In, Out_Range Out, Monoid const &m, flags fl, bool out_uninitialized = false) ->
    typename In_Seq::value_type {
  using T = typename In_Seq::value_type;
  size_t n = In.size();
  size_t l = num_blocks(n, _block_size);
  if (l <= 2 || fl & fl_sequential) return scan_serial(In, Out, m, m.identity, fl, out_uninitialized);
  sequence<T> Sums(l);
  sliced_for(n, _block_size,
             [&](size_t i, size_t s, size_t e) { Sums[i] = reduce_serial(make_slice(In).cut(s, e), m); });
...
auto scan(sequence<T> &&In, Monoid m, flags fl = no_flag) -> std::pair<sequence<T>, T> {
...
auto pack_serial(In_Seq const &In, Bool_Seq const &Fl) -> sequence<typename In_Seq::value_type> {
  using T = typename In_Seq::value_type;
  size_t n = In.size();
...
template<typename In_Seq, typename Bool_Seq>
auto pack(In_Seq const &In, Bool_Seq const &Fl, flags fl = no_flag) -> sequence<typename In_Seq::value_type> {
  using T = typename In_Seq::value_type;
  size_t n = In.size();
  size_t l = num_blocks(n, _block_size);
  ...
  sliced_for(n, _block_size,
             [&](size_t i, size_t s, size_t e) { Sums[i] = sum_bools_serial(make_slice(Fl).cut(s, e)); });
...
size_t pack_out(In_Seq const &In, Bool_Seq const &Fl, Out_Seq Out, flags fl = no_flag) {
  size_t n = In.size();
  size_t l = num_blocks(n, _block_size);
  if (l <= 1 || fl & fl_sequential) {
...
auto filter_map(In_Seq const &In, F f, G g) {
  ...
  sliced_for(n, _block_size, [&](size_t i, size_t s, size_t e) {
    pack_serial_at(make_slice(in_mapped).cut(s, e), make_slice(Fl).cut(s, e),
                   make_slice(Out).cut(Sums[i], (i == l - 1) ? m : Sums[i + 1]));
  });
  return Out;
}
...
std::pair<size_t, size_t> split_three(slice<InIterator, InIterator> In, slice<OutIterator, OutIterator> Out,
