...
struct forward_delayed_sequence {
  ...
  struct iterator {
    ...
    bool operator!=([[maybe_unused]] const sentinal &other) const { return count != 0; }
    bool operator==([[maybe_unused]] const sentinal &other) const { return count == 0; }
  };

  struct sentinal {
    size_t operator-(const iterator &i) const { return i.count; }
    size_t operator-(iterator &i) { return i.count; }
    bool operator!=(const iterator &other) const { return other.count != 0; }
    bool operator==(const iterator &other) const { return other.count == 0; }
  };
  ...
  forward_delayed_sequence(Iter ii, size_t n) : begin_(iterator(ii, n)) {}
};
...
auto scan(F f, T const &init, const Seq &S, bool inclusive = false) {
  ...
  struct iter {
    ...
    iter(F const &f, T const &init, input_iter_t input_iterator) : f(f), value(init), input_iterator(input_iterator) {}
  };
