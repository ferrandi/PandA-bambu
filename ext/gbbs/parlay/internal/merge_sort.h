...
void merge_sort_(slice<InIterator, InIterator> In, slice<OutIterator, OutIterator> Out, const BinaryOp& f,
                 bool inplace) {
  size_t n = In.size();
  // Base case
  if (n < MERGE_SORT_BASE) {
    ...
    size_t m = n / 2;
    par_do_if(
        n > 64, [&]() { merge_sort_(In.cut(0, m), Out.cut(0, m), f, !inplace); },
        [&]() { merge_sort_(In.cut(m, n), Out.cut(m, n), f, !inplace); }, true);
