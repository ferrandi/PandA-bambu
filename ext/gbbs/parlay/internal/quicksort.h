...
std::tuple<size_t, size_t, bool> p_split3(slice<Iterator, Iterator> A, slice<Iterator, Iterator> B, const BinPred& f) {
  ...
  if (true) {
    ...
    auto r =
        split_three<assignment_tag>(A, make_slice(B), make_slice(delayed_seq<unsigned char>(n, flag)), fl_conservative);
    return std::make_tuple(r.first, r.first + r.second, !f(p1, p2));
  } else {  // currently not used
    auto buckets = dseq(n, [&](size_t i) { return f(A[i], p1) ? 0 : f(p2, A[i]) ? 2 : 1; });
...
void p_quicksort_(slice<InIterator, InIterator> In, slice<OutIterator, OutIterator> Out, const F& f,
                  bool inplace = false, long cut_size = -1) {
  size_t n = In.size();
  if (cut_size == -1) cut_size = std::max<long>((3 * n) / num_workers(), (1 << 14));
  if (n < (size_t)cut_size) {
    quicksort(In.begin(), n, f);
    if (!inplace)
      parallel_for(
          0, n, [&](size_t i) { assign_dispatch(Out[i], In[i], assignment_tag()); }, 2000);
  } else {
    size_t l, m;
    bool mid_eq;
    std::tie(l, m, mid_eq) = p_split3<assignment_tag>(In, Out, f);
    par_do3([&]() { p_quicksort_<assignment_tag>(Out.cut(0, l), In.cut(0, l), f, !inplace, cut_size); },
            [&]() {
              auto copy_in = [&](size_t i) { In[i] = Out[i]; };
              if (!mid_eq)
                p_quicksort_<assignment_tag>(Out.cut(l, m), In.cut(l, m), f, !inplace, cut_size);
              else if (inplace)
