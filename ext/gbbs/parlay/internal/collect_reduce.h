...
auto collect_reduce_few(Seq const &A, Helper const &helper, size_t num_buckets) {
  ...
  if (n < CR_SEQ_THRESHOLD || num_blocks == 1 || num_threads == 1) return seq_collect_reduce(A, helper, num_buckets);
  ...
  sliced_for(n, block_size, [&](size_t i, size_t start, size_t end) {
    Out[i] = seq_collect_reduce(make_slice(A).cut(start, end), helper, num_buckets);
  });
...
struct get_bucket {
  ...
  get_bucket(Seq const &A, HashEq const &hasheq, size_t bits) : hasheq(hasheq) {
    ...
    for (size_t i = 0; i < table_size; i++) {
      if (hash_table_count[i].second + 2 > copy_cutoff) {
        ...
        if (hash_table[idx].second == -1) hash_table[idx] = std::make_pair(key, heavy_hitters++);
      }
    }
  }
  ...
  size_t operator()(in_type const &v) const {
    ...
    if (heavy_hitters > 0) {
      ...
      if ((hash_val & bucket_mask) < heavy_hitters) return hash_val % (bucket_mask + 1 - heavy_hitters) + heavy_hitters;
...
auto collect_reduce(Seq const &A, Helper const &helper, size_t num_buckets) {
  ...
  size_t bits = std::max<size_t>(log2_up(1 + (2 * (size_t)sizeof(T) * n) / cache_per_thread), 4);
  size_t num_blocks = size_t{1} << bits;

  if (num_buckets <= 4 * num_blocks || n < CR_SEQ_THRESHOLD) return collect_reduce_few(A, helper, num_buckets);
  ...
  struct hasheq {
    ...
    static size_t hash(key_type const &a) { return hash64_2((a + shift) & ~((size_t)15)); }
  ...
  block_offsets = integer_sort_<std::false_type, uninitialized_copy_tag>(make_slice(A), make_slice(B), make_slice(Tmp),
                                                                         gb, bits, num_blocks);
  t.next("sort");
...
auto seq_collect_reduce_sparse(Slice A, Helper const &helper) {
  ...
  for (size_t j = 0; j < A.size(); j++) {
    ...
    if (flags[k]) {
      ...
      if constexpr (std::is_same<assignment_tag, uninitialized_relocate_tag>::value) helper.get_key(A[j]).~key_type();
    } else {
      flags[k] = true;
      count++;
      helper.init(table[k], A[j]);
      assign_dispatch(helper.get_key(table[k]), helper.get_key(A[j]), assignment_tag());
    }
    // if relocating input need to destruct value
    if constexpr (std::is_same<assignment_tag, uninitialized_relocate_tag>::value) helper.destruct_val(A[j]);
  }
...
auto collect_reduce_sparse_(Slice A, Helper const &helper) {
  ...
  // the counting sort uses 2 x input size due to copy
  size_t cache_per_thread = 1000000;
  size_t bits = log2_up((size_t)(1 + (1.2 * 2 * sizeof(in_type) * n) / (float)cache_per_thread));
  ...
  auto bucket_offsets = count_sort<assignment_tag>(make_slice(A), make_slice(B), make_slice(keys), num_buckets).first;
  t.next("integer sort");
      ...
      [&](size_t i) {
        ...
        if (i < heavy_cutoff) {
          ...
          if constexpr (!std::is_trivially_destructible_v<in_type>)
            parallel_for(
                0, block.size(), [&](size_t i) { block[i].~in_type(); }, 1000);
          return a;
        } else
          return seq_collect_reduce_sparse<uninitialized_relocate_tag>(block, helper);
      },
      1);
  t.next("block hash");
