...
auto group_by_key_sorted(const Range &S, const Comp &less) {
  ...
  if constexpr (std::is_integral_v<K> && std::is_unsigned_v<K> && sizeof(KV) <= 16) {
    ...
    auto pair_less = [&](auto const &a, auto const &b) { return less(a.first, b.first); };
  ...
  auto r = tabulate(idx.size(), [&](size_t i) {
    ...
    return std::pair(std::move(sorted[idx[i]].first), to_sequence(vals.cut(start, end)));
  });
  return r;
}
...
struct reduce_by_key_helper {
  ...
  Hash hash;
  Equal equal;
  reduce_by_key_helper(Monoid const &m, Hash const &h, Equal const &e) : monoid(m), hash(h), equal(e){};
  static const key_type &get_key(in_type const &p) { return p.first; }
  static key_type &get_key(in_type &p) { return p.first; }
  void init(result_type &p, in_type const &kv) const { assign_uninitialized(p.second, kv.second); }
  void update(result_type &p, in_type const &kv) const { p.second = monoid.f(p.second, kv.second); }
  static void destruct_val(in_type &kv) { kv.second.~value_type(); }
  template<typename Range>
  result_type reduce(Range &S) const {
    auto &key = S[0].first;
    auto sum = internal::reduce(delayed_map(S, [&](in_type const &kv) { return kv.second; }), monoid);
...
template<PARLAY_RANGE_TYPE R, typename Monoid = addm<typename range_value_type_t<R>::second_type>,
...
struct group_by_key_helper {
  ...
  static void init(result_type &p, in_type const &kv) { assign_uninitialized(p.second, sequence(1, kv.second)); }
  static void update(result_type &p, in_type const &kv) { p.second.push_back(kv.second); }
...
template<PARLAY_RANGE_TYPE R, typename Hash = std::hash<typename range_value_type_t<R>::first_type>,
         typename Equal = std::equal_to<typename range_value_type_t<R>::first_type>>
...
// Returned in an arbitrary order that depends on the hash function.
template<typename sum_type = size_t, PARLAY_RANGE_TYPE R, typename Hash = std::hash<range_value_type_t<R>>,
...
template<PARLAY_RANGE_TYPE R, typename Hash = std::hash<range_value_type_t<R>>,
         typename Equal = std::equal_to<range_value_type_t<R>>>
...
auto histogram_by_index(R const &A, Integer_t num_buckets) {
  struct helper {
    ...
    static void combine(val_type &d, slice<key_type *, key_type *> s) { d = s.size(); }
  };
...
auto remove_duplicate_integers(R const &A, Integer_t max_value) {
  struct helper {
    ...
    static void combine(val_type &d, slice<key_type *, key_type *>) { d = true; }
  };
