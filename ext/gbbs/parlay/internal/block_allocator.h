...
struct block_allocator {
  ...
  void print_stats() {
    ...
    std::cout << "Used: " << used << ", allocated: " << allocated << ", block size: " << sz
              << ", bytes: " << sz * allocated << std::endl;
  }

  explicit block_allocator(size_t block_size, size_t reserved_blocks = 0, size_t list_length_ = 0,
    ...
    if (list_length_ == 0)
      ...
      list_length = list_length_ / block_size;
    if (max_blocks_ == 0)
  ...
  bool clear() {
    if (num_used_blocks() > 0)
      ...
      while ((x = pool_roots.pop()))
        ::operator delete (*x, std::align_val_t{pad_size});
      pool_roots.clear();
      global_stack.clear();
      blocks_allocated.store(0);
      return false;
