...
namespace operations_research
{
   ...
   {
    public:
      ZVector() : base_(nullptr), min_index_(0), max_index_(-1), size_(0), storage_()
      {
      }
          ...
          : base_(nullptr), min_index_(0), max_index_(-1), size_(0), storage_()
      {
         ...
         {
            std::cerr << "Could not reserve memory for indices ranging from " << min_index << " to " << max_index
                      << "\n";
            assert(0);
         }
      }
      ...
      void SetAll(T value)
      {
         ...
         {
            std::cerr << "Trying to set values to uninitialized vector.\n";
            assert(0);
         }
