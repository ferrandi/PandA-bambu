...
namespace operations_research
{
   ...
   class StarGraphBase
   {
    public:
      ...
      // To be used in an assert; also used internally to validate
      ...
      {
       public:
         explicit NodeIterator(const DerivedGraph& graph) : graph_(graph), head_(graph_.StartNode(kFirstNode))
         {
         }
         ...
         bool Ok() const
         {
            return head_ != kNilNode;
         }

         // Advances the current node index.
         void Next()
         {
            head_ = graph_.NextNode(head_);
         }
      ...
      {
       public:
         explicit ArcIterator(const DerivedGraph& graph) : graph_(graph), arc_(graph_.StartArc(kFirstArc))
         {
         }
      ...
      class OutgoingArcIterator
      {
       public:
         OutgoingArcIterator(const DerivedGraph& graph, NodeIndexType node)
             : graph_(graph), node_(graph_.StartNode(node)), arc_(graph_.StartArc(graph_.FirstOutgoingArc(node)))
         {
         ...
         OutgoingArcIterator(const DerivedGraph& graph, NodeIndexType node, ArcIndexType arc)
             : graph_(graph), node_(graph_.StartNode(node)), arc_(graph_.StartArc(arc))
         {
            assert(CheckInvariant());
         }
    ...
    protected:
      ...
      {
         assert(IsNodeValid(node));
         return ThisAsDerived()->FindNextOutgoingArc(ThisAsDerived()->FirstOutgoingOrOppositeIncomingArc(node));
      }
   ...
   {
    public:
      explicit PermutationIndexComparisonByArcHead(const ZVector<NodeIndexType>& head) : head_(head)
      {
      }
   ...
   template <typename NodeIndexType, typename ArcIndexType>
   class ForwardStaticGraph
       : public StarGraphBase<NodeIndexType, ArcIndexType, ForwardStaticGraph<NodeIndexType, ArcIndexType>>
   {
      typedef StarGraphBase<NodeIndexType, ArcIndexType, ForwardStaticGraph<NodeIndexType, ArcIndexType>> Base;
      friend class StarGraphBase<NodeIndexType, ArcIndexType, ForwardStaticGraph<NodeIndexType, ArcIndexType>>;
...
// CycleHandlerForAnnotatedArcs class.
#if !defined(SWIG)
      class CycleHandlerForAnnotatedArcs : public ArrayIndexCycleHandler<NodeIndexType, ArcIndexType>
      {
      ...
      ForwardStaticGraph(const NodeIndexType num_nodes, const ArcIndexType num_arcs, const bool sort_arcs_by_head,
                         ...
                         operations_research::PermutationCycleHandler<ArcIndexType>* const client_cycle_handler)
      {
         ...
         std::vector<std::pair<NodeIndexType, NodeIndexType>>& input_arcs = *client_input_arcs;
         ...
         for(ArcIndexType arc = kFirstArc; arc < kFirstArc + num_arcs; ++arc)
         {
            ...
            num_nodes_ = std::max(num_nodes_, static_cast<NodeIndexType>(input_arcs[arc].first + 1));
            num_nodes_ = std::max(num_nodes_, static_cast<NodeIndexType>(input_arcs[arc].second + 1));
         }
         ...
         else
         {
            if(sizeof(input_arcs[0].first) >= sizeof(first_incident_arc_[0]))
            {
               ...
               for(ArcIndexType input_arc = 0; input_arc < num_arcs; ++input_arc)
               {
                  ArcIndexType arc = static_cast<ArcIndexType>(input_arcs[input_arc].first);
                  NodeIndexType head = input_arcs[input_arc].second;
         ...
         // first_incident_arc_.
         for(NodeIndexType node = kFirstNode + num_nodes; node > /* kFirstNode */ 0; --node)
         {
         ...
         if(sort_arcs_by_head)
         {
            ...
            if(client_cycle_handler != nullptr)
            {
               for(NodeIndexType node = 0; node < num_nodes; ++node)
               {
                  ...
                  std::sort(&arc_permutation[begin], &arc_permutation[end],
                            PermutationIndexComparisonByArcHead<NodeIndexType, ArcIndexType>(head_));
                  begin = end;
               }
            }
            else
            {
               for(NodeIndexType node = 0; node < num_nodes; ++node)
               {
                  ...
                  std::sort(&head_[begin_index] + begin_offset, &head_[end_index] + end_offset);
                  begin = end;
               }
            }
         }
         if(client_cycle_handler != nullptr && num_arcs > 0)
         {
            ...
            CycleHandlerForAnnotatedArcs handler_for_constructor(client_cycle_handler, &head_[kFirstArc] - kFirstArc);
      ...
      {
         return ((tail_ != nullptr) && (arc >= kFirstArc) && (arc <= tail_->max_index()));
      }
    ...
    private:
      ...
      bool RepresentationClean() const
      {
         return true;
      }
   ...
   // The index of the 'nil' node in the graph.
   template <typename NodeIndexType, typename ArcIndexType, typename DerivedGraph>
   const NodeIndexType StarGraphBase<NodeIndexType, ArcIndexType, DerivedGraph>::kNilNode = -1;

   // The index of the 'nil' arc in the graph.
   template <typename NodeIndexType, typename ArcIndexType, typename DerivedGraph>
   const ArcIndexType
       StarGraphBase<NodeIndexType, ArcIndexType, DerivedGraph>::kNilArc = std::numeric_limits<ArcIndexType>::min();

   // The index of the first node in the graph.
   template <typename NodeIndexType, typename ArcIndexType, typename DerivedGraph>
   const NodeIndexType StarGraphBase<NodeIndexType, ArcIndexType, DerivedGraph>::kFirstNode = 0;

   // The index of the first arc in the graph.
   template <typename NodeIndexType, typename ArcIndexType, typename DerivedGraph>
   const ArcIndexType StarGraphBase<NodeIndexType, ArcIndexType, DerivedGraph>::kFirstArc = 0;

   // The maximum possible node index in the graph.
   template <typename NodeIndexType, typename ArcIndexType, typename DerivedGraph>
   const NodeIndexType StarGraphBase<NodeIndexType, ArcIndexType, DerivedGraph>::kMaxNumNodes =
   ...
   template <typename NodeIndexType, typename ArcIndexType, typename DerivedGraph>
   const ArcIndexType
       StarGraphBase<NodeIndexType, ArcIndexType, DerivedGraph>::kMaxNumArcs = std::numeric_limits<ArcIndexType>::max();
   ...
   // template argument.
   template <typename NodeIndexType, typename ArcIndexType, typename DerivedGraph>
   class EbertGraphBase : public StarGraphBase<NodeIndexType, ArcIndexType, DerivedGraph>
   {
    ...
    public:
      ...
      bool Reserve(NodeIndexType new_max_num_nodes, ArcIndexType new_max_num_arcs)
      {
         ...
         for(NodeIndexType node = max_num_nodes_; node <= first_incident_arc_.max_index(); ++node)
         {
      ...
      {
         if(num_arcs_ >= max_num_arcs_ || !IsNodeValid(tail) || !IsNodeValid(head))
         {
            return kNilArc;
         }
...
// member template and the CycleHandlerForAnnotatedArcs class.
      ...
      {
         std::unique_ptr<ArcIndexType[]> arc_permutation(new ArcIndexType[end_arc_index()]);
         ...
         std::sort(&arc_permutation[kFirstArc], &arc_permutation[end_arc_index()], compare);

         // Now we actually permute the head_ array and the
         // scaled_arc_cost_ array according to the sorting permutation.
         CycleHandlerForAnnotatedArcs cycle_handler(annotation_handler, ThisAsDerived());
      ...
      class CycleHandlerForAnnotatedArcs : public PermutationCycleHandler<ArcIndexType>
      {
       public:
         CycleHandlerForAnnotatedArcs(PermutationCycleHandler<ArcIndexType>* annotation_handler, DerivedGraph* graph)
             : annotation_handler_(annotation_handler), graph_(graph), head_temp_(kNilNode), tail_temp_(kNilNode)
         {
         }
    ...
    protected:
      ...
      void Initialize(NodeIndexType max_num_nodes, ArcIndexType max_num_arcs)
      {
         ...
         {
            std::cerr << "Could not reserve memory for " << static_cast<int64_t>(max_num_nodes) << " nodes and "
      ...
      // before the adjacency lists are examined. Only for assert in debug
      // builds.
   ...
   template <typename NodeIndexType, typename ArcIndexType>
   class EbertGraph : public EbertGraphBase<NodeIndexType, ArcIndexType, EbertGraph<NodeIndexType, ArcIndexType>>
   {
      typedef EbertGraphBase<NodeIndexType, ArcIndexType, EbertGraph<NodeIndexType, ArcIndexType>> Base;
       ...
       public:
         OutgoingOrOppositeIncomingArcIterator(const EbertGraph& graph, NodeIndexType node)
             : graph_(graph),
               ...
               arc_(graph_.StartArc(graph_.FirstOutgoingOrOppositeIncomingArc(node)))
         {
            assert(CheckInvariant());
         }
      ...
      class IncomingArcIterator
      {
       public:
         IncomingArcIterator(const EbertGraph& graph, NodeIndexType node)
             : graph_(graph), node_(graph_.StartNode(node)), arc_(graph_.StartArc(graph_.FirstIncomingArc(node)))
         {
            assert(CheckInvariant());
         }
         ...
         IncomingArcIterator(const EbertGraph& graph, NodeIndexType node, ArcIndexType arc)
             : graph_(graph),
               node_(graph_.StartNode(node)),
               arc_(arc == kNilArc ? kNilArc : graph_.StartArc(graph_.Opposite(arc)))
         {
            assert(CheckInvariant());
         }
   ...
   template <typename NodeIndexType, typename ArcIndexType>
   class ForwardEbertGraph
       : public EbertGraphBase<NodeIndexType, ArcIndexType, ForwardEbertGraph<NodeIndexType, ArcIndexType>>
   {
      typedef EbertGraphBase<NodeIndexType, ArcIndexType, ForwardEbertGraph<NodeIndexType, ArcIndexType>> Base;
      friend class EbertGraphBase<NodeIndexType, ArcIndexType, ForwardEbertGraph<NodeIndexType, ArcIndexType>>;
      friend class StarGraphBase<NodeIndexType, ArcIndexType, ForwardEbertGraph<NodeIndexType, ArcIndexType>>;
    ...
    private:
      ...
      void ReserveTailArray(ArcIndexType new_max_num_arcs)
      {
         if(tail_ != nullptr)
         {
            ...
            {
               for(ArcIndexType arc = tail_->max_index() + 1; arc < new_max_num_arcs; ++arc)
               {
   ...
   class TailArrayManager
   {
    public:
      ...
      {
         or_internal::TailArrayBuilder<GraphType, graph_traits<GraphType>::has_reverse_arcs> tail_array_builder(graph_);
         return tail_array_builder.BuildTailArray();
      }

      void ReleaseTailArrayIfForwardGraph() const
      {
         or_internal::TailArrayReleaser<GraphType, graph_traits<GraphType>::has_reverse_arcs> tail_array_releaser(
             graph_);
   ...
   {
    public:
      explicit ArcFunctorOrderingByTailAndHead(const GraphType& graph) : graph_(graph)
      {
      }
      ...
      bool operator()(typename GraphType::ArcIndex a, typename GraphType::ArcIndex b) const
      {
         return ((graph_.Tail(a) < graph_.Tail(b)) ||
                 ((graph_.Tail(a) == graph_.Tail(b)) && (graph_.Head(a) < graph_.Head(b))));
      }
   ...
   namespace or_internal
   {
      ...
      {
       public:
         GraphBuilderFromArcs(typename GraphType::NodeIndex max_num_nodes, typename GraphType::ArcIndex max_num_arcs,
                              bool sort_arcs)
         ...
         typename GraphType::ArcIndex AddArc(typename GraphType::NodeIndex tail, typename GraphType::NodeIndex head)
         {
            ...
            if(num_arcs_ < max_num_arcs_ && tail < GraphType::kFirstNode + max_num_nodes_ &&
         ...
         {
            GraphType* graph = new GraphType(max_num_nodes_, num_arcs_, sort_arcs_, &arcs_, client_cycle_handler);
            delete this;
            return graph;
         }

       private:
         bool Reserve(typename GraphType::NodeIndex new_max_num_nodes, typename GraphType::ArcIndex new_max_num_arcs)
         {
            max_num_nodes_ = new_max_num_nodes;
            max_num_arcs_ = new_max_num_arcs;
         ...
         std::vector<std::pair<typename GraphType::NodeIndex, typename GraphType::NodeIndex>> arcs_;
      ...
      class GraphBuilderFromArcs<GraphType, true>
      {
       public:
         GraphBuilderFromArcs(typename GraphType::NodeIndex max_num_nodes, typename GraphType::ArcIndex max_num_arcs,
                              bool sort_arcs)
             : graph_(new GraphType(max_num_nodes, max_num_arcs)), sort_arcs_(sort_arcs)
         {
         }
         ...
         typename GraphType::ArcIndex AddArc(const typename GraphType::NodeIndex tail,
   ...
   class AnnotatedGraphBuildManager
       : public or_internal::GraphBuilderFromArcs<GraphType, graph_traits<GraphType>::is_dynamic>
   {
    public:
      AnnotatedGraphBuildManager(typename GraphType::NodeIndex num_nodes, typename GraphType::ArcIndex num_arcs,
                                 bool sort_arcs)
          : or_internal::GraphBuilderFromArcs<GraphType, graph_traits<GraphType>::is_dynamic>(num_nodes, num_arcs,
                                                                                              sort_arcs)
      {
      }
   };
   ...
   bool BuildLineGraph(const GraphType& graph, GraphType* const line_graph)
   {
      ...
      {
         std::cerr << "line_graph must not be NULL\n";
         assert(0);
         return false;
      }
      if(line_graph->num_nodes() != 0)
      {
         std::cerr << "line_graph must be empty\n";
         assert(0);
         return false;
      }
      ...
      for(ArcIterator arc_iterator(graph); arc_iterator.Ok(); arc_iterator.Next())
      {
         const typename GraphType::ArcIndex arc = arc_iterator.Index();
         const typename GraphType::NodeIndex head = graph.Head(arc);
         for(OutgoingArcIterator iterator(graph, head); iterator.Ok(); iterator.Next())
         {
            ++num_arcs;
         }
      }
