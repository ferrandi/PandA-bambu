...
// Divisor for epsilon at each Refine step of LinearSumAssignment.
#define FLAGS_assignment_alpha 5
#define FLAGS_assignment_stack_order true
...
namespace operations_research
{
   ...
   class LinearSumAssignment
   {
    public:
      ...
      {
         assert(0 == scaled_arc_cost_[arc] % cost_scaling_factor_);
      ...
      {
         assert(left_node < num_left_nodes_);
      ...
      inline NodeIndex GetMate(NodeIndex left_node) const
      {
         ...
         assert(GraphType::kNilArc != matching_arc);
    ...
    private:
      ...
      class ActiveNodeStack : public ActiveNodeContainerInterface
      {
       public:
         ...
         void Add(NodeIndex node) override
         {
            v_.push_back(node);
         }
      ...
      // epsilon-optimal. To be used in an assert.
      ...
      // contains a safety assert() that its argument is a left-side node,
      ...
      inline CostValue PriceChangeBound(CostValue old_epsilon, CostValue new_epsilon, bool* in_range) const
      {
         ...
         // pencil handy. :-)
         const double result = static_cast<double>(std::max<CostValue>(1, n / 2 - 1)) *
                               (static_cast<double>(old_epsilon) + static_cast<double>(new_epsilon));
         const double limit = static_cast<double>(std::numeric_limits<CostValue>::max());
         if(result > limit)
         {
   ...
   void LinearSumAssignment<GraphType>::SetArcCost(ArcIndex arc, CostValue cost)
   {
      ...
      {
         assert(arc >= 0);
         assert(arc < graph_->num_arcs());
         NodeIndex head = Head(arc);
         assert(num_left_nodes_ <= head);
      }
      cost *= cost_scaling_factor_;
      const CostValue cost_magnitude = std::abs(cost);
      largest_scaled_cost_magnitude_ = std::max(largest_scaled_cost_magnitude_, cost_magnitude);
   ...
   {
    public:
      explicit CostValueCycleHandler(std::vector<CostValue>* cost) : temp_(0), cost_(cost)
      {
      }
   ...
   // Passes ownership of the cycle handler to the caller.
   template <typename GraphType>
   PermutationCycleHandler<typename GraphType::ArcIndex>* LinearSumAssignment<GraphType>::ArcAnnotationCycleHandler()
   {
      return new CostValueCycleHandler<typename GraphType::ArcIndex>(&scaled_arc_cost_);
   }
   ...
   void LinearSumAssignment<GraphType>::OptimizeGraphLayout(GraphType* graph)
   {
      ...
      CostValueCycleHandler<typename GraphType::ArcIndex> cycle_handler(&scaled_arc_cost_);
   ...
   template <typename GraphType>
   CostValue LinearSumAssignment<GraphType>::NewEpsilon(const CostValue current_epsilon) const
   {
...
#endif
      assert(slack_relabeling_price_ > 0);
   ...
   template <typename GraphType>
   inline bool LinearSumAssignment<GraphType>::IsActive(NodeIndex left_node) const
   {
      assert(left_node < num_left_nodes_);
   ...
   template <typename GraphType>
   inline bool LinearSumAssignment<GraphType>::IsActiveForDebugging(NodeIndex node) const
   {
   ...
   void LinearSumAssignment<GraphType>::InitializeActiveNodeContainer()
   {
      ...
      for(BipartiteLeftNodeIterator node_it(*graph_, num_left_nodes_); node_it.Ok(); node_it.Next())
      {
   ...
   {
      assert(num_left_nodes_ > source);
      if(!IsActive(source))
      {
         std::cerr << "Node " << source << "must be active (unmatched)!\n";
         assert(0);
      }
   ...
   {
      if(!IsActive(left_node))
      {
         std::cerr << "Node " << left_node << " must be active (unmatched)!\n";
         assert(0);
      }
      assert(epsilon_ > 0);
      ...
      CostValue second_min_partial_reduced_cost = min_partial_reduced_cost + max_gap;
      ...
      const CostValue gap = std::min<CostValue>(second_min_partial_reduced_cost - min_partial_reduced_cost, max_gap);
      assert(gap >= 0);
   ...
   template <typename GraphType>
   inline CostValue LinearSumAssignment<GraphType>::ImplicitPrice(NodeIndex left_node) const
   {
      assert(num_left_nodes_ > left_node);
      assert(epsilon_ > 0);
      typename GraphType::OutgoingArcIterator arc_it(*graph_, left_node);
   ...
   bool LinearSumAssignment<GraphType>::EpsilonOptimal() const
   {
      for(BipartiteLeftNodeIterator node_it(*graph_, num_left_nodes_); node_it.Ok(); node_it.Next())
      {
         ...
         for(typename GraphType::OutgoingArcIterator arc_it(*graph_, left_node); arc_it.Ok(); arc_it.Next())
         {
            const ArcIndex arc = arc_it.Index();
   ...
   bool LinearSumAssignment<GraphType>::FinalizeSetup()
   {
      ...
      do
      {
         ...
         double_price_lower_bound -=
             2.0 * static_cast<double>(PriceChangeBound(old_error_parameter, new_error_parameter, &in_range));
      ...
      if(!in_range)
      {
         std::cerr << "Price change bound exceeds range of representable "
