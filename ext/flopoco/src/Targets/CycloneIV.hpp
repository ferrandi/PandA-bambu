#ifndef CycloneIV_HPP
#define  CycloneIV_HPP
#include <iostream>
#include <sstream>
#include <vector>
#include <math.h>
...
namespace flopoco{
	...
	/** Class for representing an CycloneIV target */
	class CycloneIV : public Target
	{
	public:
		...
		CycloneIV() : Target()	{
			...
			elemWireDelay_			= 0.325e-9;		// obtained from Quartus 2 Chip Planner 11.1 - local line delay
			//lut2lutDelay_			= 1.5e-10;		// ???
			lut2lutDelay_			= 0.650e-9;		// ???
			...
			// all these values are set precisely to match the Cyclone IV
			lut2_					= 0.308e-9;		// obtained from Quartus 2 Chip Planner 11.1
			lut3_					= 0.308e-9;		// obtained from Quartus 2 Chip Planner 11.1
			lut4_					= 0.242e-9;		// obtained from Quartus 2 Chip Planner 11.1
			
			innerLABcarryDelay_		= 0.058e-9;		// obtained from Quartus 2 Chip Planner 11.1 -> Q2 CP sets them (inner- and inter-) equal to the fast carry; possibly compensated by the other elements on the critical path
			...
			// DSP parameters
			totalDSPs_				= 266;
			nrConfigs_				= 2;			// CycloneIV has 9, 18 bit multipliers by default
