/*
  A model of the Cyclone IV FPGA
...
namespace flopoco{
	...
	double CycloneIV::adderDelay(int size) {
		int subAdd = 0;
	...
	void CycloneIV::getAdderParameters(double &k1, double &k2, int size)
	{
		int subAdd = 0;
	...
	double CycloneIV::eqComparatorDelay(int size){
		return adderDelay(size) + elemWireDelay_ + lut2_; // TODO: check correctness
	};
	
	double CycloneIV::eqConstComparatorDelay(int size){
		return adderDelay(size/2) + elemWireDelay_ + lut2_; // // TODO: check correctness
	};
	
	double CycloneIV::carryPropagateDelay() {
		return  fastcarryDelay_; 
	};
	
	double CycloneIV::localWireDelay(int fanout){
		return elemWireDelay_;
	};
	
	double CycloneIV::distantWireDelay(int n){
		return n*elemWireDelay_;
	};
	
	double CycloneIV::lutDelay(){
		return lutDelay_;
	};
	
	double CycloneIV::ffDelay(){
		return ffDelay_;
	};
	
	long CycloneIV::sizeOfMemoryBlock()
	{
		return sizeOfBlock_;	
	};
	
	DSP* CycloneIV::createDSP() 
	{
		int multW, multH;
	...
	bool CycloneIV::suggestSubmultSize(int &x, int &y, int wInX, int wInY){
	...
	bool CycloneIV::suggestSubaddSize(int &x, int wIn){
	...
	bool  CycloneIV::suggestSlackSubaddSize(int &x, int wIn, double slack)
	{
	...
	bool CycloneIV::suggestSlackSubcomparatorSize(int& x, int wIn, double slack, bool constant)
	{
	...
	// TODO: - redo
	int CycloneIV::getIntMultiplierCost(int wInX, int wInY)
	{
		int cost = 0;
	...
	//		check validity
	void CycloneIV::getDSPWidths(int &x, int &y, bool sign)
	{ 
	...
	int CycloneIV::getEquivalenceSliceDSP(){
		int lutCost = 0;
		int x, y;
	...
	int CycloneIV::getIntNAdderCost(int wIn, int n)
	{
	...
	void CycloneIV::delayForDSP(MultiplierBlock* multBlock, double currentCp, int& cycleDelay, double& cpDelay)
	{
