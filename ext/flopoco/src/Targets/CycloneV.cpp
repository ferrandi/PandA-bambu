...
namespace flopoco{
	...
	double CycloneV::adderDelay(int size) 
	{
		int subAdd = 0;
	...
	double CycloneV::adder3Delay(int size) 
	{
		int subAdd = 0;
	...
	void CycloneV::getAdderParameters(double &k1, double &k2, int size)
	{
		int subAdd = 0;
	...
	double CycloneV::eqComparatorDelay(int size){
		return adderDelay(size) + elemWireDelay_ + lut2_; // TODO: check correctness
	};
	
	double CycloneV::eqConstComparatorDelay(int size){
		return adderDelay(size/2) + elemWireDelay_ + lut2_; // // TODO: check correctness
	};
	
	double CycloneV::carryPropagateDelay() {
		return  fastcarryDelay_; 
	};
	
	double CycloneV::localWireDelay(int fanout){
		return elemWireDelay_;
	};
	
	double CycloneV::distantWireDelay(int n){
		return n*elemWireDelay_;
	};
	
	double CycloneV::lutDelay(){
		return lutDelay_;
	};
	
	double CycloneV::ffDelay(){
		return ffDelay_;
	};
	
	long CycloneV::sizeOfMemoryBlock()
	{
		return sizeOfBlock_;	
	};
	
	DSP* CycloneV::createDSP() 
	{
		int multW, multH;
	...
	bool CycloneV::suggestSubmultSize(int &x, int &y, int wInX, int wInY){
	...
	bool CycloneV::suggestSubaddSize(int &x, int wIn){
	...
	bool CycloneV::suggestSubadd3Size(int &x, int wIn)
	{
	...
	bool  CycloneV::suggestSlackSubaddSize(int &x, int wIn, double slack)
	{		
	...
	bool  CycloneV::suggestSlackSubadd3Size(int &x, int wIn, double slack)
	{
	...
	bool CycloneV::suggestSlackSubcomparatorSize(int& x, int wIn, double slack, bool constant)
	{
	...
	// TODO: - redo
	int CycloneV::getIntMultiplierCost(int wInX, int wInY)
	{
		int cost = 0;
	...
	//		check validity
	void CycloneV::getDSPWidths(int &x, int &y, bool sign)
	{ 
	...
	int CycloneV::getEquivalenceSliceDSP(){
		int lutCost = 0;
		int x, y;
	...
	int CycloneV::getIntNAdderCost(int wIn, int n)
	{
