...
class CordRepAnalyzer {
 public:
  ...
  void AnalyzeCordRep(const CordRep* rep) {
    ...
    size_t refcount_ = rep->refcount_.Get();
    RepRef repref{rep, (refcount_ > 1) ? refcount_ - 1 : 1};

    // Process the top level CRC node, if present.
    if (repref.rep->tag == CRC) {
      statistics_.node_count++;
      statistics_.node_counts.crc++;
      memory_usage_.Add(sizeof(CordRepCrc), repref.refcount_);
 ...
 private:
  ...
  struct RepRef {
    ...
    RepRef Child(const CordRep* child) const {
      return RepRef{child, refcount_ * child->refcount_.Get()};
    }
  };
  ...
  struct MemoryUsage {
    ...
    void Add(size_t size, size_t refcount_) {
      total += size;
      fair_share += static_cast<double>(size) / refcount_;
    }
  };
  ...
  RepRef CountLinearReps(RepRef rep, MemoryUsage& memory_usage) {
    ...
    while (rep.rep->tag == SUBSTRING) {
      ...
      memory_usage.Add(sizeof(CordRepSubstring), rep.refcount_);
    ...
    if (rep.rep->tag >= FLAT) {
      ...
      CountFlat(size);
      memory_usage.Add(size, rep.refcount_);
  ...
  void AnalyzeConcat(RepRef rep) {
    ...
    while (rep.rep != nullptr) {
      ...
      memory_usage_.Add(sizeof(CordRepConcat), rep.refcount_);
  ...
  void AnalyzeRing(RepRef rep) {
    ...
    memory_usage_.Add(CordRepRing::AllocSize(ring->capacity()), rep.refcount_);
  ...
  void AnalyzeBtree(RepRef rep) {
    ...
    memory_usage_.Add(sizeof(CordRepBtree), rep.refcount_);
