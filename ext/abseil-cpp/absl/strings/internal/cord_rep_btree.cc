...
void DumpAll(const CordRep* rep, bool include_contents, std::ostream& stream,
  ...
  std::string sharing = const_cast<CordRep*>(rep)->refcount_.IsOne()
                            ? std::string("Private")
                            : absl::StrCat("Shared(", rep->refcount_.Get(), ")");
...
inline void FastUnref(R* r, Fn&& fn) {
  if (r->refcount_.IsOne()) {
    fn(r);
  } else if (!r->refcount_.DecrementExpectHighRefcount()) {
    fn(r);
  }
}
...
struct StackOperations {
  ...
  inline CordRepBtree* BuildStack(CordRepBtree* tree, int depth) {
    ...
    int current_depth = 0;
    while (current_depth < depth && tree->refcount_.IsOne()) {
      stack[current_depth++] = tree;
      tree = tree->Edge(edge_type)->btree();
    }
    share_depth = current_depth + (tree->refcount_.IsOne() ? 1 : 0);
...
static void DestroyTree(CordRepBtree* tree) {
  for (CordRep* node : tree->Edges()) {
    if (node->refcount_.Decrement()) continue;
    for (CordRep* edge : node->btree()->Edges()) {
      if (edge->refcount_.Decrement()) continue;
      if (size == 1) {
        DeleteLeafEdge(edge);
      } else {
...
CordRep* CordRepBtree::RemoveSuffix(CordRepBtree* tree, size_t n) {
  ...
  bool is_mutable = tree->refcount_.IsOne();
  ...
  while (pos.index == tree->begin()) {
    ...
    is_mutable &= edge->refcount_.IsOne();
  ...
  while (length != edge->length) {
    ...
    assert(tree->refcount_.IsOne());
    const bool edge_is_mutable = edge->refcount_.IsOne();
...
Span<char> CordRepBtree::GetAppendBufferSlow(size_t size) {
  ...
  for (int i = 0; i < depth; ++i) {
    ...
    if (!node->refcount_.IsOne()) return {};
    stack[i] = node;
  }

  // Must be a privately owned, mutable flat.
  CordRep* const edge = node->Edge(kBack);
  if (!edge->refcount_.IsOne() || edge->tag < FLAT) return {};
...
void CordRepBtree::Rebuild(CordRepBtree** stack, CordRepBtree* tree,
                           bool consume) {
  bool owned = consume && tree->refcount_.IsOne();
  if (tree->height() == 0) {
    for (CordRep* edge : tree->Edges()) {
...
CordRepBtree::ExtractResult CordRepBtree::ExtractAppendBuffer(
  ...
  while (tree->height() > 0) {
    if (!tree->refcount_.IsOne()) return result;
    stack[depth++] = tree;
  ...
  if (!(rep->IsFlat() && rep->refcount_.IsOne())) return result;
