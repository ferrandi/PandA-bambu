...
class Refcount {
 public:
  ...
  inline bool Decrement() {
    int32_t refcount0 = count_.load(std::memory_order_acquire);
    assert(refcount0 > 0);
    return refcount0 != 1 && count_.fetch_sub(1, std::memory_order_acq_rel) != 1;
  }

  // Same as Decrement but expect that refcount is greater than 1.
  inline bool DecrementExpectHighRefcount() {
    int32_t refcount0 = count_.fetch_sub(1, std::memory_order_acq_rel);
    assert(refcount0 > 0);
    return refcount0 != 1;
  }
