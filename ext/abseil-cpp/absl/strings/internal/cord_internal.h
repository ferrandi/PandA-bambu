...
class RefcountAndFlags {
 public:
  ...
  inline bool Decrement() {
    int32_t refcount_ = count_.load(std::memory_order_acquire) & kRefcountMask;
    assert(refcount_ > 0 || refcount_ & kImmortalFlag);
    return refcount_ != kRefIncrement &&
...
struct CordRep {
  ...
  constexpr CordRep(RefcountAndFlags::Immortal immortal, size_t l)
      : length(l), refcount_(immortal), tag(EXTERNAL), storage{} {}

  // The following three fields have to be less than 32 bytes since
  // that is the smallest supported flat node size.
  size_t length;
  RefcountAndFlags refcount_;
...
inline void CordRep::Unref(CordRep* rep) {
  ...
  if (ABSL_PREDICT_FALSE(!rep->refcount_.DecrementExpectHighRefcount())) {
    Destroy(rep);
  }
}
